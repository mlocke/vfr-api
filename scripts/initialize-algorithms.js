...
const ALGORITHM_CONFIGURATIONS = {
  composite: {
    ...
    weights: [
      ...
      { factor: 'value_composite', weight: 0.20, enabled: true },
  ...
  quality: {
    ...
    description: 'Conservative strategy focusing on high-quality companies with strong fundamentals',
    type: 'QUALITY',
    enabled: true,
    selectionCriteria: 'SCORE_BASED',
...
function validateConfiguration(config) {
  try {
    ...
    if (totalWeight <= 0 || totalWeight > 2) { // Allow some flexibility
      return false;
    }
...
${configs.map(config => `### ${config.name} (\`${config.id}\`)
- **Type:** ${config.type}
...
`;
  ...
  const reportPath = path.join(__dirname, '..', 'docs', 'test-output', 'algorithm-initialization-report.md');
