...
async function fetchTopStocks(limit: number = 1000): Promise<string[]> {
	...
	try {
		...
		const response = await fmpAPI["makeRequest"](
			`/stock-screener?marketCapMoreThan=1000000000&limit=${limit}`
		);

		if (!response || !response.success || !response.data || !Array.isArray(response.data)) {
			throw new Error(
				`Invalid response from FMP stock screener: ${JSON.stringify(response)}`
			);
		}
...
class MultiSourceDataCollector {
		...
		try {
			...
			const features: EnhancedFeatures = {
				...
				sec_institutional_ownership_change:
					govData?.sec_institutional_ownership_change || 0,
			...
			console.log(
				`  [${symbol}] ✓ Collected from ${this.getSourcesUsed(fmpData, eodhdData, govData)}`
			);
			return features;
		} catch (error: any) {
	...
	private async collectFMPFeatures(symbol: string, date: Date): Promise<any> {
		try {
			...
			const previous5dVolume =
				sortedData.slice(5, 10).reduce((sum, d) => sum + d.volume, 0) / 5;
			...
			const macd_histogram_trend = this.calculateMACDTrend(
				sortedData.slice(0, 26).map(d => d.close)
			);
		...
		try {
			...
			return {
				...
				sec_institutional_ownership_change: Number(
					sec_institutional_ownership_change.toFixed(4)
				),
	...
	async generateLabel(symbol: string, date: Date): Promise<number> {
		try {
			...
			const response = await this.fmpAPI["makeRequest"](
				`/upgrades-downgrades/${symbol}?limit=100`
			);
...
class DatasetGenerator {
	...
	async generateDataset(
		...
		console.log(
			`   Sample dates: ${sampleDates.length} (${samplingMode === "weekly" ? "every Wednesday" : "15th of each month"})`
		);
		...
		for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
			...
			const batchResults = await Promise.allSettled(
				batch.map(async pair => {
					...
					console.log(
						`[${pair.index + 1}/${symbolDatePairs.length}] (${progress}%) ${pair.symbol} @ ${dateStr}`
					);

					try {
						// Collect features from all sources for historical date
						const features = await this.collector.collectEnhancedFeatures(
							pair.symbol,
							pair.date
						);

						if (features) {
							const label = await this.collector.generateLabel(
								pair.symbol,
								pair.date
							);
			...
			for (const result of batchResults) {
				if (result.status === "fulfilled" && result.value.success && result.value.example) {
	...
	private async saveCheckpoint(
		dataset: EnhancedTrainingExample[],
		name: string,
		count: number
	): Promise<void> {
	...
	private exampleToCSV(ex: EnhancedTrainingExample): string {
		...
		return [
			...
			f.volume_ratio,
			f.volume_trend,
			f.sentiment_news_delta,
			f.sentiment_reddit_accel,
			f.sentiment_options_shift,
			f.social_stocktwits_24h_change,
			f.social_stocktwits_hourly_momentum,
			f.social_stocktwits_7d_trend,
			f.social_twitter_24h_change,
			f.social_twitter_hourly_momentum,
			f.social_twitter_7d_trend,
			f.earnings_surprise,
			f.revenue_growth_accel,
			f.analyst_coverage_change,
			f.rsi_momentum,
			f.macd_histogram_trend,
			f.fed_rate_change_30d,
			f.unemployment_rate_change,
			f.cpi_inflation_rate,
			f.gdp_growth_rate,
			f.treasury_yield_10y,
			f.sec_insider_buying_ratio,
			f.sec_institutional_ownership_change,
			f.sec_8k_filing_count_30d,
			f.analyst_price_target_change,
			f.earnings_whisper_vs_estimate,
			f.short_interest_change,
			f.institutional_ownership_momentum,
			f.options_put_call_ratio_change,
			f.dividend_yield_change,
			f.market_beta_30d,
			ex.label,
			ex.dataSource,
		].join(",");
...
async function main() {
	...
	if (samplingMode === "monthly") {
		console.log(
			`   → Sampling 15th of each month (${Math.ceil((dateRange.end.getTime() - dateRange.start.getTime()) / (30 * 24 * 60 * 60 * 1000))} samples expected)`
		);
	} else {
		console.log(
			`   → Sampling every Wednesday (${Math.ceil((dateRange.end.getTime() - dateRange.start.getTime()) / (7 * 24 * 60 * 60 * 1000))} samples expected)`
		);
	}
