...
async function generateTrainingData(args: CLIArgs): Promise<void> {
	...
	console.log(
		'   Label = 1 if earnings beat expectations AND current consensus is "Buy" or better'
	);
	console.log(
	...
	for (let i = 0; i < symbols.length; i++) {
		...
		try {
			...
			console.log(
				`  ✓ Found ${filteredEarnings.length} earnings releases in date range (sorted chronologically)`
			);
			...
			for (let j = 0; j < filteredEarnings.length; j++) {
				try {
					...
					console.error(
						`  ✗ Failed to process earnings ${j} for ${symbol}: ${error.message}`
					);
					errorCount++;
					totalSkipped++;
				}
	...
	if (stats.validExamples > 0) {
		console.log(
			`  ${stats.upgradePercentage >= 10 && stats.upgradePercentage <= 50 ? "✓" : "⚠️"} Positive label percentage: ${stats.upgradePercentage.toFixed(1)}% (target: 10-50% for earnings beats)`
		);
		console.log(
