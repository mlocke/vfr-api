#!/usr/bin/env python3
"""
Train Smart Money Flow Model - 18 Features (Parquet-based)

Features: 18 features from Parquet files
- 4 insider features (SEC Form 4)
- 3 price features
- 3 volume features
- 2 advanced volume features
- 1 options feature
- 4 congressional features
- 1 institutional feature (placeholder)

Model: LightGBM Binary Classifier
Target: Predict 14-day forward return >5%

Usage:
    python3 scripts/ml/train-18feature-model.py
"""
...
# Paths
DATA_DIR = "data/training/smart-money-flow-18features"
MODEL_DIR = "models/smart-money-flow/v2.0.0-18features"
TRAIN_FILE = f"{DATA_DIR}/train.csv"
VAL_FILE = f"{DATA_DIR}/val.csv"
TEST_FILE = f"{DATA_DIR}/test.csv"

# Feature names (18 features from Parquet + cached data)
FEATURE_NAMES = [
    # Insider features (4)
...
def save_model(model, means, stds, train_metrics, val_metrics, test_metrics):
    ...
    metadata = {
        ...
        'model_version': 'v2.0.0-18features',
        ...
        'total_iterations': int(model.num_trees())
    }
...
def plot_feature_importance(feature_importance_list):
    ...
    plt.title('Smart Money Flow - Feature Importance (18 Features (SEC + Cached Data))')
    plt.tight_layout()
...
def main():
    ...
    print("=" * 80)
    print("Smart Money Flow Model Training (18 Features (SEC + Cached Data))")
    print("=" * 80)
