 ...
 */
function normalizeFeatures(data: TrainingRow[]): {
	normalized: TrainingRow[];
	mean: number[];
	std: number[];
} {
...
async function trainModel() {
	...
	fs.writeFileSync(path.join(modelDir, "model.json"), JSON.stringify(modelData, null, 2));

	// Save normalizer
	const normalizer = { mean, std };
	fs.writeFileSync(path.join(modelDir, "normalizer.json"), JSON.stringify(normalizer, null, 2));
