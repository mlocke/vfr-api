...
async function testEODHDOptions() {
  ...
  try {
    const options = await eodhd.getOptionsData(testSymbol)
    console.log('✅ Standard options endpoint SUCCESS')
    console.log(`   Contracts returned: ${options?.data?.length || 0}`)
    if (options?.data?.[0]) {
      console.log(`   Sample contract:`, {
        type: options.data[0].type,
        strike: options.data[0].strike,
        expiration: options.data[0].expirationDate
      })
    }
  } catch (error: any) {
