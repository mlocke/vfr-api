...
function splitDataset(args: CLIArgs): void {
	...
	console.log("\nSplit Results:");
	console.log(
		`  Train: ${trainRows.length} examples (${args.trainStart.toISOString().split("T")[0]} to ${args.trainEnd.toISOString().split("T")[0]})`
	);
	console.log(
		`  Validation: ${valRows.length} examples (${args.valStart.toISOString().split("T")[0]} to ${args.valEnd.toISOString().split("T")[0]})`
	);
	console.log(
		`  Test: ${testRows.length} examples (${args.testStart.toISOString().split("T")[0]} to ${args.testEnd.toISOString().split("T")[0]})`
	);
	...
	const splitRatioOk =
		trainPctNum >= 70 && trainPctNum <= 90 && valPctNum >= 5 && testPctNum >= 5;
	console.log(
		`  ${splitRatioOk ? "✓" : "⚠️"} Split ratio appropriate: Train ${trainPct}%, Val ${valPct}%, Test ${testPct}%`
	);

	const noTemporalOverlap = args.trainEnd < args.valStart && args.valEnd < args.testStart;
	console.log(
		`  ${noTemporalOverlap ? "✓" : "✗"} No temporal overlap: ${noTemporalOverlap ? "Yes" : "No"}`
	);

	const allFilesSaved =
		fs.existsSync(trainPath) && fs.existsSync(valPath) && fs.existsSync(testPath);
