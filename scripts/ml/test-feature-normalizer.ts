...
function createMockFeatureVector(multiplier: number = 1): FeatureVector {
  return {
    ...
    price_change_10d: 0.10 * multiplier,
...
async function testNormalizer() {
  ...
  console.log(`  Sample normalized values: [${normalized.slice(0, 3).map(v => v.toFixed(3)).join(', ')}, ...]`)
  ...
  for (let featureIdx = 0; featureIdx < numFeatures; featureIdx++) {
    ...
    if (featureIdx === 0) {
      console.log(`  Feature ${featureIdx}: mean=${mean.toFixed(4)}, std=${stdDev.toFixed(4)}`)
    }
  }
  ...
  const constantData: FeatureVector[] = Array(10).fill(null).map(() => ({
