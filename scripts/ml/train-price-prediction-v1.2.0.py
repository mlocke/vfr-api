#!/usr/bin/env python3
"""
Price Prediction Model Training Script v1.2.0
...
Features: 48 price-relevant features (43 original + 5 sentiment)
Target: 3-class classification (UP, DOWN, NEUTRAL)
Model: LightGBM gradient boosting

NEW in v1.2.0:
- Added 5 news sentiment features from FinBERT model
- news_sentiment_24h, news_sentiment_7d, news_sentiment_30d
- news_sentiment_momentum, news_volume_24h

Usage:
    python scripts/ml/train-price-prediction-v1.2.0.py
"""
...
MODEL_DIR = "models/price-prediction/v1.2.0"  # New version
TRAIN_FILE = f"{DATA_DIR}/price-sentiment-test-train.csv"
VAL_FILE = f"{DATA_DIR}/price-sentiment-test-val.csv"
TEST_FILE = f"{DATA_DIR}/price-sentiment-test-test.csv"

# Feature columns (48 features = 43 original + 5 sentiment)
FEATURE_COLUMNS = [
    # Volume features (6)
    "volume_ratio_5d",
    "volume_spike",
...
def load_data():
    ...
    print(f"  Features: {len(FEATURE_COLUMNS)} (43 original + 5 sentiment)")
...
def save_model(model, scaler, metrics):
    ...
    metadata = {
        ...
        "feature_names": FEATURE_COLUMNS,
        "improvements_over_v1_1_0": [
            "Added 5 FinBERT-based news sentiment features",
            "news_sentiment_24h: 24-hour sentiment average",
            "news_sentiment_7d: 7-day sentiment average",
            "news_sentiment_30d: 30-day sentiment average",
            "news_sentiment_momentum: Sentiment trend direction",
            "news_volume_24h: News article volume"
        ],
        "metrics": metrics,
        "hyperparameters": {
            "objective": "multiclass",
...
def main():
    ...
    print("=" * 80)
    print("🎯 Price Prediction Model Training v1.2.0 (with Sentiment Features)")
    print("=" * 80)
    ...
    print("=" * 80)
    print(f"\nModel v1.2.0 saved to: {MODEL_DIR}/")
    ...
    print("\n💡 Next steps:")
    print("  1. Compare v1.2.0 accuracy with v1.1.0 baseline (46%)")
    print("  2. If improvement seen, generate full 73,200-row sentiment dataset")
    print("  3. Retrain v1.2.0 on full dataset for production deployment")
    print("=" * 80)
