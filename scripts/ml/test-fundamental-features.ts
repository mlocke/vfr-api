	...
	for (const symbol of testSymbols) {
		...
		try {
			...
			if (incomeStatements && incomeStatements.length >= 4) {
				...
				if (relevantStatements.length >= 4) {
					...
					const recentGrowthRate =
						recentYearAgoRevenue !== 0
							? ((recentRevenue - recentYearAgoRevenue) / recentYearAgoRevenue) * 100
							: 0;

					console.log(
						`  Recent Quarter (${relevantStatements[0].date}): $${(recentRevenue / 1e9).toFixed(2)}B`
					);
					console.log(
						`  Year Ago Quarter (${relevantStatements[3].date}): $${(recentYearAgoRevenue / 1e9).toFixed(2)}B`
					);
					...
					if (relevantStatements.length >= 5) {
						...
						const previousGrowthRate =
							previousYearAgoRevenue !== 0
								? ((previousRevenue - previousYearAgoRevenue) /
										previousYearAgoRevenue) *
									100
								: 0;
			...
			if (analystRatings && analystRatings.totalAnalysts) {
				...
				console.log(
					`  Breakdown: Buy=${analystRatings.buy || 0}, Hold=${analystRatings.hold || 0}, Sell=${analystRatings.sell || 0}`
				);
			} else {
