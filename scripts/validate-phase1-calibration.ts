...
} from "../app/services/algorithms/SectorBenchmarks";
...
console.log(
	`  P/E Ratio: p25=${techBenchmarks.peRatio.p25}, median=${techBenchmarks.peRatio.median}, p75=${techBenchmarks.peRatio.p75}, max=${techBenchmarks.peRatio.max}`
);
console.log(
	`  PEG Ratio: p25=${techBenchmarks.pegRatio.p25}, median=${techBenchmarks.pegRatio.median}, p75=${techBenchmarks.pegRatio.p75}`
);
console.log();
...
console.log(
	`  P/E Ratio: p25=${financialBenchmarks.peRatio.p25}, median=${financialBenchmarks.peRatio.median}, p75=${financialBenchmarks.peRatio.p75}, max=${financialBenchmarks.peRatio.max}`
);
console.log();
...
console.log(
	`Old Generic Score (capped at 30x): ${oldPEScore.toFixed(3)} (${(oldPEScore * 100).toFixed(1)}%)`
);
...
if (newPEScore >= 0.6 && newPEScore <= 0.8) {
...
console.log(
	`New Sector-Aware Score: ${jpmNewScore.toFixed(3)} (${(jpmNewScore * 100).toFixed(1)}%)`
);
...
console.log(
	`New Sector-Aware Score: ${tslaNewScore.toFixed(3)} (${(tslaNewScore * 100).toFixed(1)}%)`
);
...
if (tslaNewScore <= 0.3) {
	...
	console.log(
		"⚠️ WARNING: TSLA P/E score higher than expected (still within tech max), got",
		tslaNewScore
	);
}
console.log();
...
const testCases = [
	{ name: "Undervalued Growth", pe: 20, growth: 0.3, expectedRange: [0.85, 1.0] },
	{ name: "Fair Value Growth (AAPL)", pe: 35, growth: 0.25, expectedRange: [0.7, 0.85] },
	{ name: "Overvalued Growth", pe: 40, growth: 0.1, expectedRange: [0.0, 0.4] },
];
...
testCases.forEach(test => {
	...
	console.log(
		`${status} ${test.name}: P/E ${test.pe}x / ${(test.growth * 100).toFixed(0)}% growth = PEG ${pegRatio.toFixed(2)} → Score ${score.toFixed(3)} (expected ${test.expectedRange[0]}-${test.expectedRange[1]})`
	);
});
console.log();
