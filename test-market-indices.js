...
async function testMarketIndices() {
	...
	console.log(
		`FMP_API_KEY: ${process.env.FMP_API_KEY ? "✅ Set (" + process.env.FMP_API_KEY.substring(0, 10) + "...)" : "❌ Not set"}`
	);
	...
	// Import services
	const { MarketIndicesService } = await import(
		"./app/services/financial-data/MarketIndicesService.ts"
	);
	...
	Object.entries(allData.sectors).forEach(([key, value]) => {
		if (value) {
			console.log(
				`  ${key.toUpperCase()}: ${value.changePercent}% (source: ${value.source})`
			);
		} else {
