...
async function testFallbackService() {
	...
	if (batchData.size > 0) {
		...
		batchData.forEach((data, symbol) => {
			console.log(
				`  ${symbol}: $${data.price} (${data.change >= 0 ? "+" : ""}${data.changePercent}%) via ${data.source}`
			);
		});
	} else {
	...
	status.forEach(source => {
		...
		if (source.dailyLimit) {
			console.log(
				`   Daily: ${source.dailyLimit.current}/${source.dailyLimit.limit} req/day`
			);
		}
	});
	...
	console.log(
		isHealthy ? "✅ At least one data source is healthy" : "❌ All data sources are down"
	);
