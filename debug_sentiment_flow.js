...
const { SentimentAnalysisService } = require('./app/services/financial-data/SentimentAnalysisService')
...
async function debugGMESentimentFlow() {
  ...
  try {
    ...
    const sentimentImpact = await sentimentService.analyzeStockSentimentImpact('GME', 'Technology', baseScore)
    ...
    if (sentimentImpact) {
      console.log('✅ GME Sentiment Impact:', {
        ...
        adjustmentPercent: ((sentimentImpact.adjustedScore - baseScore) * 100).toFixed(2) + '%'
      })
