...
async function testGREData() {
  ...
  try {
    ...
    if (response.data && !response.data.code) {
      ...
      results.push({ source: 'TwelveData', success: false, error: data.message || 'No data found' });
  ...
  try {
    ...
    if (error.response?.status === 404) {
      ...
      try {
        ...
        console.log(`  Search response: ${JSON.stringify(searchResponse.data).substring(0, 200)}`);
      } catch (searchError) {
    ...
    console.log(`❌ ERROR: ${error.response?.status === 404 ? 'Company not found in SEC database' : error.message}`);
  }
  ...
  const volumes = successful
    .filter(r => r.data.volume !== undefined)
    .map(r => r.data.volume);
