	...
	try {
		...
		for (const weightConfig of algorithmConfig.weights) {
			...
			try {
				...
				if (factorValue !== null && !isNaN(factorValue)) {
					...
					console.log(
						`✅ ${factor}: ${factorValue.toFixed(4)} × ${weight} = ${contribution.toFixed(4)}`
					);
				} else {
					weightedContributions.push({
						factor,
				...
				console.log(
					`❌ ${factor}: ERROR - ${error instanceof Error ? error.message : String(error)}`
				);
			}
		}
		...
		weightedContributions.forEach(contrib => {
			if (contrib.enabled && contrib.score !== null) {
				if (contrib.contribution > 0.1) {
					reasoning.push(
						`Strong positive contribution from ${contrib.factor} (${contrib.score.toFixed(3)})`
					);
				} else if (contrib.contribution < -0.1) {
					reasoning.push(
						`Strong negative contribution from ${contrib.factor} (${contrib.score.toFixed(3)})`
					);
				}
			}
		});
		...
		if (discrepancy > 5) {
			if (actualAPIScore > scaledScore) {
				possibleCause =
					"API using different algorithm or additional factors not in configuration";
			} else {
