	...
	try {
		...
		switch (action) {
			...
			case "set-preference":
				...
				const currentPreference: { fallbacks: string[] } | undefined = (
					currentPreferences as Record<string, any>
				)[dataType];

				if (currentPreference) {
					// Keep existing fallbacks, just change primary
					const fallbacks: DataSourceProvider[] = currentPreference.fallbacks.filter(
						(f: string) => f !== primary
					) as DataSourceProvider[];
					enhancedService.setDataSourcePreference(
						dataType,
						primary as DataSourceProvider,
						fallbacks
					);
				} else {
