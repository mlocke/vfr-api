...
const FactorDebugRequestSchema = z.object({
  ...
  factors: z.array(z.string()).optional().default([
    'pe_ratio', 'pb_ratio', 'roe', 'debt_equity', 'current_ratio',
    'quality_composite', 'value_composite', 'momentum_composite',
    'momentum_1m', 'momentum_3m', 'revenue_growth', 'earnings_growth'
  ])
})
...
export async function POST(request: NextRequest): Promise<NextResponse<FactorDebugResponse>> {
  try {
    ...
    console.log(`🧮 Calculating ${debugRequest.factors.length} factors for ${debugRequest.symbol}...`)
    ...
    for (const factorName of debugRequest.factors) {
      ...
      try {
        ...
        console.log(`❌ ${factorName}: ERROR - ${error instanceof Error ? error.message : String(error)} (${calculationTime}ms)`)
      }
    }
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        categories: {
          ...
          value: availableFactors.filter(f => f.includes('pe_ratio') || f.includes('pb_ratio') || f.includes('value')),
          quality: availableFactors.filter(f => f.includes('roe') || f.includes('debt') || f.includes('quality')),
          growth: availableFactors.filter(f => f.includes('growth')),
          technical: availableFactors.filter(f => f.includes('rsi') || f.includes('macd') || f.includes('sma')),
          volatility: availableFactors.filter(f => f.includes('volatility') || f.includes('atr')),
