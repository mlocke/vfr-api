...
export async function POST(request: NextRequest) {
	try {
		...
		console.log("🧪 Admin API: Testing data sources with direct API calls", {
			dataSourceIds,
			testType,
			timeout,
			maxRetries,
		...
		const invalidDataSources = dataSourceIds.filter(
			id => !DATA_SOURCE_CONFIGS[id as keyof typeof DATA_SOURCE_CONFIGS]
		);
		if (invalidDataSources.length > 0) {
			return NextResponse.json(
				{
					error: `Invalid data source IDs: ${invalidDataSources.join(", ")}`,
					success: false,
				},
				{ status: 400 }
			);
		...
		const testDataSource = async (dataSourceId: string): Promise<TestResult> => {
			const dataSourceConfig =
				DATA_SOURCE_CONFIGS[dataSourceId as keyof typeof DATA_SOURCE_CONFIGS];
			...
			try {
				...
				switch (testType) {
					...
					case "comprehensive":
						...
						const performanceTest = await testDataSourcePerformance(
							dataSourceId,
							timeout
						);

						success =
							connectionTest && !!dataTest && !!endpointsTest && !!performanceTest;
						testData = {
							connection: {
			...
			switch (dataSourceId) {
				...
				case "technical_indicators":
					...
					const response = await fetch(
						"http://localhost:3000/api/admin/test-technical-indicators",
						{
							method: "POST",
				...
				case "extended_market":
					...
					apiInstance = new ExtendedMarketDataService(
						new FinancialModelingPrepAPI() as unknown as any,
						new RedisCache()
					);
					break;
				case "reddit":
	...
	try {
		...
		switch (dataSourceId) {
			...
			case "options":
				...
				const optionsBasicAnalysis = await optionsDataService.getOptionsAnalysis(
					testSymbol,
					false
				);
				const optionsEnhancedAnalysis = await optionsDataService.getOptionsAnalysis(
					testSymbol,
					true
				);
				...
				const unicornBayIntegration = await testOptionsServiceUnicornBayIntegration(
					optionsDataService,
					testSymbol
				);
				...
				for (const symbol of additionalTestSymbols) {
					try {
						const enhancedResult = await optionsDataService.getOptionsAnalysis(
							symbol,
							true
						);
						timeBasedResults.push({
							...
							shortTermSentiment:
								enhancedResult?.timeBasedAnalysis?.shortTerm?.sentiment,
							mediumTermSignals:
								enhancedResult?.timeBasedAnalysis?.mediumTerm?.institutionalSignals
									?.length || 0,
							longTermConfidence:
								enhancedResult?.timeBasedAnalysis?.longTerm?.confidence,
							leapsAvailable:
								enhancedResult?.timeBasedAnalysis?.longTerm?.leapsAnalysis !==
								"No LEAPS available - limited long-term positioning data",
							strikePositioning: {
								heavyCallActivity:
									enhancedResult?.timeBasedAnalysis?.strikePositioning
										?.heavyCallActivity?.length || 0,
								heavyPutActivity:
									enhancedResult?.timeBasedAnalysis?.strikePositioning
										?.heavyPutActivity?.length || 0,
								institutionalHedges:
									enhancedResult?.timeBasedAnalysis?.strikePositioning
										?.institutionalHedges?.length || 0,
								unusualActivity:
									enhancedResult?.timeBasedAnalysis?.strikePositioning
				...
				testData = {
						...
						enhanced: optionsEnhancedAnalysis
							? {
									...
									hasTimeBasedAnalysis:
										!!optionsEnhancedAnalysis.timeBasedAnalysis,
									timeBasedFeatures: optionsEnhancedAnalysis.timeBasedAnalysis
										? {
												shortTermAnalysis: {
													...
													daysToExpiry:
														optionsEnhancedAnalysis.timeBasedAnalysis
															.shortTerm.daysToExpiry,
													confidence:
														optionsEnhancedAnalysis.timeBasedAnalysis
															.shortTerm.confidence,
													volumeRatio:
														optionsEnhancedAnalysis.timeBasedAnalysis
															.shortTerm.volumeRatio,
												},
												mediumTermAnalysis: {
													sentiment:
														optionsEnhancedAnalysis.timeBasedAnalysis
															.mediumTerm.sentiment,
													daysToExpiry:
														optionsEnhancedAnalysis.timeBasedAnalysis
															.mediumTerm.daysToExpiry,
													institutionalSignalsCount:
														optionsEnhancedAnalysis.timeBasedAnalysis
															.mediumTerm.institutionalSignals.length,
													institutionalSignals:
														optionsEnhancedAnalysis.timeBasedAnalysis
															.mediumTerm.institutionalSignals,
												},
												longTermAnalysis: {
													sentiment:
														optionsEnhancedAnalysis.timeBasedAnalysis
													...
													daysToExpiry:
														optionsEnhancedAnalysis.timeBasedAnalysis
															.longTerm.daysToExpiry,
													leapsAnalysis:
														optionsEnhancedAnalysis.timeBasedAnalysis
															.longTerm.leapsAnalysis,
													confidence:
														optionsEnhancedAnalysis.timeBasedAnalysis
															.longTerm.confidence,
												},
												strikePositioning: {
													heavyCallActivity:
														optionsEnhancedAnalysis.timeBasedAnalysis
															.strikePositioning.heavyCallActivity,
													heavyPutActivity:
														optionsEnhancedAnalysis.timeBasedAnalysis
															.strikePositioning.heavyPutActivity,
													institutionalHedges:
														optionsEnhancedAnalysis.timeBasedAnalysis
															.strikePositioning.institutionalHedges,
													unusualActivityCount:
														optionsEnhancedAnalysis.timeBasedAnalysis
															.strikePositioning.unusualActivity
															.length,
													unusualActivity:
														optionsEnhancedAnalysis.timeBasedAnalysis
															.strikePositioning.unusualActivity,
					...
					timeBasedValidation: {
						...
						summary: {
							...
							withTimeBasedAnalysis: timeBasedResults.filter(
								r => r.hasTimeBasedAnalysis
							).length,
							avgInstitutionalSignals:
								timeBasedResults
									.filter(r => r.success)
									.reduce((sum, r) => sum + (r.mediumTermSignals || 0), 0) /
								Math.max(1, timeBasedResults.filter(r => r.success).length),
							leapsAvailableCount: timeBasedResults.filter(r => r.leapsAvailable)
								.length,
						},
					},
			...
			case "enhanced":
				...
				const switchingRecommendations =
					await enhancedService.getSwitchingRecommendations();
				...
				testData = {
					...
					capabilities: {
						...
						availableProviders: Object.values(
							enhancedServiceStatus.providerStatus
			...
			case "extended_market":
				...
				const extendedMarketService = new ExtendedMarketDataService(
					extendedFMPAPI as unknown as any,
					extendedCache
				);
				...
				for (const symbol of testSymbolsExt) {
					try {
						const extendedData =
							await extendedMarketService.getExtendedMarketData(symbol);
						const bidAskSpread = await extendedMarketService.getBidAskSpread(symbol);
						const extendedScore = extendedData
							? extendedMarketService.calculateExtendedMarketScore(extendedData)
							: 0;
				...
				testData = {
					...
					summary: {
						...
						dataQuality:
							extendedResults.filter(r => r.success).length / testSymbolsExt.length,
						timestamp: Date.now(),
					},
			...
			case "reddit":
				...
				for (const symbol of redditTestSymbols) {
					try {
						...
						sentimentResults.push({
							...
							subredditBreakdown:
								data?.subredditBreakdown?.map(sub => ({
				...
				const totalSubreddits = successfulResults.reduce(
					(sum, r) => sum + (r.subredditCount || 0),
					0
				);
				const avgDiversityScore =
					successfulResults.length > 0
						? successfulResults.reduce((sum, r) => sum + (r.diversityScore || 0), 0) /
				...
				testData = {
					...
					enhancedMetrics: {
						...
						averageWeightedSentiment: successfulResults
							...
							.reduce(
								(sum, r, _, arr) => sum + (r.weightedSentiment || 0) / arr.length,
								0
							),
						totalPostsAnalyzed: successfulResults.reduce(
						...
						subredditCoverage:
							successfulResults.length > 0
								? Math.round((totalSubreddits / successfulResults.length) * 100) /
									100
								: 0,
					},
		...
		if (testData) {
			...
			testData.isRealData =
				[
					...
					"technical_indicators",
					"reddit",
				].includes(dataSourceId) && !testData.error;
		}
		...
		const endpointMappings: Record<string, any> = {
			...
			fmp: {
				...
				endpoints: [
					...
					{
						path: "/income-statement?symbol={symbol}",
						description: "Income statement",
						method: "GET",
					},
					{
						path: "/balance-sheet-statement?symbol={symbol}",
						description: "Balance sheet",
						method: "GET",
					},
					{
						path: "/cash-flow-statement?symbol={symbol}",
						description: "Cash flow statement",
						method: "GET",
					},
					{
						path: "/financial-ratios?symbol={symbol}",
						description: "Financial ratios",
						method: "GET",
					},
					{
						path: "/ratios-ttm?symbol={symbol}",
						description: "TTM financial ratios (P/B, ROE, ROA, etc.)",
						method: "GET",
					},
					{
						path: "/key-metrics-ttm?symbol={symbol}",
						description: "TTM key metrics (P/E, PEG, etc.)",
						method: "GET",
					},
					{
						path: "/upgrades-downgrades-consensus?symbol={symbol}",
						description: "Analyst consensus ratings",
						method: "GET",
					},
					{
						path: "/price-target-consensus?symbol={symbol}",
						description: "Price target consensus",
						method: "GET",
					},
					{
						path: "/price-target-latest-news?_symbol_={symbol}",
			...
			sec_edgar: {
				...
				endpoints: [
					...
					{
						path: "/api/xbrl/frames/us-gaap/{tag}/USD/{period}.json",
			...
			treasury: {
				...
				endpoints: [
					{
						path: "/services/api/fiscal_service/v1/accounting/od/debt_to_penny",
						description: "Daily treasury debt",
						method: "GET",
					},
					{
						path: "/services/api/fiscal_service/v1/accounting/od/avg_interest_rates",
						description: "Average interest rates",
						method: "GET",
					},
					{
						path: "/services/api/fiscal_service/v1/accounting/od/securities_sales",
						description: "Securities sales",
						method: "GET",
					},
				],
				authentication: "No API key required",
			...
			bls: {
				...
				endpoints: [
					{
						path: "/v2/timeseries/data/",
						description: "Time series data",
						method: "POST",
					},
					{
						path: "/v1/timeseries/data/{seriesid}",
						description: "Single series data",
						method: "GET",
					},
				],
			...
			eia: {
				...
				endpoints: [
					...
					{
						path: "/v2/natural-gas/pri/fut/data/",
					...
					},
					{
						...
						description: "Total energy statistics",
						method: "GET",
					},
				],
				authentication: "API Key required",
			...
			eodhd_unicornbay: {
				...
				endpoints: [
					{
						path: "/options/{symbol}.US",
						description: "UnicornBay Options Chain (40+ fields)",
						method: "GET",
					},
					{
						path: "/options/{symbol}.US?from={date}&to={date}",
						description: "Historical Options Data",
						method: "GET",
					},
					{
						path: "/options/iv-surface/{symbol}.US",
						description: "Implied Volatility Surface",
						method: "GET",
					},
					{
						path: "/options/greeks/{symbol}.US",
						description: "Options Greeks Analysis",
						method: "GET",
					},
					{
						path: "/options/volume-analysis/{symbol}.US",
						description: "Options Volume & Open Interest",
						method: "GET",
					},
					{
						path: "/options/put-call-ratio/{symbol}.US",
						description: "Put/Call Ratio Analysis",
						method: "GET",
					},
					{
						path: "/options/max-pain/{symbol}.US",
						description: "Max Pain Analysis",
						method: "GET",
					},
					{
						path: "/options/flow/{symbol}.US",
						description: "Unusual Options Activity",
						method: "GET",
					},
					{
						path: "/options/strategy-analysis/{symbol}.US",
						description: "Options Strategy Recommendations",
						method: "GET",
					},
					{
						path: "/options/earnings-impact/{symbol}.US",
						description: "Earnings Impact on Options",
						method: "GET",
					},
				],
			...
			market_indices: {
				...
				documentation:
					"Internal service that aggregates data from Polygon, TwelveData, FMP, Yahoo",
				...
				endpoints: [
					{
						...
						description: "Test technical indicators with symbols",
						method: "POST",
					},
					{
						path: "/TechnicalIndicatorService.calculateAllIndicators",
						description: "Calculate 50+ technical indicators",
						method: "GET",
					},
					{
						path: "/TechnicalIndicatorService.analyzeTrends",
						description: "Analyze price trends",
						method: "GET",
					},
					{
						path: "/TechnicalIndicatorService.detectPatterns",
						description: "Detect chart patterns",
						method: "GET",
					},
					{
						path: "/TechnicalIndicatorService.getMomentumSignals",
						description: "Get momentum indicators",
						method: "GET",
					},
				],
				authentication: "Internal service - no external API keys required",
				documentation:
					"Technical analysis service using existing market data and SimpleTechnicalTestService",
		...
		if (
			...
			for (let i = 0; i < 5; i++) {
				if (dataSourceId === "market_indices") {
					...
					requests.push(
						apiInstance
							.getEnhancedSentiment(testSymbol)
							.then((result: any) => ({
								request: i + 1,
								responseTime: Date.now() - startTime,
					...
					requests.push(
						apiInstance
							.getExtendedMarketData(testSymbol)
							.then((result: any) => ({
								request: i + 1,
								responseTime: Date.now() - startTime,
								success: !!result,
...
async function testUnicornBayEndpoint(timeout: number): Promise<boolean> {
	try {
		...
		if (response.ok) {
			...
			const hasUnicornBayFields =
				data &&
				Array.isArray(data) &&
				data.length > 0 &&
				data[0].hasOwnProperty("delta") &&
				data[0].hasOwnProperty("gamma");
	...
	try {
		...
		try {
			...
			results.featureTests.optionsChain = {
				...
				latency,
				contractCount:
					(optionsChain?.calls?.length || 0) + (optionsChain?.puts?.length || 0),
				hasGreeks:
					optionsChain?.calls?.[0]?.delta !== undefined ||
					optionsChain?.puts?.[0]?.delta !== undefined,
				hasIV:
					optionsChain?.calls?.[0]?.impliedVolatility !== undefined ||
					optionsChain?.puts?.[0]?.impliedVolatility !== undefined,
			};
		} catch (error) {
			results.featureTests.optionsChain = {
				success: false,
		...
		const successfulTests = Object.values(results.featureTests).filter(
		...
		results.overallStatus = {
			...
			limitations:
				successfulTests < totalTests
					? "Some features require paid UnicornBay subscription"
...
async function benchmarkUnicornBayPerformance(symbol: string, timeout: number): Promise<any> {
	...
	try {
		...
		tests.push(
			api
				.getOptionsAnalysisFreeTier(symbol)
...
async function assessUnicornBayDataQuality(symbol: string, timeout: number): Promise<any> {
	...
	try {
		...
		if (optionsChain && (optionsChain.calls || optionsChain.puts)) {
			...
			const completeContracts = allContracts.filter(
				contract =>
		...
		quality.overallScore =
			(quality.dataCompleteness + quality.fieldAccuracy + quality.timeliness) / 3;

		// Generate recommendations
		if (quality.dataCompleteness < 80) {
			quality.recommendations.push(
				"Consider upgrading to UnicornBay premium for more complete data"
			);
		}
		...
		for (const symbol of testSymbols) {
			...
			try {
				...
				results.push({
					...
					hasUnicornBayFields:
						optionsData?.calls?.[0]?.delta !== undefined ||
						optionsData?.puts?.[0]?.delta !== undefined,
				});
 ...
 * Test UnicornBay integration within OptionsDataService
 */
async function testOptionsServiceUnicornBayIntegration(
	optionsService: any,
	symbol: string
): Promise<any> {
	try {
