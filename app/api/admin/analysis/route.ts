 ...
 */
async function enhanceStockWithComprehensiveAnalysis(
	stock: StockData
): Promise<AdminAnalysisStockData> {
		...
		try {
			...
			if (vwapAnalysis) {
				enhancedStock.vwapAnalysis = {
					...
					deviationPercent:
						((vwapAnalysis.currentPrice - vwapAnalysis.vwap) / vwapAnalysis.vwap) * 100,
					strength:
						Math.abs(vwapAnalysis.currentPrice - vwapAnalysis.vwap) >
						vwapAnalysis.vwap * 0.02
							? "strong"
							: "moderate",
	...
	if (enhancedStock.technicalAnalysis?.score !== undefined) {
		weightedSum += enhancedStock.technicalAnalysis.score * 0.4;
		totalWeight += 0.4;
	}
	if (enhancedStock.macroeconomicAnalysis?.score !== undefined) {
		weightedSum += enhancedStock.macroeconomicAnalysis.score * 0.2;
		totalWeight += 0.2;
	}
	if (enhancedStock.sentimentAnalysis?.score !== undefined) {
		weightedSum += enhancedStock.sentimentAnalysis.score * 0.1;
		totalWeight += 0.1;
	}
	...
	if (enhancedStock.macroeconomicAnalysis)
		enhancedStock.primaryFactors.push("Macroeconomic Analysis");
...
function buildAnalysisInputServicesMetadata(stocks: AdminAnalysisStockData[]): any {
	...
	return {
		...
		sentimentAnalysis: {
			...
			status: AdminAnalysisServiceManager.isServiceAvailable("sentiment")
				? ("active" as const)
		...
		macroeconomicAnalysis: {
			...
			status: AdminAnalysisServiceManager.isServiceAvailable("macroeconomic")
				? ("active" as const)
				: ("unavailable" as const),
			...
			components: {
				...
				blsData: {
					enabled: !!process.env.BLS_API_KEY,
					coverage: "Employment & inflation",
					frequency: "monthly",
				},
				eiaData: {
					enabled: !!process.env.EIA_API_KEY,
		...
		esgAnalysis: {
			...
			status: AdminAnalysisServiceManager.isServiceAvailable("esg")
				? ("active" as const)
				: ("unavailable" as const),
		...
		shortInterestAnalysis: {
			...
			status: AdminAnalysisServiceManager.isServiceAvailable("shortInterest")
				? ("active" as const)
				: ("unavailable" as const),
			description: "FINRA data + squeeze detection algorithms",
			components: {
				shortInterestData: {
					enabled: !!process.env.FINRA_API_KEY,
		...
		vwapAnalysis: {
			...
			status: AdminAnalysisServiceManager.isServiceAvailable("vwap")
				? ("active" as const)
				: ("unavailable" as const),
		...
		extendedMarketData: {
			...
			status: AdminAnalysisServiceManager.isServiceAvailable("extendedMarket")
				? ("active" as const)
				: ("unavailable" as const),
	...
	try {
		...
		switch (adminRequest.mode) {
			...
			case "sector":
				...
				stocks = await financialDataService.getStocksBySector(
					adminRequest.sector,
					adminRequest.limit
				);
			...
			case "multiple":
				...
				stocks = await financialDataService.getMultipleStocks(
					adminRequest.symbols.slice(0)
				);
				stocks.forEach(stock => sources.add(stock.source));
				break;
			...
			data: {
				...
				metadata: {
					...
					technicalAnalysisEnabled:
						AdminAnalysisServiceManager.isServiceAvailable("technical"),
					fundamentalDataEnabled: true,
					analystDataEnabled: true,
					sentimentAnalysisEnabled:
						AdminAnalysisServiceManager.isServiceAvailable("sentiment"),
					macroeconomicAnalysisEnabled:
						AdminAnalysisServiceManager.isServiceAvailable("macroeconomic"),
					esgAnalysisEnabled: AdminAnalysisServiceManager.isServiceAvailable("esg"),
					shortInterestAnalysisEnabled:
						AdminAnalysisServiceManager.isServiceAvailable("shortInterest"),
					extendedMarketDataEnabled:
						AdminAnalysisServiceManager.isServiceAvailable("extendedMarket"),
