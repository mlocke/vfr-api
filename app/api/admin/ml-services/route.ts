  ...
  try {
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        avgResponseTime: mlServices.reduce((sum, s) => sum + (s.responseTime || 0), 0) / mlServices.length
      },
      timestamp: Date.now()
    })
    ...
    switch (action) {
      case 'test_services':
        ...
        const testResults = await Promise.allSettled(
          serviceIds.map(serviceId =>
            mlMonitoringService.testMLService(
              serviceId,
              testType || 'health'
            )
          )
        ...
        const results = testResults.map((result, index) => {
          if (result.status === 'fulfilled') {
            ...
            return {
              ...
              timestamp: Date.now(),
              testType: (testType || 'health') as 'health' | 'integration' | 'performance' | 'stress'
            }
          }
        })
      ...
      case 'performance_benchmark':
        ...
        return NextResponse.json({
          ...
          summary: {
            avgResponseTime: benchmarkResults.reduce((sum, r) => sum + (r.responseTime || 0), 0) / benchmarkResults.length,
