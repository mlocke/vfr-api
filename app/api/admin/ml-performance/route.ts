...
export async function GET(request: NextRequest) {
	try {
		...
		const systemStats = {
			...
			avgResponseTime:
				filteredMetrics.reduce((sum, m) => sum + m.metrics.averageLatency, 0) /
				(filteredMetrics.length || 1),
			totalRequests: filteredMetrics.reduce((sum, m) => sum + m.metrics.requestsPerMinute, 0),
			systemErrorRate:
				filteredMetrics.reduce((sum, m) => sum + m.metrics.errorRate, 0) /
				(filteredMetrics.length || 1),
			overallCacheHitRate:
				filteredMetrics.reduce((sum, m) => sum + m.metrics.cacheHitRate, 0) /
				(filteredMetrics.length || 1),
		};
...
async function runMLPerformanceBenchmark(config: any = {}) {
	...
	try {
		...
		results.systemPerformance.avgResponseTime =
			allResponseTimes.reduce((a, b) => a + b, 0) / allResponseTimes.length;
		results.systemPerformance.p95ResponseTime = results.serviceResults.reduce(
			(max, sr) => Math.max(max, sr.p95ResponseTime),
			0
		);
		results.systemPerformance.p99ResponseTime = results.serviceResults.reduce(
			(max, sr) => Math.max(max, sr.p99ResponseTime),
			0
		);
		results.systemPerformance.throughput = results.serviceResults.reduce(
			(sum, sr) => sum + sr.throughput,
			0
		);
		results.systemPerformance.errorRate =
			1 -
			results.serviceResults.reduce((sum, sr) => sum + sr.successRate, 0) /
				results.serviceResults.length;
...
function generatePerformanceInsights(metrics: any[], services: any[]) {
	...
	if (cacheMetrics.length > 0) {
		const avgCacheHitRate =
			cacheMetrics.reduce((sum, m) => sum + m.metrics.cacheHitRate, 0) / cacheMetrics.length;
	...
	if (healthyServices.length < totalEnabledServices.length) {
		...
		healthyServices.length === totalEnabledServices.length &&
		totalEnabledServices.length > 0
	) {
		insights.push({
			type: "success",
			category: "availability",
