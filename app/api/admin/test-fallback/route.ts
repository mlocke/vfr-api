...
export async function POST(request: NextRequest) {
	try {
		...
		const { symbols = ["AAPL", "GOOGL", "MSFT"], testType = "single", timeout = 10000 } = body;
		...
		switch (testType) {
			...
			case "stress":
				...
				results.stressTest = {
					...
					successRate:
						(stressResults.filter(r => r.success).length / stressResults.length) * 100,
					avgResponseTime:
						stressResults.reduce((acc, r) => acc + r.responseTime, 0) /
		...
		results.healthCheck = {
			healthy: isHealthy,
			message: isHealthy
				? "At least one data source is available"
