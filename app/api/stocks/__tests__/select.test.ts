...
describe("Enhanced Stock Selection API", () => {
	...
	describe("GET Health Check Endpoint", () => {
		test("should return health status with service availability", async () => {
			...
			console.log(
				`✅ Health check: ${Object.keys(result.data.services).length} services checked`
			);
		}, 15000);
		...
		test("should include provider health information", async () => {
			...
			if (result.success && result.data.providers) {
				...
				console.log(
					`✅ Provider health: ${healthyProviders}/${result.data.providers.length} providers healthy`
				);
			}
		}, 10000);
	});
	...
	describe("Single Stock Analysis (mode: single)", () => {
		test("should analyze single large-cap stock with enhanced data", async () => {
			...
			console.log(
				`✅ ${symbol} single analysis: Score=${stock.compositeScore}, Rec=${stock.recommendation}, Time=${analysisTime}ms`
			);
		}, 20000);
		...
		test("should include fundamental ratios when available", async () => {
			...
			if (stock.fundamentals) {
				...
				console.log(
					`✅ ${symbol} fundamentals: P/E=${stock.fundamentals.peRatio?.toFixed(2) || "N/A"}, ROE=${stock.fundamentals.roe?.toFixed(2) || "N/A"}`
				);
			}
		}, 20000);
		...
		test("should include analyst ratings when available", async () => {
			...
			if (stock.analystRating) {
				...
				console.log(
					`✅ ${symbol} analyst rating: ${stock.analystRating.consensus} (${stock.analystRating.totalAnalysts} analysts, sentiment=${stock.analystRating.sentimentScore})`
				);
			}
		}, 20000);
		...
		test("should include price targets when available", async () => {
			...
			if (stock.priceTarget) {
				...
				expect(stock.priceTarget.targetHigh).toBeGreaterThanOrEqual(
					stock.priceTarget.targetLow
				);
				...
				console.log(
					`✅ ${symbol} price target: $${stock.priceTarget.targetConsensus.toFixed(2)} (upside=${stock.priceTarget.upside?.toFixed(1) || "N/A"}%)`
				);
			}
		}, 20000);
		...
		test("should include technical analysis when sufficient data available", async () => {
			...
			if (stock.technicalAnalysis) {
				...
				expect(["bullish", "bearish", "neutral"]).toContain(
					stock.technicalAnalysis.trend.direction
				);
				...
				console.log(
					`✅ ${symbol} technical: Score=${stock.technicalAnalysis.score}, Trend=${stock.technicalAnalysis.trend.direction}, Signal=${stock.technicalAnalysis.momentum.signal}`
				);
			}
		}, 25000);
	});
	...
	describe("Multiple Stock Analysis (mode: multiple)", () => {
		test("should analyze multiple stocks with parallel processing", async () => {
			...
			console.log(
				`✅ Multiple analysis: ${result.data.stocks.length} stocks, ${analysisTime}ms`
			);
		}, 30000);
		...
		test("should handle mixed valid and invalid symbols gracefully", async () => {
			...
			const symbols = [
				TEST_SYMBOLS.largeCap[0],
				TEST_SYMBOLS.invalid[0],
				TEST_SYMBOLS.largeCap[1],
			];
			...
			console.log(
				`✅ Mixed symbols: ${results.length} valid stocks returned from ${symbols.length} requested`
			);
		}, 10000);
	});
	...
	describe("Composite Scoring Algorithm", () => {
		...
		test("should provide valid recommendations based on composite scores", async () => {
			...
			result.data.stocks.forEach((stock: any) => {
				...
				console.log(
					`✅ ${stock.symbol}: Score=${stock.compositeScore.toFixed(1)} → ${stock.recommendation}`
				);
			});
		}, 25000);
		...
		test("should factor in fundamental data when available", async () => {
			...
			if (stock.fundamentals) {
				...
				if (hasStrongFundamentals) {
					console.log(
						`✅ ${symbol} strong fundamentals reflected in score: ${stock.compositeScore.toFixed(1)}`
					);
				}
			}
		}, 20000);
		...
		test("should factor in analyst ratings when available", async () => {
			...
			if (stock.analystRating) {
				...
				if (stock.analystRating.consensus === "Strong Buy") {
					console.log(
						`✅ ${symbol} Strong Buy rating reflected in score: ${stock.compositeScore.toFixed(1)}`
					);
				}
			}
		}, 20000);
	...
	describe("Performance and Parallel Processing", () => {
		...
		test("should demonstrate parallel processing efficiency", async () => {
			...
			console.log(
				`✅ Parallel efficiency: ${parallelTime}ms vs estimated ${sequentialEstimate}ms sequential (${((1 - parallelTime / sequentialEstimate) * 100).toFixed(1)}% improvement)`
			);
		}, 40000);
		...
		test("should handle concurrent requests efficiently", async () => {
			...
			console.log(
				`✅ Concurrent requests: ${responses.length} requests in ${concurrentTime}ms`
			);
		}, 30000);
	});
	...
	describe("Error Handling and Graceful Degradation", () => {
		...
		test("should degrade gracefully when data sources are unavailable", async () => {
			...
			if (result.success) {
				...
				console.log(
					`✅ ${symbol} graceful degradation: Basic data=${!!stock.price}, Fundamentals=${!!stock.fundamentals}, Analyst=${!!stock.analystRating}, Technical=${!!stock.technicalAnalysis}`
				);
			} else {
				console.log(
					"✅ Service appropriately returned error when data sources unavailable"
				);
			}
		}, 20000);
	...
	describe("Data Quality and Integration", () => {
		...
		test("should validate numerical data ranges", async () => {
			...
			if (result.success) {
				...
				console.log(
					`✅ ${symbol} numerical validation: Price=$${stock.price.toFixed(2)}, Score=${stock.compositeScore.toFixed(1)}`
				);
			}
		}, 15000);
	});
	...
	describe("Memory Management and Cleanup", () => {
		test("should not leak memory during intensive analysis", async () => {
			...
			console.log(
				`✅ Memory management: ${memoryIncreaseMB.toFixed(1)}MB increase after ${symbols.length} analyses`
			);
		}, 60000);
	});
	...
	describe("Early Signal Detection Integration (Phase 4)", () => {
		test("should accept include_early_signal parameter", async () => {
			...
			console.log(
				`✅ Early signal parameter accepted, latency: ${result.data.metadata.early_signal_latency_ms}ms`
			);
		}, 30000);
		...
		test("should return early_signal field when requested", async () => {
			...
			if (stock.early_signal) {
				...
				console.log(
					`✅ ${symbol} early signal: ${stock.early_signal.upgrade_likely ? "UPGRADE" : "NO UPGRADE"} (confidence=${(stock.early_signal.confidence * 100).toFixed(1)}%)`
				);
			} else {
		...
		test("should handle multiple stocks with early signal predictions", async () => {
			...
			console.log(
				`✅ Multiple stocks early signal: ${predictionsCount}/${symbols.length} predictions, total time: ${analysisTime}ms, early signal latency: ${result.data.metadata.early_signal_latency_ms}ms`
			);
		}, 45000);
		...
		test("should complete early signal predictions within performance target", async () => {
			...
			console.log(
				`✅ ${symbol} early signal performance: ${result.data.metadata.early_signal_latency_ms}ms (target: <5s first call, <100ms cached)`
			);
		}, 35000);
