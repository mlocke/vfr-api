...
describe('Frontend Analysis API Security Tests', () => {
  ...
  beforeEach(() => {
    ...
    mockSecurityValidator.checkCircuitBreaker.mockReturnValue({ allowed: true, state: 'CLOSED' })
    mockSecurityValidator.validateSymbol.mockReturnValue({ isValid: true, sanitized: 'AAPL', errors: [] })
    mockSecurityValidator.validateSymbolBatch.mockReturnValue({ isValid: true, sanitized: '["AAPL"]', errors: [] })
    mockSecurityValidator.sanitizeErrorMessage.mockImplementation((error: any, isProd?: boolean) =>
  ...
  describe('OWASP A03: Injection Attack Prevention', () => {
    it('should prevent SQL injection in symbol input', async () => {
      const maliciousRequest = new NextRequest('http://localhost:3000/api/stocks/analysis-frontend', {
        method: 'POST',
    ...
    it('should prevent XSS in sector label input', async () => {
      const xssRequest = new NextRequest('http://localhost:3000/api/stocks/analysis-frontend', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          mode: 'sector',
    ...
    it('should prevent command injection in sector ID', async () => {
      const cmdInjectionRequest = new NextRequest('http://localhost:3000/api/stocks/analysis-frontend', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          mode: 'sector',
    ...
    it('should sanitize path traversal attempts in filename generation', async () => {
      ...
      const pathTraversalRequest = new NextRequest('http://localhost:3000/api/stocks/analysis-frontend', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          mode: 'single',
  ...
  describe('OWASP A04: Insecure Design - Rate Limiting', () => {
    ...
    it('should prevent DoS via large request payloads', async () => {
      const largeSymbolArray = Array(100).fill(undefined).map((_, i) => `SYM${i}`)
  ...
  describe('OWASP A05: Security Misconfiguration', () => {
    it('should validate request structure strictly', async () => {
      const invalidRequest = new NextRequest('http://localhost:3000/api/stocks/analysis-frontend', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          mode: 'single',
    ...
    it('should enforce timeout limits', async () => {
      const timeoutRequest = new NextRequest('http://localhost:3000/api/stocks/analysis-frontend', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          mode: 'single',
    ...
    it('should validate enum values strictly', async () => {
      const invalidModeRequest = new NextRequest('http://localhost:3000/api/stocks/analysis-frontend', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          mode: 'invalid_mode',
  ...
  describe('OWASP A09: Security Logging - Error Information Disclosure', () => {
    it('should sanitize error messages to prevent information disclosure', async () => {
      ...
      mockFs.mkdir.mockRejectedValue(new Error('Database connection failed: postgres://user:password@localhost:5432/db'))
    ...
    it('should not expose stack traces in production', async () => {
      ...
      mockFs.mkdir.mockRejectedValue(new Error('Internal system failure with sensitive data'))
  ...
  describe('Input Validation Security', () => {
    it('should validate business logic constraints', async () => {
      const invalidBusinessLogicRequest = new NextRequest('http://localhost:3000/api/stocks/analysis-frontend', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          mode: 'single',
    ...
    it('should validate symbol format with strict patterns', async () => {
      const invalidSymbolRequest = new NextRequest('http://localhost:3000/api/stocks/analysis-frontend', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          mode: 'single',
