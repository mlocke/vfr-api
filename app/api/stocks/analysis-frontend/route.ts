...
const FrontendAnalysisRequestSchema = z
	.object({
	...
	.refine(
		data => {
			...
			if (
				(data.mode === "single" || data.mode === "multiple") &&
		...
		{
			message:
				"Request must include sector for 'sector' mode or symbols for 'single'/'multiple' modes",
		}
	);
		...
		const fileContent = {
			metadata: {
				...
				serviceHealth: {
					totalServices: analysisResult.metadata?.analysisInputServices
						? Object.keys(analysisResult.metadata.analysisInputServices).length
						: 0,
					activeServices: analysisResult.metadata?.analysisInputServices
						? Object.values(analysisResult.metadata.analysisInputServices).filter(
								(s: any) => s.enabled
							).length
						: 0,
						...
						? Object.entries(analysisResult.metadata.analysisInputServices)
								.filter(
									([_, service]: [string, any]) =>
										service.enabled && service.errors?.length > 0
								)
								.map(([name]) => name)
						: [],
				},
			...
			detailedErrors: errorLogger
				? {
						...
						rateLimitIssues: errorLogger
							.getErrorsFor(undefined, undefined)
							.filter((e: any) => e.errorType === "RATE_LIMIT"),
						serviceIssues: errorLogger
							.getErrorsFor(undefined, undefined)
							.filter((e: any) => e.errorType === "API_UNAVAILABLE"),
						criticalIssues: errorLogger
							.getErrorsFor(undefined, undefined)
	...
	try {
		...
		if (analysisResult.errors && analysisResult.errors.length > 0) {
			analysisResult.errors.forEach((error: string) => {
				...
				const serviceMatch = error.match(
					/(Polygon|FMP|TwelveData|News|Reddit|FRED|BLS|EIA)/i
				);
		...
		if (analysisResult.metadata?.analysisInputServices) {
			Object.entries(analysisResult.metadata.analysisInputServices).forEach(
