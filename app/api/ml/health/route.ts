  ...
  try {
    ...
    const response = {
      ...
      summary: {
        ...
        healthyServices: Object.values(healthChecks).filter((check: any) => check.healthy).length,
        degradedServices: Object.values(healthChecks).filter((check: any) => !check.healthy).length
      }
    }
