...
export class MLFeatureToggleService {
  ...
  private async initializeFeatures(): Promise<void> {
    try {
      ...
      if (!existingStatus) {
        ...
        await this.cache.set(esdKey, JSON.stringify(initialStatus), MLFeatureToggleService.CACHE_TTL)
        this.inMemoryFallback.set(esdKey, initialStatus)

        console.log(`✅ Initialized ML feature: ${initialStatus.featureName} (default: ${initialStatus.enabled ? 'enabled' : 'disabled'})`)
      } else {
        // Load existing status into memory fallback
        const status = JSON.parse(existingStatus)
        this.inMemoryFallback.set(esdKey, status)
        console.log(`✅ Loaded ML feature: ${status.featureName} (current: ${status.enabled ? 'enabled' : 'disabled'})`)
      }
    } catch (error) {
  ...
  public async setFeatureEnabled(
    ...
    try {
      ...
      // Save to cache
      const key = this.getFeatureKey(featureId)
      await this.cache.set(key, JSON.stringify(updatedStatus), MLFeatureToggleService.CACHE_TTL)
      ...
      console.log(`✅ ML Feature ${enabled ? 'ENABLED' : 'DISABLED'}: ${currentStatus.featureName}`)
      ...
      throw new Error(`Failed to update feature toggle: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
  ...
  public async getAuditLog(featureId: string, limit: number = 100): Promise<ToggleAuditLog[]> {
    try {
      ...
      console.warn('Audit log retrieval is simplified - implement full SCAN pattern for production')
      return []
    } catch (error) {
