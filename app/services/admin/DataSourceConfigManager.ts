...
export class DataSourceConfigManager {
		...
		try {
			...
			if (persistedState && persistedState.enabledDataSources) {
				...
				console.log(
					...
					persistedState.enabledDataSources
				);
			} else {
				...
				console.log(
					"🔧 Initialized default data source states:",
					Array.from(this.enabledDataSources)
				);
	...
	async testDataSource(
		dataSourceId: string,
		testType: "connection" | "health" | "data_fetch" | "rate_limit" = "health"
	 ...
	 */
	async testDataSourceGroup(
		groupType: "commercial" | "government" | "free" | "all"
	): Promise<DataSourceGroupTestResult> {
		...
		const averageResponseTime =
			dataSourceResults.reduce((sum, r) => sum + r.responseTime, 0) /
			dataSourceResults.length;
	...
	private async testRateLimit(dataSourceId: string): Promise<any> {
		...
		const requests = Array(3)
			.fill(null)
			.map(() => this.testConnection(dataSourceId));
	...
	async validateAdminAccess(token: string): Promise<boolean> {
		try {
			...
			return (
				authService.hasPermission(user, "ADMINISTRATOR" as any) ||
				user.role === UserRole.ADMINISTRATOR
			);
		} catch (error) {
			if (process.env.NODE_ENV === "development") {
				console.log(
					"🔧 Auth validation failed in development, allowing access anyway:",
					error
				);
				return true;
			}
	...
	async toggleDataSource(dataSourceId: string): Promise<DataSourceToggleResult> {
		...
		try {
			...
			console.log(
				`🔄 Data source ${dataSourceId} toggled: ${wasEnabled ? "ENABLED" : "DISABLED"} → ${isNowEnabled ? "ENABLED" : "DISABLED"}`
			);
	...
	private async saveEnabledDataSources(): Promise<void> {
		try {
			...
			console.log(
				"✅ Data source enabled states saved:",
				Array.from(this.enabledDataSources)
			);
		} catch (error) {
		...
		try {
			if (typeof window === "undefined") {
				...
				try {
					...
					console.warn(
						"Failed to save to file, state will not persist across restarts:",
						fsError
					);
				}
			}
