...
export class MLMonitoringService {
  ...
  async getAllMLServices(): Promise<MLServiceInfo[]> {
    ...
    for (const service of services) {
      ...
      if (performanceData) {
        service.performance = { ...performanceData.metrics, uptime: Date.now() - performanceData.timestamp }
      }
  ...
  async testMLServiceGroup(): Promise<MLGroupTestResult> {
    ...
    const averageResponseTime = serviceResults.reduce((sum, r) => sum + r.responseTime, 0) / serviceResults.length
  ...
  private async getMLServiceHealth(serviceId: string): Promise<{
    ...
    try {
      ...
      switch (serviceId) {
        case 'ml_prediction':
          healthResult = this.mlPredictionService ? await this.mlPredictionService.healthCheck() : { success: false, error: 'Service not initialized' }
          break
  ...
  private async performMLHealthCheck(serviceId: string): Promise<any> {
    switch (serviceId) {
      case 'ml_prediction':
        return this.mlPredictionService ? await this.mlPredictionService.healthCheck() : { success: false, error: 'Service not initialized' }
    ...
    try {
      switch (serviceId) {
        case 'ml_prediction':
          ...
          const enhancementResult = this.mlPredictionService ? await this.mlPredictionService.enhanceVFRScore('AAPL', 75, testFactors) : null
          ...
          return {
            success: true,
            enhancementWorking: enhancementResult.enhancedScore !== enhancementResult.vfrWeight * 75,
        ...
        case 'feature_engineering':
          ...
          const features = this.featureEngineeringService ? await this.featureEngineeringService.generateFeatures(['AAPL', 'TSLA']) : new Map()

          if (!this.featureEngineeringService) {
            return { success: false, error: 'Feature Engineering Service not initialized' }
          }
      ...
      const promises = Array(concurrentRequests).fill(null).map(async (_, index) => {
        ...
        try {
          switch (serviceId) {
            case 'ml_prediction':
              if (this.mlPredictionService) {
                await this.mlPredictionService.enhanceVFRScore(`TEST${index}`, 75, {})
              }
              break
            case 'feature_engineering':
              if (this.featureEngineeringService) {
                await this.featureEngineeringService.generateFeatures([`TEST${index}`])
              }
              break
          }
      ...
      if (this.enabledServices.has(serviceId)) {
        ...
        metrics.push({
          ...
          metrics: {
            ...
            featureGenerationTime: serviceId === 'feature_engineering' ? (service.responseTime || 0) : 0,
            mlEnhancementTime: serviceId === 'ml_prediction' ? (service.responseTime || 0) : 0,
   ...
   */
  async toggleMLService(serviceId: string): Promise<{ success: boolean; enabled: boolean; message: string }> {
    const service = this.mlServices.get(serviceId)
    if (!service) {
      return {
        success: false,
