...
export class SelectionCache {
	 ...
	 */
	async cacheMarketData(symbol: string, data: any, source: string): Promise<boolean> {
	...
	async warmCache(
		...
		console.log(
			`🔥 Intelligent cache warming for ${symbols.length} symbols and ${sectors.length} sectors...`
		);
		...
		const warmingTasks = [
			...
			...this.createBatchedTasks(
				prioritizedSymbols.slice(0, 20),
				symbol => this.warmStockData(symbol, options),
				BATCH_SIZE,
				MAX_CONCURRENT
			),
			...
			...this.createBatchedTasks(
				prioritizedSectors,
				sectorId => this.warmSectorData(sectorId, options),
				BATCH_SIZE,
				MAX_CONCURRENT
			),
		];
	...
	private initializeCacheStrategies(): void {
		const strategies: { [name: string]: CacheStrategy } = {
			single_stock: {
				name: "single_stock",
				ttl: 60000, // 60 seconds (1 minute) - matches stock_price cache TTL
	...
	private selectStockResultStrategy(symbol: string, result: EnhancedStockResult): string {
		...
		if (result.context.marketCap > 100000000000) {
			// Large cap
			return "single_stock";
		} else {
	...
	private async storeInHierarchy(
		...
		if (strategy.priorityLevel === "high") {
			storePromises.push(
				this.redisCache
					.set(key, entry, strategy.ttl / 1000, {
	...
	private createBatchedTasks<T>(
		...
		return batches.map(async batch => {
			const semaphore = Array(maxConcurrent)
				.fill(null)
				.map(() => Promise.resolve());
			let currentIndex = 0;
