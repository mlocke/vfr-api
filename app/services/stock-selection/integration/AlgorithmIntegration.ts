...
export class AlgorithmIntegration implements AlgorithmIntegrationInterface {
	 ...
	 */
	private async buildAlgorithmConfiguration(
		...
		if (!baseConfig) {
			...
			baseConfig = await this.configManager.getConfiguration(
				serviceConfig.fallbackAlgorithmId
			);
		}
		...
		const customizedConfig: AlgorithmConfiguration = {
			...
			dataFusion: {
				...
				minQualityScore:
					options?.dataPreferences?.minQualityScore ||
					baseConfig.dataFusion.minQualityScore,
				requiredSources:
					options?.dataPreferences?.sources || baseConfig.dataFusion.requiredSources,
	...
	private buildAlgorithmContext(request: SelectionRequest): AlgorithmContext {
		...
		return {
			algorithmId:
				options?.algorithmId || this.selectionConfig.getConfig().defaultAlgorithmId,
	...
	private buildDataStatus(options?: SelectionOptions): any {
		...
		if (serviceConfig.dataSources) {
			...
			serviceConfig.getDataSourceConfig &&
			typeof serviceConfig.getDataSourceConfig === "function"
		) {
	 ...
	 */
	private enhanceSelectionResult(
	 ...
	 */
	private async buildFallbackConfiguration(
		request: SelectionRequest
	): Promise<AlgorithmConfiguration> {
