...
export class SectorIntegration implements SectorIntegrationInterface {
	...
	constructor(financialDataService: FinancialDataService, selectionConfig?: any) {
	...
	private async getIndustryStocks(sector: SectorOption): Promise<string[]> {
		const classification =
			SECTOR_CLASSIFICATIONS[sector.id as keyof typeof SECTOR_CLASSIFICATIONS];
	 ...
	 */
	private async getPolygonSectorStocks(
		sector: SectorOption,
		classification: any
	): Promise<string[]> {
		try {
	 ...
	 */
	private async getWebSearchSectorStocks(
		sector: SectorOption,
		classification: any
	): Promise<string[]> {
		try {
	...
	private extractSymbolsFromText(data: any): string[] {
		...
		if (Array.isArray(data)) {
			text = data
				.map(item => item.markdown || item.content || JSON.stringify(item))
				.join(" ");
