...
export class StockSelectionService extends EventEmitter implements DataIntegrationInterface {
	...
	constructor(
		...
		if (technicalService) {
			...
			const enhancedFactorLibrary = new FactorLibrary(
				cache,
				technicalService,
				this.vwapService
			);
	...
	async selectStocks(request: SelectionRequest): Promise<SelectionResponse> {
		...
		try {
			...
			// Apply timeout
			const timeout =
				request.options?.timeout || this.config.getConfig().limits.defaultTimeout;
	 ...
	 */
	private async executeSingleStockAnalysis(
		...
		if (cached) {
			// CRITICAL: Always validate cached price against current price
			// Stock prices can change frequently while analysis cache is still valid
			console.log(`🔍 Cache HIT for ${symbol} - validating price freshness...`);

			try {
				const currentStockData = await this.financialDataService.getStockPrice(symbol);
				const cachedPrice = cached.score?.marketData?.price;
				const currentPrice = currentStockData?.price;

				if (cachedPrice && currentPrice && cachedPrice !== currentPrice) {
					const priceChange = (
						((currentPrice - cachedPrice) / cachedPrice) *
						100
					).toFixed(2);
					console.log(
						`🔍 Price changed for ${symbol}: $${cachedPrice} → $${currentPrice} (${priceChange}%) - invalidating cache`
					);

					// Price changed - invalidate cache and continue to fresh analysis
					this.emitEvent({
						type: "cache_miss",
						timestamp: Date.now(),
						requestId,
					...
					// DO NOT return cached - fall through to fresh analysis below
				} else if (cachedPrice === currentPrice) {
					console.log(
						`🔍 Price unchanged for ${symbol} ($${currentPrice}) - returning cached result`
					);
					this.emitEvent({
						type: "cache_hit",
						timestamp: Date.now(),
					...
					console.warn(
						`⚠️ Could not validate price for ${symbol} - returning cached result`
					);
					this.emitEvent({
						type: "cache_hit",
						timestamp: Date.now(),
				...
				// If price fetch fails, use cached result but log warning
				console.warn(`⚠️ Failed to fetch current price for ${symbol}:`, error);
				console.log(`🔍 Using cached result despite price validation failure`);
				this.emitEvent({
					type: "cache_hit",
					timestamp: Date.now(),
					requestId,
		...
		console.log(
			`🔍 No cache or price invalidated, proceeding with fresh analysis for ${symbol}`
		);
		...
		const enhancedResult = await this.enhanceSingleStockResult(
			symbol,
			algorithmResult,
			request
		);
	 ...
	 */
	private async executeSectorAnalysis(
		request: SelectionRequest,
		requestId: string
	): Promise<any> {
		const { scope } = request;
	 ...
	 */
	private async executeMultiStockAnalysis(
		request: SelectionRequest,
		requestId: string
	): Promise<any> {
		const { scope } = request;
	...
	private async enhanceSingleStockResult(
		...
		if (this.optionsService) {
			try {
				...
				if (optionsAnalysis) {
					console.log(
						`📈 Options analysis completed for ${symbol}: P/C ratio ${optionsAnalysis.currentRatio?.volumeRatio || "N/A"}`
					);
				}
			} catch (error) {
		...
		const scoreBasedAction = getRecommendation(
			...
			additionalData.analystData as AnalystData
		);
	...
	private async buildSectorAnalysis(
		...
		const topSelections = await Promise.all(
			algorithmResult.selections.slice(0, 10).map(async selection => {
				return await this.enhanceSingleStockResult(
					selection.symbol,
					{
						...algorithmResult,
	...
	private async buildResponse(
		...
		return {
			...
			singleStock:
				request.scope.mode === SelectionMode.SINGLE_STOCK ? analysisResult : undefined,
			sectorAnalysis: [
				SelectionMode.SECTOR_ANALYSIS,
				SelectionMode.INDEX_ANALYSIS,
				SelectionMode.ETF_ANALYSIS,
			].includes(request.scope.mode)
				? analysisResult
				: undefined,
			multiStockAnalysis:
				request.scope.mode === SelectionMode.MULTIPLE_STOCKS ? analysisResult : undefined,
			...
			metadata: {
				algorithmUsed:
					request.options?.algorithmId || this.config.getConfig().defaultAlgorithmId,
	 ...
	 */
	private async buildAnalysisInputServiceStatus(
		...
		const serviceStatuses = {
			...
			technicalAnalysis: {
				...
				utilizationInResults: this.calculateServiceUtilization(
					topSelections,
					"technicalAnalysis"
				),
	 ...
	 */
	private calculateServiceUtilization(
		topSelections: EnhancedStockResult[],
		serviceKey: string
	): string {
		...
		topSelections.forEach(selection => {
			...
			if (reasoning && reasoning.primaryFactors) {
				hasServiceData = reasoning.primaryFactors.some(
					factor =>
						factor.toLowerCase().includes(serviceKey.toLowerCase()) ||
						factor
							.toLowerCase()
							.includes(serviceKey.replace("Analysis", "").toLowerCase())
				);
			}
			...
			if (!hasServiceData && selection.score && selection.score.factorScores) {
				...
				switch (serviceKey) {
					case "technicalAnalysis":
						hasServiceData = Object.keys(factorScores).some(
							factor =>
								factor.includes("rsi") ||
					...
					case "fundamentals":
						hasServiceData = Object.keys(factorScores).some(
							factor =>
								factor.includes("pe_ratio") ||
								factor.includes("pb_ratio") ||
								factor.includes("roe") ||
								factor.includes("debt_equity") ||
								factor.includes("quality") ||
								factor.includes("value") ||
								factor.includes("current_ratio") ||
								factor.includes("revenue_growth")
						);
						break;
					case "sentimentAnalysis":
						hasServiceData = Object.keys(factorScores).some(
							factor =>
								factor.includes("sentiment") ||
								factor.includes("news") ||
								factor.includes("social")
						);
						break;
					case "macroeconomicAnalysis":
						hasServiceData = Object.keys(factorScores).some(
							factor =>
								factor.includes("macro") ||
								factor.includes("economic") ||
								factor.includes("fred") ||
								factor.includes("bls") ||
								factor.includes("eia") ||
								factor === "macroeconomic_sector_impact" ||
								factor === "macroeconomic_composite" // 🆕 DIRECT FACTOR TRACKING
						);
						break;
					case "vwapAnalysis":
						hasServiceData = Object.keys(factorScores).some(
					...
					case "esgAnalysis":
						hasServiceData = Object.keys(factorScores).some(
							factor =>
								factor.includes("esg") ||
								factor === "esg_composite" ||
								factor === "esg_environmental" ||
								factor === "esg_social" ||
					...
					case "extendedMarketData":
						hasServiceData = Object.keys(factorScores).some(
							factor =>
								factor.includes("premarket") ||
								factor.includes("afterhours") ||
								factor.includes("extended") ||
								factor === "extended_hours_activity"
						);
						break;
					case "optionsAnalysis":
						hasServiceData = Object.keys(factorScores).some(
							factor =>
								factor.includes("options") ||
								factor === "options_composite" ||
								factor === "options_flow_score" ||
								factor === "put_call_ratio" ||
								factor.includes("max_pain") ||
								factor.includes("implied_volatility")
						);
						break;
				}
			}
			...
			if (!hasServiceData && serviceKey === "technicalAnalysis" && selection.score) {
				...
				if (
					selection.score.factorScores.momentum_composite &&
			...
			if (!hasServiceData && serviceKey === "fundamentals" && selection.score) {
				...
				if (
					(selection.score.factorScores.quality_composite &&
			...
			if (!hasServiceData && serviceKey === "vwapAnalysis" && selection.score) {
				if (
					(selection.score.factorScores.vwap_deviation_score &&
			...
			if (!hasServiceData && serviceKey === "macroeconomicAnalysis" && selection.score) {
				if (
					(selection.score.factorScores.macroeconomic_sector_impact &&
			...
			if (!hasServiceData && serviceKey === "esgAnalysis" && selection.score) {
				if (
					(selection.score.factorScores.esg_composite &&
			...
			if (!hasServiceData && serviceKey === "shortInterestAnalysis" && selection.score) {
				if (
					(selection.score.factorScores.short_interest_composite &&
		...
		console.log(
			`🔍 Service utilization for ${serviceKey}: ${utilizationCount}/${topSelections.length} = ${utilizationPercentage}%`
		);
		...
		if (["vwapAnalysis", "shortInterestAnalysis", "institutionalData"].includes(serviceKey)) {
			console.log(
				`🔍 ${serviceKey} utilization debug: ${utilizationCount}/${topSelections.length} stocks detected`
			);
			topSelections.slice(0, 2).forEach((selection, idx) => {
				const factorKeys = selection.score?.factorScores
					? Object.keys(selection.score.factorScores)
					: [];
				const relevantFactors = factorKeys.filter(f => {
					if (serviceKey === "vwapAnalysis")
						return f.includes("vwap") || f.includes("volume") || f.includes("obv");
					if (serviceKey === "shortInterestAnalysis") return f.includes("short");
					if (serviceKey === "institutionalData")
						return f.includes("institutional") || f.includes("insider");
					return false;
				});
	...
	private calculateAnalystCoverage(topSelections: EnhancedStockResult[]): string {
		...
		topSelections.forEach(selection => {
			if (
				selection.dataQuality.sourceBreakdown?.analystRatings &&
	...
	private calculatePriceTargetCoverage(topSelections: EnhancedStockResult[]): string {
		...
		topSelections.forEach(selection => {
			if (
				selection.dataQuality.sourceBreakdown?.priceTargets &&
	...
	private calculateInstitutionalUtilization(topSelections: EnhancedStockResult[]): string {
		...
		topSelections.forEach(selection => {
			...
			const hasInstitutionalSentiment =
				selection.score?.factorScores?.["institutional_sentiment"] !== undefined &&
				selection.score?.factorScores?.["institutional_sentiment"] !== 0.5;
			const hasInsiderActivity =
				selection.score?.factorScores?.["insider_activity"] !== undefined &&
				selection.score?.factorScores?.["insider_activity"] !== 0.5;
			const hasInstitutionalComposite =
				selection.score?.factorScores?.["institutional_composite"] !== undefined &&
				selection.score?.factorScores?.["institutional_composite"] !== 0.5;

			// Check if ANY institutional factors are present and non-neutral
			const hasAnyInstitutionalFactors =
				selection.score?.factorScores &&
				Object.keys(selection.score.factorScores).some(
					factor =>
			...
			const hasFMPInstitutionalData =
				selection.score?.dataQuality?.source === "fmp_institutional" ||
				(selection.score?.timestamp &&
					selection.score.factorScores &&
					hasAnyInstitutionalFactors);

			// 🔄 FALLBACK: Check text-based indicators for SEC EDGAR fallback
			const hasInstitutionalWarnings =
				selection.reasoning?.warnings?.some(
					warning =>
						warning.toLowerCase().includes("institution") ||
						warning.toLowerCase().includes("insider")
				) || false;

			const hasInstitutionalOpportunities =
				selection.reasoning?.opportunities?.some(
					opportunity =>
						opportunity.toLowerCase().includes("institution") ||
						opportunity.toLowerCase().includes("insider")
				) || false;
			...
			const hasSentimentWithInstitutionalData =
				selection.score?.factorScores?.sentiment_composite !== undefined;

			if (
				hasInstitutionalSentiment ||
				hasInsiderActivity ||
				hasInstitutionalComposite ||
				hasAnyInstitutionalFactors ||
				hasFMPInstitutionalData ||
				hasInstitutionalWarnings ||
				hasInstitutionalOpportunities ||
	...
	private calculateExtendedMarketUtilization(topSelections: EnhancedStockResult[]): string {
		...
		topSelections.forEach(selection => {
			...
			const hasNonNeutralExtendedFactors =
				selection.score?.factorScores &&
				((selection.score.factorScores.extended_hours_activity &&
					selection.score.factorScores.extended_hours_activity !== 0.5) ||
					(selection.score.factorScores.premarket_momentum &&
			...
			if (
				hasExtendedMarketContext ||
				hasExtendedMarketFactors ||
				hasNonNeutralExtendedFactors
			) {
				utilizationCount++;
			}
		});
		...
		if (symbols.length <= chunkSize) {
			...
			for (let i = 0; i < symbols.length; i += chunkSize) {
				...
				const chunkPromises = chunk.map(symbol =>
					this.fetchSingleStockData(symbol, options)
				);
	...
	async healthCheck(): Promise<{ status: "healthy" | "unhealthy"; details: any }> {
		try {
			...
			const dataServiceHealthy = await this.financialDataService
				.healthCheck()
				.catch(() => false);
	 ...
	 */
	private validateAndPrepareSymbol(symbol: string): {
	...
	private async getCachedCompanyInfo(sanitizedSymbol: string): Promise<any> {
		...
		try {
			...
			console.warn(
				`Failed to get cached company info for ${sanitizedSymbol}, fetching directly`
			);
	 ...
	 */
	private extractDataFromResults(
		...
		return results.map((result, index) => {
			if (result.status === "fulfilled") {
				...
				console.warn(
					`Failed to fetch ${dataType} for ${sanitizedSymbol}: ${sanitizedError}`
				);
				return null;
			}
		});
	...
	private validateCoreData(rawData: any[], serviceId: string, sanitizedSymbol: string): void {
		...
		if (!companyInfo) {
			console.warn(
				`Missing company info for ${sanitizedSymbol} - continuing with available data`
			);
		}
		if (!marketData) {
			console.warn(
				`Missing market data for ${sanitizedSymbol} - continuing with available data`
			);
		}
	}
	...
	private transformStockData(rawData: any[], sanitizedSymbol: string): any {
		...
		return {
			...
			fundamentalRatios: fundamentalRatios
				? this.validateFundamentalRatios(fundamentalRatios, sanitizedSymbol)
				: null,
			...
			sourceBreakdown: this.createSourceBreakdown(
				stockPrice,
				companyInfo,
				marketData,
				fundamentalRatios,
	...
	private calculatePriceChanges(
		...
		marketData: any,
		companyInfo: any
	): { priceChange24h: number; volumeChange24h: number } {
		const priceChange24h = stockPrice?.change || 0;
		const volumeChange24h =
			marketData?.volume && companyInfo?.marketCap
	...
	private validateFundamentalRatiosStructure(fundamentalRatios: any, symbol: string): boolean {
		...
		if (!responseValidation.isValid) {
			console.warn(
				`Invalid fundamental ratios structure for ${symbol}: ${responseValidation.errors.join(", ")}`
			);
			return false;
		}
	...
	private async fetchAdditionalStockData(
		symbol: string,
	...
	private generateAnalystInsights(additionalData: any): string[] {
		...
		const { consensus, totalAnalysts, sentimentScore, distribution } =
			additionalData.analystData;
		...
		if (additionalData.priceTargets) {
			...
			insights.push(
				`Price target: $${target} (${upside > 0 ? "+" : ""}${upside?.toFixed(1)}% upside)`
			);
		}

		// Distribution insights
		const buyPercent = (
			((distribution.strongBuy + distribution.buy) / totalAnalysts) *
			100
		).toFixed(0);
	...
	private generateMacroeconomicInsights(macroImpact: any): string[] {
		...
		if (macroImpact.sectorImpact) {
			...
			if (Math.abs(sector.economicSensitivity.interestRates) > 0.5) {
				const direction =
					sector.economicSensitivity.interestRates > 0 ? "benefits from" : "hurt by";
				sensitivities.push(`${direction} interest rate changes`);
			}
			if (Math.abs(sector.economicSensitivity.inflation) > 0.5) {
				const direction =
					sector.economicSensitivity.inflation > 0 ? "benefits from" : "hurt by";
		...
		if (macroImpact.adjustedScore && macroImpact.macroWeight) {
			const adjustment =
				macroImpact.adjustedScore -
				macroImpact.adjustedScore / (1 - macroImpact.macroWeight + macroImpact.macroWeight);
			if (Math.abs(adjustment) > 0.05) {
				const direction = adjustment > 0 ? "boosted" : "reduced";
				insights.push(
					`Macro factors ${direction} score by ${(Math.abs(adjustment) * 100).toFixed(1)}%`
				);
			}
		}
	...
	private generateSentimentInsights(sentimentImpact: any): string[] {
		...
		if (sentimentImpact.adjustedScore && sentimentImpact.sentimentWeight) {
			const baseScore =
				sentimentImpact.adjustedScore /
				(1 - sentimentImpact.sentimentWeight + sentimentImpact.sentimentWeight);
			const adjustment = sentimentImpact.adjustedScore - baseScore;
			if (Math.abs(adjustment) > 0.02) {
				const direction = adjustment > 0 ? "boosted" : "reduced";
				insights.push(
					`Sentiment factors ${direction} score by ${(Math.abs(adjustment) * 100).toFixed(1)}%`
				);
			}
		}
	...
	private calculateCacheHitRate(): number {
		...
		console.log(
			`Actual cache hit rate: ${totalHits}/${totalRequests} = ${(hitRate * 100).toFixed(1)}%`
		);
		return hitRate;
	}
	...
	private calculateOverallQualityScore(selections: EnhancedStockResult[]): QualityScore {
		...
		const avgQuality =
			selections.reduce((sum, s) => sum + s.dataQuality.overall.overall, 0) /
			selections.length;
	...
	private updateStats(
		...
		if (success) {
			this.stats.averageExecutionTime =
				(this.stats.averageExecutionTime + response.executionTime) / 2;
		}

		this.stats.successRate =
			(this.stats.successRate * (this.stats.totalRequests - 1) + (success ? 1 : 0)) /
			this.stats.totalRequests;
	}
	...
	static async createOptimized(
		...
		try {
			sentimentService = new (
				await import("../financial-data/SentimentAnalysisService")
			).default(cache);
		...
		if (availableApiKeys.has("macroeconomic")) {
			try {
				macroeconomicService = new (
					await import("../financial-data/MacroeconomicAnalysisService")
		...
		if (sharedFmpAPI) {
			try {
				vwapService = new (await import("../financial-data/VWAPService")).VWAPService(
					sharedFmpAPI,
					cache
				);
		...
		if (availableApiKeys.has("esg")) {
			try {
				const { default: ESGDataService } = await import(
					"../financial-data/ESGDataService"
				);
		...
		if (availableApiKeys.has("finra") || availableApiKeys.has("polygon")) {
			try {
				const { default: ShortInterestService } = await import(
		...
		if (sharedFmpAPI) {
			try {
				extendedMarketService = new (
					await import("../financial-data/ExtendedMarketDataService")
				).ExtendedMarketDataService(sharedFmpAPI, cache);
		...
		try {
			optionsService = new (
				await import("../financial-data/OptionsDataService")
			).OptionsDataService();
		...
		if (enableMLEnhancement) {
			try {
				mlPredictionService = new (
					await import("../ml/prediction/MLPredictionService")
				).MLPredictionService();
		...
		console.log(
			`   🧠 Memory footprint: ${Math.round(config.memoryFootprint / 1024 / 1024)}MB`
		);
		...
		if (health.status === "unhealthy") {
			console.warn(
				"⚠️ StockSelectionService initialized with unhealthy dependencies:",
				health.details
			);
		}
