...
describe('Options Data Integration into Analysis Engine', () => {
  ...
  describe('Options Data Pre-fetch Integration', () => {
      ...
      try {
        ...
        if (optionsAnalysis && optionsChain) {
          ...
          const optionsData: OptionsDataPoint = {
            ...
            optionsFlow: {
              sentiment: optionsAnalysis.sentiment === 'greed' ? 0.5 :
                        optionsAnalysis.sentiment === 'fear' ? -0.5 : 0,
          ...
          console.log(`   Options Flow Sentiment: ${optionsData.optionsFlow?.sentiment.toFixed(3)}`)
          console.log(`   Total Volume: ${optionsData.optionsFlow?.volume.toLocaleString()}`)
          console.log(`   Total OI: ${optionsData.optionsFlow?.openInterest.toLocaleString()}`)
          console.log(`   Processing Time: ${processingTime}ms`)

          // Performance validation
          expect(processingTime).toBeLessThan(10000) // Should complete within 10 seconds
        } else {
          console.warn(`⚠️ Options data not available for ${INDEX_SYMBOL} - test will validate fallback behavior`)
        }
      } catch (error) {
    ...
    test('should_calculate_options_composite_score_with_weighted_factors', async () => {
      ...
      if (putCallRatioScore !== null) {
        ...
        console.log(`📊 P/C Ratio Score: ${putCallRatioScore.toFixed(3)} (from ratio: ${testOptionsData.putCallRatio})`)
      }

      if (ivPercentileScore !== null) {
        expect(ivPercentileScore).toBeGreaterThanOrEqual(0)
        expect(ivPercentileScore).toBeLessThanOrEqual(1)
        console.log(`📊 IV Percentile Score: ${ivPercentileScore.toFixed(3)} (from percentile: ${testOptionsData.impliedVolatilityPercentile}%)`)
      }

      if (optionsFlowScore !== null) {
        expect(optionsFlowScore).toBeGreaterThanOrEqual(0)
        expect(optionsFlowScore).toBeLessThanOrEqual(1)
        console.log(`📊 Options Flow Score: ${optionsFlowScore.toFixed(3)} (from sentiment: ${testOptionsData.optionsFlow?.sentiment})`)
      }
      ...
      if (volumeDivergenceScore !== null) {
        ...
        console.log(`📊 Volume Divergence Score: ${volumeDivergenceScore.toFixed(3)} (from ratio: ${testOptionsData.volumeDivergence})`)
      }
      ...
      if (optionsComposite !== null) {
        ...
        const expectedComposite = (
          (putCallRatioScore || 0.5) * 0.30 +
          (ivPercentileScore || 0.5) * 0.25 +
          (optionsFlowScore || 0.5) * 0.20 +
          (greeksScore || 0.5) * 0.15 +
          (volumeDivergenceScore || 0.5) * 0.10
        )

        // Allow for some tolerance in calculation
        const tolerance = 0.05
        expect(Math.abs(optionsComposite - expectedComposite)).toBeLessThan(tolerance)
        console.log(`✅ Composite score validation: Expected ${expectedComposite.toFixed(3)}, Got ${optionsComposite.toFixed(3)}`)
      } else {
        console.warn('⚠️ Options composite score not calculated - may indicate factor calculation issues')
      }
    }, 30000)
    ...
    test('should_integrate_options_into_enhanced_technical_analysis_with_correct_weight', async () => {
      ...
      if (enhancedTechnicalScore !== null) {
        ...
        if (traditionalTechnicalScore !== null) {
          ...
          console.log(`   Traditional Technical Score: ${traditionalTechnicalScore.toFixed(3)}`)
          console.log(`   Enhanced Score (with options): ${enhancedTechnicalScore.toFixed(3)}`)
          console.log(`   Options influence: ${((enhancedTechnicalScore - traditionalTechnicalScore) * 100).toFixed(1)}%`)

          // Scores should be different when options data is present
          expect(enhancedTechnicalScore).not.toBe(traditionalTechnicalScore)
        } else {
          console.log(`📊 Enhanced Technical Score: ${enhancedTechnicalScore.toFixed(3)}`)
        }
      } else {
    ...
    test('should_calculate_put_call_ratio_score_accurately', async () => {
      ...
      for (const testCase of testCases) {
        ...
        if (score !== null) {
          ...
          console.log(`📊 P/C Ratio ${testCase.ratio} → Score: ${score.toFixed(3)} (${testCase.expected})`)
        }
      }
    }, 30000)
      ...
      try {
        ...
        if (optionsAnalysis) {
          ...
          if (optionsComposite !== null) {
            ...
            console.log(`   P/C Ratio: ${optionsAnalysis.currentRatio.volumeRatio.toFixed(3)}`)
    ...
    test('should_validate_options_data_quality_and_freshness', async () => {
      ...
      if (optionsAnalysis) {
        ...
        if (optionsAnalysis.confidence > 0.7) {
          console.log(`   ✅ High confidence options data (${(optionsAnalysis.confidence * 100).toFixed(1)}%)`)
        } else {
          console.log(`   ⚠️ Lower confidence options data - may impact analysis quality`)
        }
        ...
        if (pcRatio > 1.5) {
          console.log(`   📈 High put volume detected (P/C: ${pcRatio.toFixed(3)}) - bearish sentiment`)
        } else if (pcRatio < 0.5) {
          console.log(`   📉 High call volume detected (P/C: ${pcRatio.toFixed(3)}) - bullish sentiment`)
        } else {
          console.log(`   📊 Normal P/C ratio (${pcRatio.toFixed(3)}) - neutral sentiment`)
        }
      } else {
  ...
  describe('Performance and Memory Optimization', () => {
    test('should_maintain_analysis_performance_with_options_integration', async () => {
      ...
      for (const symbol of HIGH_OPTIONS_VOLUME_SYMBOLS.slice(0, 2)) { // Test first 2 symbols
        const startTime = Date.now()
        ...
        try {
          ...
          console.log(`⏱️ Performance for ${symbol}: ${latency}ms (options: ${hasOptionsData ? 'yes' : 'no'})`)

          // Performance target: should complete within reasonable time
          expect(latency).toBeLessThan(15000) // 15 second max per symbol
        } catch (error) {
          console.warn(`Performance test failed for ${symbol}:`, error instanceof Error ? error.message : error)
        }
      }

      // Analyze performance results
      const avgLatency = performanceResults.reduce((sum, result) => sum + result.latency, 0) / performanceResults.length
      const optionsEnabledCount = performanceResults.filter(r => r.hasOptions).length

      console.log(`📊 Performance Summary:`)
      console.log(`   Average Latency: ${Math.round(avgLatency)}ms`)
      console.log(`   Symbols with Options: ${optionsEnabledCount}/${performanceResults.length}`)
      console.log(`   Performance Grade: ${avgLatency < 5000 ? 'A' : avgLatency < 10000 ? 'B' : 'C'}`)
    ...
    test('should_optimize_memory_usage_during_batch_options_processing', async () => {
      ...
      try {
        ...
        const batchResults = await optionsService.getBatchOptionsAnalysis(HIGH_OPTIONS_VOLUME_SYMBOLS)
        ...
        console.log(`   Memory per Symbol: ${(memoryIncreaseMB / batchResults.size).toFixed(1)}MB`)
        ...
        if (global.gc) {
          ...
          console.log(`   Memory freed by GC: ${Math.round(memoryReduction / (1024 * 1024))}MB`)
        }
