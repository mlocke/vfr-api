...
describe("SectorBenchmarks - Comprehensive Test Suite", () => {
	...
	describe("Schema Validation", () => {
			...
			REQUIRED_SECTORS.forEach(sector => {
				REQUIRED_RATIOS.forEach(ratio => {
					test(`${sector}.${ratio} should have correct percentiles`, () => {
						const benchmarks = SECTOR_BENCHMARKS[sector];
						const ratioData =
							benchmarks[ratio as keyof Omit<SectorValuationBenchmarks, "sector">];

						if (ratio === "pegRatio") {
							// PEG ratio doesn't have max percentile
							PEG_PERCENTILES.forEach(percentile => {
								expect(ratioData).toHaveProperty(percentile);
								expect(typeof ratioData[percentile as keyof typeof ratioData]).toBe(
									"number"
								);
							});
		...
		test("DEFAULT_BENCHMARKS should have identical structure to sector benchmarks", () => {
			...
			REQUIRED_RATIOS.forEach(ratio => {
				...
				const ratioData =
					DEFAULT_BENCHMARKS[ratio as keyof Omit<SectorValuationBenchmarks, "sector">];
		...
		describe("All numeric values should be positive numbers", () => {
			...
			test("All DEFAULT_BENCHMARKS values should be positive", () => {
				REQUIRED_RATIOS.forEach(ratio => {
					const ratioData =
						DEFAULT_BENCHMARKS[
							ratio as keyof Omit<SectorValuationBenchmarks, "sector">
						];
					Object.entries(ratioData).forEach(([percentile, value]) => {
						expect(value).toBeGreaterThan(0);
						expect(Number.isFinite(value)).toBe(true);
	...
	describe("Internal Consistency - Percentile Ordering", () => {
			...
			REQUIRED_SECTORS.forEach(sector => {
				RATIOS_WITH_MAX.forEach(ratio => {
					test(`${sector}.${ratio}: p25 < median < p75 < max`, () => {
						...
						if (ratio === "peRatio") {
							console.log(
								`${sector}.${ratio}: ${ratioData.p25} < ${ratioData.median} < ${ratioData.p75} < ${ratioData.max}`
							);
						}
					});
				});
		...
		describe("PEG ratio: p25 < median < p75 (no max)", () => {
			REQUIRED_SECTORS.forEach(sector => {
				test(`${sector}.pegRatio: p25 < median < p75`, () => {
					...
					console.log(
						`${sector}.pegRatio: ${pegData.p25} < ${pegData.median} < ${pegData.p75}`
					);
				});
			});
		});
		...
		test("DEFAULT_BENCHMARKS should maintain percentile ordering", () => {
			...
			RATIOS_WITH_MAX.forEach(ratio => {
				const ratioData = DEFAULT_BENCHMARKS[
					ratio as keyof Omit<SectorValuationBenchmarks, "sector">
				] as {
					p25: number;
					median: number;
			...
			// Test PEG ratio
			expect(DEFAULT_BENCHMARKS.pegRatio.p25).toBeLessThan(
				DEFAULT_BENCHMARKS.pegRatio.median
			);
			expect(DEFAULT_BENCHMARKS.pegRatio.median).toBeLessThan(
				DEFAULT_BENCHMARKS.pegRatio.p75
			);
		});
	});
	...
	describe("Reasonableness - Industry Standard Validation", () => {
		...
		describe("Healthcare sector validation", () => {
			...
			test("Healthcare P/E median should be lower than Technology", () => {
				...
				console.log(
					`Healthcare P/E: ${healthcare.peRatio.median} < Tech P/E: ${tech.peRatio.median}`
				);
			});
		...
		describe("Cross-sector reasonableness checks", () => {
			test("Growth sectors should have higher P/E than value sectors", () => {
				...
				const avgGrowthPE =
					growthSectors.reduce((sum, s) => sum + SECTOR_BENCHMARKS[s].peRatio.median, 0) /
					growthSectors.length;

				const avgValuePE =
					valueSectors.reduce((sum, s) => sum + SECTOR_BENCHMARKS[s].peRatio.median, 0) /
					valueSectors.length;

				expect(avgGrowthPE).toBeGreaterThan(avgValuePE);
				console.log(
					`Growth PE avg: ${avgGrowthPE.toFixed(1)} > Value PE avg: ${avgValuePE.toFixed(1)}`
				);
			});
	...
	describe("Integration Scenarios", () => {
		...
		test("should handle cross-sector comparisons", () => {
			...
			const techScore = calculatePercentileScore(
				techPE,
				SECTOR_BENCHMARKS["Technology"].peRatio
			);
			const financialsScore = calculatePercentileScore(
				financialsPE,
				SECTOR_BENCHMARKS["Financial Services"].peRatio
			);
		...
		test("should calculate realistic scores for all sectors at median", () => {
			Object.entries(SECTOR_BENCHMARKS).forEach(([sector, benchmarks]) => {
				const score = calculatePercentileScore(
					benchmarks.peRatio.median,
					benchmarks.peRatio
				);
				expect(score).toBeCloseTo(0.75, 2);
			});
