...
import { AlgorithmConfiguration, StockScore, SelectionResult, AlgorithmCacheKeys } from "./types";
...
export class AlgorithmCache {
	...
	async setStockScores(algorithmId: string, scores: StockScore[]): Promise<void> {
		...
		const metadata = {
			...
			timestamp,
			averageQuality:
				scores.reduce((sum, s) => sum + s.dataQuality.overall, 0) / scores.length,
	...
	async setFundamentalData(
		symbol: string,
		data: { data: any; timestamp: number }
	): Promise<void> {
	 ...
	 */
	async getRecentSelectionResults(
		algorithmId: string,
		limit: number = 10
	...
	async getFactorValue(
		factorName: string,
		symbol: string
	): Promise<{ value: number; metadata?: any; timestamp: number } | null> {
		const key = `factor:${factorName}:${symbol}`;
	...
	async clearExpiredEntries(): Promise<number> {
		...
		const patterns = ["algorithm:scores:*", "algorithm:selections:*", "market:*", "factor:*"];
	...
	private serialize(data: any): string {
		...
		if (
			this.config.performance.enableCompression &&
