...
export class FactorLibrary {
		...
		try {
			...
			switch (factorName) {
				...
				case "technical_overall_score_with_options":
					result = await this.calculateTechnicalOverallScoreWithOptions(
						symbol,
				...
				case "put_call_ratio_score":
					result = technicalData?.optionsData?.putCallRatio
						? this.calculatePutCallRatioScore(technicalData.optionsData.putCallRatio)
						: null;
					break;
				case "iv_percentile_score":
					result = technicalData?.optionsData?.impliedVolatilityPercentile
						? this.calculateIVPercentileScore(
				...
				case "options_flow_score":
					result = technicalData?.optionsData?.optionsFlow
						? this.calculateOptionsFlowScore(technicalData.optionsData.optionsFlow)
						: null;
					break;
				case "greeks_score":
					result = technicalData?.optionsData?.greeks
						? this.calculateGreeksScore(technicalData.optionsData.greeks)
						: null;
					break;
				case "volume_divergence_score":
					result = technicalData?.optionsData?.volumeDivergence
						? this.calculateVolumeDivergenceScore(
								technicalData.optionsData.volumeDivergence
							)
						: null;
					break;
				case "max_pain_score":
					result = technicalData?.optionsData?.maxPain
						? this.calculateMaxPainScore(
								technicalData.optionsData.maxPain,
								marketData.price
							)
						: null;
					break;
				...
				case "vwap_trend_analysis":
					result = await this.calculateVWAPTrendScore(
						symbol,
						technicalData?.vwapAnalysis
					);
					break;
				case "macroeconomic_sector_impact":
					result = this.calculateMacroeconomicSectorImpact(
						technicalData?.macroeconomicContext,
						marketData?.sector
					);
					break;
				case "macroeconomic_composite":
					result = this.calculateMacroeconomicComposite(
						technicalData?.macroeconomicContext
					);
					break;
				...
				case "esg_environmental":
					result = await this.calculateESGFactor(
						symbol,
						marketData.sector,
						"environmental"
					);
				...
				case "momentum_composite":
					result = await this.calculateMomentumComposite(
						symbol,
						marketData,
						technicalData
					);
	 ...
	 */
	private async calculateTechnicalOverallScoreWithOptions(
		symbol: string,
	 ...
	 */
	private calculateOptionsFlowScore(optionsFlow: {
		sentiment: number;
		volume: number;
		openInterest: number;
	}): number {
	 ...
	 */
	private calculateGreeksScore(greeks: {
		delta: number;
		gamma: number;
		theta: number;
		vega: number;
	 ...
	 * @returns Score from 0 to 1 (higher = more attractive valuation)
	 */
	private calculateEVEBITDAScore(
		fundamentalData?: FundamentalDataPoint,
		sector?: string
	): number | null {
	...
	private calculatePEGScore(
		...
		const earningsGrowth = fundamentalData?.earningsGrowth || fundamentalData?.revenueGrowth; // Fallback to revenue growth
		...
		if (pegRatio < 0.5) {
			...
			return 0.9 - position * 0.15; // 0.90 → 0.75
			...
			return 0.6 - position * 0.3; // 0.60 → 0.30
		} else {
	...
	private calculateRevenueAccelerationScore(
		fundamentalData?: FundamentalDataPoint
	): number | null {
	...
	private async calculateVolatilityScore(
		symbol: string,
		periods: number
	): Promise<number | null> {
	...
	private calculateQualityComposite(fundamentalData?: FundamentalDataPoint): number | null {
		...
		const factors: { name: string; value: number | null; weight: number }[] = [
			...
			{
				name: "Debt/Equity",
				value: this.calculateDebtEquityScore(fundamentalData.debtToEquity),
				weight: 0.25,
			},
			...
			},
			{
				...
				value: this.calculateOperatingMarginScore(fundamentalData.grossProfitMargin),
				weight: 0.1,
			},
		];
		...
		factors.forEach(factor => {
			if (factor.value !== null && !isNaN(factor.value) && isFinite(factor.value)) {
				...
				validFactors++;
				console.log(
					`Quality factor ${factor.name}: ${factor.value.toFixed(3)} (weight: ${factor.weight})`
				);
			} else {
		...
		console.log(
			`Quality composite score: ${qualityScore.toFixed(3)} (based on ${validFactors} factors, ${totalWeight.toFixed(2)} total weight)`
		);
	...
	private async calculateMomentumComposite(
		...
		const factors: { name: string; value: number | null; weight: number }[] = [
			// Short-term momentum (3 weeks)
			{
				...
				value: await this.calculateMomentum(symbol, 21),
				weight: 0.25,
			},
			...
			// Medium-term momentum (3 months)
			{
				...
				value: await this.calculateMomentum(symbol, 63),
				weight: 0.3,
			},
			...
			// Long-term momentum (6 months)
			{
				...
				value: await this.calculateMomentum(symbol, 126),
				weight: 0.25,
			},

			// Technical momentum indicators
			{ name: "RSI", value: this.calculateRSIScore(technicalData?.rsi), weight: 0.1 },
			{ name: "MACD", value: this.calculateMACDScore(technicalData?.macd), weight: 0.1 },
		];
	...
	private calculateValueComposite(
		...
		const factors: { name: string; value: number | null; weight: number }[] = [
			...
			{
				name: "P/E Ratio",
				value: this.calculatePEScore(fundamentalData.peRatio),
				weight: 0.35,
			},
			...
			{
				name: "P/B Ratio",
				value: this.calculatePBScore(fundamentalData.pbRatio),
				weight: 0.25,
			},
			...
			{
				name: "P/S Ratio",
				value: this.calculatePriceToSalesDirectScore(fundamentalData.priceToSales),
				weight: 0.25,
			},
			...
			{
				name: "EV/EBITDA",
				value: this.calculateEVEBITDAScore(fundamentalData),
				weight: 0.15,
			},
		];
		...
		factors.forEach(factor => {
			if (factor.value !== null && !isNaN(factor.value) && isFinite(factor.value)) {
				...
				validFactors++;
				console.log(
					`Value factor ${factor.name}: ${factor.value.toFixed(3)} (weight: ${factor.weight})`
				);
			} else {
		...
		console.log(
			`Value composite score: ${valueScore.toFixed(3)} (based on ${validFactors} factors, ${totalWeight.toFixed(2)} total weight)`
		);
	...
	private adjustWeightsForMarketCap(marketCap: number): {
		...
		if (marketCap >= MEGA_CAP) {
			...
			return {
				fundamentalMultiplier: 0.9, // -10%
				technicalMultiplier: 1.1, // +10%
			};
		}
	}
	...
	private async calculateMainComposite(
		...
		const baseWeightsSum = technicalWeight + fundamentalWeight + 0.2 + 0.18 + 0.06; // Include macro, sentiment, alternative
		...
		if (shortInterestScore !== null) {
			console.log(
				`📊 Short Interest Analysis: ${shortInterestScore.toFixed(3)} (weight: 1.5%)`
			);
		...
		if (extendedMarketScore !== null) {
			console.log(
				`💹 Extended Market Data: ${extendedMarketScore.toFixed(3)} (weight: 0.5%)`
			);
		...
		console.log(
			`   🔍 DEBUG totalScore: ${totalScore.toFixed(4)}, totalWeight: ${totalWeight.toFixed(4)}`
		);
		console.log(
		...
		console.log(
			`   ✅ WEIGHT VERIFICATION: Total weights = ${totalWeight.toFixed(3)} (target: 1.000)`
		);
		...
		if (technicalScore !== null) {
			this.factorCache.set(
				`technical_overall_score_${symbol}_${Math.floor(Date.now() / 60000)}`,
				{
		...
		console.log(
			`   🔍 DEBUG: Returning clamped score: ${clampedScore.toFixed(4)} (original: ${finalScore.toFixed(4)})`
		);
		console.log(
	 ...
	 */
	private async calculateESGFactor(
		symbol: string,
		sector: string,
	 ...
	 */
	private async calculateVWAPTrendScore(
		symbol: string,
		vwapAnalysis?: any
	): Promise<number | null> {
	 ...
	 */
	private async calculateShortInterestComposite(
		symbol: string,
		technicalData?: TechnicalDataPoint
	): Promise<number> {
		try {
	 ...
	 */
	private async calculateExtendedMarketComposite(
		symbol: string,
		technicalData?: TechnicalDataPoint
	): Promise<number> {
		try {
