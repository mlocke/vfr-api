...
export class AlgorithmEngine {
	 ...
	 */
	private async getStockUniverse(
		config: AlgorithmConfiguration,
		context?: AlgorithmContext
	): Promise<string[]> {
	...
	private async fetchMarketData(
		...
		console.log(
			`📊 Using optimized batch size: ${optimalBatchSize} (FMP Starter: ${fmpCapacity.isStarterPlan})`
		);
		...
		console.log(
			`✅ Market data fetched for ${marketData.size}/${symbols.length} symbols (${((marketData.size / symbols.length) * 100).toFixed(1)}% success rate)`
		);
		return marketData;
	}
	...
	private async processBatchWithRateLimiting(
		...
		const symbolPromises = batch.map(symbol =>
			this.fetchSymbolMarketData(symbol, config).catch(error => {
	...
	private async fetchFundamentalData(
		...
		const requiresFundamentals = config.weights.some(
			w => w.enabled && this.factorLibrary.requiresFundamentalData(w.factor)
		);
		...
		for (let i = 0; i < symbols.length; i += batchSize) {
			...
			const batchPromises = batch.map(symbol =>
				this.fetchSymbolFundamentalData(symbol, config)
			);
	...
	private async fetchSymbolFundamentalData(
		...
		try {
			...
			if (
				cached &&
				Date.now() - cached.timestamp < (config.dataFusion?.cacheTTL || 300000) * 10
			) {
				return cached.data;
			}
	...
	private async calculateStockScores(
		...
		for (const symbol of symbols) {
			console.log(
				`Processing symbol ${symbol}, marketData keys: [${Array.from(marketData.keys()).join(", ")}]`
			);
			...
			if (!market) {
				...
				console.warn(
					`Available market data keys: [${Array.from(marketData.keys()).join(", ")}]`
				);
				continue;
			}

			try {
				console.log(`Calculating score for ${symbol} with market data:`, {
					price: market.price,
				...
				if (score) {
					console.log(
						`Successfully calculated score for ${symbol}: ${score.overallScore}`
					);
	...
	private async calculateSingleStockScore(
			...
			try {
				...
				try {
					const analystRatings =
						await this.financialDataService.getAnalystRatings(symbol);
					if (analystRatings) {
						analystData = {
				...
				if (compositeScore !== null && !isNaN(compositeScore)) {
					...
					console.log(
						`🔍 DEBUG: compositeScore type: ${typeof compositeScore}, value: ${compositeScore}, clamped: ${Math.max(0, Math.min(1, compositeScore))}`
					);
					console.log(
					...
					const componentFactors: { [factor: string]: number } = {
					...
					try {
						...
						const technicalOverallScore = await this.factorLibrary.calculateFactor(
						...
						if (technicalOverallScore !== null) {
							...
							console.log(
								`✅ Technical overall score for ${symbol}: ${technicalOverallScore.toFixed(3)} (${(technicalOverallScore * 100).toFixed(1)}) - TRACKED`
							);
						} else {
							console.warn(
								`⚠️ Technical score null for ${symbol} - will show as 0 in UI`
							);
						}

						// 🆕 FUNDAMENTAL SCORE CALCULATION - Add fundamental score to factorScores
						const qualityScore = await this.factorLibrary.calculateFactor(
							"quality_composite",
							symbol,
							marketData,
							fundamentalData
						...
						if (qualityScore !== null) {
							...
							console.log(
								`✅ Quality composite score for ${symbol}: ${qualityScore.toFixed(3)} (${(qualityScore * 100).toFixed(1)})`
							);
						} else {
						...
						if (fundamentalData && Object.keys(fundamentalData).length > 0) {
							...
							console.log(
								`✅ Fundamental score for ${symbol}: ${fundamentalScore.toFixed(1)} - TRACKED`
							);
						}
						...
						if (analystData) {
							...
							console.log(
								`✅ Analyst score for ${symbol}: ${analystScore.toFixed(1)} - TRACKED`
							);
						}

						const momentumScore = await this.factorLibrary.calculateFactor(
							"momentum_composite",
							symbol,
							marketData,
							fundamentalData
						...
						const valueScore = await this.factorLibrary.calculateFactor(
							"value_composite",
							symbol,
							marketData,
							fundamentalData
						...
						const volatilityScore = await this.factorLibrary.calculateFactor(
							"volatility_30d",
							symbol,
							marketData,
							fundamentalData
						...
						if (
							...
							console.log(
								`✅ Sentiment score for ${symbol}: ${sentimentScore.toFixed(3)} - TRACKED`
							);
						}

						// 🆕 VWAP SERVICE TRACKING - Calculate VWAP factors for utilization tracking
						if (vwapAnalysis) {
							const vwapDeviationScore = await this.factorLibrary.calculateFactor(
							...
							if (vwapDeviationScore !== null && vwapDeviationScore !== 0.5) {
								...
								console.log(
									`✅ VWAP deviation score for ${symbol}: ${vwapDeviationScore.toFixed(3)} - TRACKED`
								);
							}

							const vwapTradingSignals = await this.factorLibrary.calculateFactor(
							...
							if (vwapTradingSignals !== null && vwapTradingSignals !== 0.5) {
								...
								console.log(
									`✅ VWAP trading signals for ${symbol}: ${vwapTradingSignals.toFixed(3)} - TRACKED`
								);
							}

							// 🆕 VWAP TREND ANALYSIS - Enhanced historical trend integration
							const vwapTrendAnalysis = await this.factorLibrary.calculateFactor(
								"vwap_trend_analysis",
								symbol,
								marketData,
								fundamentalData,
							...
							if (vwapTrendAnalysis !== null && vwapTrendAnalysis !== 0.5) {
								...
								console.log(
									`✅ VWAP trend analysis for ${symbol}: ${vwapTrendAnalysis.toFixed(3)} - TRACKED`
								);
							}
						}
						...
						if (macroeconomicContext) {
							const macroSectorImpact = await this.factorLibrary.calculateFactor(
							...
							if (macroSectorImpact !== null && macroSectorImpact !== 0.5) {
								...
								console.log(
									`✅ Macro sector impact for ${symbol}: ${macroSectorImpact.toFixed(3)} - TRACKED`
								);
							}

							const macroComposite = await this.factorLibrary.calculateFactor(
							...
							if (macroComposite !== null && macroComposite !== 0.5) {
								...
								console.log(
									`✅ Macroeconomic composite for ${symbol}: ${macroComposite.toFixed(3)} - TRACKED`
								);
							}
						}
						...
						if (institutionalData) {
							const institutionalSentiment = await this.factorLibrary.calculateFactor(
							...
							if (institutionalSentiment !== null && institutionalSentiment !== 0.5) {
								componentFactors["institutional_sentiment"] =
									institutionalSentiment;
								console.log(
									`✅ Institutional sentiment for ${symbol}: ${institutionalSentiment.toFixed(3)} - TRACKED`
								);
							}

							const insiderActivity = await this.factorLibrary.calculateFactor(
							...
							if (insiderActivity !== null && insiderActivity !== 0.5) {
								...
								console.log(
									`✅ Insider activity score for ${symbol}: ${insiderActivity.toFixed(3)} - TRACKED`
								);
							}

							// 🆕 INSTITUTIONAL COMPOSITE - Enhanced institutional intelligence integration
							const institutionalComposite = await this.factorLibrary.calculateFactor(
							...
							if (institutionalComposite !== null && institutionalComposite !== 0.5) {
								componentFactors["institutional_composite"] =
									institutionalComposite;
								console.log(
									`✅ Institutional composite for ${symbol}: ${institutionalComposite.toFixed(3)} - TRACKED`
								);
							}
						}
						...
						if (esgScore !== undefined && esgScore !== null) {
							const esgComposite = await this.factorLibrary.calculateFactor(
								"esg_composite",
								symbol,
								marketData,
								fundamentalData,
							...
							if (esgComposite !== null && esgComposite !== 0.5) {
								...
								console.log(
									`✅ ESG composite score for ${symbol}: ${esgComposite.toFixed(3)} - TRACKED`
								);
							}
						}
						...
						if (shortInterestData) {
							const shortInterestComposite = await this.factorLibrary.calculateFactor(
								"short_interest_composite",
								symbol,
								marketData,
								fundamentalData,
								enhancedTechnicalData
							...
							if (shortInterestComposite !== null && shortInterestComposite !== 0.5) {
								componentFactors["short_interest_composite"] =
									shortInterestComposite;
								console.log(
									`✅ Short interest composite for ${symbol}: ${shortInterestComposite.toFixed(3)} - TRACKED`
								);
							}
						}
						...
						if (extendedMarketData) {
							const extendedMarketComposite =
								await this.factorLibrary.calculateFactor(
									"extended_market_composite",
									symbol,
									marketData,
									fundamentalData,
									enhancedTechnicalData
							...
							if (
								extendedMarketComposite !== null &&
								extendedMarketComposite !== 0.5
							) {
								componentFactors["extended_market_composite"] =
									extendedMarketComposite;
								console.log(
									`✅ Extended market composite for ${symbol}: ${extendedMarketComposite.toFixed(3)} - TRACKED`
								);
							}
						}
						...
						if (optionsData) {
							const optionsComposite = await this.factorLibrary.calculateFactor(
								"options_composite",
								symbol,
								marketData,
								fundamentalData,
							...
							if (optionsComposite !== null && optionsComposite !== 0.5) {
								...
								console.log(
									`✅ Options composite for ${symbol}: ${optionsComposite.toFixed(3)} - TRACKED`
								);
							}

							// Track individual options factors for detailed analysis
							const putCallRatioScore = await this.factorLibrary.calculateFactor(
							...
							if (putCallRatioScore !== null && putCallRatioScore !== 0.5) {
								...
								console.log(
									`✅ Put/Call ratio score for ${symbol}: ${putCallRatioScore.toFixed(3)} - TRACKED`
								);
							}

							const optionsFlowScore = await this.factorLibrary.calculateFactor(
								"options_flow_score",
								symbol,
								marketData,
								fundamentalData,
							...
							if (optionsFlowScore !== null && optionsFlowScore !== 0.5) {
								...
								console.log(
									`✅ Options flow score for ${symbol}: ${optionsFlowScore.toFixed(3)} - TRACKED`
								);
							}

							const maxPainScore = await this.factorLibrary.calculateFactor(
								"max_pain_score",
								symbol,
								marketData,
								fundamentalData,
							...
							if (maxPainScore !== null && maxPainScore !== 0.5) {
								...
								console.log(
									`✅ Max pain score for ${symbol}: ${maxPainScore.toFixed(3)} - TRACKED`
								);
							}

							const ivPercentileScore = await this.factorLibrary.calculateFactor(
								"iv_percentile_score",
								symbol,
								marketData,
								fundamentalData,
							...
							if (ivPercentileScore !== null && ivPercentileScore !== 0.5) {
								...
								console.log(
									`✅ IV percentile score for ${symbol}: ${ivPercentileScore.toFixed(3)} - TRACKED`
								);
							}

							const volumeDivergenceScore = await this.factorLibrary.calculateFactor(
							...
							if (volumeDivergenceScore !== null && volumeDivergenceScore !== 0.5) {
								...
								console.log(
									`✅ Volume divergence score for ${symbol}: ${volumeDivergenceScore.toFixed(3)} - TRACKED`
								);
							}
						}
		...
		console.log(
			`🧮 Calculating individual factors for ${symbol} (${config.weights.length} factors configured)`
		);
		...
		for (const weight of config.weights) {
			...
			try {
				console.log(
					`🔧 Calculating factor ${weight.factor} for ${symbol} (weight: ${weight.weight})...`
				);
				...
				if (factorScore !== null && !isNaN(factorScore)) {
					...
					console.log(
						`✅ Factor ${weight.factor} = ${factorScore.toFixed(4)}, weight = ${weight.weight}, contribution = ${(factorScore * weight.weight).toFixed(4)}`
					);
				} else {
		...
		console.log(
			`📊 Factor calculation summary for ${symbol}: ${Object.keys(factorScores).length} factors calculated, total weight: ${totalWeight}`
		);

		// If no factors calculated successfully, use a simple default scoring
		if (totalWeight === 0) {
			console.warn(
				`No factors calculated successfully for ${symbol}, using simple default scoring`
			);
	...
	private async applySelectionCriteria(
		...
		const qualifiedScores = stockScores.filter(
			score => score.dataQuality.overall >= (config.dataFusion?.minQualityScore || 0.7)
		);
	 ...
	 */
	private determineAction(symbol: string, context: AlgorithmContext): "BUY" | "SELL" | "HOLD" {
	 ...
	 */
	private calculateConfidence(score: StockScore, rank: number, totalSelections: number): number {
	...
	private buildWhereClause(criteria: any): string {
		...
		if (criteria.sectors?.length) {
			conditions.push(
				`sector IN (${criteria.sectors.map((s: string) => `'${s}'`).join(",")})`
			);
		}
		...
		if (criteria.exchanges?.length) {
			conditions.push(
				`exchange IN (${criteria.exchanges.map((e: string) => `'${e}'`).join(",")})`
			);
		}
		if (criteria.excludeSymbols?.length) {
			conditions.push(
				`symbol NOT IN (${criteria.excludeSymbols.map((s: string) => `'${s}'`).join(",")})`
			);
		}
	...
	private calculateExecutionMetrics(
		...
		return {
			...
			averageDataQuality:
				stockScores.reduce((sum, s) => sum + s.dataQuality.overall, 0) / stockScores.length,
	...
	private calculateDataCompleteness(stockScores: StockScore[]): number {
		return (
			stockScores.reduce((sum, s) => sum + s.dataQuality.metrics.completeness, 0) /
			stockScores.length
		);
	}
	...
	private calculateDataFreshness(stockScores: StockScore[]): number {
		return (
			stockScores.reduce((sum, s) => sum + s.dataQuality.metrics.freshness, 0) /
			stockScores.length
		);
	}
	...
	private calculateFactorContributions(
		...
		weights.forEach(weight => {
			if (weight.enabled && factorScores[weight.factor] !== undefined) {
				contributions[weight.factor] =
					(weight.weight / totalWeight) * factorScores[weight.factor];
			}
		});
	...
	private calculateFundamentalScore(fundamentalData: FundamentalDataPoint): number {
		...
		if (fundamentalData.currentRatio !== undefined) {
			if (fundamentalData.currentRatio >= 1.5 && fundamentalData.currentRatio <= 2.5)
				score += 5;
