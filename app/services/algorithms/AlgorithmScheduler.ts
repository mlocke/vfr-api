...
export class AlgorithmScheduler extends EventEmitter {
	...
	async scheduleAlgorithm(algorithmId: string, config: AlgorithmConfiguration): Promise<void> {
		...
		console.log(
			`Scheduled algorithm ${config.name} (${algorithmId}) with ${refreshInterval}s refresh interval`
		);
	...
	async unscheduleAlgorithm(algorithmId: string): Promise<void> {
		...
		const runningExecution = Array.from(this.runningExecutions.values()).find(
			exec => exec.config.id === algorithmId
		);
		...
		Promise.allSettled(executionPromises).then(results => {
			results.forEach((result, index) => {
				if (result.status === "rejected") {
					console.error(
						`Algorithm execution failed for ${dueAlgorithms[index]}:`,
						result.reason
					);
				}
			});
	...
	private async executeAlgorithm(algorithmId: string): Promise<void> {
		...
		try {
			...
			console.log(
				`Algorithm execution completed: ${config.name} (${runId}) - ${result.selections.length} selections in ${executionTime}ms`
			);
	...
	private async handleExecutionFailure(
		...
		if (scheduled.failureCount <= this.config.retryAttempts) {
			...
			console.log(
				`Scheduling retry for ${algorithmId} in ${retryDelay}ms (attempt ${scheduled.failureCount})`
			);
		} else {
	...
	getStatus() {
		return {
			...
			scheduledAlgorithms: Array.from(this.scheduledAlgorithms.entries()).map(
				([id, scheduled]) => ({
					algorithmId: id,
