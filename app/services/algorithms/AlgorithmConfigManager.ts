...
export class AlgorithmConfigManager {
	...
	async createConfiguration(
		...
		if (!validation.configValidation.isValid) {
			throw new Error(
				`Configuration validation failed: ${validation.configValidation.errors.join(", ")}`
			);
		}
	...
	async updateConfiguration(
		...
		console.log(
			`Updated algorithm configuration: ${updatedConfig.name} (${configId}) v${updatedConfig.metadata.version}`
		);
	...
	async validateConfiguration(config: AlgorithmConfiguration): Promise<AlgorithmValidation> {
		...
		if (
			validation.configValidation.isValid &&
			config.metadata.backtestedFrom &&
			config.metadata.backtestedTo
		) {
			try {
	 ...
	 */
	private validateFactors(config: AlgorithmConfiguration): {
		errors: string[];
		warnings: string[];
	} {
		const errors: string[] = [];
	...
	private async performCustomValidations(
		...
		if (
			config.selectionCriteria === SelectionCriteria.QUANTILE_BASED &&
			!config.selection.quantile
		) {
			errors.push("Quantile value required for quantile-based selection");
		}

		if (
			config.selectionCriteria === SelectionCriteria.THRESHOLD_BASED &&
		...
		if (config.dataFusion.requiredSources.length === 0) {
			warnings.push(
				"No required data sources specified - algorithm may fail if no data is available"
			);
		}
		...
		if (
			config.selection.minHoldingPeriod &&
	...
	private createSingleCompositeConfiguration(): AlgorithmConfiguration {
		return {
			...
			name: "Composite Analysis Algorithm",
			description:
				"Comprehensive analysis with individual factor tracking for proper utilization reporting",
			type: "quality" as any,
			enabled: true,
