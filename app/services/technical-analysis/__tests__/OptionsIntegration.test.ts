...
describe.skip('TechnicalIndicatorService Options Integration - SKIPPED: Methods need updating', () => {
  ...
  const sampleOHLCData: OHLCData[] = [
    { timestamp: Date.now() - 86400000 * 10, open: 100, high: 105, low: 95, close: 103, volume: 1000000 },
    { timestamp: Date.now() - 86400000 * 9, open: 103, high: 108, low: 101, close: 106, volume: 1200000 },
    { timestamp: Date.now() - 86400000 * 8, open: 106, high: 110, low: 104, close: 108, volume: 900000 },
    { timestamp: Date.now() - 86400000 * 7, open: 108, high: 112, low: 107, close: 111, volume: 1100000 },
    { timestamp: Date.now() - 86400000 * 6, open: 111, high: 115, low: 109, close: 113, volume: 1300000 },
    { timestamp: Date.now() - 86400000 * 5, open: 113, high: 117, low: 111, close: 115, volume: 1000000 },
    { timestamp: Date.now() - 86400000 * 4, open: 115, high: 119, low: 113, close: 117, volume: 1400000 },
    { timestamp: Date.now() - 86400000 * 3, open: 117, high: 121, low: 115, close: 119, volume: 1200000 },
    { timestamp: Date.now() - 86400000 * 2, open: 119, high: 123, low: 117, close: 121, volume: 1500000 },
    { timestamp: Date.now() - 86400000 * 1, open: 121, high: 125, low: 119, close: 123, volume: 1100000 }
  ]
  ...
  describe('Options Data Available', () => {
    it('should integrate options signals when options data is available', async () => {
      ...
      console.log('✅ Options integration test passed - signals included in technical analysis')
    })
    ...
    it('should calculate IV signals correctly', async () => {
      ...
      console.log(`IV Signal (20th percentile): ${result.options?.signals?.impliedVolatilitySignal}`)
    })
  })
  ...
  describe('Options Data Unavailable', () => {
    it('should gracefully handle when options data is unavailable', async () => {
      ...
      console.log('✅ Options unavailable fallback test passed - traditional analysis maintained')
    })
  ...
  describe('Performance Requirements', () => {
    it('should maintain <3s total analysis time with options integration', async () => {
      ...
      console.log(`✅ Performance test passed - analysis completed in ${duration.toFixed(0)}ms`)
    })
    ...
    it('should maintain backward compatibility with existing technical analysis', async () => {
      ...
      console.log('✅ Backward compatibility test passed - all traditional components preserved')
    })
  })
  ...
  describe('Options Signal Quality', () => {
    it('should apply confidence weighting to options signals', async () => {
      ...
      expect(highConfResult.score?.breakdown?.options).toBeGreaterThan(lowConfResult.score?.breakdown?.options || 0)

      console.log(`✅ Confidence weighting test passed - High: ${highConfResult.score?.breakdown?.options}, Low: ${lowConfResult.score?.breakdown?.options}`)
    })
  })
})