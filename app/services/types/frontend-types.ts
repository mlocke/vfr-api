...
export const FrontendAnalysisRequestSchema = z.object({
  mode: z.enum(['single', 'sector', 'multiple']),
  symbols: z.array(z.string().min(1).max(5).regex(/^[A-Z0-9]{1,5}$/)).optional(),
  ...
  switch (data.mode) {
    ...
    case 'sector':
      return !!data.sector
    case 'multiple':
      return data.symbols && data.symbols.length >= 1 && data.symbols.length <= data.limit
    default:
      return false
  }
}, {
  message: 'Invalid request: single mode requires one symbol, sector mode requires sector name, multiple mode requires 1-50 symbols',
  path: ['mode']
})
