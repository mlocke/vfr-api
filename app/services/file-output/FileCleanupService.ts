...
export class FileCleanupService {
  ...
  start(config: Partial<FileCleanupConfig> = {}): void {
    ...
    console.log(`FileCleanupService started with cleanup interval: ${finalConfig.cleanupIntervalHours} hours`)
    console.log(`Configuration: maxFiles=${finalConfig.maxFiles}, maxAge=${finalConfig.maxAgeDays} days, retainLatest=${finalConfig.retainLatest}`)
  }
  ...
  private async performCleanup(config: FileCleanupConfig): Promise<CleanupStats> {
    ...
    try {
      ...
      console.log(`Cleanup completed in ${duration}ms: ${deleteResults.deleted} deleted, ${this.stats.retainedFiles} retained`)
  ...
  private validateConfig(config: FileCleanupConfig): void {
    ...
    if (config.maxFiles < 10 && config.retainLatest) {
      console.warn('Warning: Very low maxFiles setting with retainLatest=true may cause frequent deletions')
    }
