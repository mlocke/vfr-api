...
export class ExtendedMarketDataService {
	...
	async getExtendedMarketData(symbol: string): Promise<ExtendedMarketData | null> {
		try {
			...
			const liquidityMetrics = bidAskData
				? await this.calculateLiquidityMetrics(symbol, bidAskData)
				: null;
			...
			console.error(
				`ExtendedMarketDataService.getExtendedMarketData error for ${symbol}:`,
				error
			);
			return null;
		}
	...
	private async calculateExtendedHoursData(
		...
		const previousClose =
			historicalData.length > 0 ? historicalData[0].close : currentData.price;
		...
		if (marketStatus === "pre-market") {
			...
			extendedHoursData.preMarketChangePercent =
				((currentData.price - previousClose) / previousClose) * 100;
	...
	private async calculateLiquidityMetrics(
		...
		try {
			...
			const marketMakingActivity = this.estimateMarketMakingActivity(
				currentSpread,
				averageSpread
			);
			...
			console.error(
				`ExtendedMarketDataService.calculateLiquidityMetrics error for ${symbol}:`,
				error
			);
			return null;
		}
	...
	private calculateVolatility(spreads: number[]): number {
		...
		const variance =
			spreads.reduce((sum, spread) => sum + Math.pow(spread - mean, 2), 0) / spreads.length;
	 ...
	 */
	private estimateMarketMakingActivity(
		currentSpread: BidAskSpread,
	...
	async getExtendedHoursData(symbol: string): Promise<ExtendedHoursData | null> {
		try {
			...
			console.error(
				`ExtendedMarketDataService.getExtendedHoursData error for ${symbol}:`,
				error
			);
			return null;
		}
	 ...
	 */
	async getBatchLiquidityMetrics(
		symbols: string[]
	): Promise<Map<string, LiquidityMetrics | null>> {
	...
	calculateExtendedMarketScore(extendedData: ExtendedMarketData): number {
		...
		if (extendedData.extendedHours.preMarketChangePercent !== undefined) {
			...
			const preMarketStrength =
				Math.min(Math.abs(extendedData.extendedHours.preMarketChangePercent), 5) / 5;
