...
describe.skip("OptionsAnalysisService Unit Tests - SKIPPED: Methods need updating", () => {
	...
	describe("Implied Volatility Calculations", () => {
		...
		test("should_handle_deep_in_the_money_options", () => {
			...
			const deepItmCall = {
				strike: 100,
				currentPrice: 130,
				optionPrice: 31.5, // Intrinsic + small time value
		...
		test("should_handle_deep_out_of_the_money_options", () => {
			...
			const deepOtmCall = {
				strike: 200,
				currentPrice: 150,
				optionPrice: 0.5, // Low premium for OTM
				timeToExpiry: 0.25,
				riskFreeRate: 0.05,
				optionType: "call" as const,
			};
			...
			expect(iv).toBeLessThan(1.5); // Very high but not unrealistic
			expect(isFinite(iv)).toBe(true);
		...
		test("should_handle_near_expiry_options", () => {
			...
			expect(iv).toBeLessThan(3.0); // Very high but possible near expiry
			expect(isFinite(iv)).toBe(true);
	...
	describe("Put/Call Ratio Calculations", () => {
		...
		test("should_handle_zero_call_volume_gracefully", () => {
			const optionsData = [{ type: "put", volume: 1000, openInterest: 5000 }];
		...
		test("should_handle_zero_put_volume_gracefully", () => {
			const optionsData = [{ type: "call", volume: 1000, openInterest: 5000 }];
	...
	describe("Mathematical Utility Functions", () => {
		...
		test("should_calculate_black_scholes_put_price_accurately", () => {
			...
			const expectedParity =
				params.currentPrice -
				params.strike * Math.exp(-params.riskFreeRate * params.timeToExpiry);
