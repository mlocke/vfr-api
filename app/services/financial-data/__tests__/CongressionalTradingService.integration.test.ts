...
describe("CongressionalTradingService Integration Tests", () => {
	...
	describe("Real FMP API Integration and Rate Limit Compliance", () => {
		test("should_fetch_congressional_trades_with_real_fmp_api_under_rate_limits", async () => {
			...
			console.log(
				`✓ Congressional trades fetched: ${apiCallCount} symbols in ${apiDuration}ms`
			);
		}, 15000);
		...
		test("should_handle_fmp_rate_limit_gracefully_with_fallback", async () => {
			...
			const rapidCalls = Array(10)
				.fill(0)
				.map(() => service.getCongressionalTrades(symbol));
			...
			console.log(
				`✓ Rate limit handling: ${successCount} successful, ${rateLimitedCount} rate-limited`
			);
		}, 20000);
		...
		test("should_validate_congressional_data_quality_and_integrity", async () => {
			...
			if (trades.length > 0) {
				trades.forEach(trade => {
					...
					expect(["BUY", "SELL", "PARTIAL_SELL", "EXCHANGE"]).toContain(
		...
		test("should_calculate_political_insider_signals_with_weight_contribution", async () => {
			...
			if (analysis) {
				...
				console.log(
					`✓ Political signal calculated: ${symbol} - Signal: ${analysis.sentimentScore}, Confidence: ${analysis.confidence}, Sentiment: ${analysis.netSentiment}`
				);
			}
		});
	});
	...
	describe("Performance and Memory Management", () => {
		...
		test("should_handle_concurrent_requests_without_race_conditions", async () => {
			...
			const results = await Promise.allSettled(
				promises.flatMap(p => [p.trades, p.signals, p.analysis])
			);
	...
	describe("Data Quality and Business Logic Validation", () => {
			...
			if (trades.length > 0) {
				...
				if (overallSignal) {
					...
					if (buyTrades.length > sellTrades.length) {
						...
						expect(overallSignal.recentPurchases).toBeGreaterThan(
							overallSignal.recentSales
						);
					}

					// More sell trades should result in negative/lower signal
					if (sellTrades.length > buyTrades.length) {
						expect(overallSignal.recentSales).toBeGreaterThan(
							overallSignal.recentPurchases
						);
					}
					...
					console.log(
						`✓ Signal logic validated: ${symbol} - ${buyTrades.length} buys, ${sellTrades.length} sells, confidence: ${overallSignal.confidence}`
					);
				}
			}
		});
		...
		test("should_validate_politician_identification_and_classification", async () => {
			...
			if (trades.length > 0) {
				...
				console.log(
					`✓ Politician data validated: ${trades.length} trades with proper identification`
				);
			}
		});
		...
		test("should_calculate_accurate_weight_contribution_to_overall_analysis", async () => {
			...
			if (analysis && analysis.confidence > 0) {
				...
				console.log(
					`✓ Congressional analysis validated: ${symbol} - Rating: ${analysis.overallRating}`
				);
			}
		});
	});
