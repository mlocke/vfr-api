/**
 * Comprehensive Test Suite for SECEdgarAPI
 * Tests enhanced SEC EDGAR integration for institutional holdings and insider transaction parsing
...
describe("SECEdgarAPI", () => {
	...
	describe("Health Check and API Connectivity", () => {
		test("should_pass_health_check_with_real_sec_edgar_endpoint", async () => {
			...
			if (isHealthy) {
				...
				console.warn(
					"⚠ SEC EDGAR API health check failed - this may be due to rate limiting, network issues, or temporary unavailability"
				);
			}
	...
	describe("Company Information Retrieval", () => {
		test("should_retrieve_company_info_for_known_symbol_with_real_api", async () => {
			...
			if (companyInfo) {
				...
				console.log(
					`⚠ No company info found for ${symbol} - this may be due to CIK mapping issues or API limitations`
				);
	...
	describe("13F Institutional Holdings Analysis", () => {
		test("should_retrieve_13f_holdings_for_known_symbol_with_real_api", async () => {
			...
			if (holdings.length > 0) {
				holdings.forEach(holding => {
					...
					expect(holding).toHaveProperty("accessionNumber");
					...
					// Validate accession number format (SEC format: XXXXXXXXXX-XX-XXXXXX)
					if (holding.accessionNumber) {
						expect(holding.accessionNumber).toMatch(/^\d{10}-\d{2}-\d{6}$/);
					}
				});
		...
		test("should_respect_quarters_parameter_for_13f_holdings", async () => {
			const symbol = "MSFT";

			// Test different quarter parameters
			const oneQuarter = await api.get13FHoldings(symbol, 1);
			const twoQuarters = await api.get13FHoldings(symbol, 2);

			expect(Array.isArray(oneQuarter)).toBe(true);
			expect(Array.isArray(twoQuarters)).toBe(true);

			// If data is available, two quarters should have same or more holdings
			if (oneQuarter.length > 0 && twoQuarters.length > 0) {
				expect(twoQuarters.length).toBeGreaterThanOrEqual(oneQuarter.length);
			}
	...
	describe("Form 4 Insider Transaction Analysis", () => {
			...
			if (transactions.length > 0) {
				transactions.forEach(transaction => {
					...
					expect([
						...
						"X",
						"Z",
					]).toContain(transaction.transactionCode);
		...
		test("should_respect_days_parameter_for_form4_transactions", async () => {
			const symbol = "AMZN";

			// Test different day parameters
			const thirtyDays = await api.getForm4Transactions(symbol, 30);
			const ninetyDays = await api.getForm4Transactions(symbol, 90);

			expect(Array.isArray(thirtyDays)).toBe(true);
			expect(Array.isArray(ninetyDays)).toBe(true);

			// If data is available, 90 days should have same or more transactions
			if (thirtyDays.length > 0 && ninetyDays.length > 0) {
				expect(ninetyDays.length).toBeGreaterThanOrEqual(thirtyDays.length);
			}
		...
		test("should_sort_form4_transactions_by_filing_date_descending", async () => {
			...
			if (transactions.length > 1) {
				...
				console.log(
					`✓ Form 4 transactions for ${symbol} are properly sorted by filing date`
				);
			}
		}, 45000);
	});

	describe("Institutional Sentiment Analysis", () => {
		test("should_calculate_institutional_sentiment_from_13f_data", async () => {
			...
			if (sentiment) {
				...
				// Validate quarterly change structure
				expect(sentiment.quarterlyChange).toHaveProperty("newPositions");
				expect(sentiment.quarterlyChange).toHaveProperty("closedPositions");
				expect(sentiment.quarterlyChange).toHaveProperty("increasedPositions");
				expect(sentiment.quarterlyChange).toHaveProperty("decreasedPositions");
		...
		test("should_handle_symbol_without_institutional_data_for_sentiment", async () => {
			const symbolWithoutData = "UNKNOWN_SYMBOL_NO_INSTITUTIONAL";

			const sentiment = await api.getInstitutionalSentiment(symbolWithoutData);

			expect(sentiment).toBeNull();
		}, 20000);
	});

	describe("Insider Sentiment Analysis", () => {
		test("should_calculate_insider_sentiment_from_form4_data", async () => {
			...
			if (sentiment) {
				...
				expect(sentiment).toHaveProperty("insiderTypes");
				expect(sentiment).toHaveProperty("recentActivity");
				expect(sentiment).toHaveProperty("sentiment");
				expect(sentiment).toHaveProperty("sentimentScore");
				expect(sentiment).toHaveProperty("confidence");
				expect(sentiment).toHaveProperty("timestamp");
				...
				expect(typeof sentiment.sentimentScore).toBe("number");
				expect(typeof sentiment.confidence).toBe("number");
				expect(Array.isArray(sentiment.recentActivity)).toBe(true);
				expect(sentiment.source).toBe("sec_edgar");
				...
				expect(sentiment.totalTransactions).toBe(
					sentiment.buyTransactions + sentiment.sellTransactions
				);

				// Validate insider types structure
				expect(sentiment.insiderTypes).toHaveProperty("officers");
				expect(sentiment.insiderTypes).toHaveProperty("directors");
				expect(sentiment.insiderTypes).toHaveProperty("tenPercentOwners");
				expect(sentiment.insiderTypes).toHaveProperty("other");

				console.log(`✓ Calculated insider sentiment for ${symbol}:`);
				console.log(`  Sentiment: ${sentiment.sentiment} (${sentiment.sentimentScore}/10)`);
				console.log(
					`  Transactions: ${sentiment.totalTransactions} (${sentiment.buyTransactions} buys, ${sentiment.sellTransactions} sells)`
				);
		...
		test("should_handle_symbol_without_insider_data_for_sentiment", async () => {
			const symbolWithoutData = "UNKNOWN_SYMBOL_NO_INSIDER";

			const sentiment = await api.getInsiderSentiment(symbolWithoutData);
	...
	describe("Rate Limiting and SEC Compliance", () => {
		test("should_enforce_sec_edgar_rate_limiting_between_requests", async () => {
			...
			console.log(
				`✓ Rate limiting enforced: ${totalTime}ms for 3 requests (expected ≥190ms)`
			);
		}, 30000);
		...
		test("should_handle_concurrent_requests_with_proper_queuing", async () => {
			...
			console.log(
				`✓ Concurrent requests handled properly: ${totalTime}ms for ${symbols.length} symbols`
			);
		}, 45000);

		test("should_maintain_request_delay_consistency", async () => {
			const requestStartTimes: number[] = [];

			// Make multiple requests and track start timing
			for (let i = 0; i < 3; i++) {
				const start = Date.now();
				requestStartTimes.push(start);
				await api.getCompanyInfo("AAPL");
			}

			// Check that delays between request starts are consistent with rate limiting
			const delays = requestStartTimes
				.slice(1)
				.map((time, index) => time - requestStartTimes[index]);
			delays.forEach(delay => {
				expect(delay).toBeGreaterThan(50); // Should have at least 50ms delay between starts due to rate limiting
			});
		}, 45000);
	});
	...
	describe("Data Parsing and Validation", () => {
		...
		test("should_validate_accession_number_format_in_responses", async () => {
			const symbol = "MSFT";
			...
			holdings.forEach(holding => {
				if (holding.accessionNumber) {
					// SEC accession number format: XXXXXXXXXX-XX-XXXXXX
					expect(holding.accessionNumber).toMatch(/^\d{10}-\d{2}-\d{6}$/);
				}
			});
		}, 30000);
