...
describe("Enhanced SentimentAnalysisService Integration Tests", () => {
	...
	describe("Multi-Source Sentiment Integration and Real API Processing", () => {
		test("should_aggregate_sentiment_from_multiple_sources_with_real_apis", async () => {
			...
			const promises = testSymbols.map(symbol => service.getMultiSourceSentiment(symbol));
			...
			results.forEach((result, index) => {
				...
				if (result.status === "fulfilled" && result.value) {
					...
					expect([
						...
						"NEGATIVE",
						"VERY_NEGATIVE",
					]).toContain(sentiment.composite.category);
				}
			});

			console.log(
				`✓ Multi-source sentiment aggregated: ${apiCallCount} symbols in ${apiDuration}ms`
			);
		}, 25000);
		...
		test("should_integrate_earnings_transcript_sentiment_with_other_sources", async () => {
			...
			if (transcriptIntegratedSentiment) {
				...
				expect(typeof transcriptIntegratedSentiment.news.aggregatedSentiment).toBe(
					"number"
				);
				expect(
					transcriptIntegratedSentiment.news.aggregatedSentiment
				).toBeGreaterThanOrEqual(-1);
				expect(transcriptIntegratedSentiment.news.aggregatedSentiment).toBeLessThanOrEqual(
					1
				);

				expect(typeof transcriptIntegratedSentiment.social.aggregatedSentiment).toBe(
					"number"
				);
				expect(
					transcriptIntegratedSentiment.social.aggregatedSentiment
				).toBeGreaterThanOrEqual(-1);
				expect(
					transcriptIntegratedSentiment.social.aggregatedSentiment
				...
				console.log(
					`✓ Multi-source sentiment: ${symbol} - Composite: ${transcriptIntegratedSentiment.composite.score.toFixed(2)}, Category: ${transcriptIntegratedSentiment.composite.category}`
				);
			}
		});
		...
		test("should_analyze_sentiment_trends_over_time_with_momentum_calculation", async () => {
			...
			if (trendAnalysis) {
				...
				console.log(
					`✓ Sentiment signal analysis: ${symbol} - Signal: ${trendAnalysis.signal}, Strength: ${trendAnalysis.strength}, Composite: ${trendAnalysis.sentiment.composite.score.toFixed(2)}`
				);
			}
		});
		...
		test("should_calculate_sentiment_factors_with_weight_contribution_accuracy", async () => {
			...
			if (singleFactor) {
				...
				expect(singleFactor.sentiment.social.aggregatedSentiment).toBeGreaterThanOrEqual(
					-1
				);
				...
				console.log(
					`✓ Sentiment batch signal: ${symbol} - Signal: ${singleFactor.signal}, Strength: ${singleFactor.strength}, Confidence: ${(singleFactor.confidence * 100).toFixed(1)}%`
				);
			}
		});
	});
	...
	describe("Performance Benchmarks and NLP Processing", () => {
		test("should_process_multi_source_sentiment_within_performance_limits", async () => {
			...
			const results = await Promise.all(
				symbols.map(symbol => service.getMultiSourceSentiment(symbol))
			);
		...
		test("should_handle_nlp_processing_timeouts_and_fallbacks", async () => {
			...
			if (sentiment) {
				...
				console.log(
					`✓ NLP timeout handling: completed in ${processingTime}ms, confidence: ${sentiment.composite.confidence.toFixed(2)}`
				);
			}
		});
	...
	describe("Data Quality and Sentiment Calculation Validation", () => {
		test("should_validate_sentiment_aggregation_weights_and_calculations", async () => {
			...
			if (sentiment) {
				...
				console.log(
					`✓ Sentiment aggregation validated: ${symbol} - Composite: ${sentiment.composite.score.toFixed(3)}`
				);
			}
		});
		...
		test("should_detect_sentiment_conflicts_between_sources", async () => {
			...
			if (sentiment) {
				...
				if (sourceScores.length > 1) {
					...
					if (variance > 0.3) {
						// Threshold for high conflict
						expect(sentiment.composite.confidence).toBeLessThan(0.7); // Lower confidence for conflicting signals

						console.log(
							`✓ Sentiment conflict detected: ${symbol} - Variance: ${variance.toFixed(3)}, Confidence: ${sentiment.composite.confidence.toFixed(2)}`
						);
					}
				}
			}
		...
		test("should_calculate_appropriate_confidence_levels_based_on_data_quality", async () => {
			...
			if (sentiment) {
				...
				if (avgSourceConfidence > 0) {
					...
					console.log(
						`✓ Confidence calculation: ${symbol} - Avg Quality: ${avgSourceConfidence.toFixed(2)}, Overall: ${sentiment.composite.confidence.toFixed(2)}`
					);
				}
			}
		});
		...
		test("should_validate_weight_contribution_calculation_for_composite_scoring", async () => {
			...
			if (sentiment && sentiment.composite) {
				...
				console.log(
					`✓ Sentiment validated: ${symbol} - Score: ${score.toFixed(3)}, Confidence: ${confidence.toFixed(2)}`
				);
			}
		});
	});
	...
	describe("Error Handling and Resilience", () => {
		...
		test("should_implement_fallback_sentiment_when_sources_unavailable", async () => {
			...
			if (sentiment) {
				...
				console.log(
					`✓ Limited sources handled: confidence ${sentiment.composite.confidence.toFixed(2)} with composite score ${sentiment.composite.score.toFixed(3)}`
				);
			}
		});
