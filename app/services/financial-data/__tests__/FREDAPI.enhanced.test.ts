...
describe("FREDAPI Enhanced Economic Analysis", () => {
	...
	describe("API Configuration and Health Check", () => {
		...
		test(
			"should_pass_health_check_for_real_fred_api",
			async () => {
				...
				if (process.env.FRED_API_KEY) {
					...
					if (isHealthy) {
						...
						console.warn(
							"⚠ FRED API health check failed - may be due to API key or network issues"
						);
					}
				} else {
					console.warn(
						"⚠ FRED_API_KEY not configured - skipping health check validation"
					);
	...
	describe("Bulk Economic Context Collection", () => {
		test(
			"should_fetch_economic_context_with_target_response_time",
			async () => {
				if (!process.env.FRED_API_KEY) {
				...
				if (economicContext) {
					...
					if (economicContext.dataCompleteness > 0) {
						...
						if (economicContext.m1MoneySupply !== undefined)
							presentIndicators.push("m1MoneySupply");
						if (economicContext.m2MoneySupply !== undefined)
							presentIndicators.push("m2MoneySupply");
						if (economicContext.federalFundsRate !== undefined)
							presentIndicators.push("federalFundsRate");
						if (economicContext.unemploymentRate !== undefined)
							presentIndicators.push("unemploymentRate");
						expect(presentIndicators.length).toBeGreaterThan(0);
					} else {
						console.warn(
							"⚠ No economic data retrieved - likely due to API key issues"
						);
					}
					...
					if (economicContext.yieldCurve) {
						...
						expect(["normal", "flat", "inverted", "steep"]).toContain(
							economicContext.yieldCurve.shape
						);
					}

					// Log key metrics
					console.log(
						`✓ Data completeness: ${(economicContext.dataCompleteness * 100).toFixed(1)}%`
					);
					console.log(
						`✓ GDP: ${economicContext.gdp?.value || "N/A"} (${economicContext.gdp?.momentum || "N/A"})`
					);
					console.log(
						`✓ CPI: ${economicContext.cpi?.value || "N/A"}% (${economicContext.cpi?.momentum || "N/A"})`
					);
					console.log(
						`✓ Fed Funds: ${economicContext.federalFundsRate?.value || "N/A"}%`
					);
					console.log(
						`✓ Yield Curve: ${economicContext.yieldCurve?.shape || "N/A"} (${economicContext.yieldCurve?.slope10Y2Y?.toFixed(2) || "N/A"}%)`
					);
					...
					if (responseTime < 200) {
						...
						console.log(
							`⚠ Performance target missed: ${responseTime}ms >= 200ms (may be due to network or cache miss)`
						);
					}
					...
					console.warn(
						"⚠ Economic context returned null - may be due to API issues or missing data"
					);
		...
		test(
			"should_implement_proper_caching_for_economic_context",
			async () => {
				if (!process.env.FRED_API_KEY) {
	...
	describe("Economic Cycle Analysis", () => {
		test(
			"should_analyze_economic_cycle_position_with_confidence_metrics",
			async () => {
				if (!process.env.FRED_API_KEY) {
				...
				if (cyclePosition) {
					...
					expect(["expansion", "peak", "contraction", "trough", "recovery"]).toContain(
						cyclePosition.phase
					);
					...
					expect(["accelerating", "decelerating", "stable"]).toContain(
						cyclePosition.gdpMomentum.trend
					);
					expect(["above", "below", "normal"]).toContain(
						cyclePosition.gdpMomentum.vsHistorical
					);
					...
					expect(
						cyclePosition.yieldCurveSignal.recessionProbability
					).toBeGreaterThanOrEqual(0);
					expect(cyclePosition.yieldCurveSignal.recessionProbability).toBeLessThanOrEqual(
						100
					);
					...
					console.log(
						`✓ GDP Growth: ${cyclePosition.gdpMomentum.current}% (${cyclePosition.gdpMomentum.trend})`
					);
					console.log(
						`✓ Recession Probability: ${cyclePosition.yieldCurveSignal.recessionProbability}%`
					);
					...
					console.warn(
						"⚠ Economic cycle analysis returned null - may be due to insufficient data"
					);
	...
	describe("Inflation Trend Analysis", () => {
		test(
			"should_analyze_inflation_trends_with_cpi_ppi_momentum",
			async () => {
				if (!process.env.FRED_API_KEY) {
					console.warn(
						"⚠ FRED_API_KEY not configured - skipping inflation analysis test"
					);
					return;
				}
				...
				if (inflationTrend) {
					...
					expect(["low", "moderate", "high", "declining"]).toContain(
						inflationTrend.environment
					);

					// Validate momentum trends
					expect(["accelerating", "decelerating", "stable"]).toContain(
						inflationTrend.cpiMomentum.trend
					);
					expect(["accelerating", "decelerating", "stable"]).toContain(
						inflationTrend.ppiMomentum.trend
					);
					...
					// Log analysis results
					console.log(
						`✓ Inflation Environment: ${inflationTrend.environment.toUpperCase()}`
					);
					console.log(
						`✓ CPI: ${inflationTrend.currentCPI} (YoY: ${inflationTrend.cpiMomentum.yearOverYear}%, ${inflationTrend.cpiMomentum.trend})`
					);
					console.log(
						`✓ PPI: ${inflationTrend.currentPPI} (YoY: ${inflationTrend.ppiMomentum.yearOverYear}%, ${inflationTrend.ppiMomentum.trend})`
					);
					console.log(`✓ Pressure Score: ${inflationTrend.pressureScore}/100`);
					console.log(
						`✓ Fed Deviation: ${inflationTrend.deviation > 0 ? "+" : ""}${inflationTrend.deviation}% from ${inflationTrend.fedTarget}% target`
					);
					console.log(`✓ Outlook: ${inflationTrend.outlook.toUpperCase()}`);
				} else {
					console.warn(
						"⚠ Inflation trend analysis returned null - may be due to insufficient data"
					);
	...
	describe("Monetary Policy Context", () => {
		test(
			"should_analyze_monetary_policy_with_money_supply_and_equity_impact",
			async () => {
				if (!process.env.FRED_API_KEY) {
				...
				if (monetaryContext) {
					...
					expect(monetaryContext.federalFundsRate).toHaveProperty(
						"nextMeetingProbability"
					);
					expect(["tightening", "easing", "neutral"]).toContain(
						monetaryContext.federalFundsRate.trend
					);
					...
					expect(["expanding", "contracting", "stable"]).toContain(
						monetaryContext.moneySupply.m1Growth.trend
					);
					expect(["expanding", "contracting", "stable"]).toContain(
						monetaryContext.moneySupply.m2Growth.trend
					);

					// Validate liquidity conditions
					expect(["abundant", "adequate", "tight", "very_tight"]).toContain(
						monetaryContext.liquidityConditions
					);
					...
					expect([
						...
						"hawkish",
						"very_hawkish",
					]).toContain(monetaryContext.policyStance);
					...
					expect(["supportive", "neutral", "headwind"]).toContain(
						monetaryContext.equityValuationImpact.sentiment
					);
					expect(Array.isArray(monetaryContext.equityValuationImpact.reasoning)).toBe(
						true
					);
					...
					// Log analysis results
					console.log(
						`✓ Fed Funds Rate: ${monetaryContext.federalFundsRate.current}% (${monetaryContext.federalFundsRate.trend})`
					);
					console.log(
						`✓ M1 Growth: ${monetaryContext.moneySupply.m1Growth.yearOverYear}% YoY (${monetaryContext.moneySupply.m1Growth.trend})`
					);
					console.log(
						`✓ M2 Growth: ${monetaryContext.moneySupply.m2Growth.yearOverYear}% YoY (${monetaryContext.moneySupply.m2Growth.trend})`
					);
					console.log(
						`✓ Liquidity: ${monetaryContext.liquidityConditions.toUpperCase()}`
					);
					console.log(`✓ Policy Stance: ${monetaryContext.policyStance.toUpperCase()}`);
					console.log(
						`✓ Equity Impact: ${monetaryContext.equityValuationImpact.sentiment.toUpperCase()} (${monetaryContext.equityValuationImpact.score}/100)`
					);
					console.log(
						`✓ Market Correlation: ${(monetaryContext.marketPerformanceCorrelation * 100).toFixed(1)}%`
					);

					if (monetaryContext.equityValuationImpact.reasoning.length > 0) {
						console.log(`📝 Equity Impact Reasoning:`);
						monetaryContext.equityValuationImpact.reasoning.forEach(reason =>
							console.log(`   - ${reason}`)
						);
					}
				} else {
					console.warn(
						"⚠ Monetary policy analysis returned null - may be due to insufficient data"
					);
	...
	describe("Integration and Performance Tests", () => {
		test(
			"should_handle_all_enhanced_methods_concurrently",
			async () => {
				if (!process.env.FRED_API_KEY) {
				...
				const [economicContext, cyclePosition, inflationTrend, monetaryContext] =
		...
		test(
			"should_maintain_data_consistency_across_methods",
			async () => {
				if (!process.env.FRED_API_KEY) {
				...
				if (economicContext && monetaryContext) {
					...
					if (economicContext.federalFundsRate && monetaryContext.federalFundsRate) {
						...
						console.log(
							`✓ Fed funds rate consistency: ${contextRate}% vs ${monetaryRate}% (diff: ${difference.toFixed(3)}%)`
						);
					}
