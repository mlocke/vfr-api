...
describe('Fundamental Ratios Integration', () => {
  ...
  describe('FinancialModelingPrepAPI.getFundamentalRatios()', () => {
    test('should fetch fundamental ratios for valid large-cap stock', async () => {
      ...
      if (ratios) {
        ...
        expect(
          ...
          ratios.currentRatio || ratios.grossProfitMargin
        ).toBeDefined()
        ...
        console.log(`✅ AAPL fundamental ratios: P/E=${ratios.peRatio?.toFixed(2) || 'N/A'}, P/B=${ratios.pbRatio?.toFixed(2) || 'N/A'}, ROE=${ratios.roe?.toFixed(2) || 'N/A'}`)
      }
    ...
    test('should fetch fundamental ratios for valid mid-cap stock', async () => {
      ...
      if (ratios) {
        ...
        console.log(`✅ MSFT fundamental ratios: Current Ratio=${ratios.currentRatio?.toFixed(2) || 'N/A'}, D/E=${ratios.debtToEquity?.toFixed(2) || 'N/A'}, Gross Margin=${ratios.grossProfitMargin?.toFixed(2) || 'N/A'}`)
      }
    }, 20000)
    ...
    test('should handle network timeout gracefully', async () => {
      ...
      const apiWithShortTimeout = new FinancialModelingPrepAPI(process.env.FMP_API_KEY, 100, false)
  ...
  describe('FallbackDataService.getFundamentalRatios()', () => {
    test('should fetch fundamental ratios through fallback service', async () => {
      ...
      if (ratios) {
        ...
        expect(
          ratios.peRatio || ratios.pbRatio || ratios.roe || ratios.roa ||
          ratios.currentRatio || ratios.grossProfitMargin || ratios.operatingMargin
        ).toBeDefined()

        console.log(`✅ TSLA fallback service ratios: P/E=${ratios.peRatio?.toFixed(2) || 'N/A'}, Operating Margin=${ratios.operatingMargin?.toFixed(2) || 'N/A'}`)
      }
    ...
    test('should handle rate limiting properly with multiple requests', async () => {
      ...
      const promises = symbols.map(symbol =>
        fallbackService.getFundamentalRatios(symbol)
      )
      ...
      console.log(`✅ Rate limiting test: ${successfulResults.length}/${symbols.length} requests successful`)
    }, 30000)
    ...
    test('should provide source attribution in fallback service', async () => {
      ...
      if (ratios) {
        ...
        expect(['fmp', 'fallback', 'unavailable'].some(source =>
          ratios.source.includes(source) || ratios.source === source
        )).toBe(true)
  ...
  describe('Data Quality Validation', () => {
    ...
    test('should validate liquidity ratios for financial health', async () => {
      ...
      if (ratios) {
        ...
        console.log(`✅ AAPL liquidity ratios: Current=${ratios.currentRatio?.toFixed(2) || 'N/A'}, Quick=${ratios.quickRatio?.toFixed(2) || 'N/A'}`)
      }
    }, 15000)
    ...
    test('should validate profitability margins', async () => {
      ...
      if (ratios) {
        ...
        console.log(`✅ MSFT margin validation: Gross=${ratios.grossProfitMargin?.toFixed(2) || 'N/A'}, Operating=${ratios.operatingMargin?.toFixed(2) || 'N/A'}, Net=${ratios.netProfitMargin?.toFixed(2) || 'N/A'}`)
      }
    }, 15000)
    ...
    test('should validate return ratios for performance metrics', async () => {
      ...
      if (ratios) {
        ...
        console.log(`✅ TSLA return validation: ROE=${ratios.roe?.toFixed(2) || 'N/A'}, ROA=${ratios.roa?.toFixed(2) || 'N/A'}`)
      }
    }, 15000)
    ...
    test('should validate dividend metrics for income stocks', async () => {
      ...
      if (ratios) {
        ...
        console.log(`✅ JNJ dividend validation: Yield=${ratios.dividendYield?.toFixed(3) || 'N/A'}, Payout=${ratios.payoutRatio?.toFixed(2) || 'N/A'}`)
      }
    }, 15000)
    ...
    test('should validate data freshness and completeness', async () => {
      ...
      if (ratios) {
        ...
        const availableRatios = Object.entries(ratios).filter(([key, value]) =>
          !['symbol', 'source', 'timestamp', 'period'].includes(key) && value !== undefined
        ).length

        expect(availableRatios).toBeGreaterThan(0)
        console.log(`✅ META data completeness: ${availableRatios} ratios available, data age: ${hoursSinceUpdate.toFixed(1)} hours`)
      }
    }, 15000)
  })
  ...
  describe('Performance and Memory Optimization', () => {
    ...
    test('should handle concurrent requests efficiently', async () => {
      ...
      console.log(`✅ Concurrent test: ${successCount}/${symbols.length} successful in ${duration}ms`)
    }, 15000)
  })
})