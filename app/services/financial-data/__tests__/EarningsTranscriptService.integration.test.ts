...
describe('EarningsTranscriptService Integration Tests', () => {
  ...
  describe('Real FMP API Integration and Transcript Processing', () => {
    test('should_fetch_earnings_transcripts_with_real_fmp_api_under_rate_limits', async () => {
      ...
      console.log(`✓ Earnings transcripts fetched: ${testSymbols.length} symbols in ${apiDuration}ms`)
    }, 25000)
    ...
    test('should_perform_nlp_sentiment_analysis_on_transcript_content', async () => {
      ...
      if (analysis) {
        ...
        expect(['CONFIDENT', 'CAUTIOUS', 'DEFENSIVE', 'OPTIMISTIC', 'NEUTRAL']).toContain(analysis.managementTone)

        // Analyst sentiment classification
        expect(['POSITIVE', 'NEGATIVE', 'MIXED', 'NEUTRAL']).toContain(analysis.analystSentiment)
        ...
        console.log(`✓ NLP analysis completed: ${symbol} - Management Tone: ${analysis.managementTone}, Overall Sentiment: ${analysis.sentimentBreakdown.overall.toFixed(2)}`)
      }
    }, 20000)

    test('should_extract_financial_keywords_and_topics_from_transcripts', async () => {
      const symbol = 'TSLA'
      const transcript = await service.getEarningsTranscript(symbol, 'Q1', new Date().getFullYear())
      ...
      if (transcript) {
        ...
        if (keywordAnalysis.length > 0) {
          ...
          console.log(`✓ Insights extraction: ${keywordAnalysis.length} insights found, top insights: ${topInsights.map((i: any) => i.topic).join(', ')}`)
        }
      }
    })
    ...
    test('should_calculate_earnings_sentiment_score_with_weight_contribution', async () => {
      ...
      if (sentimentAnalysis) {
        ...
        expect(['IMPROVING', 'DECLINING', 'STABLE']).toContain(sentimentAnalysis.trendingSentiment)
        ...
        console.log(`✓ Earnings sentiment calculated: ${symbol} - Score: ${sentimentAnalysis.sentimentScore}, Trend: ${sentimentAnalysis.trendingSentiment}`)
      }
    })
  })
  ...
  describe('NLP Processing Performance and Memory Management', () => {
    test('should_process_large_transcripts_within_memory_limits', async () => {
      ...
      if (transcript && transcript.transcript.length > 10000) { // Only test with substantial content
        ...
        if (analysis) {
          ...
          console.log(`✓ Large transcript processed: ${transcript.transcript.length} chars in ${nlpProcessingTime}ms, ${(memoryIncrease / 1024 / 1024).toFixed(2)}MB`)
        }
      }
    }, 30000)
    ...
    test('should_implement_streaming_analysis_for_very_large_transcripts', async () => {
      ...
      if (transcript && transcript.transcript.length > 10000) { // Large transcript
    ...
    test('should_cache_nlp_analysis_results_effectively', async () => {
      ...
      if (transcript) {
        ...
        if (analysis1) {
          ...
          console.log(`✓ NLP cache efficiency: ${cacheEfficiency.toFixed(1)}% improvement (${duration1}ms -> ${duration2}ms)`)
        }
      }
    })
  })
  ...
  describe('Data Quality and Business Logic Validation', () => {
    test('should_validate_transcript_authenticity_and_completeness', async () => {
      ...
      if (transcript) {
        ...
        console.log(`✓ Transcript authenticity validated: ${symbol} - ${transcript.transcript.length} chars, ${transcript.participants?.length || 0} participants`)
      }
    })
    ...
    test('should_identify_forward_looking_statements_and_guidance', async () => {
      ...
      if (analysis && analysis.keyInsights.length > 0) {
        ...
        const forwardLookingInsights = analysis.keyInsights.filter(insight =>
        ...
        if (forwardLookingInsights.length > 0) {
          ...
          console.log(`✓ Forward-looking statements identified: ${forwardLookingInsights.length} guidance insights`)
        }

        // Should identify key financial metrics discussed
        const financialInsights = analysis.keyInsights.filter(insight =>
          insight.topic && insight.mentions > 2
        )

        expect(financialInsights.length).toBeGreaterThan(0)
        console.log(`✓ Financial insights identified: ${financialInsights.length} high-mention topics`)
      }
    })
    ...
    test('should_calculate_accurate_management_confidence_indicators', async () => {
      ...
      if (sentimentAnalysis) {
        ...
        console.log(`✓ Management confidence validated: ${symbol} - Score: ${sentimentAnalysis.sentimentScore}, Trend: ${sentimentAnalysis.trendingSentiment}`)
      }
    })
  })
  ...
  describe('Error Handling and Resilience', () => {
    test('should_handle_missing_transcripts_gracefully', async () => {
      ...
      const transcript = await service.getEarningsTranscript(symbol, `Q${futureQuarter}`, futureYear)
      const analysis = transcript ? await service.analyzeTranscript(transcript) : null
  ...
  describe('Integration with Enhanced SentimentAnalysisService', () => {
    test('should_provide_earnings_data_for_sentiment_integration', async () => {
      ...
      if (sentimentData) {
        ...
        console.log(`✓ Sentiment integration data: ${symbol} - Score: ${sentimentData.sentimentScore}, Themes: ${sentimentData.keyThemes.join(', ')}`)
      }
    })
  })
})