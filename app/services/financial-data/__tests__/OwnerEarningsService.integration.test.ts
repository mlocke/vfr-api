...
describe('OwnerEarningsService Integration Tests', () => {
  ...
  describe('Real FMP API Integration and Owner Earnings Calculations', () => {
    test('should_calculate_owner_earnings_with_real_fmp_financial_data', async () => {
      ...
      const promises = testSymbols.map(symbol =>
        service.getOwnerEarnings(symbol)
      )
      ...
      results.forEach((result, index) => {
        ...
        if (result.status === 'fulfilled' && result.value) {
          ...
          const calculatedOwnerEarnings = earnings.reportedEarnings + earnings.depreciation + earnings.amortization - earnings.maintenanceCapex
          const tolerance = Math.abs(calculatedOwnerEarnings * 0.05) // 5% tolerance for rounding/adjustments

          expect(Math.abs(earnings.ownerEarnings - calculatedOwnerEarnings)).toBeLessThan(tolerance)
        }
      })
    ...
    test('should_analyze_owner_earnings_analysis_with_quality_metrics', async () => {
      ...
      console.log(`✓ Analysis: ${symbol} - Trend: ${analysis.trend}, Quality: ${analysis.qualityAssessment}, Rating: ${analysis.investmentRating}`)
    })
    ...
    test('should_screen_stocks_by_owner_earnings_criteria', async () => {
      ...
      console.log(`✓ Screening: found ${screeningResults.length} stocks meeting criteria (yield >= ${minYield}%, quality >= ${minQuality})`)
    })
  })
  ...
  describe('Performance Benchmarks and Calculation Accuracy', () => {
    ...
    test('should_handle_complex_financial_statement_analysis_efficiently', async () => {
      ...
      console.log(`✓ Comprehensive analysis: ${symbol} in ${processingTime}ms, ${(memoryIncrease / 1024 / 1024).toFixed(2)}MB, Quality: ${analysis.qualityAssessment}, Rating: ${analysis.investmentRating}`)
    }, 20000)
  })
  ...
  describe('Calculation Accuracy and Business Logic Validation', () => {
    test('should_validate_owner_earnings_calculation_components_and_formulas', async () => {
      ...
      if (earnings) {
        ...
        const manualCalculation = reportedEarnings + depreciation + amortization - maintenanceCapex
        const calculationTolerance = Math.abs(manualCalculation * 0.01) // 1% tolerance

        expect(Math.abs(earnings.ownerEarnings - manualCalculation)).toBeLessThan(calculationTolerance)
        ...
        console.log(`✓ Calculation validation: ${symbol} - OE: $${(earnings.ownerEarnings / 1000000).toFixed(0)}M, Ratio: ${ownerEarningsToReportedRatio.toFixed(2)}`)
      }
    })
    ...
    test('should_calculate_accurate_quality_assessment', async () => {
      ...
      console.log(`✓ Quality assessment: ${symbol} - Quality: ${analysis.qualityAssessment}, Rating: ${analysis.investmentRating}`)
    })
    ...
    test('should_calculate_reasonable_yield_estimates', async () => {
      ...
      console.log(`✓ Yield validation: ${symbol} - Average Yield: ${analysis.averageYield.toFixed(2)}%, Quality: ${analysis.qualityAssessment}`)
    })
    ...
    test('should_calculate_appropriate_confidence_scores_based_on_quality_metrics', async () => {
      ...
      for (const symbol of symbols) {
        ...
        console.log(`✓ Confidence calculation: ${symbol} - Quality: ${analysis.qualityAssessment}, Confidence: ${(analysis.confidence * 100).toFixed(1)}%`)
      }
    }, 20000)
  })
  ...
  describe('Error Handling and Data Validation', () => {
    ...
    test('should_validate_financial_data_consistency_and_detect_anomalies', async () => {
      ...
      if (earnings) {
        ...
        if (maintenanceCapex > depreciation * 3) { // More than 3x depreciation
          console.warn(`High Maintenance CapEx detected for ${symbol}: CapEx ${maintenanceCapex / 1000000}M vs Depreciation ${depreciation / 1000000}M`)
        }

        // Reported earnings should be reasonable relative to owner earnings
        if (earnings.ownerEarnings < reportedEarnings * 0.1) { // Owner earnings less than 10% of reported earnings
          console.warn(`Very low owner earnings for ${symbol}: ${(earnings.ownerEarnings / reportedEarnings * 100).toFixed(1)}% of reported earnings`)
        }
    ...
    test('should_implement_consistent_calculations_across_calls', async () => {
      ...
      if (earnings1 && earnings2) {
        ...
        console.log(`✓ Consistent calculations: confidence ${(earnings1.confidence * 100).toFixed(0)}%`)
      }
    })
  })
