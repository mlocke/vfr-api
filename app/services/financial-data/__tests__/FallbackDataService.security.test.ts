...
describe('FallbackDataService Security Integration', () => {
  ...
  describe('getFundamentalRatios Security Tests', () => {
    ...
    test('should sanitize error messages', async () => {
      ...
      const mockProvider = {
        getFundamentalRatios: jest.fn().mockRejectedValue(
          new Error('API key: secret123, Database connection: mongodb://user:pass@localhost')
        )
      }
    ...
    test('should validate each response in batch', async () => {
      ...
      const mockProvider = {
        getBatchPrices: jest.fn().mockResolvedValue(new Map([
          ['AAPL', { symbol: 'AAPL', price: 150, timestamp: Date.now(), source: 'test' }],
          ['MSFT', { symbol: 'MSFT', price: -100, timestamp: Date.now(), source: 'test' }] // Invalid price
        ]))
      }
  ...
  describe('Error Handling Security Tests', () => {
    test('should not leak sensitive information in error responses', async () => {
      ...
      const mockProvider = {
        getFundamentalRatios: jest.fn().mockRejectedValue(
          new Error('Database error: connection failed to mongodb://admin:secret@localhost:27017/financials')
        )
      }
