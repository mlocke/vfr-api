...
describe("Yahoo Finance Sentiment API Performance Tests", () => {
	...
	describe("Batch Processing Performance", () => {
		it("should efficiently process multiple symbols via batching", async () => {
			...
			const metrics = await measurePerformance(async () => {
				...
				const promises = batchSymbols.map(symbol => sentimentAPI.getNewsSentiment(symbol));
			...
			console.log(
				`⚡ Batch processing: ${metrics.responseTime}ms for ${batchSymbols.length} symbols`
			);
	...
	describe("Caching Performance", () => {
		it("should achieve high cache hit rate after warmup", async () => {
			...
			const cacheTestPromises = Array(5)
				.fill(null)
				.map(() => sentimentAPI.getNewsSentiment(symbol));
			...
			console.log(
				`⚡ Cache response time: ${Math.round(cacheMetrics.responseTime / cacheTestPromises.length)}ms avg`
			);
		...
		it("should prevent memory bloat with LRU eviction", async () => {
			...
			console.log(
				`🧠 Memory after large batch: ${Math.round(metrics.memoryDelta * 100) / 100}MB`
			);
	...
	describe("Error Handling Performance", () => {
		it("should handle invalid symbols efficiently", async () => {
			...
			console.log(
				`🚨 Error handling: ${metrics.responseTime}ms for ${invalidSymbols.length} invalid symbols`
			);
			console.log(`💾 Memory usage: ${Math.round(metrics.memoryDelta * 100) / 100}MB`);
	...
	describe("Overall Performance Summary", () => {
		it("should meet all performance targets in comprehensive test", async () => {
			...
			console.log(
				`🧠 Memory usage: ${Math.round(comprehensiveMetrics.memoryDelta * 100) / 100}MB`
			);
