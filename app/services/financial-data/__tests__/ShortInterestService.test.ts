...
describe('ShortInterestService Integration Tests', () => {
  ...
  describe('NO MOCK DATA Compliance', () => {
    test('should return unavailable when no API keys configured', async () => {
      const result = await shortInterestServiceNoKeys.getShortInterestImpactForStock('TSLA', 'technology', 75)
      ...
      const hasUnavailableMessage = result.factors.some(factor =>
        factor.includes('Short interest data unavailable') || factor.includes('no API keys')
      )
    ...
    test('should return null for bulk analysis when no API keys', async () => {
      ...
      expect(result.error).toMatch(/Short interest analysis unavailable|No short interest data available/)
      expect(result.data).toBeUndefined()
    })

    test('should return null for direct data fetch when no API keys', async () => {
      const result = await shortInterestServiceNoKeys.getShortInterestData('TSLA', 'technology')
      expect(result).toBeNull()
    })

    test('should return null for comprehensive analysis when no API keys', async () => {
      const result = await shortInterestServiceNoKeys.analyzeStockShortInterestImpact('TSLA', 'technology', 75)
      expect(result).toBeNull()
    })
  })
  ...
  describe('Real API Integration (when keys available)', () => {
    test('should attempt real API calls when keys are configured', async () => {
      if (!hasApiKeys) {
        console.log('⚠️  Skipping real API tests - no FINRA_API_KEY or POLYGON_API_KEY configured')
        return
      }

      const result = await shortInterestService.getShortInterestImpactForStock('AAPL', 'technology', 75)
    ...
    test('should handle real API failures gracefully', async () => {
      ...
      const result = await shortInterestService.getShortInterestImpactForStock('INVALID_SYMBOL_TEST_123', 'technology', 50)
  ...
  describe('Data Structure Validation', () => {
    test('should return proper structure when real data available', async () => {
      ...
      if (result) {
        ...
        console.log('✅ No short interest data returned - API keys may not provide access to this data')
      }
    })

    test('should handle invalid symbols gracefully without mock data', async () => {
      const result = await shortInterestService.getShortInterestImpactForStock('INVALID123', 'technology', 50)
    ...
    test('should validate input symbols for security', async () => {
      ...
      for (const symbol of maliciousSymbols) {
        const result = await shortInterestService.getShortInterestImpactForStock(symbol, 'technology', 50)
  ...
  describe('Risk Analysis (Real Data Only)', () => {
    test('should provide squeeze analysis only when real data available', async () => {
      ...
      for (const symbol of symbols) {
        const result = await shortInterestService.analyzeStockShortInterestImpact(symbol, 'technology', 50)

        if (result && result.confidence > 0) {
          // Only validate when real data is available
          expect(result.squeezeScenarios).toBeDefined()

          if (result.riskFactors.squeezeRisk.level === 'high' || result.riskFactors.squeezeRisk.level === 'extreme') {
  ...
  describe('Alternative Data Component Integration', () => {
    test('should provide compatible data structure for Alternative Data component', async () => {
      const result = await shortInterestService.getShortInterestImpactForStock('AAPL', 'technology', 65)
  ...
  describe('Error Handling and Resilience', () => {
    test('should handle service failures gracefully without mock data', async () => {
      ...
      const result = await shortInterestService.getShortInterestImpactForStock('', 'technology', 50)
    ...
    test('should provide clear messaging when data unavailable', async () => {
      const result = await shortInterestService.getShortInterestImpactForStock('UNKNOWN_SYMBOL_123', 'technology', 70)
      ...
      const hasUnavailableMessage = result.factors.some(factor =>
        factor.includes('unavailable') || factor.includes('error')
      )
      expect(hasUnavailableMessage).toBe(true)
    })
  })
  ...
  describe('Caching and Performance', () => {
    ...
    test('should complete analysis within reasonable time', async () => {
      ...
      const result = await shortInterestService.getShortInterestImpactForStock('AAPL', 'technology', 80)
