...
describe('MacroeconomicAnalysisService - Data Foundation Enhancement', () => {
  ...
  describe('Core Implementation', () => {
    ...
    test('should orchestrate parallel data collection with performance targets', async () => {
      ...
      if (context) {
        ...
        expect(['low', 'moderate', 'high', 'declining']).toContain(context.inflationEnvironment)
        expect(['accommodative', 'neutral', 'restrictive']).toContain(context.monetaryPolicy)
        expect(['expansion', 'peak', 'contraction', 'trough']).toContain(context.economicCycle)
    ...
    test('should calculate sector-specific impact multipliers', async () => {
      const context = await service.getMacroeconomicContext()
      ...
      if (context && context.sectorImpacts) {
        ...
        const expectedSectors = [
          ...
          'industrials', 'materials', 'consumerDiscretionary',
          'consumerStaples', 'realEstate', 'communication'
        ]
        ...
        console.log(`✓ Sector coverage: ${foundSectors}/${expectedSectors.length} sectors`)

        // Show sector performance distribution
        const impacts = Object.entries(context.sectorImpacts)
        const sorted = impacts.sort(([, a], [, b]) => b - a)
        if (sorted.length >= 2) {
          console.log(`✓ Best performer: ${sorted[0][0]} (${sorted[0][1].toFixed(2)})`)
          console.log(`✓ Worst performer: ${sorted[sorted.length-1][0]} (${sorted[sorted.length-1][1].toFixed(2)})`)
        }
      }
    }, testTimeout)

    test('should demonstrate 20% weight integration readiness', async () => {
      const context = await service.getMacroeconomicContext()
      ...
      if (context) {
        ...
        const weightedScore =
          (mockAnalysis.technical * 0.40) +
          (mockAnalysis.fundamental * 0.25) +
          (mockAnalysis.macroeconomic * 0.20) +
          (mockAnalysis.other * 0.15)

        console.log(`✓ 20% Weight Integration Simulation:`)
        console.log(`  - Technical (40%): ${mockAnalysis.technical} → ${(mockAnalysis.technical * 0.40).toFixed(2)}`)
        console.log(`  - Fundamental (25%): ${mockAnalysis.fundamental} → ${(mockAnalysis.fundamental * 0.25).toFixed(2)}`)
        console.log(`  - Macroeconomic (20%): ${mockAnalysis.macroeconomic} → ${(mockAnalysis.macroeconomic * 0.20).toFixed(2)}`)
        console.log(`  - Other (15%): ${mockAnalysis.other} → ${(mockAnalysis.other * 0.15).toFixed(2)}`)
        ...
        const macroContribution = mockAnalysis.macroeconomic * 0.20
        console.log(`✅ Macro contributes ${macroContribution.toFixed(2)}/2.0 points to final score`)
      }
    }, testTimeout)
  })

  describe('Enhanced API Integration', () => {
    test('should validate data source integration', async () => {
      const context = await service.getMacroeconomicContext()
    ...
    test('should implement consistent data retrieval', async () => {
      // First call
  ...
  describe('Health Check and Resilience', () => {
    ...
    test('should handle graceful degradation', async () => {
      ...
      if (context) {
        ...
        console.log(`✓ Graceful degradation: Score ${context.overallScore}, Confidence ${(context.confidence*100).toFixed(1)}%`)
      } else {
