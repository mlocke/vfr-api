...
describe("CurrencyDataService", () => {
	...
	describe("Dollar Index (DXY) Data", () => {
		test("should fetch DXY data with performance under 10 seconds", async () => {
			...
			if (dxyData) {
				...
				console.log(
					`✅ DXY: ${dxyData.value} (Strength: ${dxyData.strength}/10, Trend: ${dxyData.trend})`
				);
			} else {
	...
	describe("Currency Pairs", () => {
		test("should fetch major currency pairs", async () => {
			...
			if (pairs.length > 0) {
				...
				console.log(
					`   Example: ${pair.symbol} = ${pair.rate} (${pair.changePercent > 0 ? "+" : ""}${pair.changePercent.toFixed(2)}%)`
				);
			} else {
		...
		test("should fetch individual currency pair", async () => {
			...
			if (pair) {
				...
				console.log(
					`✅ EUR/USD: ${pair.rate} (${pair.changePercent > 0 ? "+" : ""}${pair.changePercent.toFixed(2)}%)`
				);
			} else {
	...
	describe("Currency Strength Analysis", () => {
		test("should calculate currency strengths", async () => {
			...
			if (strengths.length > 0) {
				...
				console.log(
					`   Example: ${strength.currency} strength: ${strength.strengthScore}/10 (${strength.trend})`
				);
			} else {
	...
	describe("Comprehensive Currency Analysis", () => {
		test("should provide complete currency analysis with performance under 5 seconds", async () => {
			...
			if (analysis) {
				...
				console.log(
					`   Risk sentiment: ${analysis.marketSentiment.riskOn ? "Risk-On" : "Risk-Off"}`
				);
		...
		test("should have proper sector exposure configurations", async () => {
			...
			for (const sector of sectors) {
				...
				if (sectorContext) {
					...
					expect(["strengthening", "weakening", "stable"]).toContain(
						sectorContext.currencyTrend
					);
	...
	describe("Performance Requirements", () => {
		...
		test("should handle concurrent requests efficiently", async () => {
			...
			const promises = Array(5)
				.fill(null)
				.map(() => service.getCurrencyAnalysis());
			...
			console.log(
				`🔄 Concurrent test: ${successCount}/5 requests successful in ${elapsed}ms`
			);
	...
	describe("Error Handling & Resilience", () => {
		...
		test("should maintain service availability during partial failures", async () => {
			...
			console.log(
				`✅ Service resilience: ${successfulResults.length}/${results.length} methods successful`
			);
		});
