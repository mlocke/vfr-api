...
export class RateLimitComplianceStrategy {
	 ...
	 */
	async executeWithRateLimit<T>(apiCall: () => Promise<T>, maxRetries: number = 3): Promise<T> {
		let attempts = 0;
	...
	async executeBatch<T>(
		...
		for (let i = 0; i < apiCalls.length; i += maxConcurrency) {
			...
			const batchPromises = batch.map(apiCall => this.executeWithRateLimit(apiCall));
	...
	generateComplianceReport(): ComplianceTestResult {
		...
		const compliance =
			violations.length === 0 && requestsPerSecond <= this.plan.requestsPerSecond;
	...
	private classifyViolationSeverity(
		actual: number,
		expected: number
		...
		if (violations.length > 0) {
			...
			if (severeViolations > 0) {
				recommendations.push(
					`Severe rate limit violations detected (${severeViolations}). Implement exponential backoff.`
				);
			}

			if (moderateViolations > 0) {
				recommendations.push(
					`Moderate violations detected (${moderateViolations}). Increase delay between requests.`
				);
			}

			recommendations.push(
				`Consider implementing request queuing with ${this.calculateOptimalDelay()}ms delays.`
			);
		}

		if (efficiency < 50) {
			recommendations.push(
				"Low API utilization efficiency. Consider increasing request rate within limits."
			);
		}

		if (efficiency > 90) {
			recommendations.push(
				"High utilization rate may risk rate limiting. Consider reducing to 80% target."
			);
		}
...
export function createRateLimitTestSuite(
	planType: keyof (typeof RateLimitComplianceStrategy)["FMP_PLANS"] = "starter"
) {
	return {
		...
		toBeCompliant() {
			if (result.compliance) {
				...
				return {
					pass: false,
					message: () =>
						`Rate limit compliance failed for ${result.planType}. Efficiency: ${result.efficiency.toFixed(1)}%${violationsSummary}\nRecommendations: ${result.recommendations.join(", ")}`,
				};
			}
		},
