...
describe("FREDAPI Real API Integration Tests", () => {
			...
			async () => {
				...
				if (hasRealApiKey) {
					...
					if (isHealthy) {
						...
						console.warn(
							"⚠ FRED API health check failed - may be due to network issues or rate limiting"
						);
					}
				} else {
					console.warn(
						"⚠ FRED_API_KEY not configured or invalid - skipping health check validation"
					);
	...
	describe("Core Financial Data Provider Methods", () => {
		test(
			"should have getStockPrice method and handle economic data",
			async () => {
				...
				if (!hasRealApiKey) {
					console.warn(
						"⚠ FRED_API_KEY not configured or invalid - skipping live API tests"
					);
					return;
				}
				...
				if (stockData) {
					...
					console.warn(
						"⚠ No data returned for UNRATE - API may be rate limited or unavailable"
					);
				}
			},
			testTimeout
		...
		test(
			"should have getCompanyInfo method and return series metadata",
			async () => {
				// Verify method exists
		...
		test(
			"should have getMarketData method and return observation data",
			async () => {
				// Verify method exists
	...
	describe("FRED-Specific Methods", () => {
		test(
			"should have searchSeries method and search capabilities",
			async () => {
				// Verify method exists
				...
				if (!hasRealApiKey) {
					console.warn(
						"⚠ FRED_API_KEY not configured or invalid - skipping search test"
					);
					return;
				}
				...
				if (searchResults && searchResults.length > 0) {
					...
					console.log(
						`✓ FRED search found ${searchResults.length} series for 'unemployment'`
					);
					console.log(
						"  First result:",
						searchResults[0].id,
		...
		test(
			"should have getTreasuryRates method and return treasury data",
			async () => {
				// Verify method exists
				...
				if (!hasRealApiKey) {
					console.warn(
						"⚠ FRED_API_KEY not configured or invalid - skipping treasury rates test"
					);
					return;
				}
				...
				if (treasuryRates) {
					...
					const foundPeriods = expectedPeriods.filter(
						period =>
							treasuryRates.hasOwnProperty(period) &&
							typeof (treasuryRates as any)[period] === "number"
					);

					if (foundPeriods.length > 0) {
						console.log(
							`✓ FRED returned treasury rates for periods: ${foundPeriods.join(", ")}`
						);
						console.log(
							"  Sample rates:",
							foundPeriods.reduce((acc, period) => {
		...
		test(
			"should have getTreasuryAnalysisData method and return analysis",
			async () => {
				// Verify method exists
				...
				if (!hasRealApiKey) {
					console.warn(
						"⚠ FRED_API_KEY not configured or invalid - skipping treasury analysis test"
					);
					return;
				}
				...
				if (analysisData) {
					...
					if (
						analysisData.yieldCurve &&
						typeof analysisData.yieldCurve.shape === "string"
					) {
		...
		test("should have getPopularIndicators method and return indicator list", () => {
			...
			console.log(
				"  Sample indicators:",
				indicators.slice(0, 3).map(i => `${i.symbol}: ${i.name}`)
			);
		});
	});
	...
	describe("Error Handling and Edge Cases", () => {
		...
		test(
			"should handle invalid series IDs",
			async () => {
				const hasRealApiKey =
					process.env.FRED_API_KEY &&
					process.env.FRED_API_KEY.length === 32 &&
				...
				if (!hasRealApiKey) {
					console.warn(
						"⚠ FRED_API_KEY not configured or invalid - skipping invalid series test"
					);
					return;
				}
		...
		test(
			"should handle API timeouts and errors",
			async () => {
	...
	describe("Performance and Caching", () => {
		test(
			"should measure response times for real API calls",
			async () => {
				const hasRealApiKey =
					process.env.FRED_API_KEY &&
					process.env.FRED_API_KEY.length === 32 &&
				...
				if (!hasRealApiKey) {
					console.warn(
						"⚠ FRED_API_KEY not configured or invalid - skipping performance test"
					);
					return;
				}
		...
		test(
			...
			async () => {
				...
				if (!hasRealApiKey) {
					console.warn(
						"⚠ FRED_API_KEY not configured or invalid - skipping concurrent test"
					);
					return;
				}
				...
				console.log(
					`📊 Concurrent requests for ${testSeries.length} series completed in ${totalTime}ms`
				);
				...
				console.log(
					`✓ ${successfulResults.length}/${testSeries.length} concurrent requests succeeded`
				);
				...
				if (successfulResults.length === 0) {
					console.warn(
						"⚠ All concurrent requests failed - this may indicate API rate limiting, network issues, or service unavailability"
					);
					...
					results.forEach((result, index) => {
						if (result.status === "rejected") {
							...
							console.warn(
								`  ${testSeries[index]} returned null (likely API error or missing data)`
							);
						}
					});
				}
	...
	describe("Data Quality and Validation", () => {
		test(
			"should return properly formatted data structures",
			async () => {
				const hasRealApiKey =
					process.env.FRED_API_KEY &&
					process.env.FRED_API_KEY.length === 32 &&
				...
				if (!hasRealApiKey) {
					console.warn(
						"⚠ FRED_API_KEY not configured or invalid - skipping data validation test"
					);
					return;
				}
				...
				if (stockData) {
					...
					console.log(
						`  CPI value: ${stockData.price}, timestamp: ${new Date(stockData.timestamp).toISOString()}`
					);
				} else {
