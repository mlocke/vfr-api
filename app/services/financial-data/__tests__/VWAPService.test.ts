...
describe("VWAPService Tests - Business Logic", () => {
	...
	describe("VWAP Score Calculation", () => {
		test(
			"should calculate VWAP score correctly for different scenarios",
			() => {
				const testCases = [
					// At VWAP (neutral)
				...
				testCases.forEach(testCase => {
					...
					console.log(
						`VWAP Score Test: ${testCase.deviationPercent}% → ${testCase.signal}/${testCase.strength} = ${score}`
					);
				});
			},
			TEST_TIMEOUT
	...
	describe("Signal Determination Logic", () => {
		test(
			"should determine signals correctly based on deviation thresholds",
			() => {
				...
				signalTests.forEach(test => {
					...
					console.log(
						`Signal Test: ${test.deviationPercent}% → ${test.expectedSignal} (score: ${score})`
					);
				});
			},
			TEST_TIMEOUT
	...
	describe("Strength Determination Logic", () => {
		test(
			"should determine strength correctly based on deviation magnitude",
			() => {
				const strengthTests = [
				...
				strengthTests.forEach(test => {
					...
					[test.absDeviation, -test.absDeviation].forEach(deviation => {
						const signal =
							Math.abs(deviation) < 0.1 ? "at" : deviation > 0 ? "above" : "below";
						...
						const expectedMagnitude =
							test.expectedStrength === "weak"
								? 0.3
								: test.expectedStrength === "moderate"
									? 0.6
									: 1.0;
						...
						console.log(
							`Strength Test: ${deviation}% → ${test.expectedStrength} (score: ${score})`
						);
					});
				});
			},
	...
	describe("Edge Cases and Error Handling", () => {
		test(
			"should handle zero and extreme values correctly",
			() => {
				const edgeCases = [
				...
				edgeCases.forEach(testCase => {
					...
					console.log(
						`Edge Case Test: ${testCase.deviationPercent}% → ${testCase.signal}/${testCase.strength} = ${score}`
					);
				});
			},
			TEST_TIMEOUT
		...
		test(
			"should be consistent with calculation results",
			() => {
	...
	describe("Integration Logic Verification", () => {
		test(
			"should demonstrate complete signal-to-score workflow",
			() => {
				...
				realWorldScenarios.forEach(scenario => {
					...
					console.log(
						`  Signal: ${scenario.expectedSignal}, Strength: ${scenario.expectedStrength}`
					);
