...
import {
	...
	LiquidityMetrics,
	ExtendedMarketData,
} from "../ExtendedMarketDataService";
...
describe("ExtendedMarketDataService Integration Tests", () => {
	...
	describe("Core Extended Market Data Functionality", () => {
		test(
			"should get comprehensive extended market data for valid symbol",
			async () => {
				...
				expect(["pre-market", "market-hours", "after-hours", "closed"]).toContain(
					result?.extendedHours.marketStatus
				);
			},
			TEST_TIMEOUT
		);
	...
	describe("Bid/Ask Spread Analysis", () => {
		test(
			"should get bid/ask spread data for liquid stock",
			async () => {
				...
				if (spread) {
					...
					console.log(
						`Bid/ask data not available for ${symbol} - may be outside market hours`
					);
				}
			},
			TEST_TIMEOUT
		...
		test(
			"should handle multiple symbols for batch spread analysis",
			async () => {
		...
		test(
			"should cache bid/ask spread data",
			async () => {
	...
	describe("Liquidity Metrics Calculation", () => {
		test(
			"should calculate liquidity metrics when spread data available",
			async () => {
				const symbol = TEST_SYMBOLS[0]; // AAPL
				...
				if (extendedData?.liquidityMetrics) {
					...
					console.log(
						`Liquidity metrics not available for ${symbol} - may be outside market hours`
					);
				}
			},
			TEST_TIMEOUT
		...
		test(
			"should provide consistent liquidity scoring",
			async () => {
	...
	describe("Extended Hours Trading Data", () => {
		test(
			"should get extended hours data",
			async () => {
		...
		test(
			"should get current market status",
			async () => {
		...
		test(
			"should handle extended hours data when available",
			async () => {
				const symbol = TEST_SYMBOLS[0]; // AAPL
				const extendedData = await service.getExtendedMarketData(symbol);
	...
	describe("Integration Scoring for StockSelectionService", () => {
		test(
			"should calculate extended market score for technical analysis",
			async () => {
				const symbol = TEST_SYMBOLS[0]; // AAPL
				const extendedData = await service.getExtendedMarketData(symbol);
		...
		test(
			"should provide meaningful scores for different liquidity levels",
			async () => {
	...
	describe("Performance and Memory Management", () => {
		test(
			...
			async () => {
				...
				console.log(
					`Processed ${validResults.length}/${symbols.length} symbols in ${duration}ms`
				);
			},
			TEST_TIMEOUT * 2
		);
		...
		test(
			"should cache data with appropriate TTL",
			async () => {
		...
		test(
			...
			async () => {
				...
				await Promise.all(symbols.map(symbol => service.getExtendedMarketData(symbol)));
	...
	describe("Real-time Data Quality", () => {
		test(
			"should provide recent timestamp data",
			async () => {
				const symbol = TEST_SYMBOLS[0];
				const result = await service.getExtendedMarketData(symbol);
		...
		test(
			...
			async () => {
				...
				const requests = Array(5)
					.fill(null)
					.map(() => service.getBidAskSpread(symbol));
