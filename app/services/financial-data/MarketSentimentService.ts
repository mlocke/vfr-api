...
export class MarketSentimentService {
	...
	private async calculateOverallSentiment(
		...
		const economicData =
			economicDataResult.status === "fulfilled" ? economicDataResult.value : null;
	...
	private calculateVIXSentiment(vixData: any): SentimentScore {
		...
		if (vix >= 30)
			sentimentValue = 10; // Extreme fear
		else if (vix >= 25)
			sentimentValue = 25; // Fear
		else if (vix >= 20)
			sentimentValue = 40; // Cautious
		else if (vix >= 15)
			sentimentValue = 60; // Optimistic
		else if (vix >= 12)
			sentimentValue = 75; // Confident
	...
	private calculateSectorSentiments(marketData: MarketIndicesData | null): SectorSentiment[] {
		...
		if (!marketData?.sectors) {
			console.warn(
				"MarketSentimentService: No market data available, using default sector sentiments"
			);
			return sectors.map(sector =>
				this.createDefaultSectorSentiment(sector.symbol, sector.name)
			);
		}
		...
		const sectorSentiments = sectors.map(sector => {
			...
			if (!sectorData || sectorData.changePercent === undefined) {
				console.warn(
					`MarketSentimentService: No data for ${sector.symbol}, using default sentiment`
				);
			...
			if (
				sectorData.changePercent === 0 &&
				sectorData.change === 0 &&
				sectorData.value === 0
			) {
				console.warn(
					`MarketSentimentService: ${sector.symbol} has all zero values (invalid data), using default sentiment`
				);
				return this.createDefaultSectorSentiment(sector.symbol, sector.name);
			}
		...
		if (defaultsUsed > 2) {
			console.warn(
				`MarketSentimentService: Using default sentiment for ${defaultsUsed}/${sectors.length} sectors due to API rate limiting`
			);
		}
	...
	private calculateEconomicSentiment(economicData: any): SentimentScore {
		...
		if (economicData?.creditSpreads?.data?.length > 0) {
			...
			if (spreads > 500)
				score -= 15; // High spreads are bearish
	...
	private combinesentiments(
		weightedSentiments: Array<{ weight: number; sentiment: SentimentScore }>
	): SentimentScore {
		let totalValue = 0;
		let totalWeight = 0;
	...
	private averageSectorSentiment(sectors: SectorSentiment[]): SentimentScore {
		...
		const avgConfidence =
			sectors.reduce((sum, s) => sum + s.sentiment.confidence, 0) / sectors.length;
	 ...
	 */
	async getEnhancedMarketSentimentWithRankings(): Promise<
		MarketSentimentData & {
			sectorRankings: {
				topPerformers: Array<{
					sector: string;
	...
	> {
		try {
			...
			const baseSentiment =
				sentimentData.status === "fulfilled"
					? sentimentData.value
					: await this.getMarketSentiment(); // Fallback

			const rankings = sectorRankings.status === "fulfilled" ? sectorRankings.value : null;
	...
	async getSectorRankingsSummary(): Promise<{
		...
		try {
			...
			const rotationStrength = Math.min(
				10,
				Math.max(0, (topPerformance - bottomPerformance) / 2)
			);

			// Determine market phase from average performance
			const avgPerformance =
				sorted.reduce((sum, sector) => sum + sector.returns.oneMonth, 0) / sorted.length;
