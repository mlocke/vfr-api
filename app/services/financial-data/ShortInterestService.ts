...
export class ShortInterestService {
	...
	constructor(options?: {
		...
		const { finraApiKey, polygonApiKey, timeout = 10000, throwErrors = false } = options || {};
		...
		if (!this.isAPIKeyAvailable) {
			console.log(
				"⚠️ Short Interest Service initialized without API keys - service will return unavailable"
			);
			console.log(
				"💡 To use real short interest data, configure FINRA_API_KEY or POLYGON_API_KEY environment variables"
			);
		} else {
			const availableAPIs = [];
			...
			console.log(
				`✅ Short Interest Service initialized with ${availableAPIs.join(" + ")} API integration`
			);
		}
	}
	 ...
	 */
	async analyzeStockShortInterestImpact(
		symbol: string,
		sector: string,
		baseScore: number
	): Promise<StockShortInterestImpact | null> {
		try {
			...
			if (!this.isAPIKeyAvailable) {
				console.log(
					`📊 Short interest analysis unavailable for ${symbol} - no API keys configured`
				);
				return null;
			}
			...
			// Generate insights
			const insights = this.generateShortInterestInsights(
				shortInterestData,
				riskFactors,
				sector
			);
			...
			const dataSource = this.finraApiKey
				? "finra"
				: this.polygonApiKey
					? "polygon"
					: "unavailable";
	 ...
	 */
	async analyzeBulkShortInterestImpact(
		stocks: Array<{ symbol: string; sector: string; baseScore: number }>
	): Promise<BulkShortInterestAnalysisResponse> {
		...
		try {
			...
			const portfolioRisk =
				averageShortRatio > 15 ? "high" : averageShortRatio > 8 ? "moderate" : "low";
	...
	async getShortInterestData(symbol: string, sector: string): Promise<ShortInterestData | null> {
		try {
			...
			if (!this.isAPIKeyAvailable) {
				console.log(
					`📊 Short interest data unavailable for ${symbol} - no API keys configured`
				);
				return null;
			}
	 ...
	 */
	private processPolygonShortInterestData(
		apiData: any,
		symbol: string
		...
		try {
			...
			console.log(
				`📊 Polygon aggregate proxy for ${symbol}: Volume ${volume.toLocaleString()}, Est. short interest ${estimatedShortInterest.toLocaleString()}`
			);
	 ...
	 */
	private generateShortInterestRiskFactors(
		...
		// Determine timeframe
		const timeframe =
			shortData.daysTooCover > 5
				? "weeks"
				: shortData.daysTooCover > 2
					? "days"
					: shortData.daysTooCover > 1
						? "days"
						: "unlikely";
		...
		return {
			...
			liquidityRisk: {
				level:
					shortData.shortInterestRatio > 15
						? "high"
						: shortData.shortInterestRatio > 8
							? "moderate"
							: "low",
			...
			momentum: {
				...
				volumePattern:
					shortData.shortVolumeRatio && shortData.shortVolumeRatio > 40
						? "extreme"
						: shortData.shortVolumeRatio && shortData.shortVolumeRatio > 30
							? "elevated"
							: "normal",
			},
		};
	 ...
	 */
	private generateShortInterestInsights(
		shortData: ShortInterestData,
		...
		if (
			riskFactors.squeezeRisk.level === "high" ||
			riskFactors.squeezeRisk.level === "extreme"
		) {
			opportunities.push(
				`High short squeeze potential with ${riskFactors.squeezeRisk.probability}% probability`
			);
		}
		...
		if (shortData.daysTooCover > 3) {
			opportunities.push(
				"High days-to-cover suggests potential for sustained upward pressure"
			);
		}
	...
	private calculateAdjustedScore(
		...
		const adjustedScore =
			baseScore * (1 - shortInterestWeight) + shortInterestImpact * shortInterestWeight;
	 ...
	 * Generate squeeze scenarios
	 */
	private generateSqueezeScenarios(
		shortData: ShortInterestData,
		riskFactors: ShortInterestRiskFactors
	) {
		const scenarios = [];
	...
	private determineHistoricalPattern(
		shortData: ShortInterestData
	): "cyclical" | "trending" | "volatile" | "stable" {
	...
	private calculateSupportLevels(shortData: ShortInterestData): number[] {
		...
		const basePrice = 100;
		return [basePrice * 0.95, basePrice * 0.9, basePrice * 0.85];
	}

	private calculateResistanceLevels(shortData: ShortInterestData): number[] {
		const basePrice = 100;
		return [basePrice * 1.05, basePrice * 1.1, basePrice * 1.15];
	}
	...
	private determineInstitutionalConsensus(
		shortData: ShortInterestData
	...
	private getConflictIndicators(
		shortData: ShortInterestData,
		riskFactors: ShortInterestRiskFactors
	): string[] {
		const indicators = [];
		if (
			shortData.shortInterestRatio > 20 &&
			riskFactors.liquidityRisk.institutionalOwnership > 70
		) {
			indicators.push(
				"High institutional ownership vs high short interest suggests disagreement"
			);
		}
		return indicators;
	}
	...
	private createDefaultConfig(): ShortInterestConfig {
		const primarySource = this.finraApiKey
			? "finra"
			: this.polygonApiKey
				? "polygon"
				: "unavailable";
		...
		try {
			...
			return {
				...
				details: {
					...
					lastReportDate: anyApiAvailable
	 ...
	 */
	async getShortInterestImpactForStock(
		symbol: string,
		sector: string,
		currentScore: number
	): Promise<{
		...
		try {
			...
			if (
				shortImpact.riskFactors.squeezeRisk.level === "high" ||
				shortImpact.riskFactors.squeezeRisk.level === "extreme"
			) {
			...
			if (shortImpact.riskFactors.squeezeRisk.level === "extreme") {
				factors.push(
					`Extreme short squeeze risk (${shortImpact.riskFactors.squeezeRisk.probability}% probability)`
				);
			} else if (shortImpact.riskFactors.squeezeRisk.level === "high") {
				factors.push(
					`High short squeeze potential with ${shortImpact.shortInterestData.daysTooCover} days to cover`
				);
			}

			if (shortImpact.shortInterestData.shortInterestRatio > 20) {
				factors.push(
					`High short interest (${shortImpact.shortInterestData.shortInterestRatio}%) indicates strong bearish sentiment`
				);
			}
			...
			if (factors.length === 0) {
				factors.push(
					`Short interest ratio: ${shortImpact.shortInterestData.shortInterestRatio}%`
				);
			}
