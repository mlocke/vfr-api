...
export class EODHDAPI implements FinancialDataProvider {
  ...
  async getUnicornBayOptionsChain(symbol: string, options?: {
    ...
    try {
      ...
      const transformUnicornBayContract = (contract: UnicornBayOptionsContract): OptionsContract => {
        ...
        const spreadQuality = contract.spread_percent <= 2 ? 'tight' :
                             contract.spread_percent <= 5 ? 'moderate' : 'wide'
        ...
        return {
          ...
          volumeQuality: contract.volume >= 100 ? 'high' : contract.volume >= 20 ? 'medium' : 'low'
        } as OptionsContract & {
          rho?: number
          midpoint?: number
      ...
      console.error(`❌ EODHD UnicornBay: Error fetching enhanced options chain for ${symbol}:`, error)
      return null
    }
  }
  ...
  async getUnicornBayPutCallRatio(symbol: string): Promise<PutCallRatio | null> {
    try {
      ...
      if (!optionsChain) {
        console.warn(`⚠️ EODHD UnicornBay: No options chain data for put/call ratio calculation`)
        return null
      }

      if (!optionsChain.calls || !optionsChain.puts ||
      ...
      const totalCallVolume = optionsChain.calls.reduce((sum, call) => sum + (call.volume || 0), 0)
      const totalPutVolume = optionsChain.puts.reduce((sum, put) => sum + (put.volume || 0), 0)
      const totalCallOpenInterest = optionsChain.calls.reduce((sum, call) => sum + (call.openInterest || 0), 0)
      const totalPutOpenInterest = optionsChain.puts.reduce((sum, put) => sum + (put.openInterest || 0), 0)

      // Calculate weighted ratios based on liquidity scores
      const callsWithLiquidity = optionsChain.calls.filter(c => (c as any).liquidityScore > 2)
      const putsWithLiquidity = optionsChain.puts.filter(p => (p as any).liquidityScore > 2)

      const liquidCallVolume = callsWithLiquidity.reduce((sum, call) => sum + (call.volume || 0), 0)
      const liquidPutVolume = putsWithLiquidity.reduce((sum, put) => sum + (put.volume || 0), 0)
      ...
      return {
        ...
        metadata: {
          ...
          enhancedMetrics: {
            avgCallLiquidity: callsWithLiquidity.length > 0 ?
              callsWithLiquidity.reduce((sum, c) => sum + ((c as any).liquidityScore || 0), 0) / callsWithLiquidity.length : 0,
            avgPutLiquidity: putsWithLiquidity.length > 0 ?
              putsWithLiquidity.reduce((sum, p) => sum + ((p as any).liquidityScore || 0), 0) / putsWithLiquidity.length : 0
          }
        }
      }
    } catch (error) {
      console.error(`❌ EODHD UnicornBay: Error fetching enhanced put/call ratio for ${symbol}:`, error)
      return null
    }
  }
   ...
   */
  async getPutCallRatio(symbol: string, useEnhanced: boolean = true): Promise<PutCallRatio | null> {
    try {
   ...
   * Requires EODHD options add-on subscription
   */
  async getOptionsChain(symbol: string, expiration?: string, useEnhanced: boolean = true): Promise<OptionsChain | null> {
    try {
      ...
      console.log(`✅ EODHD API: Retrieved ${calls.length} calls, ${puts.length} puts for ${symbol} (standard data)`)
   ...
   */
  async getUnicornBayOptionsFlow(symbols: string[], timeframe: '1D' | '5D' | '1M' = '1D'): Promise<{
    symbol: string
    flowMetrics: {
      callFlow: number
      putFlow: number
    ...
    try {
      console.log(`📊 EODHD UnicornBay: Fetching options flow data for ${symbols.length} symbols`)
      ...
      for (const symbol of symbols) {
        ...
        const avgCallLiquidity = optionsChain.calls.length > 0 ?
          optionsChain.calls.reduce((sum, c) => sum + ((c as any).liquidityScore || 0), 0) / optionsChain.calls.length : 0
        const avgPutLiquidity = optionsChain.puts.length > 0 ?
          optionsChain.puts.reduce((sum, p) => sum + ((p as any).liquidityScore || 0), 0) / optionsChain.puts.length : 0

        const unusualActivity = avgCallLiquidity > 7 || avgPutLiquidity > 7 || Math.abs(flowRatio - 1) > 2
      ...
      console.log(`✅ EODHD UnicornBay: Calculated options flow for ${flowData.length} symbols`)
      return flowData
   ...
   */
  async getUnicornBayGreeksAnalysis(symbol: string, expirations?: string[]): Promise<{
    symbol: string
    portfolioGreeks: {
      totalDelta: number
      totalGamma: number
    ...
    try {
      ...
      const totalDelta = calls.reduce((sum, c) => sum + (c.delta || 0) * (c.volume || 0), 0) +
                        puts.reduce((sum, p) => sum + (p.delta || 0) * (p.volume || 0), 0)

      const totalGamma = calls.reduce((sum, c) => sum + (c.gamma || 0) * (c.volume || 0), 0) +
                        puts.reduce((sum, p) => sum + (p.gamma || 0) * (p.volume || 0), 0)

      const totalTheta = calls.reduce((sum, c) => sum + (c.theta || 0) * (c.volume || 0), 0) +
                        puts.reduce((sum, p) => sum + (p.theta || 0) * (p.volume || 0), 0)

      const totalVega = calls.reduce((sum, c) => sum + (c.vega || 0) * (c.volume || 0), 0) +
      ...
      const ivStdDev = Math.sqrt(ivs.reduce((sum, iv) => sum + Math.pow(iv - avgIV, 2), 0) / ivs.length)
      ...
      console.error(`❌ EODHD UnicornBay: Error fetching Greeks analysis for ${symbol}:`, error)
      return null
    }
  }
  ...
  async getUnicornBayIVSurface(symbol: string): Promise<{
    ...
    try {
      ...
      const ivSurface = Array.from(expirationGroups.entries()).map(([expiration, { calls, puts }]) => {
        // Calculate days to expiration
        const daysToExpiration = Math.max(1, Math.round((new Date(expiration).getTime() - Date.now()) / (1000 * 60 * 60 * 24)))
        ...
        const validIVs = strikes.flatMap(s => [s.callIV, s.putIV]).filter(iv => iv !== null) as number[]
        const avgIV = validIVs.length > 0 ? validIVs.reduce((sum, iv) => sum + iv, 0) / validIVs.length : 0

        // Calculate skew (put vs call IV difference at similar moneyness)
        const atmStrikes = strikes.filter(s => s.moneyness >= 0.95 && s.moneyness <= 1.05)
        const atmCallIVs = atmStrikes.map(s => s.callIV).filter(iv => iv !== null) as number[]
        const atmPutIVs = atmStrikes.map(s => s.putIV).filter(iv => iv !== null) as number[]

        const avgCallIV = atmCallIVs.length > 0 ? atmCallIVs.reduce((sum, iv) => sum + iv, 0) / atmCallIVs.length : 0
        const avgPutIV = atmPutIVs.length > 0 ? atmPutIVs.reduce((sum, iv) => sum + iv, 0) / atmPutIVs.length : 0
      ...
      const allIVs = ivSurface.flatMap(exp => exp.strikes.flatMap(s => [s.callIV, s.putIV])).filter(iv => iv !== null) as number[]
      const atmIV = allIVs.length > 0 ? allIVs.reduce((sum, iv) => sum + iv, 0) / allIVs.length : 0
      ...
      const termStructure = shortTermIV > longTermIV ? 'inverted' :
                           Math.abs(shortTermIV - longTermIV) < 0.02 ? 'flat' : 'normal'
  ...
  async checkOptionsAvailability(): Promise<{ [key: string]: boolean | string }> {
    try {
      ...
      const availability = {
        ...
        dataQuality: unicornBayAvailable ? 'enhanced' : standardAvailable ? 'standard' : 'unavailable'
      }
