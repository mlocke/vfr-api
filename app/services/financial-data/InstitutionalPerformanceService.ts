...
export class InstitutionalPerformanceService {
	...
	private readonly performanceCache = new Map<
		string,
	...
	private optimizeMemoryCache(): void {
		...
		this.errorHandler.logger.debug(
			`Optimized performance cache: removed ${toRemove.length} entries`
		);
	}
	 ...
	 */
	async getInstitutionalPerformance(
		symbol: string,
		institution?: string
		...
		try {
			...
			const cacheKey = this.fmpCache.generateKey(
				`${symbol}:${institution || "all"}`,
				"institutional_performance"
			);
			const cachedData = this.fmpCache.get<InstitutionalPerformance[]>(
				cacheKey,
				"institutional_performance"
			);
			...
			if (cachedData) {
				...
				this.performanceCache.set(memoryKey, {
					data: cachedData,
					timestamp: Date.now(),
					hits: 1,
				});
			...
			const holdersData =
				institutionalHolders.status === "fulfilled" ? institutionalHolders.value : [];
	...
	async getTopPerformers(symbol: string, limit: number = 5): Promise<InstitutionalPerformance[]> {
		try {
			...
			const sorted = performances.sort(
				(a, b) => b.benchmarkComparison.alpha - a.benchmarkComparison.alpha
			);
	 ...
	 */
	async getInstitutionalPerformanceBatch(
		symbols: string[]
	): Promise<{ symbol: string; performances: InstitutionalPerformance[] }[]> {
		const startTime = Date.now();
		try {
			...
			const batchPromises: Promise<{
				symbol: string;
				performances: InstitutionalPerformance[];
			}>[] = [];
				...
				const batchResults = batch.map(async (symbol, index) => {
					...
					try {
						...
						this.errorHandler.logger.warn(
							`Batch performance analysis failed for ${symbol}`,
							{ error }
						);
						return { symbol: symbol.toUpperCase(), performances: [] };
					}
			...
			throw this.errorHandler.errorHandler.createErrorResponse(
				error,
				"getInstitutionalPerformanceBatch"
			);
		}
	}
	...
	async analyzePerformanceTrends(
		symbol: string
	): Promise<{ trend: string; analysis: string; performers: string[] }> {
		try {
			SecurityValidator.validateSymbol(symbol);
			...
			const avgAlpha =
				performances.reduce((sum, p) => sum + p.benchmarkComparison.alpha, 0) /
				performances.length;

			const trend =
				avgAlpha > 2 ? "OUTPERFORMING" : avgAlpha < -2 ? "UNDERPERFORMING" : "MIXED";
	...
	private async fetchFMPInstitutionalHolders(symbol: string): Promise<any[]> {
		...
		try {
			...
			this.errorHandler.logger.warn("FMP institutional holders fetch failed", {
				symbol,
				error,
			});
			return [];
	...
	private async processInstitutionalData(
		...
		const targetInstitutions = institution
			? holders.filter(h => h.holder.toLowerCase().includes(institution.toLowerCase()))
			: holders.slice(0, 8);

		for (const holder of targetInstitutions) {
			const performanceData = await this.calculateInstitutionalPerformance(
				holder,
				ownership,
				financials,
				symbol
		...
		return performances.length > 0
			? performances
			: this.generateDefaultPerformances(symbol, institution);
	}

	/**
	 * Calculate institutional performance from real data
	 */
	private async calculateInstitutionalPerformance(
		holder: any,
		ownership: any,
		financials: any,
		symbol: string
		...
		const performanceMetrics: PerformanceMetric[] = [
			...
			},
			{
				metric: "Ownership %",
				value:
					holder.weightPercent ||
					(shares / (ownership?.outstandingShares || 1000000000)) * 100,
		...
		const benchmarkComparison = this.generateEnhancedBenchmarkComparison(
			symbol,
			institutionName,
			changePercent,
			financials
	...
	private generateEnhancedBenchmarkComparison(
		symbol: string,
		institution: string,
		...
		return {
			...
			informationRatio:
				(institutionalReturn - marketReturn) /
				Math.max(Math.abs(institutionalReturn - marketReturn) * 0.5, 0.1),
	 ...
	 */
	private generateDefaultPerformances(
		symbol: string,
		institution?: string
