...
export class SectorDataService {
  ...
  async getSectorPerformanceRankings(): Promise<SectorRankingResponse> {
    try {
      ...
      for (const sectorInfo of this.sectorETFs) {
        ...
        const historicalData = validHistoricalData.find(h => h.symbol === sectorInfo.symbol)

        if (currentData && historicalData) {
          const ranking = this.createSectorRanking(currentData, historicalData, sectorInfo)
      ...
      throw this.errorHandler.errorHandler.createErrorResponse(error, 'getSectorPerformanceRankings')
    }
  }
  ...
  private calculateReturns(prices: Array<{ close: number; date: string }>): {
    ...
    const ytdPrice = prices.find(p => new Date(p.date).getFullYear() === currentYear && new Date(p.date).getMonth() === 0)?.close ||
                     prices[prices.length - 1]?.close || current
  ...
  private calculateTechnicalIndicators(prices: Array<{ close: number }>): {
    ...
    const ema12 = this.calculateEMA(prices.slice(0, 12).map(p => p.close), 12)
    const ema26 = this.calculateEMA(prices.slice(0, 26).map(p => p.close), 26)
   ...
   */
  private createSectorRanking(currentData: SectorData, historicalData: SectorHistoricalData, sectorInfo: any): SectorPerformanceRanking {
   ...
   */
  private analyzeMomentum(currentData: SectorData, historicalData: SectorHistoricalData): {
    ...
    const avgReturn = (Math.abs(returns.oneDay) + Math.abs(returns.fiveDay) + Math.abs(returns.oneMonth)) / 3
   ...
   */
  private analyzeVolume(currentData: SectorData, historicalData: SectorHistoricalData): {
    current: number;
    average: number;
    ratio: number;
  } {
  ...
  private calculateVolatility(historicalData: SectorHistoricalData): number {
    const returns = historicalData.prices.slice(0, 20).map((price, index) => {
   ...
   */
  private calculateConfidence(currentData: SectorData, historicalData: SectorHistoricalData): number {
  ...
  private calculateRankings(rankings: SectorPerformanceRanking[]): void {
    ...
    const timeframes: Array<keyof SectorPerformanceRanking['returns']> = ['oneDay', 'fiveDay', 'oneMonth', 'threeMonth', 'yearToDate']

    timeframes.forEach(timeframe => {
      const sorted = [...rankings].sort((a, b) => b.returns[timeframe] - a.returns[timeframe])
    ...
    rankings.forEach(ranking => {
      const weights = { oneDay: 0.1, fiveDay: 0.2, oneMonth: 0.3, threeMonth: 0.3, yearToDate: 0.1 }
  ...
  private analyzeMarketContext(rankings: SectorPerformanceRanking[]): {
    ...
    const avgOneMonth = rankings.reduce((sum, r) => sum + r.returns.oneMonth, 0) / rankings.length
    const avgThreeMonth = rankings.reduce((sum, r) => sum + r.returns.threeMonth, 0) / rankings.length
  ...
  private analyzeRotationSignals(rankings: SectorPerformanceRanking[]): {
    ...
    const intoSectors = rankings
      .filter(r => r.momentum.direction === 'up' && r.momentum.strength !== 'weak' && r.volume.ratio > 1.2)
   ...
   */
  private assessDataQuality(rankings: SectorPerformanceRanking[]): 'excellent' | 'good' | 'fair' | 'poor' {
   ...
   */
  async getSectorRankingsQuick(): Promise<Array<{
    sector: string;
    symbol: string;
    ranking: number;
    returns: {
    ...
    try {
      ...
      return fullRankings.rankings.map(ranking => ({
        ...
        trend: ranking.returns.fiveDay > 1 ? 'up' :
               ranking.returns.fiveDay < -1 ? 'down' : 'neutral'
      }))
    } catch (error) {
      throw this.errorHandler.errorHandler.createErrorResponse(error, 'getSectorRankingsQuick')
    }
  }
