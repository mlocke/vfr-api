...
export class VWAPService {
	...
	calculateVWAPScore(vwapAnalysis: VWAPAnalysis): number {
		...
		if (vwapAnalysis.signal === "above") {
			...
			score =
				vwapAnalysis.strength === "strong"
					? 1.0
					: vwapAnalysis.strength === "moderate"
						? 0.6
						: 0.3;
	...
	async isVWAPSignificant(
		...
		try {
			...
			const direction =
				analysis.deviationPercent > threshold
					? "above"
					: analysis.deviationPercent < -threshold
						? "below"
						: "neutral";
	 ...
	 */
	async analyzeVWAPTrend(
		symbol: string,
		timeframe: "1D" | "5D" | "1M" | "3M"
	): Promise<VWAPTrendAnalysis | null> {
		try {
	...
	async getVWAPTrendInsights(symbol: string): Promise<VWAPTrendInsights | null> {
		try {
			...
			const trendConvergence = this.determineTrendConvergence(
				oneDayTrend,
				fiveDayTrend,
				oneMonthTrend
			);
			const trendScore = this.calculateCombinedTrendScore(
				oneDayTrend,
				fiveDayTrend,
				oneMonthTrend
			);
			const keyInsights = this.generateTrendInsights(
				oneDayTrend,
				fiveDayTrend,
				oneMonthTrend,
				trendConvergence
	...
	private getTimeframeDays(timeframe: "1D" | "5D" | "1M" | "3M"): number {
		switch (timeframe) {
			case "1D":
				return 2; // Get 2 days to calculate 1-day trend
			case "5D":
				return 7; // Get 7 days to account for weekends
			case "1M":
				return 30;
			case "3M":
				return 90;
	...
	private calculateTrendScore(
		...
		const directionMultiplier =
			direction === "uptrend" ? 1 : direction === "downtrend" ? -1 : 0;
	...
	private calculateVolatility(data: HistoricalVWAP[]): number {
		...
		const variance =
			deviations.reduce((sum, dev) => sum + Math.pow(dev - mean, 2), 0) / deviations.length;
	...
	private calculateCombinedTrendScore(
		...
		const weightedSum = trends.reduce(
			(sum, { trend, weight }) => sum + trend!.trendScore * weight,
			0
		);
	...
	private generateTrendInsights(
		...
		if (fiveDay) {
			insights.push(
				`5-day VWAP trend shows ${fiveDay.trendDirection} with ${fiveDay.trendStrength} strength`
			);
		}
		...
		if (oneMonth && Math.abs(oneMonth.momentum) > 1.0) {
			insights.push(
				`Strong momentum detected in monthly VWAP trend (${oneMonth.momentum.toFixed(2)}%)`
			);
		}
