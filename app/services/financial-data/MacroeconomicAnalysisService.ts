...
export class MacroeconomicAnalysisService {
	...
	async getMacroeconomicContext(): Promise<MacroeconomicContext | null> {
		...
		try {
			...
			if (economicContext.status === "fulfilled" && economicContext.value)
				dataSourcesUsed.push("FRED-Economic");
			if (inflationTrend.status === "fulfilled" && inflationTrend.value)
				dataSourcesUsed.push("FRED-Inflation");
			if (monetaryContext.status === "fulfilled" && monetaryContext.value)
				dataSourcesUsed.push("FRED-Monetary");
			if (cyclePosition.status === "fulfilled" && cyclePosition.value)
				dataSourcesUsed.push("FRED-Cycle");
			if (employmentStrength.status === "fulfilled" && employmentStrength.value)
				dataSourcesUsed.push("BLS-Employment");
			if (unemploymentTrend.status === "fulfilled" && unemploymentTrend.value)
				dataSourcesUsed.push("BLS-Unemployment");
			if (commoditySnapshot.status === "fulfilled" && commoditySnapshot.value)
				dataSourcesUsed.push("EIA-Energy");

			// Calculate composite macroeconomic score (0-10 scale)
			const overallScore = this.calculateCompositeScore({
				economicContext:
					economicContext.status === "fulfilled" ? economicContext.value : null,
				...
				unemploymentTrend:
					unemploymentTrend.status === "fulfilled" ? unemploymentTrend.value : null,
				commoditySnapshot:
			...
			console.log(
				`✅ Macro context generated in ${totalTime}ms with ${parallelEfficiency.toFixed(1)}% efficiency`
			);
			...
			if (parallelEfficiency < 80) {
				console.warn(
					`⚠️ Parallel efficiency below target: ${parallelEfficiency.toFixed(1)}% < 80%`
				);
			}
	...
	async getComprehensiveMacroAnalysis(): Promise<ComprehensiveMacroAnalysis | null> {
		...
		try {
			...
			const [trendAnalysis, currencyAnalysis, energyAnalysis, riskFactors] = enhancedResults;
			...
			const keyInsights = this.generateKeyInsights(baseContext, {
				...
				currencyAnalysis:
					currencyAnalysis.status === "fulfilled" ? currencyAnalysis.value : null,
			...
			const comprehensiveAnalysis: ComprehensiveMacroAnalysis = {
				...baseContext,
				trendAnalysis:
					trendAnalysis.status === "fulfilled"
						? (trendAnalysis.value as MacroTrendAnalysis)
						: this.getDefaultTrendAnalysis(),
				currencyAnalysis:
					currencyAnalysis.status === "fulfilled"
						? (currencyAnalysis.value as CurrencyAnalysis)
						: undefined,
				energyAnalysis:
					energyAnalysis.status === "fulfilled"
						? (energyAnalysis.value as EnergyInflationPressure)
						: undefined,
				riskFactors:
					riskFactors.status === "fulfilled"
						? (riskFactors.value as MacroRiskFactors)
						: this.getDefaultRiskFactors(),
				opportunities,
				keyInsights,
				performanceMetrics: {
					...
					parallelEfficiency:
						(enhancedResults.filter(r => r.status === "fulfilled").length /
	...
	private calculateCompositeScore(data: {
		...
		if (data.unemploymentTrend) {
			const unemploymentScore =
				data.unemploymentTrend.economicSignal === "bullish"
					? 8
					: data.unemploymentTrend.economicSignal === "bearish"
						? 3
						: 5;
			score += unemploymentScore * 0.1;
			totalWeight += 0.1;
		}
	...
	private scoreEnergyConditions(commoditySnapshot: any): number {
		...
		if (commoditySnapshot.wti) {
			...
			if (wtiPrice >= 60 && wtiPrice <= 80)
				score += 1; // Goldilocks range
			else if (wtiPrice >= 40 && wtiPrice <= 100)
	...
	private async analyzeEnergyInflationPressure(): Promise<EnergyInflationPressure | null> {
		try {
			...
			return {
				...
				energyInflationScore:
					(energyData.oilInflationImpact + energyData.gasInflationImpact) / 2,
				sectorImpacts: {
	...
	private assessInflationRisk(context: MacroeconomicContext): {
		level: "low" | "moderate" | "high";
		description: string;
		probability: number;
	} {
		...
		if (isHighInflation) {
			return {
				level: "high",
				description:
					"Persistent high inflation pressures consumer spending and corporate margins",
				probability: 0.75,
			};
		}
	...
	private assessRecessionRisk(context: MacroeconomicContext): {
		level: "low" | "moderate" | "high";
		description: string;
		probability: number;
	} {
	...
	private assessMonetaryRisk(context: MacroeconomicContext): {
		level: "low" | "moderate" | "high";
		description: string;
		impact: string;
	} {
	...
	private assessEnergyRisk(context: MacroeconomicContext): {
		level: "low" | "moderate" | "high";
		description: string;
		sectorImpact: string[];
	} {
	...
	private identifyOpportunities(context: MacroeconomicContext, insights: string[]): string[] {
		...
		if (
			context.inflationEnvironment === "declining" &&
			context.monetaryPolicy !== "restrictive"
		) {
