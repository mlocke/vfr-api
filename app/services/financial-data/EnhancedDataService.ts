  ...
  async getSwitchingRecommendations(): Promise<{
    ...
    Object.entries(preferences).forEach(([dataType, pref]) => {
      ...
      if (currentProvider?.costTier === 'paid' || currentProvider?.costTier === 'premium') {
        const freeAlternatives = pref.fallbacks.filter(f =>
          configs[f]?.costTier === 'free' && configs[f]?.enabled
        )
        if (freeAlternatives.length > 0) {
          costOptimization.push(`💰 Switch ${dataType} from ${pref.primary} to ${freeAlternatives[0]} for cost savings`)
        }
      }
    })
    ...
    Object.entries(preferences).forEach(([dataType, pref]) => {
      const currentProvider = configs[pref.primary]
      const betterProviders = Object.entries(configs).filter(([provider, config]) =>
      ...
      if (betterProviders.length > 0) {
        qualityImprovement.push(`📈 Switch ${dataType} from ${pref.primary} to ${betterProviders[0][0]} for better data quality`)
      }
    })
    ...
    Object.entries(status.providerStatus).forEach(([provider, status]) => {
      if (!status.available && configs[provider as DataSourceProvider]?.enabled) {
        reliabilityImprovement.push(`⚠️ Provider ${provider} is currently unavailable - consider switching primary data sources`)
      }
    })
