...
export class InstitutionalCacheManager {
  ...
  async get13FHoldings(symbol: string): Promise<InstitutionalHolding[] | null> {
    ...
    try {
      ...
      if (cached) {
        ...
        const timeUntilExpiration = (cached.metadata.cachedAt + cached.metadata.ttl * 1000) - Date.now()
  ...
  async cache13FHoldings(symbol: string, holdings: InstitutionalHolding[]): Promise<boolean> {
    ...
    try {
      ...
      const optimizedHoldings = holdings.map(holding => ({
        ...
        securityName: holding.securityName.length > 50 ? holding.securityName.substring(0, 50) + '...' : holding.securityName
      }))
   ...
   */
  async cacheForm4Transactions(symbol: string, transactions: InsiderTransaction[]): Promise<boolean> {
    const cacheKey = this.buildForm4CacheKey(symbol)
    ...
    try {
      ...
      this.errorHandler.logger.error(`Failed to cache Form 4 transactions for ${symbol}`, { error })
      return false
    }
  }
  ...
  async getInstitutionalIntelligence(symbol: string): Promise<InstitutionalIntelligence | null> {
    ...
    try {
      ...
      if (cached) {
        ...
        const dependenciesValid = await this.validateCacheDependencies(symbol, cached.metadata.dependencies)
      ...
      this.errorHandler.logger.warn(`Intelligence cache retrieval failed for ${symbol}`, { error })
      return null
    }
  }
  ...
  async cacheInstitutionalIntelligence(
    ...
    try {
      ...
      this.errorHandler.logger.error(`Failed to cache institutional intelligence for ${symbol}`, { error })
      return false
    }
  }
  ...
  async warmupPopularSymbols(limit = 50): Promise<void> {
    ...
    this.errorHandler.logger.info(`Starting cache warmup for ${symbols.length} popular symbols`)
    ...
    for (let i = 0; i < symbols.length; i += batchSize) {
      ...
      const warmupPromises = batch.map(symbol =>
        this.scheduleWarmup(symbol, 'intelligence', 3) // Lower priority for batch warmup
      )
   ...
   */
  async invalidateByMarketEvent(event: 'earnings' | 'acquisition' | 'insider_activity', symbols: string[]): Promise<void> {
    ...
    this.errorHandler.logger.info(`Invalidated caches for ${event} affecting ${symbols.length} symbols`)
  }
  ...
  private scheduleWarmup(symbol: string, type: 'thirteenF' | 'form4' | 'intelligence', priority: number): void {
    // Avoid duplicate warmup jobs
    const existingJob = this.warmupQueue.find(job => job.symbol === symbol && job.type === type)
    if (existingJob) {
    ...
    try {
      ...
      const warmupPromises = jobsToProcess.map(async (job) => {
        try {
          ...
          this.errorHandler.logger.warn(`Cache warmup failed for ${job.symbol}:${job.type}`, { error, job })
        }
      })
  ...
  private async validateCacheDependencies(symbol: string, dependencyTimestamps: number[]): Promise<boolean> {
    try {
  ...
  private trackPopularSymbol(symbol: string): void {
    ...
    if (this.popularSymbols.size > this.config.thirteenF.maxSymbols) {
      ...
      const toRemove = symbolArray.slice(0, symbolArray.length - this.config.thirteenF.maxSymbols)
  ...
  private updateResponseTime(responseTime: number): void {
    if (this.metrics.responseTimeMs.min === 0 || responseTime < this.metrics.responseTimeMs.min) {
    ...
    const total = this.metrics.hits.thirteenF + this.metrics.hits.form4 + this.metrics.hits.intelligence +
                  this.metrics.misses.thirteenF + this.metrics.misses.form4 + this.metrics.misses.intelligence

    this.metrics.responseTimeMs.avg = ((this.metrics.responseTimeMs.avg * (total - 1)) + responseTime) / total
  }
