...
export class ESGDataService {
	...
	constructor(options?: { apiKey?: string; timeout?: number; throwErrors?: boolean }) {
		const { apiKey, timeout = 10000, throwErrors = false } = options || {};
		...
		if (!this.isAPIKeyAvailable) {
			console.log(
				"⚠️ ESG Data Service initialized without API key - using baseline defaults"
			);
			console.log(
				"💡 To use real ESG data, configure ESG_API_KEY or FMP_API_KEY environment variable"
			);
		} else {
	 ...
	 */
	async analyzeStockESGImpact(
		symbol: string,
		sector: string,
		baseScore: number
	): Promise<StockESGImpact | null> {
		try {
	 ...
	 */
	async analyzeBulkESGImpact(
		stocks: Array<{ symbol: string; sector: string; baseScore: number }>
	): Promise<BulkESGAnalysisResponse> {
		...
		try {
			...
			const averageESGScore =
				stockImpacts.length > 0 ? totalESGScore / stockImpacts.length : 60;
	...
	private async fetchESGFromAPI(symbol: string, sector: string): Promise<ESGScore> {
		...
		try {
			...
			if (esgRating && esgRating.ESGScore > 0) {
				...
				console.log(
					`⚠️ FMP ESG data not available for ${symbol}, using enhanced synthetic`
				);
			}
		} catch (error) {
	 ...
	 */
	private generateSyntheticESGScore(
		symbol: string,
		sector: string,
		...
		const industryBaseline =
			this.INDUSTRY_BASELINES[sector] || this.INDUSTRY_BASELINES["unknown"];
		...
		const environmental = Math.max(
			0,
			Math.min(100, industryBaseline + (Math.random() - 0.5) * variation)
		);
	...
	private generateESGRiskFactors(esgScore: ESGScore, sector: string): ESGRiskFactors {
		...
		return {
			...
			carbonFootprint: {
				...
				scope1: Math.round(
					this.calculateCarbonIntensity(esgScore.environmental, sector) * 0.6
				),
				scope2: Math.round(
					this.calculateCarbonIntensity(esgScore.environmental, sector) * 0.3
				),
				scope3: Math.round(
					this.calculateCarbonIntensity(esgScore.environmental, sector) * 0.1
				),
				trend:
					esgScore.environmental >= 70
						? "improving"
						: esgScore.environmental >= 50
							? "stable"
							: "worsening",
				industryComparison:
					esgScore.environmental >= 70
			...
			governance: {
				boardDiversity: Math.max(
					0,
					Math.min(100, esgScore.governance + (Math.random() - 0.5) * 10)
				),
				boardIndependence: Math.max(
					30,
					Math.min(95, esgScore.governance + (Math.random() - 0.5) * 15)
				),
				executiveCompensation:
					esgScore.governance >= 75
						? "reasonable"
						: esgScore.governance >= 50
							? "concerning"
							: "excessive",
				transparency: Math.max(
					0,
					Math.min(100, esgScore.governance + (Math.random() - 0.5) * 8)
				),
				auditQuality: Math.max(
					0,
					Math.min(100, esgScore.governance + (Math.random() - 0.5) * 12)
				),
				shareholderRights: Math.max(
					0,
					Math.min(100, esgScore.governance + (Math.random() - 0.5) * 10)
				),
			},
			social: {
				workplaceSafety: Math.max(
					0,
					Math.min(100, esgScore.social + (Math.random() - 0.5) * 15)
				),
				laborPractices: Math.max(
					0,
					Math.min(100, esgScore.social + (Math.random() - 0.5) * 12)
				),
				communityImpact: Math.max(
					0,
					Math.min(100, esgScore.social + (Math.random() - 0.5) * 10)
				),
				dataPrivacy:
					sector === "technology"
						? Math.max(0, Math.min(100, esgScore.social + (Math.random() - 0.5) * 20))
						: Math.max(40, Math.min(80, esgScore.social + (Math.random() - 0.5) * 10)),
				productSafety: Math.max(
					0,
					Math.min(100, esgScore.social + (Math.random() - 0.5) * 8)
				),
			},
		};
	}
	...
	private generateESGInsights(
		...
		if (esgScore.environmental < 70 && sector === "energy") {
			opportunities.push(
				"Transition to renewable energy could significantly improve ESG profile"
			);
		}
		if (esgScore.social < 70) {
			opportunities.push(
				"Enhanced diversity and inclusion programs could boost social scores"
			);
		}
		...
		// Generate warnings
		if (
			riskFactors.controversies.level === "high" ||
			riskFactors.controversies.level === "severe"
		) {
			warnings.push(
				"High controversy level poses significant reputational and regulatory risks"
			);
		}
		...
		return {
			...
			regulatoryRisks: {
				level:
					esgScore.overall >= 70 ? "low" : esgScore.overall >= 50 ? "moderate" : "high",
				...
				employees:
					esgScore.social >= 65
						...
						: esgScore.social >= 45
							...
							: "negative",
				communities:
					esgScore.environmental >= 60
						? "positive"
						: esgScore.environmental >= 40
							? "neutral"
							: "negative",
			},
		};
	...
	private generateMaterialityFactors(
		sector: string,
		esgScore: ESGScore
	): Array<{
		factor: string;
	...
	async getESGImpactForStock(
		...
		try {
			...
			if (esgImpact.esgScore.grade === "A" || esgImpact.esgScore.grade === "B") {
				factors.push(
					`Strong ESG rating (${esgImpact.esgScore.grade}) supports long-term value`
				);
			}
			if (
				esgImpact.riskFactors.controversies.level === "high" ||
				esgImpact.riskFactors.controversies.level === "severe"
			) {
