...
export class FinancialDataService implements FinancialDataProvider {
	...
	private initializeDataSources(): void {
		...
		this.errorHandler.logger.info(
			`Financial Data Service initialized with ${this.dataSources.length} authorized sources`,
			{
	...
	private canMakeFMPRequest(): boolean {
		...
		const rateInfo = this.requestCounts.get(fmpSource.name) || {
			count: 0,
			resetTime: now + windowSize,
		};
		const burstKey = `${fmpSource.name}_burst`;
		const burstInfo = this.requestCounts.get(burstKey) || {
			count: 0,
		...
		if (!canMakeRequest) {
			...
			this.errorHandler.logger.debug(
				`FMP ${limitType} limit approached: ${currentCount}/${limitValue}`,
				{
					limitType,
	...
	private async processFMPQueue(): Promise<void> {
		...
		while (this.fmpRequestQueue.length > 0 && this.canMakeFMPRequest()) {
			...
			if (this.fmpRequestQueue.length > 0) {
				...
				const adaptiveDelay =
					remainingCapacity > 0.5 ? delayBetweenRequests * 0.5 : delayBetweenRequests;
	...
	private canMakeRequest(source: DataSourceConfig): boolean {
		...
		const rateInfo = this.requestCounts.get(source.name) || {
			count: 0,
			resetTime: now + 60000,
		};
		if (now > rateInfo.resetTime) {
		...
		if (rateInfo.count >= source.rateLimit) {
			console.log(
				`⚠️ Rate limit reached for ${source.name}: ${rateInfo.count}/${source.rateLimit} per minute`
			);
			return false;
		}
		...
		if (source.dailyLimit) {
			...
			if (dailyInfo.count >= source.dailyLimit) {
				console.log(
					`⚠️ Daily limit reached for ${source.name}: ${dailyInfo.count}/${source.dailyLimit}`
				);
				return false;
			}
		}
	...
	private async executeGetStockPrice(symbol: string): Promise<StockData | null> {
		...
		// ✅ CACHE CHECK: Check FMPCacheManager first (60-second TTL)
		const cacheKey = this.fmpCacheManager.generateKey(sanitizedSymbol, "stock_price");
		const cachedData = this.fmpCacheManager.get<StockData>(cacheKey, "stock_price");

		if (cachedData) {
			this.errorHandler.logger.debug(`Stock price cache hit for ${sanitizedSymbol}`, {
				age: Math.floor((Date.now() - cachedData.timestamp) / 1000) + "s",
			});
			return cachedData;
		}
		...
		for (const source of sourcesToTry) {
			...
			try {
				...
				if (data) {
					...
					if (!responseValidation.isValid) {
						this.errorHandler.logger.warn(
							`Invalid stock price response from ${source.name}`,
							{
					...
					if (!priceValidation.isValid) {
						this.errorHandler.logger.warn(
							`Invalid price value for ${sanitizedSymbol}`,
							{
					...
					// ✅ CACHE STORE: Store in cache for 60 seconds (FMPCacheManager TTL)
					this.fmpCacheManager.set(cacheKey, result, "stock_price");
					this.errorHandler.logger.debug(`Stock price cached for ${sanitizedSymbol}`, {
						ttl: "60s",
					});
				...
				this.errorHandler.logger.logApiError("GET", "stock_price", error, undefined, {
					symbol: sanitizedSymbol,
					source: source.name,
				});
			}
	...
	async getCompanyInfo(symbol: string): Promise<CompanyInfo | null> {
		return this.errorHandler
			.validateAndExecute(() => this.executeGetCompanyInfo(symbol), [symbol], {
				timeout: 20000,
				retries: 1,
				context: "getCompanyInfo",
			})
			.catch(error => {
				this.errorHandler.logger.warn(`Failed to get company info for ${symbol}`, {
					error,
				});
				return null;
			});
	...
	private async executeGetCompanyInfo(symbol: string): Promise<CompanyInfo | null> {
		...
		for (const source of sourcesToTry) {
			...
			try {
				...
				this.errorHandler.logger.logApiError("GET", "company_info", error, undefined, {
					symbol,
					source: source.name,
				});
			}
	...
	async getMarketData(symbol: string): Promise<MarketData | null> {
		return this.errorHandler
			.validateAndExecute(() => this.executeGetMarketData(symbol), [symbol], {
				timeout: 20000,
				retries: 1,
	...
	async getBatchPrices(symbols: string[]): Promise<Map<string, StockData>> {
		...
		const batchSources = this.dataSources.filter(
			source =>
		...
		if (batchSources.length > 0) {
			const batchPromises = batchSources.map(async source => {
				try {
					console.log(
						`🔄 Trying batch fetch from ${source.name} for ${sanitizedSymbols.length} symbols...`
					);
					const batchData = await (source.provider as any).getBatchPrices(
						sanitizedSymbols
					);
					...
					if (batchData.size > 0) {
						this.recordRequest(source);
						console.log(
							`✅ ${source.name} batch fetch returned ${batchData.size} symbols`
						);
					...
					batchData.forEach((data: StockData, symbol: string) => {
						if (!results.has(symbol)) {
							...
							if (validation.isValid) {
								...
								const priceValidation = SecurityValidator.validateNumeric(
									data.price,
									{
										min: 0,
										max: 100000,
		...
		if (missingSymbols.length > 0 && missingSymbols.length <= 10) {
			// Limit parallel individual requests
	 ...
	 */
	private getOptimalDataSource(
		dataType: "stock_price" | "fundamental_ratios" | "company_info" | "market_data"
	): DataSourceConfig[] {
		const now = Date.now();
		const today = new Date().toDateString();
		...
		const scoredSources = candidateSources.map(source => {
			...
			const isFMPStarter =
				source.name === "Financial Modeling Prep" && source.rateLimit >= 300;
			...
			return {
				...
				score,
				capacityUtilization,
				dailyUtilization: source.dailyLimit
					? (this.dailyCounts.get(source.name)?.count || 0) / source.dailyLimit
					: 0,
			};
		});
	...
	getSourceStatus(): Array<{
		...
		return this.dataSources.map(source => {
			...
			const dailyUtilization = source.dailyLimit
				? (dailyInfo?.count || 0) / source.dailyLimit
				: 0;
	...
	async getStocksBySector(sector: string, limit?: number): Promise<StockData[]> {
		...
		for (const source of this.dataSources) {
			...
			try {
				...
				this.errorHandler.logger.warn(`${source.name} sector fetch failed`, {
					error,
					sector,
				});
				continue;
	...
	async getAnalystRatings(symbol: string): Promise<AnalystRatings | null> {
		...
		const fmpSource = this.dataSources.find(
			source => source.name === "Financial Modeling Prep"
		);
	...
	async getBatchFundamentalRatios(symbols: string[]): Promise<Map<string, FundamentalRatios>> {
		try {
			...
			const cachedResults = this.fmpCacheManager.getBatch<FundamentalRatios>(
				cacheKeys,
				"fundamental_ratios"
			);
			...
			if (
				fmpSource &&
				this.canMakeFMPRequest() &&
				(fmpCapacity.isStarterPlan || fmpCapacity.isProfessionalPlan)
			) {
				this.errorHandler.logger.info(
					`Using enhanced FMP batch processing for ${symbolsToFetch.length} symbols`,
					{
				...
				if (typeof fmpAPI.getBatchFundamentalRatios === "function") {
					...
					if (fmpResults.size > 0) {
						...
						const cacheEntries = Array.from(fmpResults.entries()).map(
							([symbol, data]) => ({
			...
			this.errorHandler.logger.info(
				"Using fallback individual processing for fundamental ratios",
				{
			...
			const promises = sanitizedSymbols.map(symbol =>
				this.getFundamentalRatios(symbol)
					.then(ratios => (ratios ? { symbol, ratios } : null))
					.catch(() => null)
			);
	...
	async getFundamentalRatios(symbol: string): Promise<FundamentalRatios | null> {
		try {
			...
			const cacheKey = this.fmpCacheManager.generateKey(
				sanitizedSymbol,
				"fundamental_ratios"
			);
			const cachedData = this.fmpCacheManager.get<FundamentalRatios>(
				cacheKey,
				"fundamental_ratios"
			);
			...
			if (cachedData) {
				this.errorHandler.logger.debug(
					`Cache hit for fundamental ratios: ${sanitizedSymbol}`
				);
				return cachedData;
			}
			...
			if (!rateLimitCheck.allowed) {
				this.errorHandler.logger.warn(
					`Rate limit exceeded for fundamental ratios: ${symbol}`
				);
				return null;
			}
			...
			if (!circuitCheck.allowed) {
				this.errorHandler.logger.warn(
					`Circuit breaker open for fundamental ratios: ${symbol}`
				);
				return null;
			}
	...
	private async executeGetFundamentalRatios(symbol: string): Promise<FundamentalRatios | null> {
		...
		const fundamentalSources = this.dataSources.filter(
			source =>
		...
		if (fundamentalSources.length === 0) {
			...
			this.errorHandler.logger.warn(
				`No available fundamental ratio sources for ${sanitizedSymbol}`
			);
			return null;
		}
		...
		try {
			...
			const promises = fundamentalSources.map(async source => {
				try {
					...
					if (data) {
						...
						if (!responseValidation.isValid) {
							this.errorHandler.logger.warn(
								`Invalid fundamental ratios response structure from ${source.name}`,
								{
									errors: responseValidation.errors,
									symbol: sanitizedSymbol,
								}
						...
						for (const field of numericFields) {
							...
							if (value !== undefined && value !== null) {
								const validation = SecurityValidator.validateNumeric(value, {
									...
									max:
										field.includes("Margin") || field === "payoutRatio"
											? 100
											: undefined,
								...
								if (!validation.isValid) {
									this.errorHandler.logger.warn(
										`Invalid ${field} value for ${sanitizedSymbol}`,
										{
											field,
											value,
						...
						this.recordRequest(source);
						this.errorHandler.logger.info(
							`Fundamental ratios from ${source.name} for ${sanitizedSymbol}`,
							{
			...
			for (const result of results) {
				if (result.status === "fulfilled" && result.value?.data) {
					this.errorHandler.errorHandler.recordSuccess(
						`fundamental_ratios_${sanitizedSymbol}`
					);
			...
			this.errorHandler.errorHandler.recordFailure(`fundamental_ratios_${sanitizedSymbol}`);
			this.errorHandler.logger.warn(
				`No fundamental ratios available for ${sanitizedSymbol}`,
				{
			...
			this.errorHandler.logger.error(
				`Fundamental ratios service error for ${sanitizedSymbol}`,
				{
					error,
					symbol: sanitizedSymbol,
				}
	...
	async getInstitutionalIntelligence(symbol: string): Promise<InstitutionalIntelligence | null> {
		return this.errorHandler
			...
			.catch(error => {
				this.errorHandler.logger.warn(
					`Failed to get institutional intelligence for ${symbol}`,
					{ error }
				);
				return null;
			});
	...
	async getInstitutionalHoldings(symbol: string, quarters = 4): Promise<InstitutionalHolding[]> {
		return this.errorHandler
			...
			.catch(error => {
				this.errorHandler.logger.warn(
					`Failed to get institutional holdings for ${symbol}`,
					{ error }
				);
				return [];
			});
	...
	async getInsiderTransactions(symbol: string, days = 180): Promise<InsiderTransaction[]> {
		return this.errorHandler
			...
			.catch(error => {
				this.errorHandler.logger.warn(`Failed to get insider transactions for ${symbol}`, {
					error,
				});
				return [];
			});
	...
	async getInstitutionalSentiment(symbol: string) {
		try {
			...
			this.errorHandler.logger.warn(`Failed to get institutional sentiment for ${symbol}`, {
				error,
			});
			return null;
		}
	...
	async getInsiderSentiment(symbol: string) {
		try {
			...
			this.errorHandler.logger.warn(`Failed to get insider sentiment for ${symbol}`, {
				error,
			});
			return null;
		}
	...
	getFmpPerformanceMetrics(): {
		...
		const burstInfo = this.requestCounts.get(`${fmpSource.name}_burst`) || {
			count: 0,
			resetTime: now + 10000,
		};
		...
		const remainingCapacity =
			Math.max(0, (fmpSource.rateLimit - rateInfo.count) / fmpSource.rateLimit) * 100;
		const burstUtilization =
			(burstInfo.count / Math.min(50, Math.floor(fmpSource.rateLimit / 6))) * 100;
	...
	async getHistoricalOHLC(
		...
		try {
			const historicalData = await (fmpSource.provider as any).getHistoricalData(
				symbol,
				days,
				endDate
			);
	...
	async getProviderHealth(): Promise<
		Array<{ name: string; healthy: boolean; responseTime?: number }>
	> {
