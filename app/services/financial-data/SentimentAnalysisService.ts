...
export class SentimentAnalysisService {
	 ...
	 */
	async analyzeStockSentimentImpact(
		symbol: string,
		sector: string,
		baseScore: number
	): Promise<StockSentimentImpact | null> {
		try {
			...
			const sentimentWeight = 0.1; // 10% as per roadmap
	 ...
	 */
	async analyzeBulkSentimentImpact(
		stocks: Array<{ symbol: string; sector: string; baseScore: number }>
	): Promise<BulkSentimentAnalysisResponse> {
	...
	async getSentimentIndicators(
		symbol: string,
		analystData?: any
	): Promise<SentimentIndicators | null> {
		try {
			// Validate symbol first to prevent injection attacks
			const validation = this.securityValidator.validateSymbol(symbol);
			if (!validation.isValid) {
			...
			const [newsResult, redditResult, optionsResult] = await Promise.allSettled([
				...
				this.withComponentTimeout(this.getNewsSentiment(sanitizedSymbol), 5000, "News"),
				...
				this.redditAPI
					? this.withComponentTimeout(
							this.redditAPI.getEnhancedSentiment(sanitizedSymbol).then(response => {
								...
								if (
									!response.success &&
									response.error?.includes("Invalid symbol")
								) {
									console.warn(
										"Symbol validation failed - potential security issue"
									);
									return null;
								}
			...
			if (redditData) {
				console.log(
					`✅ Enhanced Reddit sentiment: ${redditData.sentiment.toFixed(2)} (${redditData.postCount} posts across ${(redditData as any).subredditBreakdown?.length || 1} subreddits)`
				);
			...
			if (optionsData) {
				console.log(
					`✅ Options sentiment: ${optionsData.sentiment.toFixed(2)} (P/C: ${optionsData.putCallRatio.toFixed(2)}, ${optionsData.sentimentSignal})`
				);
			...
			if (analystData && analystData.sentimentScore !== undefined) {
				...
				baseConfidence =
					analystData.totalAnalysts > 0
						? Math.min(0.9, 0.5 + analystData.totalAnalysts / 100)
						: 0.7;
				console.log(
					`📊 Analyst sentiment: ${normalizedAnalystScore.toFixed(3)} (${analystData.sentimentScore}/5 from ${analystData.totalAnalysts} analysts, weight: ${(analystWeight * 100).toFixed(0)}%)`
				);
			}
	...
	private async getOptionsSentiment(symbol: string): Promise<OptionsSentimentData | null> {
		try {
			...
			const putCallData =
				await this.optionsAnalysisService.calculateUnicornBayPutCallSignals(symbol);
			if (!putCallData) {
				return null;
			}
			...
			const signalStrength = this.determineOptionsSignalStrength(
				putCallData,
				optionsAnalysis
			);
	...
	private calculateSentimentScore(indicators: SentimentIndicators): SentimentScore {
		...
		if (indicators.reddit) {
			...
			reasoning.push(
				`Reddit WSB sentiment: ${(redditScore * 100).toFixed(0)}% (${indicators.reddit.postCount} posts)`
			);
		...
		if (indicators.options) {
			...
			reasoning.push(
				`Options sentiment: ${(optionsScore * 100).toFixed(0)}% (P/C: ${indicators.options.putCallRatio.toFixed(2)}, ${indicators.options.sentimentSignal})`
			);
	...
	private generateSentimentInsights(
		...
		insights.push(
			`News sentiment: ${sentimentLevel} (${(sentimentScore.overall * 100).toFixed(0)}/100)`
		);

		// Confidence insights
		const confidenceLevel =
			sentimentScore.confidence > 0.8
				? "high"
				: sentimentScore.confidence > 0.5
					? "moderate"
					: "low";
		insights.push(
			`Sentiment confidence: ${confidenceLevel} (${(sentimentScore.confidence * 100).toFixed(0)}%)`
		);
		...
		if (articleCount > 50) {
			insights.push(
				`High news volume (${articleCount} articles) indicates strong market attention`
			);
	...
	private createDefaultConfig(): SentimentConfig {
		return {
			...
			weights: {
				analyst: 0.4, // 40% weight for analyst consensus (FMP) - highest priority professional opinion
				news: 0.3, // 30% weight for company sentiment (Yahoo Finance) - reduced from 55%
				reddit: 0.2, // 20% weight for Reddit WSB sentiment - reduced from 30%
				options: 0.1, // 10% weight for options sentiment (P/C ratio analysis) - reduced from 15%
			},
			thresholds: {
	 ...
	 */
	private determineOptionsSignalStrength(
		putCallData: any,
		optionsAnalysis: any
	 ...
	 * Generate options-specific insights
	 */
	private generateOptionsInsights(
		putCallData: any,
		optionsAnalysis: any,
		sentiment: number
	): string[] {
		...
		if (pcRatio > 1.5) {
			insights.push(
				`Very high put/call ratio (${pcRatio.toFixed(2)}) indicates strong bearish sentiment`
			);
		} else if (pcRatio > 1.2) {
			insights.push(
				`Elevated put/call ratio (${pcRatio.toFixed(2)}) suggests bearish positioning`
			);
		} else if (pcRatio < 0.5) {
			insights.push(
				`Very low put/call ratio (${pcRatio.toFixed(2)}) indicates strong bullish sentiment`
			);
		} else if (pcRatio < 0.8) {
			insights.push(
				`Low put/call ratio (${pcRatio.toFixed(2)}) suggests bullish positioning`
			);
		}
		...
		if (largeTransactions > 0) {
			insights.push(
				`${largeTransactions} large block transaction${largeTransactions > 1 ? "s" : ""} detected`
			);
		}
		...
		if (putCallData.openInterestRatio !== putCallData.volumeRatio) {
			...
			if (oiDiff > 0.3) {
				insights.push(
					`Divergence between volume and open interest P/C ratios suggests fresh positioning`
				);
			}
		}
	...
	async healthCheck(): Promise<{ status: "healthy" | "unhealthy"; details: any }> {
		try {
			...
			if (this.optionsAnalysisService) {
				try {
					const optionsHealthCheck =
						await this.optionsAnalysisService.healthCheckEnhanced();
