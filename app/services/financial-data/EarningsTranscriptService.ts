...
export class EarningsTranscriptService {
  ...
  async getEarningsTranscript(symbol: string, quarter: string, year: number): Promise<EarningsTranscript | null> {
    ...
    try {
      ...
      const cacheKey = this.fmpCache.generateKey(`${symbol}:${quarter}:${year}`, 'earnings_transcript')
      ...
      // Check FMP cache
      const cachedData = this.fmpCache.get<EarningsTranscript>(cacheKey, 'earnings_transcript')
      if (cachedData) {
      ...
      const processedTranscript = this.processTranscriptData(earnings, transcript, symbol, quarter, year)
  ...
  async analyzeTranscript(transcript: EarningsTranscript): Promise<TranscriptAnalysis> {
    ...
    try {
      ...
      const cachedData = this.fmpCache.get<TranscriptAnalysis>(cacheKey, 'transcript_analysis')
      ...
      const sentimentData = sentimentResults.status === 'fulfilled' ? sentimentResults.value : this.getDefaultSentiment()
      const insights = insightsResults.status === 'fulfilled' ? insightsResults.value : this.getDefaultInsights()
      const toneData = toneResults.status === 'fulfilled' ? toneResults.value : {
        management: 'NEUTRAL' as 'NEUTRAL' | 'CONFIDENT' | 'CAUTIOUS' | 'DEFENSIVE' | 'OPTIMISTIC',
      ...
      const analysis: TranscriptAnalysis = {
        ...
        managementTone: toneData.management as 'NEUTRAL' | 'CONFIDENT' | 'CAUTIOUS' | 'DEFENSIVE' | 'OPTIMISTIC',
  ...
  async getEarningsSentiment(symbol: string): Promise<EarningsSentiment> {
    ...
    try {
      ...
      const validTranscripts = transcripts
        .filter((result): result is PromiseFulfilledResult<EarningsTranscript | null> => result.status === 'fulfilled')
        .map(result => result.value)
        .filter((transcript): transcript is EarningsTranscript => transcript !== null)

      // Parallel analysis of valid transcripts
      const analysisPromises = validTranscripts.map(transcript => this.analyzeTranscript(transcript))
      const analysisResults = await Promise.allSettled(analysisPromises)

      const recentTranscripts = analysisResults
        .filter((result): result is PromiseFulfilledResult<TranscriptAnalysis> => result.status === 'fulfilled')
        .map(result => result.value)
  ...
  async getEarningsSentimentBatch(symbols: string[]): Promise<EarningsSentiment[]> {
    try {
      ...
      throw this.errorHandler.errorHandler.createErrorResponse(error, 'getEarningsSentimentBatch')
    }
  }
  ...
  async searchTranscriptsByKeyword(symbol: string, keyword: string, limit: number = 10): Promise<NLPInsight[]> {
    try {
      ...
      throw this.errorHandler.errorHandler.createErrorResponse(error, 'searchTranscriptsByKeyword')
    }
  }
   ...
   */
  private async fetchFMPEarningsData(symbol: string, quarter: string, year: number): Promise<any> {
    ...
    try {
      ...
      this.errorHandler.logger.warn('FMP earnings data fetch failed', { symbol, quarter, year, error })
      return null
    }
  }
   ...
   */
  private async fetchFMPTranscriptData(symbol: string, quarter: string, year: number): Promise<any> {
    if (!this.apiKey) return null
    ...
    try {
      ...
      this.errorHandler.logger.warn('FMP transcript data fetch failed', { symbol, quarter, year, error })
      return null
    }
  }
   ...
   */
  private processTranscriptData(earnings: any, transcript: any, symbol: string, quarter: string, year: number): EarningsTranscript {
    return {
      symbol: symbol.toUpperCase(),
      quarter,
      year,
      ...
      transcript: transcript?.content || earnings?.transcript || `Earnings call transcript for ${symbol.toUpperCase()} ${quarter} ${year}`,
      keyTopics: this.extractKeyTopics(transcript?.content || earnings?.transcript || ''),
      sentiment: this.determineSentimentFromText(transcript?.content || earnings?.transcript || ''),
  ...
  private async analyzeSentiment(text: string, topics: string[]): Promise<any> {
    ...
    const positiveWords = words.filter(word => ['growth', 'strong', 'positive', 'increase', 'expansion'].includes(word)).length
    const negativeWords = words.filter(word => ['decline', 'weak', 'negative', 'decrease', 'challenges'].includes(word)).length
  ...
  private async extractInsights(text: string, topics: string[]): Promise<NLPInsight[]> {
    ...
    for (const topic of limitedTopics) {
      const mentions = (text.toLowerCase().match(new RegExp(topic.toLowerCase(), 'g')) || []).length
      if (mentions > 0) {
        insights.push({
          topic,
   ...
   */
  private async analyzeTone(participants: any[], text: string): Promise<{ management: string, analyst: string }> {
   ...
   * Calculate aggregated sentiment efficiently
   */
  private calculateAggregatedSentiment(analyses: TranscriptAnalysis[]): { trend: 'IMPROVING' | 'DECLINING' | 'STABLE', score: number, confidence: number } {
    if (analyses.length === 0) {
      return { trend: 'STABLE' as const, score: 5, confidence: 0.3 }
    }

    const avgSentiment = analyses.reduce((sum, analysis) => sum + analysis.sentimentBreakdown.overall, 0) / analyses.length
    ...
    analyses.forEach(analysis => {
      if (analysis.keyInsights.length > 0) {
        ...
        const insightStrings = analysis.keyInsights
          .slice(0, 2)
          .map(insight => typeof insight === 'string' ? insight : insight.topic || insight.context || String(insight))
  ...
  private extractKeyTopics(text: string): string[] {
    const commonTopics = ['Revenue Growth', 'Margin Expansion', 'Market Position', 'Future Guidance', 'Cost Management']
  ...
  private determineSentimentFromText(text: string): 'POSITIVE' | 'NEGATIVE' | 'NEUTRAL' {
    ...
    const positiveWords = (text.toLowerCase().match(/(growth|strong|positive|increase|expansion)/g) || []).length
    const negativeWords = (text.toLowerCase().match(/(decline|weak|negative|decrease|challenges)/g) || []).length
