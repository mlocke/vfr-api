...
export class EIAAPI implements FinancialDataProvider {
  ...
  private readonly COMMODITY_CATEGORIES: Record<string, {
    category: 'oil' | 'gas' | 'metals' | 'currency',
  ...
  async getCommodityPrices(symbols?: string[]): Promise<CommodityPrice[]> {
    const targetSymbols = symbols || ['PET.RWTC.D', 'PET.RBRTE.D', 'NG.RNGWHHD.D', 'PET.EMM_EPMR_PTE_NUS_DPG.W']
   ...
   */
  async analyzeSectorCorrelation(commoditySymbol: string, sectors: string[]): Promise<SectorCorrelation[]> {
    ...
    try {
      ...
      for (const sector of sectors) {
        try {
          ...
          let analysis = this.generateCorrelationAnalysis(sector, commoditySymbol, correlationStrength, categoryInfo)
   ...
   */
  async getMetalsInflationCorrelation(): Promise<{gold: number, silver: number, correlation: number} | null> {
  ...
  private generateCorrelationAnalysis(sector: string, commoditySymbol: string, correlation: number, categoryInfo: any): string {
    ...
    if (sector === 'Energy' && categoryInfo.category === 'oil') {
      analysis += ' Energy companies directly benefit from higher oil prices through improved margins and exploration activity.'
    } else if (sector === 'Transportation' && categoryInfo.category === 'oil') {
      analysis += ' Transportation costs increase with higher oil prices, negatively impacting profit margins for airlines, trucking, and shipping.'
    } else if (sector === 'Utilities' && categoryInfo.category === 'gas') {
      analysis += ' Natural gas prices significantly impact utility companies using gas-fired power generation.'
    } else if (sector === 'Consumer Discretionary') {
      analysis += ' Higher energy costs reduce disposable income, potentially impacting consumer spending on non-essential goods.'
    }
  ...
  async getEnergyMarketIntelligence(options?: {
    ...
    try {
      ...
      const inflationIndex = includeDXY && includeInflationImpact ? 2 : includeInflationImpact ? 1 : -1
      if (includeInflationImpact && results[inflationIndex] && results[inflationIndex].status === 'fulfilled') {
  ...
  async getQuickCommoditySnapshot(): Promise<{
    ...
    try {
      ...
      const symbols = ['PET.RWTC.D', 'PET.RBRTE.D', 'NG.RNGWHHD.D', 'PET.EMM_EPMR_PTE_NUS_DPG.W']
      ...
      return {
        ...
        gasoline: results[3].status === 'fulfilled' ? (results[3].value?.value || null) : null,
        responseTime
      }
    } catch (error) {
