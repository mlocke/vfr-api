...
export class EnhancedSentimentAnalysisService {
  ...
  async getSentimentFromSource(symbol: string, source: 'news' | 'social' | 'analyst' | 'insider'): Promise<SentimentAnalysis> {
    try {
      ...
      throw this.errorHandler.errorHandler.createErrorResponse(error, 'getSentimentFromSource')
    }
  }
  ...
  async getMultiSourceSentiment(symbol: string): Promise<MultiSourceSentiment> {
    ...
    try {
      ...
      this.sentimentCache.set(memoryKey, { data: multiSourceSentiment, timestamp: Date.now() })
      ...
      throw this.errorHandler.errorHandler.createErrorResponse(error, 'getMultiSourceSentiment')
    }
  }
  ...
  async getSentimentSignalBatch(symbols: string[]): Promise<SentimentSignal[]> {
    ...
    try {
      ...
      throw this.errorHandler.errorHandler.createErrorResponse(error, 'getSentimentSignalBatch')
    }
  }
  ...
  private async fetchFMPSentimentData(symbol: string, source: 'news' | 'social'): Promise<any> {
    ...
    try {
      ...
      this.errorHandler.logger.warn(`FMP ${source} sentiment fetch failed`, { symbol, error })
      return null
    }
  }
  ...
  private processNewsData(newsData: any[]): number {
    ...
    const sentiments = newsData.slice(0, 10).map(article => {
      ...
      const positiveWords = (title + ' ' + text).match(/(positive|bullish|growth|strong|surge|rally|gains)/g) || []
      const negativeWords = (title + ' ' + text).match(/(negative|bearish|decline|weak|crash|losses|drop)/g) || []

      return (positiveWords.length - negativeWords.length) / Math.max(positiveWords.length + negativeWords.length, 1)
    })
  ...
  private calculateDynamicWeights(results: SentimentAnalysis[]): Record<string, number> {
    const sources = ['news', 'social', 'analyst', 'insider']
    const baseWeights: Record<string, number> = { news: 0.3, social: 0.2, analyst: 0.3, insider: 0.2 }
   ...
   */
  private calculateWeightedComposite(results: SentimentAnalysis[], weights: Record<string, number>): number {
    const sources = ['news', 'social', 'analyst', 'insider']
   ...
   */
  private calculateCompositeConfidence(results: SentimentAnalysis[], weights: Record<string, number>): number {
    const sources = ['news', 'social', 'analyst', 'insider']
