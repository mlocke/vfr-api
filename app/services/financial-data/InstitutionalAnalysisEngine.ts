...
export class InstitutionalAnalysisEngine {
  ...
  async analyzeSymbol(
    ...
    try {
      ...
      if (useCache) {
        intelligence = await this.cacheManager.getInstitutionalIntelligence(sanitizedSymbol)
      ...
      if (!intelligence) {
        intelligence = await this.institutionalService.getInstitutionalIntelligence(sanitizedSymbol)

        // Cache the result
        if (intelligence && useCache) {
          await this.cacheManager.cacheInstitutionalIntelligence(sanitizedSymbol, intelligence)
        }
      }
      ...
      if (includeHoldings || includeInsiderActivity) {
        const [holdingsResult, insiderResult] = await Promise.allSettled([
          includeHoldings ? this.getHoldingsWithCache(sanitizedSymbol, useCache, maxAge) : Promise.resolve(undefined),
          includeInsiderActivity ? this.getInsiderActivityWithCache(sanitizedSymbol, useCache, maxAge) : Promise.resolve(undefined)
        ])
  ...
  private async executeParallelAnalysis(
    ...
    for (let i = 0; i < symbols.length; i += this.batchSize) {
      ...
      this.errorHandler.logger.debug(`Processing batch ${Math.floor(i / this.batchSize) + 1}`, {
   ...
   */
  private async smartDelay(batchResults: PromiseSettledResult<BatchAnalysisResult>[]): Promise<void> {
    // Calculate average response time for this batch
    const successfulResults = batchResults
      .filter((r): r is PromiseFulfilledResult<BatchAnalysisResult> => r.status === 'fulfilled')
      .map(r => r.value)

    const avgResponseTime = successfulResults.reduce((sum, r) => sum + r.processingTime, 0) / successfulResults.length
  ...
  private async checkMemoryPressure(): Promise<void> {
    if (typeof process !== 'undefined' && process.memoryUsage) {
      ...
      if (heapUsedMB > this.memoryThresholdMB) {
        this.errorHandler.logger.warn(`High memory usage detected: ${heapUsedMB.toFixed(1)}MB`, {
          heapUsed: heapUsedMB,
  ...
  private updateMetrics(results: BatchAnalysisResult[], processingTime: number): void {
    ...
    this.metrics.cacheHitRate =
      (this.metrics.cacheHitRate * (this.metrics.totalRequests - 1) + (cacheHits / totalRequests)) /
      this.metrics.totalRequests
