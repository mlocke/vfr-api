...
export class BLSAPI implements FinancialDataProvider {
	...
	getTier2Indicators(): Array<{ symbol: string; name: string; category: string }> {
		const tier2Series = {
			...
			JTS000000000000000JOL: {
				name: "Job Openings Rate (Total Nonfarm)",
				category: "Labor Dynamics",
			},
			JTS000000000000000QUR: {
				name: "Quit Rate (Total Nonfarm)",
				category: "Labor Dynamics",
			},
	...
	private async makeRequest(
	...
	): Promise<ApiResponse<BLSResponse>> {
		...
		try {
			...
			if (data.status === "REQUEST_NOT_PROCESSED") {
				throw new Error(
					`BLS API Error: ${data.message?.join(", ") || "Request not processed"}`
				);
			}
	 ...
	 */
	async analyzeUnemploymentTrend(
		monthsToAnalyze: number = 12
	): Promise<UnemploymentTrendResult | null> {
		if (!this.apiKey) {
		...
		try {
			...
			const isAtExtreme =
				currentRate <= historicalMin + range * 0.1 ||
	 ...
	 */
	async analyzePayrollMomentum(
		monthsToAnalyze: number = 12
	): Promise<PayrollMomentumResult | null> {
		if (!this.apiKey) {
		...
		try {
			...
			const recentAvg =
				recentChanges.reduce((sum, val) => sum + val, 0) / recentChanges.length;
	...
	private calculateTrendAnalysis(values: number[]): TrendAnalysis {
		...
		const confidence = Math.min(
			100,
			Math.max(0, rSquared * 70 + (n >= 12 ? 30 : (n / 12) * 30))
		);
	...
	async calculateEmploymentStrengthScore(): Promise<EmploymentStrengthScore | null> {
		...
		try {
			...
			if (overallScore >= 8.5) {
				interpretation =
					"Exceptionally strong employment conditions with robust job growth and low unemployment";
	...
	async calculateJobMarketHealthScore(): Promise<JobMarketHealthScore | null> {
		...
		try {
			...
			// Extract current values
			const unemployment = parseFloat(latestData.get("LNS14000000")?.value || "0");
			const jobOpeningsRate = parseFloat(
				latestData.get("JTS000000000000000JOL")?.value || "0"
			);
			...
			const unemploymentHealth = this.scoreUnemploymentForHealth(
				unemployment,
				unemploymentTrend?.trend.direction
			);
			const jobOpeningsHealth = this.scoreJobOpenings(
				jobOpeningsRate,
				jobOpeningsTrend?.direction
			);
			const quitRateHealth = this.scoreQuitRate(quitRate, quitRateTrend?.direction);

			// Overall health is weighted average
			const overallHealth =
				unemploymentHealth * 0.4 + jobOpeningsHealth * 0.35 + quitRateHealth * 0.25;
	 ...
	 */
	private async getSimpleTrend(
		seriesId: string,
		months: number
	): Promise<{ direction: string; strength: number } | null> {
		try {
			const currentYear = new Date().getFullYear();
	...
	private scoreJobOpenings(rate: number, trend?: string): number {
		...
		let baseScore = 0;
		if (rate >= 7.0)
			baseScore = 10.0; // Very high job availability
		else if (rate >= 6.0)
			baseScore = 9.0; // High job availability
		else if (rate >= 5.0)
			baseScore = 8.0; // Good job availability
		else if (rate >= 4.0)
			baseScore = 7.0; // Moderate job availability
		else if (rate >= 3.5)
			baseScore = 6.0; // Below average availability
		else if (rate >= 3.0)
			baseScore = 5.0; // Low job availability
		else if (rate >= 2.5)
			baseScore = 4.0; // Poor job availability
		else if (rate >= 2.0)
			baseScore = 3.0; // Very poor availability
		else if (rate >= 1.5)
			baseScore = 2.0; // Extremely poor
	...
	private scoreQuitRate(rate: number, trend?: string): number {
		...
		let baseScore = 0;
		if (rate >= 2.8 && rate <= 3.2)
			baseScore = 10.0; // Optimal confidence range
		else if (rate >= 2.5 && rate <= 3.5)
			baseScore = 9.0; // Very good range
		else if (rate >= 2.2 && rate <= 3.8)
			baseScore = 8.0; // Good range
		else if (rate >= 2.0 && rate <= 4.0)
			baseScore = 7.0; // Acceptable range
		else if (rate >= 1.8 && rate <= 4.2)
			baseScore = 6.0; // Suboptimal
		else if (rate >= 1.5 && rate <= 4.5)
			baseScore = 5.0; // Poor
		else if (rate >= 1.2 && rate <= 5.0)
			baseScore = 4.0; // Very poor
		else if (rate >= 1.0 && rate <= 5.5)
			baseScore = 3.0; // Extremely poor
		else if (rate >= 0.8 && rate <= 6.0)
			baseScore = 2.0; // Crisis indicators
	...
	async analyzeWageGrowth(monthsToAnalyze: number = 12): Promise<WageGrowthAnalysis | null> {
		...
		try {
			...
			const sortedInflationData = inflationData
				.sort((a, b) => {
					const dateA = this.parseDate(a.year, a.period);
					const dateB = this.parseDate(b.year, b.period);
					return dateB.getTime() - dateA.getTime();
	...
	async calculateConsumerSpendingPower(): Promise<ConsumerSpendingPower | null> {
		...
		try {
			...
			const [wageGrowthAnalysis, unemploymentTrend, employmentStrength] = await Promise.all([
			...
			const realWageComponent = this.scoreRealWageGrowth(
				wageGrowthAnalysis.inflationAdjustedGrowth
			);
			const employmentComponent = this.scoreEmploymentForSpending(
				unemploymentTrend.currentRate
			);
	...
	async calculateCompositeEmploymentHealth(): Promise<CompositeEmploymentHealth | null> {
		...
		try {
			...
			const [employmentStrength, jobMarketHealth, wageGrowthAnalysis, consumerSpendingPower] =
				await Promise.all([
					this.calculateEmploymentStrengthScore(),
			...
			const compositeScore =
				employmentScore * 0.3 + healthScore * 0.25 + wageScore * 0.25 + consumerScore * 0.2;

			// Determine overall trend
			const trends = [
				employmentStrength.interpretation.includes("strong")
					? 1
					: employmentStrength.interpretation.includes("weak")
						? -1
						: 0,
				jobMarketHealth.marketCondition === "tight"
					? 1
					: jobMarketHealth.marketCondition === "distressed"
						? -1
						: 0,
				wageGrowthAnalysis.consumptionImpact === "positive"
					? 1
					: wageGrowthAnalysis.consumptionImpact === "negative"
						? -1
						: 0,
			];
			...
			const economicPhase = this.determineEconomicPhase(
				compositeScore,
				overallTrend,
				jobMarketHealth
			);
	 ...
	 */
	private scoreConsumerConfidence(
		realWageGrowth: boolean,
		unemploymentImproving: boolean
	): number {
