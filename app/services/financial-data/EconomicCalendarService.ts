...
export class EconomicCalendarService {
  ...
  private cache: Map<string, { data: EconomicEvent[], timestamp: number, ttl: number }> = new Map()
  ...
  private static readonly RECURRING_EVENTS = [
    // CPI - Monthly, around 12th-15th of each month at 8:30 AM ET
    {
      ...
      description: 'Monthly measure of inflation based on a basket of consumer goods and services',
      time: '08:30',
  ...
  private getCacheTTL(timeframe: string, hasUpcomingEvents: boolean): number {
    ...
    // Normal cache times
    switch (timeframe) {
      case 'today': return 5 * 60 * 1000 // 5 minutes
      case 'week': return 15 * 60 * 1000 // 15 minutes
      case 'month': return 30 * 60 * 1000 // 30 minutes
    ...
    for (const meeting of EconomicCalendarService.FOMC_MEETINGS) {
      ...
      if (meetingDate >= startDate && meetingDate <= endDate) {
        events.push({
          ...
          impact: 'high',
          description: 'Federal Open Market Committee meeting where interest rate decisions are made',
