...
export class CurrencyDataService extends BaseFinancialDataProvider {
  ...
  private readonly sectorExposures: Record<string, SectorCurrencyImpact> = {
    'Technology': {
      ...
      impactDescription: 'High foreign revenue exposure; USD strength reduces international earnings',
      riskLevel: 'medium'
    },
    'Energy': {
      sector: 'Energy',
    ...
    'Consumer Discretionary': {
      ...
      impactDescription: 'Import-heavy sector; strong USD reduces input costs but may hurt exports',
      riskLevel: 'medium'
    },
    'Consumer Staples': {
      ...
      impactDescription: 'Moderate sensitivity; defensive nature provides some currency hedging',
      riskLevel: 'low'
    },
    'Financials': {
      sector: 'Financials',
    ...
    'Materials': {
      ...
      impactDescription: 'Commodity-heavy sector; strong USD typically pressures material prices',
      riskLevel: 'high'
    },
    'Utilities': {
      sector: 'Utilities',
    ...
    'Real Estate': {
      ...
      impactDescription: 'Limited direct exposure; mainly affected through interest rate channels',
      riskLevel: 'low'
    },
    'Communication Services': {
      ...
      impactDescription: 'Global tech platforms with significant international revenue exposure',
      riskLevel: 'medium'
    }
  }
  ...
  async getCurrencyAnalysis(): Promise<CurrencyAnalysis | null> {
    ...
    try {
      ...
      if (cached) {
        console.log(`🔄 Currency analysis served from cache in ${Date.now() - startTime}ms`)
        return cached
      }
      ...
      const strengths = currencyStrengths.status === 'fulfilled' ? currencyStrengths.value : []
      ...
      return this.handleApiError(error, 'currency_analysis', 'comprehensive currency analysis', null)
    }
  }
  ...
  async getDollarIndex(): Promise<DollarIndex | null> {
    try {
      ...
      if (dxyData) {
        await this.cache.set(cacheKey, dxyData, this.cacheTTL / 2, { // 7.5 minute cache for DXY
  ...
  async getMajorCurrencyPairs(): Promise<CurrencyPair[]> {
    try {
      ...
      const currencyPairs: CurrencyPair[] = results
        .filter((result): result is PromiseFulfilledResult<CurrencyPair | null> =>
  ...
  private calculateCurrencyStrength(currency: string, pairs: CurrencyPair[]): CurrencyStrength | null {
    try {
      let totalScore = 0
      let pairCount = 0
      ...
      return {
        ...
        volatility: this.calculateVolatility(pairs.filter(p =>
          p.symbol.includes(currency)
        )),
  ...
  private calculateMarketSentiment(dxy: DollarIndex | null, pairs: CurrencyPair[]) {
    const avgVolatility = pairs.length > 0
  ...
  private calculateConfidence(dxy: DollarIndex | null, pairs: CurrencyPair[], strengths: CurrencyStrength[]): number {
    let confidence = 0
