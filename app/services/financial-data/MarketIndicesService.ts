...
export class MarketIndicesService {
	...
	private async getIndexData(symbol: string, name: string): Promise<MarketIndex | null> {
			...
			try {
				...
				if (stockData) {
					...
					if (
						...
						console.warn(
							`MarketIndicesService: ${symbol} from ${providerName} returned all zeros, trying next provider`
						);
						continue;
					}
		...
		console.error(
			`MarketIndicesService: Failed to get index data for ${symbol} from all providers`
		);
		return null;
	}
	...
	async getSectorRotation(): Promise<{
		...
		const sectorPromises = this.SECTOR_ETFS.map(idx => this.getIndexData(idx.symbol, idx.name));
