...
export class YahooFinanceAPI implements FinancialDataProvider {
	...
	async getPutCallRatio(symbol: string): Promise<PutCallRatio | null> {
		try {
			...
			const totalCallOI = optionsChain.calls.reduce(
				(sum, contract) => sum + (contract.openInterest || 0),
				0
			);
			const totalPutOI = optionsChain.puts.reduce(
				(sum, contract) => sum + (contract.openInterest || 0),
				0
			);
	...
	async getOptionsAnalysisFreeTier(symbol: string): Promise<OptionsAnalysis | null> {
		try {
			console.warn(
				"⚠️ Using unofficial Yahoo Finance for options analysis - limited features"
			);
			...
			const analysis =
				`Put/Call Volume Ratio: ${putCallRatio.volumeRatio.toFixed(2)} | ` +
	...
	async getOptionsChain(symbol: string, expiration?: string): Promise<OptionsChain | null> {
		try {
			...
			const expirationDates =
				optionData.expirationDates?.map((exp: number) =>
	...
	async checkOptionsAvailability(): Promise<{ [key: string]: boolean | string }> {
		try {
			...
			return {
				...
				unreliable: true,
				message:
					"Yahoo Finance unofficial API error: " +
