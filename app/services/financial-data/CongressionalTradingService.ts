...
export class CongressionalTradingService {
  ...
  async getCongressionalTrades(symbol: string): Promise<CongressionalTrade[]> {
    ...
    try {
      ...
      const cachedData = this.fmpCache.get<CongressionalTrade[]>(cacheKey, 'congressional_trades')
      ...
      throw this.errorHandler.errorHandler.createErrorResponse(error, 'getCongressionalTrades')
    }
  }
  ...
  async getPoliticalInsiderSignal(symbol: string): Promise<PoliticalInsiderSignal> {
    try {
      ...
      const netSentiment = recentPurchases > recentSales ? 'BULLISH' :
                          recentSales > recentPurchases ? 'BEARISH' : 'NEUTRAL'
      ...
      throw this.errorHandler.errorHandler.createErrorResponse(error, 'getPoliticalInsiderSignal')
    }
  }
    ...
    try {
      ...
      for (let i = 0; i < symbols.length; i += this.batchSize) {
        ...
        const batchPromises = batch.map(async (symbol, index) => {
          ...
          const sentimentData = politicalSentiment.status === 'fulfilled' ? politicalSentiment.value :
      ...
      results.push(...batchResults
        .filter((result): result is PromiseFulfilledResult<CongressionalAnalysis> => result.status === 'fulfilled')
      ...
      throw this.errorHandler.errorHandler.createErrorResponse(error, 'getCongressionalAnalysisBatch')
    }
  }
  ...
  private async fetchFMPInstitutionalHoldings(symbol: string): Promise<any[]> {
    ...
    try {
      ...
      this.errorHandler.logger.error('FMP institutional holdings fetch failed', { symbol, error })
      return []
    }
  }
   ...
   */
  private processPoliticalTrades(insiderData: any[], institutionalData: any[], symbol: string): CongressionalTrade[] {
   ...
   */
  private generateInsights(trades: CongressionalTrade[], sentiment: PoliticalInsiderSignal): string[] {
    const insights = [
   ...
   */
  private identifyRiskFactors(trades: CongressionalTrade[], sentiment: PoliticalInsiderSignal): string[] {
    const risks: string[] = []
   ...
   */
  private identifyOpportunities(trades: CongressionalTrade[], sentiment: PoliticalInsiderSignal): string[] {
   ...
   */
  private calculateOverallRating(sentiment: PoliticalInsiderSignal): 'POSITIVE' | 'NEGATIVE' | 'NEUTRAL' {
   ...
   */
  private calculateConfidence(trades: CongressionalTrade[], sentiment: PoliticalInsiderSignal): number {
