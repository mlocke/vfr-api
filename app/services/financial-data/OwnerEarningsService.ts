...
export class OwnerEarningsService {
  ...
  async getOwnerEarningsAnalysis(symbol: string): Promise<OwnerEarningsAnalysis> {
    try {
      ...
      const averageYield = yields.reduce((sum, currentYield) => sum + currentYield, 0) / yields.length || 0

      const qualityScores = historicalData.map(d => d.qualityScore)
      const avgQualityScore = qualityScores.reduce((sum, score) => sum + score, 0) / qualityScores.length || 5

      // Determine trend
      const recentYields = yields.slice(0, 3)
      const olderYields = yields.slice(3)
      const recentAvg = recentYields.reduce((sum, y) => sum + y, 0) / recentYields.length || 0
      ...
      const analysis: OwnerEarningsAnalysis = {
        ...
        trend,
        averageYield,
        qualityAssessment: avgQualityScore > 7 ? 'HIGH' : avgQualityScore > 4 ? 'MEDIUM' : 'LOW',
        keyInsights: this.generateKeyInsights(symbol, historicalData, trend, averageYield),
        investmentRating: this.determineInvestmentRating(averageYield, avgQualityScore, trend),
        confidence: 0.8,
        timestamp: Date.now(),
      ...
      throw this.errorHandler.errorHandler.createErrorResponse(error, 'getOwnerEarningsAnalysis')
    }
  }
   ...
   */
  async compareOwnerEarnings(symbols: string[]): Promise<{ symbol: string; analysis: OwnerEarningsAnalysis }[]> {
    try {
      ...
      return results
        .filter((result): result is PromiseFulfilledResult<{ symbol: string; analysis: OwnerEarningsAnalysis }> =>
   ...
   * @returns Promise<OwnerEarningsAnalysis[]>
   */
  async screenByOwnerEarnings(minYield: number = 5, minQuality: number = 6): Promise<OwnerEarningsAnalysis[]> {
    try {
      ...
      const validAnalyses = analyses
        .filter((result): result is PromiseFulfilledResult<OwnerEarningsAnalysis> => result.status === 'fulfilled')
        .map(result => result.value)

      // Filter by criteria
      return validAnalyses.filter(analysis =>
        analysis.averageYield >= minYield &&
        (analysis.qualityAssessment === 'HIGH' ? 8 : analysis.qualityAssessment === 'MEDIUM' ? 5 : 2) >= minQuality
      )

    } catch (error) {
      throw this.errorHandler.errorHandler.createErrorResponse(error, 'screenByOwnerEarnings')
    }
  }
  ...
  private generateMockOwnerEarnings(symbol: string, period: string): OwnerEarnings {
    ...
    const qualityScore = Math.min(10, Math.max(1,
      5 + (cashConversionRatio - 1) * 5 + (Math.random() - 0.5) * 2
    ))
   ...
   * @private
   */
  private generateKeyInsights(symbol: string, data: OwnerEarnings[], trend: string, avgOwnerYield: number): string[] {
