...
export type DataSourceProvider = "fmp" | "eodhd";
...
export class DataSourceManager {
	...
	getProvidersForDataType(dataType: DataType): DataSourceProvider[] {
		...
		return [preferences.primary, ...preferences.fallbacks].filter(
			provider =>
	...
	async getStockPrice(symbol: string): Promise<StockData | null> {
		...
		for (const providerName of providers) {
			...
			try {
				...
				console.warn(
					`❌ ${providerName} failed for stock price:`,
	...
	async getCompanyInfo(symbol: string): Promise<CompanyInfo | null> {
		...
		for (const providerName of providers) {
			...
			try {
				...
				console.warn(
					`❌ ${providerName} failed for company info:`,
					error instanceof Error ? error.message : error
				);
				continue;
			}
	...
	async getProviderStatus(): Promise<
		Record<DataSourceProvider, { available: boolean; lastCheck: number }>
	> {
		const status: Record<DataSourceProvider, { available: boolean; lastCheck: number }> =
			{} as any;
	...
	async getServiceStatus(): Promise<{
		...
		if (!providerStatus.fmp?.available) {
			recommendations.push(
				"⚠️ FMP (primary data source) is unavailable - check API key and connectivity"
			);
		}

		if (!providerStatus.eodhd?.available) {
			recommendations.push(
				"⚠️ EODHD (fallback data source) is unavailable - options data may be limited"
			);
		}
	...
	resetToDefaults(): void {
		...
		console.log(
			"🔄 Reset all data source preferences to defaults (FMP primary, EODHD fallback)"
		);
	}
}
