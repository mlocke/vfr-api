...
export class TreasuryAPI implements FinancialDataProvider {
	...
	async getLatestDebtData(): Promise<any> {
		try {
			...
			return response.success && response.data?.data?.length > 0
	...
	async getDailyTreasuryStatement(): Promise<any> {
		try {
			const response = await this.makeRequest(
				"/v1/accounting/dts/dts_table_1?sort=-record_date&limit=10"
			);
	...
	async getOperatingCashBalance(): Promise<any> {
		try {
			const response = await this.makeRequest(
				"/v1/accounting/dts/operating_cash_balance?sort=-record_date&limit=1"
			);
			return response.success && response.data?.data?.length > 0
				? response.data.data[0]
				: null;
