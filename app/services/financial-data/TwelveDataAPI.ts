...
export class TwelveDataAPI implements FinancialDataProvider {
	...
	constructor(apiKey?: string, timeout = 10000, throwErrors = false) {
		...
		if (this.apiKey && !this.isValidApiKeyFormat(this.apiKey)) {
			console.warn(
				"TwelveData API key format appears invalid. Expected a 32-character alphanumeric string."
			);
			...
			if (quoteResponse.success && quoteResponse.data) {
				...
				if (quote.close) {
					...
					const changePercent = quote.percent_change
						? parseFloat(quote.percent_change)
						: 0;
	...
	async healthCheck(): Promise<boolean> {
		try {
			...
			console.error(
				"TwelveData health check failed:",
	...
	async getAnalystRatings(symbol: string): Promise<AnalystRatings | null> {
		console.warn(
			`TwelveData: Analyst ratings not supported for ${symbol}. Use Financial Modeling Prep as primary source.`
		);
		return null;
	}
	...
	async getPriceTargets(symbol: string): Promise<PriceTarget | null> {
		console.warn(
			`TwelveData: Price targets not supported for ${symbol}. Use Financial Modeling Prep as primary source.`
		);
		return null;
	}
	...
	async getRecentRatingChanges(symbol: string, limit = 10): Promise<RatingChange[]> {
		console.warn(
			`TwelveData: Rating changes not supported for ${symbol}. Use Financial Modeling Prep as primary source.`
		);
		return [];
	}
		...
		try {
			...
			if (data.code && data.message) {
				...
				if (data.code === 400) {
					...
					errorMessage =
						"TwelveData API key is invalid. Please check your TWELVE_DATA_API_KEY environment variable.";
