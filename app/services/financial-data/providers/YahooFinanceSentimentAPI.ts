...
export class YahooFinanceSentimentAPI implements FinancialDataProvider {
  ...
  private createBatchedRequest(symbol: string): Promise<NewsSentimentData | null> {
    return new Promise((resolve, reject) => {
      ...
      let targetBatch = this.batchQueue.find(batch =>
        batch.symbols.length < this.MAX_BATCH_SIZE
      )
  ...
  private async processBatches(): Promise<void> {
    ...
    try {
      ...
      const allSymbols = Array.from(new Set(
        batches.flatMap(batch => batch.symbols)
      ))
   ...
   */
  private async executeBatchSentimentAnalysis(symbols: string[]): Promise<Map<string, NewsSentimentData>> {
  ...
  private evictLRU(): void {
    ...
    for (const [symbol, cached] of this.sentimentCache.entries()) {
      ...
      if (cached.hits < minHits || (cached.hits === minHits && cached.timestamp < oldestTime)) {
        minHits = cached.hits
  ...
  getPerformanceStats() {
    return {
      ...
      avgBatchSize: this.stats.batchRequests > 0 ? this.stats.requests / this.stats.batchRequests : 0
    }
  }
  ...
  async warmCache(symbols: string[]): Promise<number> {
    ...
    const promises = symbols.map(symbol =>
      this.getNewsSentiment(symbol).catch(() => null)
    )
