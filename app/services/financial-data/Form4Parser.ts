...
export class Form4Parser {
	...
	constructor() {
		...
		this.xmlParser = new XMLParser({
			...
			updateTag: (tagName: string) => {
				...
				return relevantTags.some(tag => tagName.toLowerCase().includes(tag.toLowerCase()))
					? tagName
					: false;
			},
		});
	...
	async parseForm4Filing(
		...
		try {
			...
			const content =
				typeof filingContent === "string" ? filingContent : filingContent.toString("utf8");
			...
			if (content.length > this.maxFileSize) {
				this.errorHandler.logger.warn(`Form 4 filing too large: ${content.length} bytes`, {
					accessionNumber,
				});
				return [];
			}
			...
			if (!this.isValidForm4Content(content)) {
				this.errorHandler.logger.warn("Invalid Form 4 content structure", {
					accessionNumber,
				});
				return [];
			}
	...
	async parseMultipleForm4Filings(
		...
		for (let i = 0; i < filings.length; i += batchSize) {
			...
			const batchPromises = batch.map(async filing => {
				try {
					return await this.parseForm4Filing(
						filing.content,
					...
					this.errorHandler.logger.warn(
						`Batch parsing failed for ${filing.accessionNumber}`,
						{ error }
					);
					return [];
				}
	...
	private extractFilingMetadata(doc: any, accessionNumber: string): Form4Filing {
		...
		const ownerName = this.extractTextValue(
			reportingOwner.reportingOwnerId?.reportingOwnerName
		);
		const ownerCik = this.extractTextValue(reportingOwner.reportingOwnerId?.reportingOwnerCik);
		const ownerTitle = this.extractTextValue(
		...
		const relationship = this.extractRelationships(
	...
	private extractNonDerivativeTransactions(doc: any, filing: Form4Filing): InsiderTransaction[] {
		...
		try {
			...
			this.errorHandler.logger.warn("Non-derivative transaction extraction failed", {
				error,
			});
		}
	...
	private convertToInsiderTransaction(
		...
		try {
			...
			const shares =
				this.extractNumericValue(transactionAmounts.transactionShares?.value) || 0;
			const pricePerShare = this.extractNumericValue(
				transactionAmounts.transactionPricePerShare?.value
			);
			const acquiredDisposed = this.extractTextValue(
			...
			const ownershipType =
				this.extractTextValue(postTransactionAmounts.directOrIndirectOwnership?.value) ||
				"D";
			const ownershipNature = this.extractTextValue(
				postTransactionAmounts.natureOfOwnership?.value
			);
			...
			if (isDerivative) {
				...
				exercisePrice = this.extractNumericValue(
					derivativeSecurityInfo.exercisePrice?.value
				);
				expirationDate = this.extractTextValue(
					derivativeSecurityInfo.expirationDate?.value
				);

				const underlyingSecurity = entry.underlyingSecurity || {};
				underlyingSecurityTitle = this.extractTextValue(
					underlyingSecurity.underlyingSecurityTitle?.value
				);
				underlyingShares = this.extractNumericValue(
	...
	private extractRelationships(relationshipData: any): string[] {
		...
		if (
			relationshipData.isTenPercentOwner === "true" ||
			relationshipData.isTenPercentOwner === true
		) {
	...
	): "BUY" | "SELL" | "GRANT" | "EXERCISE" | "GIFT" | "OTHER" {
	...
	private calculateConfidenceScore(entry: any, transactionCode: string): number {
		...
		const shares =
			this.extractNumericValue(entry.transactionAmounts?.transactionShares?.value) || 0;
		if (shares < 100) {
			confidence -= 0.1;
		}
	...
	private isValidForm4Content(content: string): boolean {
		...
		return (
			content.includes("ownershipDocument") &&
