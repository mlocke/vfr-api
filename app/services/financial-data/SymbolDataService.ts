...
export class SymbolDataService {
	...
	private async isRefreshNeeded(): Promise<boolean> {
		try {
			...
			if (isExpired) {
				console.log(
					`📅 Symbol data expired (${new Date(metadata.lastRefresh).toLocaleString()}), refresh needed`
				);
				return true;
			}
	...
	private async refreshSymbolsBackground(): Promise<void> {
		try {
			...
			await this.updateRefreshMetadata("failed", 0, "background-refresh", [
	...
	private async downloadSymbolsFromSources(): Promise<StockSymbol[]> {
		...
		for (const source of this.dataSources) {
			try {
				...
				if (symbols.length > 0) {
					console.log(
						`✅ Successfully downloaded ${symbols.length} symbols from ${source.name}`
					);
					await this.updateRefreshMetadata(
						"success",
						symbols.length,
						source.name,
						errors
	...
	private async getStaticSymbols(): Promise<StockSymbol[]> {
		try {
			...
			console.log(
				`📁 Loaded ${fileData.symbols?.length || 0} symbols from static file (updated: ${fileData.lastUpdated})`
			);
	...
	private async updateRefreshMetadata(
		...
		try {
			...
			console.log(
				`📋 Updated refresh metadata: ${status}, ${totalSymbols} symbols from ${source}`
			);
		} catch (error) {
