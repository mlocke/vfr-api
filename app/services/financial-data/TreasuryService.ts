...
export class TreasuryService {
	...
	async getTreasuryRates(): Promise<TreasuryDataResponse> {
		...
		try {
			...
			const ratePromises = Object.entries(this.TREASURY_SERIES).map(
				async ([duration, seriesId]) => {
					try {
			...
			console.log(
				`📈 2Y/10Y spread: ${spreads["2Y10Y"]?.toFixed(2) || "N/A"}% ${indicators.isInverted ? "(INVERTED)" : ""}`
			);
	...
	private calculateSpreads(rates: TreasuryRates["rates"]): TreasuryRates["spreads"] {
		...
		return {
			"2Y10Y":
				rate10Y !== null && rate2Y !== null ? Number((rate10Y - rate2Y).toFixed(2)) : null,
			"10Y30Y":
				rate30Y !== null && rate10Y !== null
					? Number((rate30Y - rate10Y).toFixed(2))
					: null,
			"3M10Y":
				rate10Y !== null && rate3M !== null ? Number((rate10Y - rate3M).toFixed(2)) : null,
		};
	}
	...
	private calculateDataQuality(rates: TreasuryRates["rates"]): number {
		...
		const availableKeyRates = keyRates.filter(
			key => rates[key as keyof typeof rates] !== null
		).length;
	...
	async getTreasurySummary(): Promise<{
		...
		if (indicators.isInverted) {
			...
			riskLevel = "high";
			insights.push(
				`Yield curve inverted (${spreads["2Y10Y"]?.toFixed(2)}% spread) - potential recession signal`
			);
		...
		if (rates["10Y"]) {
			if (rates["10Y"] > 5) {
				insights.push(
					`High 10Y yield at ${rates["10Y"].toFixed(2)}% - restrictive monetary policy`
				);
			} else if (rates["10Y"] < 2) {
				insights.push(
					`Low 10Y yield at ${rates["10Y"].toFixed(2)}% - accommodative environment`
				);
			}
		}
