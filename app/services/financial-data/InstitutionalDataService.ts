...
export class InstitutionalDataService extends BaseFinancialDataProvider {
	...
	async getInstitutionalHoldings(
		...
		try {
			...
			const validationError = await this.errorHandler.errorHandler.validateOperation(
				"getInstitutionalHoldings",
				[symbol]
			);
			if (validationError) {
				this.errorHandler.logger.debug(
					`Validation failed for symbol ${symbol}, returning empty array`
				);
				return [];
			}
			...
			this.errorHandler.logger.warn(`Failed to get institutional holdings for ${symbol}`, {
				error,
			});
			return [];
		}
	...
	async getInstitutionalIntelligence(symbol: string): Promise<InstitutionalIntelligence | null> {
		try {
			...
			if (validationError) {
				this.errorHandler.logger.debug(
					`Validation failed for symbol ${symbol}, returning null`
				);
				return null;
			}
	...
	private async executeGetInstitutionalHoldings(
		...
		if (cached?.holdings && Date.now() - cached.lastUpdated < 6 * 60 * 60 * 1000) {
			this.errorHandler.logger.debug(
				`Using cached institutional holdings for ${sanitizedSymbol}`
			);
	...
	private async executeGetInsiderTransactions(
		...
		if (cached?.insiderTransactions && Date.now() - cached.lastUpdated < 2 * 60 * 60 * 1000) {
			this.errorHandler.logger.debug(
				`Using cached insider transactions for ${sanitizedSymbol}`
			);
		...
		try {
			...
			const transactions = await this.fetchForm4Transactions(
				sanitizedSymbol,
				companyCik,
				days
			);
			...
			this.errorHandler.logger.logApiError("GET", "insider_transactions", error, undefined, {
	...
	private async executeGetInstitutionalIntelligence(
		symbol: string
	): Promise<InstitutionalIntelligence | null> {
		const sanitizedSymbol = this.normalizeSymbol(symbol);
		...
		try {
			...
			if (this.fmpAPI) {
				...
				if (institutionalData && institutionalData.summary) {
					this.errorHandler.logger.info(
						`FMP institutional data success for ${sanitizedSymbol}: ${institutionalData.summary.totalInstitutionalHolders} holders, sentiment ${institutionalData.summary.compositeSentiment}`
					);
					...
					const intelligence: InstitutionalIntelligence = {
						...
						institutionalSentiment:
							this.convertFMPInstitutionalSentiment(institutionalData),
						...
						dataQuality: {
							institutionalDataAvailable:
								(institutionalData.institutionalOwnership?.length || 0) > 0,
							insiderDataAvailable:
								(institutionalData.insiderTrading?.length || 0) > 0,
			...
			const institutionalSentiment =
				this.analyzeInstitutionalSentiment(institutionalHoldings);
			...
			const intelligence: InstitutionalIntelligence = {
				...
				weightedSentiment: this.getWeightedSentiment(
					institutionalSentiment,
					insiderSentiment
				),
	 ...
	 */
	private async processFilingsInParallel(
		filings: any[],
		symbol: string
	): Promise<InstitutionalHolding[]> {
		...
		const processingPromises = filings.map(async filing => {
			return semaphore(async () => {
				try {
					...
					this.errorHandler.logger.warn(
						`Failed to process filing ${filing.accessNumber}`,
						{ error }
					);
					return [];
				}
		...
		return allHoldings.sort((a, b) => b.marketValue - a.marketValue).slice(0, 100); // Limit to top 100 institutional holders
	}

	/**
	 * Optimized 13F filing parser with memory management
	 */
	private async parse13FFiling(
		filing: any,
		targetSymbol: string
	): Promise<InstitutionalHolding[]> {
		try {
			...
			this.errorHandler.logger.warn(`13F parsing error for ${filing.accessNumber}`, {
				error,
			});
			return [];
		}
	 ...
	 */
	private async parseXmlStreaming(
		content: string,
		targetSymbol: string
	): Promise<InstitutionalHolding[]> {
	 ...
	 */
	private extractHoldingFromXmlFragment(
		xmlFragment: string,
		targetSymbol: string
		...
		try {
			...
			const sharesMatch = xmlFragment.match(
				/<shrsOrPrnAmt>.*?<sshPrnamt>(.*?)<\/sshPrnamt>/i
			);
	...
	private analyzeInstitutionalData(
		...
		const topHolders = holdings.slice(0, 10).map(holding => ({
		...
		return {
			...
			dataCompleteness: {
				...
				insiderCoverage:
					insiderActivity.length > 0 ? Math.min(insiderActivity.length / 10, 1) : 0,
	...
	private calculateCacheTtl(dataCompleteness: any): number {
		...
		const qualityMultiplier =
			(dataCompleteness.thirteenFCoverage + dataCompleteness.insiderCoverage) / 2;
	 ...
	 */
	private analyzeInstitutionalSentiment(
		holdings: InstitutionalHolding[]
	): InstitutionalSentiment | undefined {
	 ...
	 */
	private analyzeInsiderSentiment(
		...
		return {
			...
			netShares: transactions.reduce(
				(sum, t) => sum + (t.transactionType === "BUY" ? t.shares : -t.shares),
				0
			),
			netValue: transactions.reduce(
				(sum, t) =>
			...
			averageTransactionSize:
				transactions.reduce((sum, t) => sum + (t.transactionValue || 0), 0) /
				transactions.length,
			insiderTypes: {
			...
			sentimentScore: Math.max(
				1,
				Math.min(9, 5 + (buyTransactions.length - sellTransactions.length))
			),
	...
	private generateKeyInsights(
		...
		if (holdings.length > 0) {
			...
			insights.push(
				`${holdings.length} institutional holders with $${(totalValue / 1e9).toFixed(1)}B total value`
			);
		}
		...
		if (transactions.length > 0) {
			...
			insights.push(
				`${transactions.length} insider transactions: ${netBuys} buys, ${netSells} sells`
			);
		}
	...
	private calculateDataFreshness(
		...
		const daysSinceLatest =
			latestTimestamp > 0 ? Math.floor((now - latestTimestamp) / (24 * 60 * 60 * 1000)) : 365;
	...
	private extractHoldingsFromParsed(
		parsed: any,
		targetSymbol: string,
		filing: any
	): InstitutionalHolding[] {
	...
	private convertFMPInstitutionalSentiment(fmpData: any): InstitutionalSentiment | undefined {
		...
		return {
			...
			quarterlyChange: {
				...
				netValueChange: 0,
				flowScore:
					fmpData.summary.institutionalSentiment === "BULLISH"
						? 1
						: fmpData.summary.institutionalSentiment === "BEARISH"
							? -1
							: 0,
			},
	...
	private convertFMPInsiderSentiment(fmpData: any): InsiderSentiment | undefined {
		...
		return {
			...
			netShares: transactions.reduce(
				(sum: number, t: any) => sum + (t.transactionType === "BUY" ? t.shares : -t.shares),
				0
			),
			netValue: transactions.reduce(
				(sum: number, t: any) =>
					sum + (t.transactionType === "BUY" ? 1 : -1) * t.transactionValue,
				0
			),
			buyTransactions: buyTransactions.length,
			sellTransactions: sellTransactions.length,
			buyValue: buyTransactions.reduce((sum: number, t: any) => sum + t.transactionValue, 0),
			sellValue: sellTransactions.reduce(
				(sum: number, t: any) => sum + t.transactionValue,
				0
			),
			averageTransactionSize:
				transactions.reduce((sum: number, t: any) => sum + t.transactionValue, 0) /
				transactions.length,
			insiderTypes: {
				officers: { transactions: 0, netShares: 0, netValue: 0 },
				directors: { transactions: 0, netShares: 0, netValue: 0 },
	...
	private generateFMPKeyInsights(fmpData: any): string[] {
		...
		if (summary.totalInstitutionalHolders > 0) {
			insights.push(
				`${summary.totalInstitutionalHolders} institutional holders with $${(summary.totalInstitutionalValue / 1e9).toFixed(1)}B total value`
			);
		}

		if (summary.recentInsiderActivity > 0) {
			insights.push(
				`${summary.recentInsiderActivity} recent insider transactions (${summary.netInsiderSentiment.toLowerCase()} sentiment)`
			);
		}

		if (summary.institutionalSentiment !== "NEUTRAL") {
			insights.push(
				`Institutional sentiment: ${summary.institutionalSentiment.toLowerCase()}`
			);
		}
	...
	private identifyFMPRiskFactors(fmpData: any): string[] {
		...
		if (summary.netInsiderSentiment === "BEARISH") {
			risks.push(
				`Recent insider selling activity detected (${summary.recentInsiderActivity} transactions)`
			);
		}
		...
		if (
			summary.compositeSentiment === "VERY_BEARISH" ||
			summary.compositeSentiment === "BEARISH"
		) {
	...
	private identifyFMPOpportunities(fmpData: any): string[] {
		...
		if (summary.netInsiderSentiment === "BULLISH") {
			opportunities.push(
				`Insider buying activity detected (${summary.recentInsiderActivity} transactions)`
			);
		}
		...
		if (
			summary.compositeSentiment === "VERY_BULLISH" ||
			summary.compositeSentiment === "BULLISH"
		) {
