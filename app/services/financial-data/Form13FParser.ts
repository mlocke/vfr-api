...
export class Form13FParser {
		...
		try {
			...
			if (content.length > this.config.streamingThreshold) {
				this.errorHandler.logger.info(
					`Large filing detected (${(content.length / 1024 / 1024).toFixed(1)}MB), using streaming parser`
				);
			...
			this.errorHandler.logger.error("13F parsing failed", {
				error,
				contentSize:
					typeof filingContent === "string" ? filingContent.length : filingContent.length,
				targetSymbol,
						...
						if (char === "<") {
							...
							if (element === "infotable") {
								...
								if (insideInfoTable && self.isValidHolding(currentHolding)) {
									const holding = self.convertToInstitutionalHolding(
										currentHolding,
										managerInfo
									);
									if (
										holding &&
										(!targetSymbol || holding.symbol === targetSymbol)
									) {
							...
							currentElement = "";
						} else if (currentElement.length < 50) {
							// Prevent memory issues with malformed XML
							currentElement += char;
						} else if (insideInfoTable && currentValue.length < 200) {
							// Limit value length
							currentValue += char;
						}
					}
	...
	private async parseInMemoryMode(
		...
		try {
			...
			this.errorHandler.logger.warn(
				"In-memory parsing failed, falling back to regex extraction",
				{ error }
			);
	...
	private parseWithRegexFallback(
		...
		while (
			(match = infoTableRegex.exec(content)) !== null &&
			holdings.length < this.config.maxHoldingsPerFiling
		) {
			try {
	...
	private parseInfoTableRegex(
		...
		try {
			...
			return {
				...
				investmentDiscretion: this.normalizeInvestmentDiscretion(
					extracted.investmentDiscretion
				),
				timestamp: Date.now(),
				source: "sec_13f",
			};
	...
	private extractInfoTables(parsed: any): ParsedInfoTable[] {
		...
		try {
			...
			this.errorHandler.logger.warn("Failed to extract info tables from parsed XML", {
				error,
			});
		}
	...
	private determineSecurityType(
		table: Partial<ParsedInfoTable>
	): "COM" | "PRF" | "PUT" | "CALL" | "NOTE" | "BOND" | "OTHER" {
	...
	private normalizeInvestmentDiscretion(discretion: string): "SOLE" | "SHARED" | "OTHER" {
		const normalized = String(discretion || "")
			.toUpperCase()
			.trim();
	...
	private processElementValue(
		element: string,
		value: string,
		holding: Partial<ParsedInfoTable>
	): void {
	...
	private updatePerformanceStats(processingTime: number, memoryUsage: number): void {
		...
		this.processingStats.averageProcessingTime =
			(this.processingStats.averageProcessingTime *
				...
				processingTime) /
			this.processingStats.filesProcessed;
		this.processingStats.memoryPeakMB = Math.max(
			this.processingStats.memoryPeakMB,
			memoryUsage
		);
	}
