...
export class FREDAPI implements FinancialDataProvider {
	...
	constructor(apiKey?: string, timeout = 15000, throwErrors = false) {
		...
		if (this.apiKey && !this.isValidApiKeyFormat(this.apiKey)) {
			console.warn(
				"FRED API key format is invalid. Key must be a 32-character lowercase alphanumeric string."
			);
			if (this.throwErrors) {
				throw new Error(
					"FRED API key format is invalid. Key must be a 32-character lowercase alphanumeric string."
	...
	async getCompanyInfo(symbol: string): Promise<CompanyInfo | null> {
		try {
			...
			return {
				...
				name:
					seriesInfo.title ||
					this.POPULAR_SERIES[symbol.toUpperCase() as keyof typeof this.POPULAR_SERIES] ||
					symbol,
			...
			const promises = Object.entries(treasurySeries).map(async ([period, seriesId]) => {
				try {
					...
					if (observation && observation.value !== ".") {
						...
						switch (period) {
							case "3M":
								treasuryRate.month3 = rate;
								break;
							case "6M":
								treasuryRate.month6 = rate;
								break;
							case "1Y":
								treasuryRate.year1 = rate;
								break;
							case "2Y":
								treasuryRate.year2 = rate;
								break;
							case "5Y":
								treasuryRate.year5 = rate;
								break;
							case "10Y":
								treasuryRate.year10 = rate;
								break;
							case "20Y":
								treasuryRate.year20 = rate;
								break;
							case "30Y":
								treasuryRate.year30 = rate;
								break;
						}
					}
				} catch (error) {
	...
	async getTreasuryAnalysisData(): Promise<any> {
		try {
			...
			if (analysisData.rates["10Y"] && analysisData.rates["2Y"]) {
				analysisData.yieldCurve.slope_10Y_2Y =
					analysisData.rates["10Y"] - analysisData.rates["2Y"];
				analysisData.yieldCurve.isInverted = analysisData.yieldCurve.slope_10Y_2Y < 0;
			}

			if (analysisData.rates["10Y"] && analysisData.rates["3M"]) {
				analysisData.yieldCurve.slope_10Y_3M =
					analysisData.rates["10Y"] - analysisData.rates["3M"];
			}

			if (analysisData.rates["30Y"] && analysisData.rates["10Y"]) {
				analysisData.yieldCurve.slope_30Y_10Y =
					analysisData.rates["30Y"] - analysisData.rates["10Y"];
			}

			// Analyze rate momentum
			const totalChanges = Object.values(analysisData.changes).filter(
				c => typeof c === "number"
			);
			if (totalChanges.length > 0) {
				const avgChange =
					totalChanges.reduce((sum: number, change: any) => sum + change, 0) /
					totalChanges.length;
	...
	private async getRecentObservations(
		...
		try {
			...
			// Filter out missing values (marked as '.')
			const validObservations = response.data.observations.filter(
	...
	async getEconomicContext(): Promise<EconomicContext | null> {
		...
		try {
				...
				if (result.status === "fulfilled" && result.value.observations) {
					...
					if (!isNaN(value)) {
						const indicator: EconomicIndicator = {
							...
							date: current.date,
							change: previous
								? Number((value - parseFloat(previous.value)).toFixed(4))
								: undefined,
							changePercent:
			...
			console.log(
				`✅ Economic context fetched in ${responseTime}ms with ${(context.dataCompleteness * 100).toFixed(1)}% completeness`
			);
		...
		try {
			...
			const cyclePosition: CyclePosition = {
				...
				gdpMomentum: {
					...
					trend:
						gdpGrowth > avgGrowth * 1.1
							? "accelerating"
							: gdpGrowth < avgGrowth * 0.9
								? "decelerating"
								: "stable",
					vsHistorical:
						gdpGrowth > avgGrowth * 1.2
							? "above"
							: gdpGrowth < avgGrowth * 0.8
								? "below"
								: "normal",
				},
				yieldCurveSignal: {
			...
			if (currentUnemployment > previousUnemployment * 1.05) {
				cyclePosition.riskFactors.push(
					"Rising unemployment suggests labor market weakening"
				);
			}
			...
			console.log(
				`✅ Economic cycle analysis complete: ${phase} phase with ${cyclePosition.confidence}% confidence`
			);
	...
	async getInflationTrendAnalysis(): Promise<InflationTrend | null> {
		try {
			...
			const cpiMoM =
				((currentCPI - parseFloat(cpiData[1].value)) / parseFloat(cpiData[1].value)) * 100;
			const cpiYoY =
				((currentCPI - parseFloat(cpiData[12].value)) / parseFloat(cpiData[12].value)) *
				100;

			const ppiMoM =
				((currentPPI - parseFloat(ppiData[1].value)) / parseFloat(ppiData[1].value)) * 100;
			const ppiYoY =
				((currentPPI - parseFloat(ppiData[12].value)) / parseFloat(ppiData[12].value)) *
				100;
			...
			else if (cpiTrend === "decelerating" && ppiTrend === "decelerating")
				outlook = "falling";
			else outlook = "stable";
			...
			console.log(
				`✅ Inflation analysis complete: ${environment} environment, ${outlook} outlook`
			);
	...
	async getMonetaryPolicyContext(): Promise<MonetaryContext | null> {
		try {
			...
			const m1YoY =
				((currentM1 - parseFloat(m1Data[12].value)) / parseFloat(m1Data[12].value)) * 100;
			const m2YoY =
				((currentM2 - parseFloat(m2Data[12].value)) / parseFloat(m2Data[12].value)) * 100;

			const m1QoQ =
				((currentM1 - parseFloat(m1Data[3].value)) / parseFloat(m1Data[3].value)) * 100 * 4; // Annualized
			const m2QoQ =
				((currentM2 - parseFloat(m2Data[3].value)) / parseFloat(m2Data[3].value)) * 100 * 4;

			// Determine trends
			const m1Trend =
				Math.abs(m1QoQ) < 2 ? "stable" : m1QoQ > 0 ? "expanding" : "contracting";
			const m2Trend =
				Math.abs(m2QoQ) < 2 ? "stable" : m2QoQ > 0 ? "expanding" : "contracting";

			// Fed policy stance analysis
			const fedTrend =
				currentFedRate > parseFloat(fedFundsData[3].value) + 0.1
					? "tightening"
					: currentFedRate < parseFloat(fedFundsData[3].value) - 0.1
						? "easing"
						: "neutral";
			...
			const equitySentiment =
				equityScore > 70 ? "supportive" : equityScore < 30 ? "headwind" : "neutral";
			...
			if (currentFedRate > 5)
				reasoning.push("High rates increase discount rates for equity valuations");
			if (liquidityConditions === "tight")
				reasoning.push("Tight liquidity conditions pressure risk assets");
			...
			const monetaryContext: MonetaryContext = {
				...
				policyStance,
				marketPerformanceCorrelation: Number(
					(0.85 - Math.abs(currentFedRate - 3) * 0.1).toFixed(2)
			...
			console.log(
				`✅ Monetary policy analysis complete: ${policyStance} stance, ${liquidityConditions} liquidity`
			);
	...
	private async makeRequest(
		...
		try {
			...
			if (data.error_message) {
				...
				if (
					data.error_message.includes(
						"api_key is not a 32 character alpha-numeric lower-case string"
					)
				) {
					errorMessage =
						"FRED API key is invalid. The key must be exactly 32 characters long and contain only lowercase letters and numbers. Please check your FRED_API_KEY environment variable.";
