...
describe('FeatureStore Integration Tests', () => {
  ...
  describe('Feature Definition Management', () => {
    ...
    it('should retrieve feature definitions by type', async () => {
      ...
      expect(technicalDefs.every(def => def.featureType === MLFeatureType.TECHNICAL)).toBe(true)
    }, 10000)
  ...
  describe('Feature Matrix Retrieval', () => {
    ...
    it('should retrieve feature matrix in <100ms for 25 symbols', async () => {
      const symbols = [
        ...
        'UBER', 'LYFT', 'SNAP', 'TWTR', 'PINS',
        'SHOP', 'SPOT', 'ZM', 'DOCU', 'CRWD'
      ]
  ...
  describe('Performance Benchmarks', () => {
    ...
    it('should demonstrate bulk insert performance', async () => {
      ...
      for (const size of batchSizes) {
        ...
        console.log(`Bulk insert ${size} features: ${duration}ms (${(duration / size).toFixed(2)}ms per feature)`)
  ...
  describe('Cache Performance', () => {
    it('should demonstrate cache effectiveness', async () => {
      ...
      const features: BulkFeatureInsert[] = []
      await featureStore.storeBulkFeatures(features.length ? features : [
        {
          ticker: 'CACHE_TEST',
          timestamp: Date.now(),
          featureId: 'test-rsi-001',
      ...
      console.log(`Cache Miss: ${missDuration}ms, Cache Hit: ${hitDuration}ms, Speedup: ${(missDuration / hitDuration).toFixed(2)}x`)
