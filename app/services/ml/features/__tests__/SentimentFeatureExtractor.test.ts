...
describe('SentimentFeatureExtractor', () => {
  ...
  describe('News Sentiment Features', () => {
    it('should_extract_news_sentiment_features_from_real_data', async () => {
      const indicators = await sentimentService.getSentimentIndicators(TEST_SYMBOLS.LARGE_CAP);
  ...
  describe('Social/Reddit Sentiment Features', () => {
    it('should_extract_reddit_sentiment_features_from_real_data', async () => {
      const indicators = await sentimentService.getSentimentIndicators(TEST_SYMBOLS.MEME_STOCK);
    ...
    it('should_detect_retail_buzz_correctly', async () => {
      ...
      const expectedBuzz = features.social.postCount > 20 &&
    ...
    it('should_calculate_reddit_strength_correctly', async () => {
      ...
      const expectedStrength = features.social.redditSentiment * features.social.redditConfidence;
    ...
    it('should_detect_institutional_flow_direction', async () => {
      ...
      if (indicators.options) {
        const expectedDirection =
          indicators.options.institutionalFlow === 'INFLOW' ? 1 :
          indicators.options.institutionalFlow === 'OUTFLOW' ? -1 : 0;
  ...
  describe('Sentiment Momentum Features', () => {
    it('should_calculate_sentiment_momentum_with_previous_indicators', async () => {
      const currentIndicators = await sentimentService.getSentimentIndicators(TEST_SYMBOLS.LARGE_CAP);
    ...
    it('should_calculate_consensus_strength_correctly', async () => {
      ...
      const avgDivergence = (features.momentum.newsVsRedditDivergence +
                            features.momentum.newsVsOptionsDivergence) / 2;
  ...
  describe('Sentiment Embeddings', () => {
    ...
    it('should_calculate_weighted_sentiment_correctly', async () => {
      ...
      const expectedWeighted = features.embeddings.overallSentiment *
                              features.embeddings.sentimentConfidence;
  ...
  describe('Default/Fallback Features', () => {
    it('should_return_default_features_when_requested', () => {
      const defaultFeatures = SentimentFeatureExtractor.getDefaultFeatures(TEST_SYMBOLS.LARGE_CAP);
    ...
    it('should_return_consistent_default_features', () => {
      ...
      expect(defaults1.embeddings.overallSentiment).toBe(defaults2.embeddings.overallSentiment);
    });
  });
  ...
  describe('Performance Requirements', () => {
    ...
    it('should_handle_multiple_extractions_efficiently', async () => {
      ...
      for (let i = 0; i < iterations; i++) {
        SentimentFeatureExtractor.extractFeatures(
          `SYMBOL${i}`,
          indicators
        );
      }
      ...
      console.log(`⚡ Average extraction time: ${avgTime.toFixed(2)}ms (${iterations} iterations)`);
  ...
  describe('Integration Tests', () => {
    it('should_extract_complete_features_for_multiple_symbols', async () => {
      ...
      console.log(`✅ Successfully extracted features for ${results.length}/${symbols.length} symbols`);
