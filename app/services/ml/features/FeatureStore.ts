...
export class FeatureStore {
  ...
  public async storeBulkFeatures(features: BulkFeatureInsert[]): Promise<void> {
    ...
    try {
      ...
      const valuesPlaceholders = features.map((_, idx) => {
        const base = idx * 8
  ...
  public async getFeatureMatrix(
    ...
    try {
      ...
      const duration = Date.now() - startTime
      this.logger.info(`Feature matrix retrieved: ${featureMatrix.size} symbols in ${duration}ms`)
   ...
   */
  public async getFeatureDefinitions(featureType?: MLFeatureType): Promise<MLFeatureDefinition[]> {
    if (!this.initialized) {
      await this.initialize()
    }
  ...
  public async batchRetrieveFeatures(
    ...
    try {
      ...
      const duration = Date.now() - startTime
      this.logger.debug(`Batch feature retrieval: ${symbols.length} symbols in ${duration}ms`)
  ...
  private async cacheFeatureVectors(vectors: Map<string, MLFeatureVector>): Promise<void> {
    try {
      ...
      for (const [symbol, vector] of vectors) {
        cachePromises.push(
          this.mlCache.cacheFeatureVector(symbol, vector)
        )
      }
