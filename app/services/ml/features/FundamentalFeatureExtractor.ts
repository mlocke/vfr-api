...
export class FundamentalFeatureExtractor {
	 ...
	 */
	static extractFeatures(symbol: string, ratios: FundamentalRatios): FundamentalFeatures {
	 ...
	 */
	private static extractValuationFeatures(ratios: FundamentalRatios): ValuationFeatures {
	 ...
	 */
	private static extractProfitabilityFeatures(ratios: FundamentalRatios): ProfitabilityFeatures {
	...
	private static extractShareholderReturnFeatures(
		...
		const dividendGrowthPotential =
			dividendYield > 0 && payoutRatio < 60 ? Math.min(1, (60 - payoutRatio) / 60) : 0;
