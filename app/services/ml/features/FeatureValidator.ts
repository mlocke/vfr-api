...
export class FeatureValidator {
	...
	private initializeDefaultRules(): void {
		...
		this.addRangeConfig("bollinger_upper", {
			featureName: "bollinger_upper",
			min: 0,
			max: 1000000,
			outlierThreshold: 4,
			allowNull: false,
		});
		this.addRangeConfig("bollinger_lower", {
			featureName: "bollinger_lower",
			min: 0,
			max: 1000000,
			outlierThreshold: 4,
			allowNull: false,
		});
		this.addRangeConfig("stochastic_k", {
			featureName: "stochastic_k",
			min: 0,
			max: 100,
			outlierThreshold: 3,
			allowNull: false,
		});
		this.addRangeConfig("stochastic_d", {
			featureName: "stochastic_d",
			min: 0,
			max: 100,
			outlierThreshold: 3,
			allowNull: false,
		});
		this.addRangeConfig("atr", {
			featureName: "atr",
			min: 0,
			max: 10000,
			outlierThreshold: 4,
		...
		this.addRangeConfig("pb_ratio", {
			featureName: "pb_ratio",
			min: 0,
			max: 100,
			outlierThreshold: 3,
			allowNull: true,
		});
		this.addRangeConfig("roe", {
			featureName: "roe",
			min: -1,
			max: 5,
			outlierThreshold: 3,
		...
		this.addRangeConfig("debt_equity", {
			featureName: "debt_equity",
			min: 0,
			max: 10,
			outlierThreshold: 3,
			allowNull: true,
		});
		this.addRangeConfig("current_ratio", {
			featureName: "current_ratio",
			min: 0,
			max: 20,
			outlierThreshold: 3,
			allowNull: true,
		});
		this.addRangeConfig("quick_ratio", {
			featureName: "quick_ratio",
			min: 0,
			max: 20,
			outlierThreshold: 3,
			allowNull: true,
		});
		this.addRangeConfig("dividend_yield", {
			featureName: "dividend_yield",
			min: 0,
			max: 0.25,
			outlierThreshold: 3,
			allowNull: true,
		...
		// Sentiment features
		this.addRangeConfig("sentiment_score", {
		...
		this.addRangeConfig("news_sentiment", {
			featureName: "news_sentiment",
			min: -1,
			max: 1,
			outlierThreshold: 3,
			allowNull: true,
		});
		this.addRangeConfig("reddit_sentiment", {
			featureName: "reddit_sentiment",
			min: -1,
			max: 1,
			outlierThreshold: 3,
			allowNull: true,
		});
		this.addRangeConfig("social_volume", {
			featureName: "social_volume",
			min: 0,
			max: 1000000,
			outlierThreshold: 4,
			allowNull: true,
	 ...
	 */
	public validateFeature(feature: MLFeature, definition?: MLFeatureDefinition): ValidationResult {
	...
	private validateSchema(
		...
		if (
			...
			(!isFinite(feature.value) || isNaN(feature.value))
		) {
			errors.push({
				featureName: feature.featureName || "unknown",
	 ...
	 */
	private validateRange(feature: MLFeature): {
		valid: boolean;
		errors: ValidationError[];
		warnings: ValidationWarning[];
	} {
	 ...
	 */
	private validateFreshness(feature: MLFeature): {
		valid: boolean;
		errors: ValidationError[];
		warnings: ValidationWarning[];
		critical: boolean;
		...
		const config =
			this.freshnessConfigs.get(feature.featureName) || this.freshnessConfigs.get("default")!;
