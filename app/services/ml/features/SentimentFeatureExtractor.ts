...
} from "../../financial-data/types/sentiment-types";
...
export class SentimentFeatureExtractor {
	 ...
	 */
	private static extractNewsFeatures(indicators: SentimentIndicators): NewsSentimentFeatures {
	 ...
	 */
	private static extractSocialFeatures(indicators: SentimentIndicators): SocialSentimentFeatures {
	...
	private static extractOptionsFeatures(
		...
		const institutionalDirection =
			optionsData.institutionalFlow === "INFLOW"
				? 1
				: optionsData.institutionalFlow === "OUTFLOW"
					? -1
					: 0;
	...
	private static extractMomentumFeatures(
		...
		const redditMomentum =
			indicators.reddit && previousIndicators.reddit
				? indicators.reddit.sentiment - previousIndicators.reddit.sentiment
				: 0;
		const optionsMomentum =
			indicators.options && previousIndicators.options
	 ...
	 */
	private static extractEmbeddings(indicators: SentimentIndicators): SentimentEmbeddings {
