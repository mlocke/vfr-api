...
export class ModelCache {
  ...
  getCacheStats(): CacheStatistics {
    ...
    const avgLoadTimeMs = this.statistics.totalRequests > 0
  ...
  async healthCheck(): Promise<{
    ...
    if (stats.avgLoadTimeMs > this.config.maxLoadTimeMs) {
      issues.push(`Average load time (${stats.avgLoadTimeMs}ms) exceeds threshold (${this.config.maxLoadTimeMs}ms)`)
    }

    // Check if P95 load time is within acceptable range (allow 2x threshold)
    if (stats.p95LoadTimeMs > this.config.maxLoadTimeMs * 2) {
      issues.push(`P95 load time (${stats.p95LoadTimeMs}ms) exceeds threshold (${this.config.maxLoadTimeMs * 2}ms)`)
    }

    // Check memory usage
    if (stats.memoryUsageMB > 500) { // 500MB warning threshold
