...
export class ModelEvaluator {
  ...
  public evaluateRegression(
    ...
    try {
      ...
      const totalSumSquares = actuals.reduce((sum, a) => sum + Math.pow(a - actualsMean, 2), 0)
      ...
      const variancePredictions = predictions.reduce(
        (sum, p) => sum + Math.pow(p - predictionsMean, 2),
        0
      ) / n
      const varianceActuals = totalSumSquares / n
      const explainedVariance = varianceActuals > 0 ? variancePredictions / varianceActuals : 0
  ...
  public evaluateFinancial(
    ...
    try {
      ...
      const directionAccuracy = directions.reduce((sum: number, d: number) => sum + d, 0) / directions.length
