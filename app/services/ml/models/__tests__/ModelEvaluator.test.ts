...
describe("ModelEvaluator", () => {
	...
	describe("Classification Metrics", () => {
		it("should evaluate classification metrics correctly", () => {
			const { predictions, actuals } = generatePredictions(100, 0.8);
	...
	describe("Financial Metrics", () => {
		it("should evaluate financial metrics correctly", () => {
			const predictions = Array(100)
				.fill(0)
				.map(() => Math.random());
			const actuals = Array(100)
				.fill(0)
				.map(() => (Math.random() > 0.5 ? 1 : 0));
			const returns = Array(100)
				.fill(0)
		...
		it("should calculate Sharpe ratio", () => {
			...
			const returns = Array(100)
				.fill(0)
				.map(() => (Math.random() - 0.3) * 0.05); // Slightly positive returns
	...
	describe("Comprehensive Evaluation", () => {
		...
		it("should work without financial returns", () => {
			...
			if (result.data) {
				...
				expect(result.data.financial.directionAccuracy).toBe(
					result.data.classification.accuracy
				);
			}
		});
	});
	...
	describe("Cross-Validation", () => {
		it("should perform k-fold cross-validation", async () => {
			...
			if (result.data) {
				...
				expect(result.data.confidenceInterval.lower).toBeLessThan(
					result.data.confidenceInterval.upper
				);
			}
		}, 30000);
