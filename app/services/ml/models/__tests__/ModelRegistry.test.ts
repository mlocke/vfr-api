...
describe('ModelRegistry', () => {
  ...
  function createTestModelInput(overrides?: Partial<RegisterModelInput>): RegisterModelInput {
    return {
      modelName: overrides?.modelName || `test_model_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
  ...
  describe('Model Listing', () => {
    ...
    it('should list models with tier filter', async () => {
      ...
      expect(response.success).toBe(true)
      expect(response.data!.every(m => m.tierRequirement === TierRequirement.ENTERPRISE)).toBe(true)
    })
    ...
    it('should list models with objective filter', async () => {
      ...
      expect(response.success).toBe(true)
      expect(response.data!.every(m => m.objective === ModelObjective.VOLATILITY_FORECAST)).toBe(true)
    })
  ...
  describe('Performance Metrics', () => {
    ...
    it('should update performance metrics', async () => {
      const response = await registry.updatePerformanceMetrics(
        testModelId,
        0.92,
        0.89
      )
    ...
    it('should track metrics over time with model history', async () => {
      ...
      const versions = [
        { version: '1.0.0', valScore: 0.80, testScore: 0.78 },
        { version: '1.1.0', valScore: 0.85, testScore: 0.83 },
        { version: '2.0.0', valScore: 0.90, testScore: 0.88 }
      ]
      ...
      expect(Number(sortedByVersion[0].validationScore)).toBe(0.80)
      expect(Number(sortedByVersion[1].validationScore)).toBe(0.85)
      expect(Number(sortedByVersion[2].validationScore)).toBe(0.90)
    })
  })
