...
import { Pool } from 'pg'
import { ModelTrainer, TrainingDataConfig, TrainingConfig, DataSplit, HyperparameterGrid } from '../ModelTrainer'
  ...
  describe('LightGBM Training', () => {
    ...
    it('should achieve >70% validation accuracy', async () => {
      ...
      if (dataResult.data) {
        ...
        expect(result.success).toBe(true)
        if (result.data) {
          expect(result.data.metrics.validationAccuracy).toBeGreaterThanOrEqual(0.70)
        }
      }
    }, 30000)
  })
  ...
  describe('Hyperparameter Optimization', () => {
    it('should optimize hyperparameters using grid search', async () => {
      ...
      if (dataResult.data) {
        ...
        const result = await trainer.optimizeHyperparameters(split, trainingConfig, paramGrid)
