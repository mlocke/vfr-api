...
export interface HyperparameterValidation extends ValidationResult {
  ...
  parameterDetails: Record<string, {
    valid: boolean
    value: any
...
export const DEFAULT_VALIDATION_THRESHOLDS = {
  minAccuracy: 0.60,        // 60% minimum accuracy
  minPrecision: 0.55,
  minRecall: 0.55,
  minF1Score: 0.55,
  minSharpeRatio: 0.5,
...
export const HYPERPARAMETER_RANGES: Record<MLModelType, Record<string, HyperparameterRange>> = {
  [MLModelType.LIGHTGBM]: {
    ...
    boosting_type: { allowedValues: ['gbdt', 'dart', 'goss', 'rf'], type: 'string', required: false, default: 'gbdt' }
  },
  [MLModelType.XGBOOST]: {
    ...
    booster: { allowedValues: ['gbtree', 'gblinear', 'dart'], type: 'string', required: false, default: 'gbtree' }
  },
  [MLModelType.LSTM]: {
    ...
    activation: { allowedValues: ['relu', 'tanh', 'sigmoid', 'linear'], type: 'string', required: false, default: 'tanh' }
  },
  [MLModelType.ENSEMBLE]: {
    voting_type: { allowedValues: ['hard', 'soft', 'weighted'], type: 'string', required: false, default: 'soft' },
    model_count: { min: 2, max: 10, type: 'number', required: true },
    weights: { type: 'array', required: false },
    aggregation_method: { allowedValues: ['mean', 'median', 'weighted_average'], type: 'string', required: false, default: 'mean' }
  }
}
...
export class ModelValidator {
  ...
  public validateModelConfig(config: MLModelConfig): ValidationResult {
    ...
    if (!config.modelId || config.modelId.trim() === '') {
      ...
      errors.push('Model ID must contain only alphanumeric characters, underscores, and hyphens')
    } else {
    ...
    if (!config.features || !Array.isArray(config.features)) {
      ...
      warnings.push(`Low feature count: ${config.features.length} features (consider adding more features for better model performance)`)
  ...
  public validatePerformanceMetrics(
    ...
    if (performance.accuracy < this.thresholds.minAccuracy) {
      ...
      if (performance.accuracy < this.thresholds.minAccuracy + 0.05) {
        warnings.push(`Accuracy ${(performance.accuracy * 100).toFixed(2)}% is only slightly above minimum threshold`)
      }
    }
    ...
    if (performance.validationLoss > performance.trainingLoss * 2) {
      warnings.push('Validation loss is significantly higher than training loss - possible overfitting or data distribution mismatch')
    }

    // Log validation results
    if (errors.length > 0) {
      this.logger.warn(`Performance metrics validation failed${modelId ? ` for ${modelId}` : ''}`, {
        errors,
        warnings,
        actualValues,
        thresholds
  ...
  public validateModelSize(
    ...
    try {
      ...
      if (sizeMB < 0.1) {
        warnings.push(`Model size ${sizeMB.toFixed(2)}MB is unusually small - verify model was saved correctly`)
      }
      ...
      errors.push(`Failed to validate model size: ${error instanceof Error ? error.message : 'Unknown error'}`)
  ...
  public validateArtifactIntegrity(
    ...
    try {
      ...
      if (errors.length > 0) {
        this.logger.warn(`Artifact integrity validation failed${modelId ? ` for ${modelId}` : ''}`, {
          artifactPath,
          errors,
          warnings
        })
      ...
      errors.push(`Failed to validate artifact integrity: ${error instanceof Error ? error.message : 'Unknown error'}`)
  ...
  public validateLoadingTime(
    ...
    return new Promise(async (resolve) => {
      ...
      try {
        ...
        if (errors.length > 0) {
          this.logger.warn(`Load time validation failed${modelId ? ` for ${modelId}` : ''}`, {
            loadTime,
        ...
        this.logger.error(`Error during model loading validation${modelId ? ` for ${modelId}` : ''}`, { error })
        errors.push(`Failed to load model: ${error instanceof Error ? error.message : 'Unknown error'}`)
  ...
  public validateFeatureCompatibility(
    ...
    const compatibilityScore = totalExpectedFeatures > 0 ? matchingFeatures / totalExpectedFeatures : 0

    if (compatibilityScore < 1.0 && compatibilityScore >= 0.9) {
      warnings.push(`Feature compatibility is ${(compatibilityScore * 100).toFixed(1)}% - some features missing`)
    }

    if (errors.length > 0) {
      this.logger.warn(`Feature compatibility validation failed${modelId ? ` for ${modelId}` : ''}`, {
        errors,
        warnings,
  ...
  public validateHyperparameters(
    ...
    if (!ranges) {
      warnings.push(`No hyperparameter validation rules defined for model type: ${modelType}`)
      return {
        isValid: true,
        errors,
        warnings,
    ...
    for (const [paramName, value] of Object.entries(hyperparameters)) {
      ...
      if (range.allowedValues && !range.allowedValues.includes(value)) {
        errors.push(`${paramName} value '${value}' is not in allowed values: ${range.allowedValues.join(', ')}`)
        isValid = false
    ...
    if (errors.length > 0) {
      this.logger.warn(`Hyperparameter validation failed for ${modelType}${modelId ? ` (${modelId})` : ''}`, {
        errors,
        warnings,
        invalidParams,
  ...
  public async validateModelForDeployment(
    ...
    const performanceResult = this.validatePerformanceMetrics(config.performanceMetrics, config.modelId)
    ...
    const integrityResult = this.validateArtifactIntegrity(artifactPath, expectedChecksum, config.modelId)
    ...
    const featureResult = this.validateFeatureCompatibility(config.features, providedFeatures, config.modelId)
    ...
    const hyperparamResult = this.validateHyperparameters(config.modelType, config.hyperparameters, config.modelId)
    ...
    if (!isValid) {
      ...
      this.logger.warn(`Deployment validation passed with warnings for model ${config.modelId}`, {
        totalWarnings: warnings.length,
...
export const createModelValidator = (customThresholds?: Partial<ValidationThresholds>): ModelValidator => {
