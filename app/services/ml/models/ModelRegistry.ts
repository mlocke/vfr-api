...
export class ModelRegistry {
	 ...
	 */
	public async updateModel(input: UpdateModelInput): Promise<MLServiceResponse<ModelMetadata>> {
		const startTime = Date.now();
	 ...
	 */
	public async listModels(filter?: ModelListFilter): Promise<MLServiceResponse<ModelMetadata[]>> {
		const startTime = Date.now();
	 ...
	 */
	public async getModelHistory(modelName: string): Promise<MLServiceResponse<ModelMetadata[]>> {
		const startTime = Date.now();
	...
	> {
      ...
      `;
        ...
        `;
			...
			) as MLServiceResponse<
				Array<{ champion: ModelMetadata; challengers: ModelMetadata[] }>
			>;
		}
	}
