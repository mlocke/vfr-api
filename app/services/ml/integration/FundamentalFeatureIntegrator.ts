...
export class FundamentalFeatureIntegrator {
	...
	constructor(cache?: RedisCache, config?: Partial<FundamentalFeatureConfig>) {
	...
	async extractBatchFeatures(symbols: string[]): Promise<FundamentalFeaturesML[]> {
		const startTime = Date.now();
		console.log(
			`📊 Extracting fundamental features for ${symbols.length} symbols in parallel...`
		);
		...
		console.log(
			`⚡ Fundamental batch extraction completed in ${elapsed}ms (${(elapsed / symbols.length).toFixed(0)}ms per symbol)`
		);
