...
export class SentimentFeatureIntegrator {
	...
	constructor(cache?: RedisCache, config?: Partial<SentimentFeatureConfig>) {
	...
	async extractBatchFeatures(
		...
		const startTime = Date.now();
		console.log(
			`📊 Extracting sentiment features for ${symbols.length} symbols in parallel...`
		);

		const results = await Promise.all(
			symbols.map(symbol =>
				this.extractSentimentFeatures(symbol, analystDataMap?.get(symbol))
			)
		);

		const elapsed = Date.now() - startTime;
		console.log(
			`⚡ Sentiment batch extraction completed in ${elapsed}ms (${(elapsed / symbols.length).toFixed(0)}ms per symbol)`
		);
