...
import type {
	FundamentalFeaturesML,
	FundamentalFeatureConfig,
} from "../FundamentalFeatureIntegrator";
...
describe("FundamentalFeatureIntegrator", () => {
	...
	describe("Single Symbol Feature Extraction - Real API", () => {
		it("should extract fundamental features for AAPL with real FMP data", async () => {
			...
			console.log(
				`✓ Extracted ${Object.keys(features.features).length} features for ${TEST_SYMBOLS.SINGLE}`
			);
		...
		it("should extract features for MSFT with real data", async () => {
			...
			console.log(
				`✓ MSFT features extracted with ${(features.metadata.completeness * 100).toFixed(1)}% completeness`
			);
		}, 10000);
	...
	describe("Caching Functionality - 15-minute TTL", () => {
		...
		it("should respect cache TTL configuration", async () => {
			...
			// First call
			const features1 =
				await shortCacheIntegrator.extractFundamentalFeatures(CACHE_TEST_SYMBOL);
			expect(features1).toBeDefined();
	...
	describe("Graceful Fallback on API Failure", () => {
		...
		it("should handle network timeouts gracefully", async () => {
			...
			const features = await timeoutIntegrator.extractFundamentalFeatures(
				TEST_SYMBOLS.SINGLE
			);
	...
	describe("Batch Parallel Processing", () => {
		it("should extract features for 5 symbols in parallel", async () => {
			...
			console.log(
				`✓ Batch extracted 5 symbols in ${duration}ms (${(duration / TEST_SYMBOLS.BATCH_SMALL.length).toFixed(0)}ms per symbol)`
			);
		...
		it("should meet performance target for 25 symbols (<1000ms)", async () => {
			...
			console.log(
				`   Target: <1000ms | Actual: ${duration}ms | Status: ${duration < 1000 ? "PASS ✓" : "FAIL ✗"}`
			);
		...
		it("should process batch with consistent data quality", async () => {
			...
			console.log(
				`    Average completeness: ${((completenessScores.reduce((a, b) => a + b, 0) / completenessScores.length) * 100).toFixed(1)}%`
			);
		}, 30000);
		...
		it("should maintain cache efficiency across batch operations", async () => {
			...
			console.log(
				`  Cache efficiency: ${((1 - duration2 / duration1) * 100).toFixed(1)}% faster`
			);
			...
			const cacheHits = cachedResults.filter(r =>
				r.metadata.sources.includes("cache")
			).length;
	...
	describe("Integration with FinancialModelingPrepAPI - Zero Breaking Changes", () => {
		...
		it("should use same data structure as FMP API", async () => {
			...
			const integratorFeatures = await integrator.extractFundamentalFeatures(
				TEST_SYMBOLS.SINGLE
			);
		...
		it("should preserve FMP API source attribution", async () => {
			...
			if (
				!features.metadata.sources.includes("default") &&
				!features.metadata.sources.includes("cache")
			) {
	...
	describe("Performance Benchmarks", () => {
		...
		it("should demonstrate parallel processing efficiency", async () => {
			...
			console.log(
				`  Efficiency gain: ${((1 - batchDuration / estimatedSequentialTime) * 100).toFixed(1)}%`
			);
