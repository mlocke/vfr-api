...
} from "../SentimentFeatureIntegrator";
...
describe("SentimentFeatureIntegrator - Real API Integration Tests", () => {
	...
	describe("Single Symbol Feature Extraction with Real SentimentAnalysisService", () => {
		test("should_extract_sentiment_features_for_single_symbol_with_real_api", async () => {
			...
			console.log(
				`✓ Single symbol extraction: ${symbol} - ${extractionTime}ms, ${features.metadata.sources.join(", ")}`
			);
		}, 30000);
		...
		test("should_extract_all_39_sentiment_features_with_correct_values", async () => {
			...
			const expectedFeatures = [
				...
				"reddit_momentum",
				"options_momentum",
				"news_reddit_divergence",
				"news_options_divergence",
				"consensus_strength",
		...
		test("should_integrate_with_sentimentanalysisservice_without_breaking_changes", async () => {
			...
			if (indicators) {
				...
				console.log(
					`✓ SentimentAnalysisService integration validated - no breaking changes`
				);
			}
		}, 30000);
	});
	...
	describe("Batch Parallel Processing for Multiple Symbols", () => {
		test("should_extract_features_for_multiple_symbols_in_parallel", async () => {
			...
			console.log(
				`✓ Batch extraction: ${symbols.length} symbols in ${batchTime}ms (${avgTimePerSymbol.toFixed(0)}ms/symbol)`
			);
		}, 60000);
		...
		test("should_process_25_symbols_under_1500ms_performance_target", async () => {
			...
			console.log(
				`✓ Large batch: ${symbols.length} symbols in ${largeBatchTime}ms (${avgTimePerSymbol.toFixed(0)}ms/symbol)`
			);
			console.log(
				`  Valid results: ${validResults.length}/${symbols.length} (${((validResults.length / symbols.length) * 100).toFixed(1)}%)`
			);
		}, 60000);
		...
		test("should_maintain_parallel_processing_efficiency_with_concurrent_calls", async () => {
			...
			console.log(
				`✓ Parallel efficiency: ${efficiency.toFixed(1)}% improvement (${sequentialTime}ms → ${parallelTime}ms)`
			);
		}, 90000);
	});
	...
	describe("Caching Functionality with 15-Minute TTL", () => {
		test("should_cache_sentiment_features_with_15_minute_ttl", async () => {
			...
			console.log(
				`✓ Cache efficiency: ${cacheEfficiency.toFixed(1)}% improvement (${time1}ms → ${time2}ms)`
			);
		}, 30000);
	...
	describe("Momentum Tracking Across Multiple Calls", () => {
		test("should_track_sentiment_momentum_across_multiple_calls", async () => {
			...
			console.log(
				`✓ Momentum tracking: sentiment_momentum=${result2.features.sentiment_momentum.toFixed(3)}, news_momentum=${result2.features.news_momentum.toFixed(3)}`
			);
		}, 45000);
		...
		test("should_calculate_divergence_between_sentiment_sources", async () => {
			...
			console.log(
				`✓ Divergence metrics: news_reddit=${features.features.news_reddit_divergence.toFixed(3)}, consensus=${features.features.consensus_strength.toFixed(3)}`
			);
		}, 30000);
	...
	describe("Health Check Functionality", () => {
		test("should_perform_health_check_successfully", async () => {
			...
			if (isHealthy) {
				...
				console.log(
					`⚠ Health check: UNHEALTHY (Redis or sentiment service may be unavailable)`
				);
			}
		}, 30000);
		...
		test("should_validate_cache_connectivity", async () => {
			try {
				...
				console.log(
					`⚠ Cache connectivity: Failed (Redis may not be available in test environment)`
				);
			}
		});
	});
	...
	describe("Error Handling and Resilience", () => {
		test("should_handle_timeout_gracefully", async () => {
			...
			console.log(
				`✓ Timeout handling: returned ${features.metadata.sources.join(", ")} features`
			);
		}, 30000);
		...
		test("should_handle_concurrent_requests_without_errors", async () => {
			...
			const promises = Array(concurrentRequests)
				.fill(0)
				.map(() => integrator.extractSentimentFeatures(symbol));
			...
			console.log(
				`✓ Concurrent requests: ${concurrentRequests} requests completed successfully`
			);
		}, 45000);
	});
	...
	describe("Performance Benchmarks and Memory Management", () => {
		test("should_maintain_memory_efficiency_with_large_batches", async () => {
			...
			console.log(
				`✓ Memory efficiency: ${(memoryIncrease / 1024 / 1024).toFixed(2)}MB for ${symbols.length} symbols`
			);
		}, 60000);
	...
	describe("Data Quality and Completeness Validation", () => {
		test("should_calculate_data_completeness_accurately", async () => {
			...
			const sourceCount = features.metadata.sources.filter(
				s => s !== "cache" && s !== "default"
			).length;
			const expectedCompleteness = sourceCount / 3; // 3 possible sources: news, reddit, options

			// Allow some tolerance
			expect(Math.abs(features.metadata.completeness - expectedCompleteness)).toBeLessThan(
				0.4
			);

			console.log(
				`✓ Data completeness: ${(features.metadata.completeness * 100).toFixed(1)}% (${sourceCount} sources)`
			);
		}, 30000);
		...
		test("should_provide_appropriate_confidence_levels", async () => {
			...
			console.log(
				`✓ Confidence levels: overall=${features.features.sentiment_confidence.toFixed(2)}, news=${features.features.news_confidence.toFixed(2)}`
			);
		}, 30000);
	});
});
