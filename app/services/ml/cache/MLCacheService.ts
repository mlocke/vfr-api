...
import { MLPrediction, MLFeatureVector, MLModelConfig, MLHealthStatus } from "../types/MLTypes";
...
export class MLCacheService {
	...
	async cachePrediction(
		...
		try {
			...
			if (shouldCompress) {
				...
				await this.redisCache.set(key, prediction, ML_CACHE_TTL.PREDICTION, {
			...
			console.error("MLCacheService.cachePrediction error:", error, {
				symbol,
				horizon,
				modelId,
			});
	 ...
	 */
	async cacheFeatureVector(symbol: string, featureVector: MLFeatureVector): Promise<boolean> {
		const startTime = Date.now();
		...
		try {
			...
			if (serialized.length > this.compressionThreshold) {
				const compressed = Buffer.from(serialized).toString("base64");
				await this.redisCache.set(`${key}:compressed`, compressed, ML_CACHE_TTL.FEATURE, {
					source: "ml-feature-vector",
					version: "1.0",
				});
				await this.redisCache.set(
					`${key}:meta`,
				...
				await this.redisCache.set(key, featureVector, ML_CACHE_TTL.FEATURE, {
					source: "ml-feature-vector",
					version: "1.0",
				});
			}
	...
	async cacheModelMetadata(modelId: string, config: MLModelConfig): Promise<boolean> {
		...
		try {
			...
			await this.redisCache.set(key, config, ML_CACHE_TTL.MODEL_METADATA, {
	...
	async cacheEnhancementStatus(status: MLEnhancementStatus): Promise<boolean> {
		...
		try {
			...
			await this.redisCache.set(key, status, ML_CACHE_TTL.ENHANCEMENT_STATUS, {
				source: "ml-enhancement-orchestrator",
				version: "1.0",
			});
