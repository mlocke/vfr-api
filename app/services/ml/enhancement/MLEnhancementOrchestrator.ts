...
export class MLEnhancementOrchestrator {
	...
	public static getInstance(
		config?: Partial<MLEnhancementOrchestratorConfig>
	 ...
	 */
	private async executeParallelEnhancements(request: EnhancementRequest): Promise<{
		technical?: any;
		fundamental?: any;
		sentiment?: any;
	}> {
		...
		if (enableFundamental) {
			promises.push(
				this.executeSafely(async () => {
					const features = await this.fundamentalIntegrator.extractBatchFeatures([
						symbol,
					]);
					return features[0];
				}, "fundamental").then(result => {
		...
		try {
			const timeoutPromise = new Promise<never>((_, reject) => {
				timeoutId = setTimeout(
					() => reject(new Error(`${category} timeout`)),
					this.config.timeout
				);
			});
