		...
		try {
			...
			switch (modelType) {
				case MLModelType.LIGHTGBM:
					({ values: optimizedValues, params: normalizationParams } =
						this.optimizeLightGBM(featureValues));
					break;
				case MLModelType.XGBOOST:
					({ values: optimizedValues, params: normalizationParams } =
						this.optimizeXGBoost(featureValues));
					break;
				case MLModelType.LSTM:
					({ values: optimizedValues, params: normalizationParams } =
						this.optimizeLSTM(featureValues));
					break;
				case MLModelType.ENSEMBLE:
					({ values: optimizedValues, params: normalizationParams } =
						this.optimizeEnsemble(featureValues));
					break;
				default:
			...
			this.logger.error(
				`Feature optimization failed: ${error instanceof Error ? error.message : "Unknown error"}`
			);
			throw error;
		}
	}
	...
	public optimizeBatch(
		...
		if (!this.config.enableVectorization) {
			...
			return featureVectors.map(fv =>
				this.optimizeFeatureVector(fv, modelType, modelMetadata)
			);
		}
