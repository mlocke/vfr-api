...
export class MLEnhancementService {
  ...
  private constructor(config?: Partial<MLEnhancementServiceConfig>) {
    ...
    this.config = {
      enableML: true,
      mlWeight: 0.10, // 10% ML contribution
      classicWeight: 0.90, // 90% classic VFR
  ...
  private async enhanceSingleStock(
    ...
    try {
      ...
      const prediction: any = {
        ...
        horizon: horizon,
        prediction: {
          direction: mlResult.mlContribution > 0 ? 'BUY' as const :
                     mlResult.mlContribution < 0 ? 'SELL' as const : 'HOLD' as const,
  ...
  private calculateCombinedConfidence(classicConfidence: number, mlConfidence: number): number {
    ...
    return (classicConfidence * this.config.classicWeight) +
           (mlConfidence * this.config.mlWeight)
  }
