...
export class ServiceInitializer {
	...
	private async initializeRedisCache(): Promise<void> {
		try {
			...
			console.warn(
				"⚠️ Redis cache initialization failed, using fallback mode:",
	...
	private async initializeAuthService(): Promise<void> {
		try {
			...
			const normalizedError = ErrorHandler.normalizeError(error);
			console.warn(
				"⚠️ Auth service initialization failed, using fallback mode:",
				normalizedError.message
			);
		}
	}
	...
	private async initializeDataSourceConfigManager(): Promise<void> {
		try {
			...
			const normalizedError = ErrorHandler.normalizeError(error);
			console.warn(
				"⚠️ Data source config manager initialization failed:",
				normalizedError.message
			);
		}
	}
