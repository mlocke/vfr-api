...
export class FinancialDataCacheService {
	...
	async getStockPrice(
		symbol: string,
		preferredSource?: string
	): Promise<DataResponse<StockData>> {
		const startTime = Date.now();
		...
		try {
			...
			const hitRate =
				this.metrics.totalRequests > 0
					? (this.metrics.totalRequests - this.metrics.missRate) /
						this.metrics.totalRequests
					: 0;
	...
	private async convertAndCacheMarketData(
		...
		const historicalData: Omit<
			HistoricalMarketData,
			"id" | "cacheCreatedAt" | "lastUpdatedAt"
		> = {
	...
	private evaluateQuality(data: HistoricalMarketData[]): number {
		...
		const avgQuality =
			data.reduce((sum, item) => sum + (item.dataQualityScore || 0), 0) / data.length;
		return avgQuality;
	}
