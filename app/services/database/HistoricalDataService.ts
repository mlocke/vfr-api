...
export class HistoricalDataService {
	...
	async checkDataFreshness(
		...
		timeframe: string,
		source?: string
	): Promise<DataFreshnessInfo | null> {
		const query = `
      SELECT
	...
	async invalidateStaleCache(): Promise<number> {
		try {
			const result = await this.db.query<{ invalidate_stale_cache: number }>(
				"SELECT invalidate_stale_cache()"
			);
    ...
    `;
		...
		try {
			...
			if (existing.length > 0) {
				...
				const priceVariance =
					Math.abs(newData.close - existingData.close_price) / existingData.close_price;
				const volumeVariance =
					Math.abs(newData.volume - existingData.volume) /
					Math.max(existingData.volume, 1);

				// Log significant variances for review
				if (priceVariance > 0.02 || volumeVariance > 0.1) {
					// 2% price or 10% volume variance
