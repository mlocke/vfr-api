...
export class DatabaseMigrationService {
	...
	async runInitialMigration(): Promise<MigrationResult> {
		...
		try {
			...
			const schemaPath = path.join(
				process.cwd(),
				"database",
				"enhanced-historical-data-schema.sql"
			);
	...
	async backfillHistoricalData(options: BackfillOptions): Promise<MigrationResult> {
		...
		try {
			console.log(
				`Starting backfill for ${options.symbols.length} symbols from ${options.startDate.toISOString().split("T")[0]} to ${options.endDate.toISOString().split("T")[0]}`
			);
			...
			for (let i = 0; i < options.symbols.length; i += options.batchSize) {
				...
				console.log(
					`Processing batch ${Math.floor(i / options.batchSize) + 1} of ${Math.ceil(options.symbols.length / options.batchSize)} (${symbolBatch.length} symbols)`
				);
				...
				if (
					...
					i + options.batchSize < options.symbols.length
				) {
				...
				for (const source of options.preferredSources) {
					try {
						...
						if (marketData) {
							...
							const historicalData = this.convertToHistoricalData(
								marketData,
								timeframe,
								source
							);
						...
						console.warn(
							`Source ${source} failed for ${symbol}:`,
	...
	async runMaintenance(options: MaintenanceOptions): Promise<MigrationResult> {
		...
		try {
			...
			if (options.validateDataIntegrity) {
				...
				tasks.push(
					`Data validation: ${validationResult.validRecords}/${validationResult.totalRecords} valid records`
				);
			}
	...
	async validateDataIntegrity(): Promise<DataValidationResult> {
      ...
      `;
			...
			console.log(
				`Data validation completed: ${result.validRecords}/${result.totalRecords} valid records`
			);
	...
	async optimizePerformance(): Promise<MigrationResult> {
		...
		try {
			...
			await this.pool.query(
				"REFRESH MATERIALIZED VIEW CONCURRENTLY mv_latest_historical_data"
			);
			await this.pool.query("REFRESH MATERIALIZED VIEW CONCURRENTLY mv_data_completeness");

			// Reindex critical tables
			await this.pool.query(
				"REINDEX INDEX CONCURRENTLY idx_historical_symbol_date_timeframe"
			);
