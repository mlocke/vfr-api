...
export function getRecommendation(score: number, analystData?: AnalystData): RecommendationTier {
  ...
  if (analystData && analystData.totalAnalysts && analystData.totalAnalysts >= 5) {
    ...
    const { totalAnalysts = 0, sentimentScore, distribution } = analystData
    ...
    // Strong Buy Consensus (sentiment >= 4.5, buy% >= 75%)
    if (sentimentScore && sentimentScore >= 4.5 && buyPercentage >= 75) {
    ...
    // Buy Consensus (sentiment >= 4.0, buy% >= 70%) - CALIBRATED for FMP analyst data
    else if (sentimentScore && sentimentScore >= 4.0 && buyPercentage >= 70) {
...
/**
 * Convert 7-tier recommendation to simple 3-tier (for legacy compatibility)
 */
export function toSimpleRecommendation(tier: RecommendationTier): 'BUY' | 'HOLD' | 'SELL' {
  switch (tier) {
    case 'STRONG_BUY':
    case 'BUY':
    case 'MODERATE_BUY':
 ...
 */
export function getRecommendationColor(recommendation: RecommendationTier | 'BUY' | 'HOLD' | 'SELL'): {
  bg: string
  text: string
  border: string
} {
