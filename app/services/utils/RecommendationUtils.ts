...
export function getRecommendation(score: number, analystData?: AnalystData): RecommendationTier {
	...
	if (analystData && analystData.totalAnalysts && analystData.totalAnalysts >= 5) {
		...
		if (sentimentScore && sentimentScore >= 4.0 && buyPercentage >= 70) {
			...
			else if (recommendation === "MODERATE_BUY")
				recommendation = "STRONG_BUY"; // ✅ FIX: Allow MODERATE_BUY → STRONG_BUY upgrade
		...
		// Buy Consensus (sentiment >= 3.6, buy% >= 65%) - CALIBRATED for FMP analyst data (lowered from 3.8 to catch NVDA with 3.7 sentiment)
			...
			if (recommendation === "BUY" && buyPercentage >= 70) {
				console.log(
					`🚀 UPGRADE: ${recommendation} → STRONG_BUY (sentiment: ${sentimentScore}, buy%: ${buyPercentage.toFixed(1)})`
				);
				recommendation = "STRONG_BUY";
			} else if (recommendation === "MODERATE_BUY") recommendation = "BUY";
			else if (recommendation === "HOLD" && normalizedScore >= 0.5)
				recommendation = "BUY"; // ✅ UPGRADE: Strong analyst consensus with good score
			else if (recommendation === "HOLD" && normalizedScore >= 0.45)
				recommendation = "MODERATE_BUY";
		}
		// Moderate Buy Consensus (sentiment >= 3.5, buy% >= 60%)
		else if (sentimentScore && sentimentScore >= 3.5 && buyPercentage >= 60) {
			if (recommendation === "HOLD" && normalizedScore >= 0.4)
				recommendation = "MODERATE_BUY";
		}
    ...
    */
		// Bearish Consensus (sentiment < 2.8, hold% > buy% OR high sell%)
			...
			sentimentScore &&
			sentimentScore < 2.8 &&
			(holdPercentage > buyPercentage || sellPercentage >= 30)
		) {
...
export function getCombinedRecommendation(
	...
	const reverseMap: [number, RecommendationTier][] = [
		[90, "STRONG_BUY"],
		[75, "BUY"],
		[60, "MODERATE_BUY"],
		[40, "HOLD"],
