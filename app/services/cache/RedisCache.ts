...
export class RedisCache {
	...
	private parseRedisUrl(): { host: string; port: number; password?: string } {
		...
		if (redisUrl && redisUrl !== "") {
			try {
				...
				CacheLogger.warn(
					`⚠️ Invalid REDIS_URL format: ${redisUrl}, falling back to individual env vars`
				);
			}
		}
	...
	async setOptimized<T>(
		...
		try {
			...
			if (shouldCompress && this.redisAvailable) {
				...
				pipeline.set(
					`${key}:meta`,
					JSON.stringify({ compressed: true, size: compressedData.length })
				);
			} else {
	...
	async setBatch<T>(
		...
		try {
			...
			for (const entry of entries) {
				...
				if (compression && serializedData.length > 4096) {
					// 4KB threshold for compression
	...
	private initializeRedis() {
		CacheLogger.log(
			`🔗 Initializing Redis connection to ${this.config.host}:${this.config.port}`
		);
	...
	private setupDevelopmentFallback(): void {
		...
		const timeoutId = setTimeout(() => {
			if (!this.redisAvailable) {
				CacheLogger.warn(
					"⚠️ Redis not available in development mode - using in-memory fallback"
				);
				CacheLogger.warn(
					"💡 To use Redis caching, ensure Redis is running on localhost:6379"
				);
			}
	...
	async get<T = any>(key: string): Promise<T | null> {
		try {
			...
			CacheLogger.warn(
				`⚠️ Cache get error for key ${key} (Redis may not be available): ${normalizedError.message}`
			);
			this.stats.errors++;
			this.redisAvailable = false;
			return null;
	 ...
	 */
	async getCachedStockPrice(
		symbol: string,
		sources: string[] = ["polygon", "yahoo"]
	): Promise<any> {
	...
	async getCachedMLPrediction<T>(
		...
		const keys = modelVersions.map(
			version => `ml:prediction:${predictionType}:${symbol.toUpperCase()}:${version}`
		);
	...
	async warmCache(
		...
		CacheLogger.log(
			`🔥 Warming cache for ${symbols.length} symbols from ${sources.length} sources...`
		);
