...
export class OptionsCache {
	 ...
	 */
	async warmCache(
		dataProvider: (symbol: string) => Promise<OptionsAnalysisMetrics | null>
	): Promise<void> {
	...
	private async getAnalysisComponents(symbol: string): Promise<any> {
		const [putCallRatio, unusualActivity, volatilityAnalysis, flowSignals] =
			await Promise.allSettled([
		...
		return {
			...
			volatilityAnalysis:
				volatilityAnalysis.status === "fulfilled" ? volatilityAnalysis.value : null,
