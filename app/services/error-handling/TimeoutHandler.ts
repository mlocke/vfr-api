...
export class TimeoutHandler {
	...
	public async fetchWithTimeout(
		...
		const timeoutMs = Math.min(timeout || this.config.defaultTimeout, this.config.maxTimeout);
	 ...
	 */
	public async raceWithTimeout<T>(promises: Promise<T>[], timeoutMs?: number): Promise<T> {
	 ...
	 */
	public async allWithTimeout<T>(promises: Promise<T>[], timeoutMs?: number): Promise<T[]> {
		const timeout = timeoutMs || this.config.defaultTimeout;

		const wrappedPromises = promises.map(promise => this.withTimeout(promise, timeout));
