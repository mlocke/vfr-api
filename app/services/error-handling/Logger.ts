...
export class Logger {
  ...
  public logApiRequest(
    ...
    this.info(`API Request: ${method} ${this.sanitizeUrl(url)}`, {
      type: 'api_request',
      method,
  ...
  public logApiResponse(
    ...
    this.log(level, `API Response: ${method} ${this.sanitizeUrl(url)} - ${statusCode} (${responseTime}ms)`, {
      type: 'api_response',
      method,
      url: this.sanitizeUrl(url),
      statusCode,
  ...
  public logApiError(
    ...
    this.error(`API Error: ${method} ${this.sanitizeUrl(url)} - ${this.extractErrorMessage(error)}`, {
      type: 'api_error',
      method,
      url: this.sanitizeUrl(url),
  ...
  public logRateLimit(
    ...
    this.warn(`Rate limit reached: ${service} (${current}/${limit})`, {
      type: 'rate_limit',
      service,
      limit,
      current,
  ...
  public logCircuitBreaker(
    ...
    this.log(level, `Circuit breaker ${state}: ${service} (${failures} failures)`, {
  ...
  public logCacheEvent(
    ...
    this.debug(`Cache ${operation}: ${this.sanitizeKey(key)}`, {
      type: 'cache_event',
      operation,
  ...
  public logDataQuality(
    ...
    this.log(level, `Data Quality ${severity}: ${issue}${symbol ? ` (${symbol})` : ''}`, {
      type: 'data_quality',
      issue,
      severity,
      symbol,
  ...
  public logSecurity(
    ...
    this.log(level, `Security ${severity}: ${event}`, {
      type: 'security',
      event,
      severity,
      ...metadata
...
export const createServiceLogger = (serviceName: string, config?: Partial<LoggerConfig>): Logger => {
