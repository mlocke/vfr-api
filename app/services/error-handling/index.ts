...
export type { RetryConfig, RetryOptions, RetryResult, RetryAttempt } from "./RetryHandler";
...
export const createStandardizedErrorHandler = (serviceName: string) => {
	...
	return {
		...
		async handleApiCall<T>(
			...
			try {
				logger.debug(
					`Starting API call: ${options.context || "unknown"}`,
					{
		...
		validateAndExecute<T>(
			...
			return errorHandler.handleAsync(
				async () => {
					...
					const validationError = await errorHandler.validateOperation(
						serviceName,
						symbols
					);
					if (validationError) {
...
export const withErrorHandling = <T extends any[], R>(
	...
	return async (...args: T): Promise<R> => {
		try {
			return await handler.handleApiCall(() => fn(...args), {
				timeout: options.timeout,
				retries: options.retries,
			...
			handler.logger[logLevel](`Function ${fn.name || "anonymous"} failed`, {
				error,
				args: args.length,
			});
			throw error;
...
export const isRetryableError = (error: any): boolean => {
	...
	if (error instanceof Error) {
		...
		return ["timeout", "network", "rate limit", "econnrefused"].some(pattern =>
