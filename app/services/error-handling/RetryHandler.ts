...
export class RetryHandler {
		...
		for (let attempt = 1; attempt <= retryConfig.maxAttempts; attempt++) {
			...
			try {
				...
				const result = options?.timeout
					? await this.timeoutHandler.withTimeout(
							operation(),
							options.timeout,
							options.signal
						)
				...
				const shouldRetry =
					!isLastAttempt && this.shouldRetryError(error, attempt, retryConfig);
	...
	public async executeParallel<T>(
		...
		const promises = operations.map(operation => this.execute(operation, config, options));
	...
	public createFastRetryConfig(): RetryConfig {
		return {
			...
			backoffMultiplier: 1,
			jitter: false,
			retryableErrors: [ErrorType.TIMEOUT_ERROR, ErrorType.NETWORK_ERROR],
		};
	}
