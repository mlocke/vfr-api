...
export class ErrorHandler {
  ...
  private classifyError(error: any): ErrorType {
    ...
    // Data quality errors
    if (message.includes('data') && (message.includes('corrupt') || message.includes('incomplete'))) {
  ...
  private getErrorCode(error: any, type: ErrorType): ErrorCode {
    ...
    switch (type) {
      ...
      case ErrorType.AUTHENTICATION_ERROR:
        if (message.includes('missing') || message.includes('not configured')) return ErrorCode.MISSING_API_KEY
        if (message.includes('invalid') || message.includes('unauthorized')) return ErrorCode.INVALID_API_KEY
