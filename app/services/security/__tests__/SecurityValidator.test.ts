    ...
    describe('Invalid Symbols - Format Validation', () => {
      ...
      test('should reject symbols with special characters', () => {
        ...
        for (const symbol of specialChars) {
          const result = validator.validateSymbol(symbol)
          expect(result.isValid).toBe(false)
          expect(result.errors).toContain('Invalid symbol format - must be 1-5 uppercase letters/numbers')
        }
      })
    })
  })
  ...
  describe('Numeric Validation', () => {
    test('should validate clean numeric inputs', () => {
      ...
      for (const num of validNumbers) {
        const result = validator.validateNumeric(num, { allowNegative: true, allowZero: true })
        expect(result.isValid).toBe(true)
  ...
  describe('Error Message Sanitization', () => {
    ...
    test('should replace technical errors with user-friendly messages', () => {
      const technicalErrors = [
        { error: 'ECONNREFUSED localhost:3000', expected: 'Service temporarily unavailable' },
        { error: 'Request timeout ETIMEDOUT', expected: 'Request timeout - please try again' },
        { error: 'Rate limit exceeded 429', expected: 'Too many requests - please wait before trying again' },
