...
export class AuthService {
	...
	private async verifyConnections(): Promise<void> {
		...
		try {
			...
			if (process.env.NODE_ENV === "development") {
				console.warn(
					"⚠️ AuthService: Redis not available in development, using fallback mode"
				);
			} else {
	...
	private async handleFailedLogin(user: User, clientInfo: any): Promise<void> {
		...
		if (attempts >= this.config.security.maxFailedLogins) {
			const lockoutUntil =
				Date.now() + this.config.security.lockoutDurationMinutes * 60 * 1000;
	...
	private async createUserSession(user: User, clientInfo: any): Promise<UserSession> {
		...
		await redisCache.set(
			`session:${sessionId}`,
			session,
			this.config.session.timeoutMinutes * 60
		);
	...
	private async generateTokens(
		...
		await redisCache.set(
			`session_tokens:${session.sessionId}`,
