...
interface DataSourceToggleProps {
	...
	onToggle: (dataSourceId: string, enabled: boolean) => Promise<void>;
	disabled?: boolean;
}
...
export default function DataSourceToggle({
	...
	const getStatusColor = (status: string, isEnabled: boolean): string => {
		...
		switch (status) {
			...
			case "degraded":
				return "rgba(245, 158, 11, 0.9)"; // orange
			case "maintenance":
	...
	const handleToggle = useCallback(async () => {
		...
		try {
			...
			await onToggle(dataSourceId, newEnabled);
		} catch (error) {
			console.error(`Failed to toggle data source ${dataSourceId}:`, error);
		} finally {
			setIsToggling(false);
		}
	}, [dataSourceId, enabled, onToggle, disabled, isToggling]);
	...
	return (
		...
		>
			...
			<div
				...
				aria-label={`${localEnabled ? "Disable" : "Enable"} ${dataSourceName}`}
