			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
				{results.map(result => (
					...
					>
						...
						{/* Success Rate */}
						<div
							className={`rounded-xl p-4 border mb-4 ${getSuccessRateBgColor(result.successRate)}`}
						>
							<div className="text-center">
								<div
									className={`text-3xl font-bold ${getSuccessRateColor(result.successRate)}`}
								>
								...
								{result.dataSources.map(dataSourceResult => (
									...
									>
										<span className="text-gray-300 truncate">
											{dataSourceResult.dataSourceId}
										</span>
										<div className="flex items-center space-x-2">
											<span className="text-gray-400">
												{dataSourceResult.responseTime}ms
											</span>
											<span
												className={
				...
				<div className="grid grid-cols-1 md:grid-cols-4 gap-4">
					...
					<div className="bg-white/5 rounded-lg p-4 text-center">
						<div className="text-2xl font-bold text-green-400">
							{results.reduce(
								(sum, r) => sum + r.dataSources.filter(s => s.success).length,
								0
							)}
						</div>
