...
export default function OptionsPerformanceChart({ testResults }: OptionsPerformanceChartProps) {
	const chartData = useMemo(() => {
		...
		const testTypePerformance = testResults.reduce(
			(acc, result) => {
		...
		// Provider comparison
		const providerComparison = testResults.reduce(
			(acc, result) => {
	...
	return (
		<div>
			...
			>
					...
					<div style={{ position: "relative", height: "200px", overflow: "hidden" }}>
						...
						>
								...
								<polyline
									points={chartData.performanceTrend
										.map((point, index) => {
											const x =
												10 +
												(index / (chartData.performanceTrend.length - 1)) *
													80;
											const y =
												100 -
												Math.min(100, (point.duration / point.target) * 50);
											return `${x},${y}`;
										})
										.join(" ")}
									fill="none"
							...
							{chartData.performanceTrend.map((point, index) => {
								const x =
									10 +
									(index / Math.max(1, chartData.performanceTrend.length - 1)) *
										80;
				...
				>
					...
					<div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
						{Object.entries(chartData.providerComparison).map(
			...
			>
				...
				>
					{Object.entries(chartData.testTypePerformance).map(
						([testType, data]: [string, any]) => {
							const avgDuration = data.totalDuration / data.count;
							const successRate = (data.successes / data.count) * 100;
