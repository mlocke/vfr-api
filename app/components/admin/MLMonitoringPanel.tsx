...
export default function MLMonitoringPanel() {
	...
	const [testType, setTestType] = useState<"health" | "integration" | "performance" | "stress">(
		"health"
	);
		...
		>
			...
			>
				...
				>
					...
					>
						{mlServices.filter(s => s.status === "healthy").length}/{mlServices.length}{" "}
						Healthy
					</div>
					<button
					...
					>
						{performanceMetrics.map(metric => (
							...
							>
								...
								>
									{mlServices.find(s => s.id === metric.serviceId)?.name ||
										metric.serviceId}
								</h4>
								...
								>
									<div>
										Avg Latency: {metric.metrics.averageLatency.toFixed(0)}ms
									</div>
									<div>P95 Latency: {metric.metrics.p95Latency.toFixed(0)}ms</div>
									<div>
										Error Rate: {(metric.metrics.errorRate * 100).toFixed(1)}%
									</div>
									<div>
										Cache Hit: {(metric.metrics.cacheHitRate * 100).toFixed(1)}%
									</div>
									<div>
										Requests/min: {metric.metrics.requestsPerMinute.toFixed(0)}
									</div>
						...
						>
							...
							<button
								...
								style={{
									...
									padding: "0.75rem",
									background:
										selectedServices.length > 0 && !isRunningTests
						...
						>
							...
							{testResults.length === 0 ? (
								...
								>
									{isRunningTests
										? "Running tests..."
										: "No test results yet. Select services and run tests."}
								</div>
							) : (
								<div
									style={{
														...
														{result.metadata && (
															...
															>
																...
																{result.metadata.dataQuality &&
																	` | Quality: ${(result.metadata.dataQuality * 100).toFixed(1)}%`}
