...
export function DataSourceCard({
	...
	const getTypeColor = (type: string) => {
		switch (type) {
			case "commercial":
				return "bg-purple-600/50 text-purple-200 border-purple-400/30";
			case "government":
				return "bg-blue-600/50 text-blue-200 border-blue-400/30";
			case "free":
				return "bg-green-600/50 text-green-200 border-green-400/30";
			default:
				return "bg-gray-600/50 text-gray-200 border-gray-400/30";
		}
	};
					...
					<div className="flex items-start space-x-4">
						...
						<div className="flex-1">
							...
							<div className="flex items-center space-x-2 mt-3">
								<span
									className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(dataSource.status)}`}
								>
									● {dataSource.status.toUpperCase()}
								</span>
								<span
									className={`px-3 py-1 rounded-full text-xs font-medium border ${getTypeColor(dataSource.type)}`}
								>
				...
				<div className="px-6 py-4 space-y-4">
					...
					<div className="flex items-center justify-between py-2 border-b border-white/10">
						...
						<div className="flex items-center space-x-2">
							{dataSource.requiresAuth ? (
								<>
									<span
										className={`text-sm ${dataSource.hasApiKey ? "text-green-400" : "text-red-400"}`}
									>
						...
						<div className="grid grid-cols-2 gap-4">
							{dataSource.responseTime && (
								<div className="bg-white/5 rounded-lg p-3">
									...
									<div className="text-lg font-bold text-white">
										{dataSource.responseTime.toFixed(0)}ms
									</div>
								</div>
							)}
							{dataSource.errorRate !== undefined && (
								<div className="bg-white/5 rounded-lg p-3">
									...
									<div
										className={`text-lg font-bold ${dataSource.errorRate > 0.1 ? "text-red-400" : "text-green-400"}`}
									>
					...
					{dataSource.lastHealthCheck && (
						<div className="text-xs text-gray-400 pt-2 border-t border-white/10">
							Last health check:{" "}
							{new Date(dataSource.lastHealthCheck).toLocaleString()}
						</div>
					)}
				</div>
			)}
