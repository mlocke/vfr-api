...
export function AdminFilters({ filters, onFiltersChange, serverCounts }: AdminFiltersProps) {
	...
	const categoryOptions = [
		...
		{
			value: "stock_data",
			label: `Stock Data (${serverCounts.byCategory.stock_data || 0})`,
			icon: "📈",
		},
		{
			value: "economic_data",
			label: `Economic Data (${serverCounts.byCategory.economic_data || 0})`,
			icon: "🏛️",
		},
		{
			value: "web_intelligence",
			label: `Web Intelligence (${serverCounts.byCategory.web_intelligence || 0})`,
			icon: "🌐",
		},
		{
			value: "filings",
			label: `SEC Filings (${serverCounts.byCategory.filings || 0})`,
			icon: "📋",
		},
	];
		...
		<div className="bg-white/10 backdrop-blur-lg rounded-2xl border border-white/20 p-6">
			...
			{(filters.type !== "all" || filters.category !== "all" || filters.search) && (
				<div className="mt-4 pt-4 border-t border-white/10">
					<button
						onClick={() =>
							onFiltersChange({ type: "all", category: "all", search: "" })
						}
