			...
			<div className="bg-white rounded-lg shadow-lg p-6">
				...
				<div className="flex items-center justify-center py-8">
					...
					<span className="ml-2 text-gray-600">
						Loading data source configurations...
					</span>
				</div>
			</div>
		);
		...
		<div className="bg-white rounded-lg shadow-lg p-6">
			<div className="flex items-center justify-between mb-6">
				...
				<div className="flex space-x-2">
					...
					<span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
						{serviceStatus.capabilities.availableProviders}/
						{serviceStatus.capabilities.totalProviders} Providers Available
					</span>
				</div>
			</div>
			...
			<div className="mb-6">
				...
				<div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
					{serviceStatus.dataTypePreferences &&
						Object.entries(serviceStatus.dataTypePreferences).map(
							([dataType, preference]) => {
								// Get available providers for this data type
								const availableProviders = serviceStatus.providerConfigurations
									? Object.entries(serviceStatus.providerConfigurations).filter(
											([provider, config]) =>
												config.enabled &&
												config.supportedDataTypes.includes(dataType as any)
										)
									: [];
									...
									>
										<div className="flex items-center justify-between mb-3">
											<h4 className="font-medium text-gray-900">
												{DATA_TYPE_NAMES[dataType] || dataType}
											</h4>
											<span
												className={`px-2 py-1 rounded text-xs font-medium ${
													serviceStatus.providerConfigurations?.[
														preference.primary
													]?.costTier
														? getCostTierColor(
																serviceStatus
				...
				<div className="overflow-x-auto">
					<table className="min-w-full divide-y divide-gray-200">
						...
						<tbody className="bg-white divide-y divide-gray-200">
							{serviceStatus.providerConfigurations &&
								Object.entries(serviceStatus.providerConfigurations).map(
									([provider, config]) => (
										<tr
											key={provider}
										...
										>
											<td className="px-6 py-4 whitespace-nowrap">
												<div className="flex items-center">
													<div
														className={`h-2 w-2 rounded-full mr-2 ${config.enabled ? "bg-green-400" : "bg-gray-400"}`}
													></div>
											...
											<td className="px-6 py-4 whitespace-nowrap">
												<span
													className={`px-2 py-1 rounded text-xs font-medium ${getCostTierColor(config.costTier)}`}
												>
													{config.costTier}
												</span>
											</td>
											<td className="px-6 py-4 whitespace-nowrap">
												<span
													className={`text-sm font-medium ${getReliabilityColor(config.reliability)}`}
												>
											...
											<td className="px-6 py-4 whitespace-nowrap">
												<span
													className={`text-sm font-medium ${getReliabilityColor(config.dataQuality)}`}
												>
			...
			{serviceStatus.switchingRecommendations &&
				(serviceStatus.switchingRecommendations.costOptimization?.length > 0 ||
					...
					<div>
						...
						<div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
							{/* Cost Optimization */}
							{serviceStatus.switchingRecommendations.costOptimization?.length >
								0 && (
								<div className="bg-green-50 border border-green-200 rounded-lg p-4">
									...
									<ul className="space-y-1">
										{serviceStatus.switchingRecommendations.costOptimization
											.slice(0, 3)
											.map((rec, idx) => (
							...
							{/* Quality Improvement */}
							{serviceStatus.switchingRecommendations.qualityImprovement?.length >
								0 && (
								<div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
									...
									<ul className="space-y-1">
										{serviceStatus.switchingRecommendations.qualityImprovement
											.slice(0, 3)
											.map((rec, idx) => (
							...
							{serviceStatus.switchingRecommendations.reliabilityImprovement?.length >
								0 && (
								<div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
									...
									<ul className="space-y-1">
										{serviceStatus.switchingRecommendations.reliabilityImprovement
											.slice(0, 3)
											.map((rec, idx) => (
