  ...
  const getMockStockData = (symbol: string): StockData => {
    const mockData: { [key: string]: StockData } = {
      'AAPL': {
        ...
        change: 2.40,
        changePercent: 1.02,
        performance: { '1D': 1.02, '5D': 3.10, '1M': 6.82, '6M': -20.28, 'YTD': -7.35, '1Y': 36.67, '5Y': 64.14, 'ALL': 195.47 },
        news: { headline: 'Apple Invests in AI Infrastructure', summary: 'Apple invested $69 billion in the last three months for data centers and AI infrastructure, reflecting a strong capital expenditure trend despite falling construction job gains.' }
      },
      'AMZN': {
        currentPrice: 238.24,
        change: 2.40,
        changePercent: 1.02,
        performance: { '1D': 1.02, '5D': 3.10, '1M': 6.82, '6M': -20.28, 'YTD': -7.35, '1Y': 36.67, '5Y': 64.14, 'ALL': 195.47 },
        news: { headline: 'Amazon Expands Cloud Services', summary: 'Amazon invested $69 billion in the last three months for data centers and AI infrastructure, reflecting a strong capital expenditure trend despite falling construction job gains.' }
      },
      'MSFT': {
        currentPrice: 415.85,
        change: 5.20,
        changePercent: 1.27,
        performance: { '1D': 1.27, '5D': 2.45, '1M': 8.15, '6M': -15.42, 'YTD': 12.35, '1Y': 25.87, '5Y': 89.23, 'ALL': 285.67 },
        news: { headline: 'Microsoft AI Integration Success', summary: 'Microsoft continues to lead in AI integration across its product suite, showing strong growth in cloud computing and enterprise solutions.' }
      },
      'TSLA': {
        currentPrice: 271.99,
        change: -8.45,
        changePercent: -3.01,
        performance: { '1D': -3.01, '5D': -1.23, '1M': 4.67, '6M': -18.92, 'YTD': 15.43, '1Y': -12.34, '5Y': 156.78, 'ALL': 1847.92 },
        news: { headline: 'Tesla Production Updates', summary: 'Tesla announces production milestone achievements and expansion plans for electric vehicle manufacturing capabilities.' }
      }
    }
        ...
        {timePeriods.map((period) => (
          ...
          >
            ...
            <div 
              ...
              style={{ color: getPerformanceColor(stockData.performance[period.key as keyof typeof stockData.performance]) }}
            >
              {formatPerformance(stockData.performance[period.key as keyof typeof stockData.performance])}
            </div>
          </button>
        ))}
      </div>
