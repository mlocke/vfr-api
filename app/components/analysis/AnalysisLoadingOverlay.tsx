 ...
 */
const CyberpunkProgressBar: React.FC<{ progress: number; stage: string }> = ({
	progress,
	stage,
}) => {
	return (
 ...
 */
const DataSourceIndicator: React.FC<{ source: { name: string; status: string } }> = ({
	source,
}) => {
	const getStatusColor = (status: string) => {
		switch (status) {
			case "connected":
				return "rgba(0, 200, 83, 0.8)";
			case "connecting":
	...
	return (
		...
		>
			<span
				style={{
					animation: source.status === "connecting" ? "spin 2s linear infinite" : "none",
				}}
			>
				{getStatusIcon(source.status)}
			</span>
			<span style={{ color: "rgba(255, 255, 255, 0.8)" }}>{source.name}</span>
			<div
				style={{
					width: "6px",
					height: "6px",
...
export const AnalysisLoadingOverlay: React.FC<AnalysisLoadingOverlayProps> = ({
	...
	useEffect(() => {
		...
		const progressStage = () => {
			...
			let currentProgress =
				currentIndex > 0 ? ANALYSIS_STAGES[stages[currentIndex - 1]].progress : 0;
					...
					>
						...
						>
							{currentStage === "initializing" &&
								"Setting up connections to premium financial data sources and validating access tokens..."}
							{currentStage === "fetching" &&
								"Gathering real-time data from SEC filings, market APIs, sentiment sources, and economic indicators..."}
							{currentStage === "analyzing" &&
								"Running advanced algorithms to analyze technical patterns, fundamental metrics, and market sentiment..."}
							{currentStage === "finalizing" &&
								"Generating personalized insights and investment recommendations with confidence scoring..."}
						</div>
					</div>
