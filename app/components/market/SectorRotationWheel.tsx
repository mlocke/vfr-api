...
export default function SectorRotationWheel({ className = "" }: SectorRotationWheelProps) {
	...
	const fetchSectorData = async () => {
		...
		try {
			...
			if (sectors.length === 0) {
				...
				console.log(
					"✅ SectorRotationWheel: Successfully loaded",
					sectors.length,
					"sectors"
				);
				setError(null);
	...
	const drawRotationWheel = () => {
		console.log(
			"🎨 SectorRotationWheel: Starting to draw wheel with",
			sectorData.length,
			"sectors"
		);
		...
		console.log(
			"🎨 SectorRotationWheel: Canvas dimensions:",
			canvas.width,
			"x",
			canvas.height,
			"radius:",
				...
				<canvas
					...
					onClick={e => {
						...
						const sectorIndex = Math.floor(
							normalizedAngle / ((2 * Math.PI) / sectorData.length)
						);
						...
						<div className="details-grid">
							...
							<div className="detail-item">
								...
								<span className="detail-value">
									{formatVolume(selectedSector.volume)}
								</span>
							</div>
							...
							<div className="detail-item">
								...
								<span className="detail-value">
									{formatMarketCap(selectedSector.marketCap)}
								</span>
							</div>
							...
							<div className="detail-item">
								...
								<span className="detail-value">
									{selectedSector.momentum.replace("-", " ").toUpperCase()}
								</span>
							</div>
						</div>
					</div>
