...
export default function MarketExplanationModal({ isOpen, onClose }: MarketExplanationModalProps) {
	...
	const createPlainLanguageExplanation = async (
		...
		const getOverallCondition = (): string => {
			if (sentimentValue <= 20)
				return "Markets are in extreme fear territory - investors are very worried about potential losses.";
			if (sentimentValue <= 40)
				return "Markets show signs of fear - there's noticeable concern among investors.";
			if (sentimentValue <= 60)
				return "Markets are in neutral territory - sentiment is balanced between optimism and caution.";
			if (sentimentValue <= 80)
				return "Markets show greed - investors are optimistic and taking on more risk.";
		...
		const getVixExplanation = (): string => {
			if (vixValue >= 30)
				return `The VIX at ${vixValue.toFixed(1)} indicates extreme market stress and high expected volatility.`;
			if (vixValue >= 25)
				return `The VIX at ${vixValue.toFixed(1)} shows elevated uncertainty and above-normal volatility expectations.`;
			if (vixValue >= 20)
				return `The VIX at ${vixValue.toFixed(1)} indicates moderate concern with some expected volatility.`;
			if (vixValue >= 15)
				return `The VIX at ${vixValue.toFixed(1)} shows normal market conditions with low fear levels.`;
			if (vixValue >= 12)
				return `The VIX at ${vixValue.toFixed(1)} indicates very calm markets with minimal expected volatility.`;
		...
		const getEconomicFactors = (): string => {
			...
			if (spread !== undefined) {
				...
				yieldExplanation =
					"Bond yield data suggests " +
					yieldData.level.replace("-", " ") +
					" economic sentiment.";
			}

			return (
				yieldExplanation +
				` Social sentiment from trending discussions is ${data.socialSentiment.sentiment}.`
			);
		};
		...
		const summary =
			`Markets are ${sentimentLevel.replace("-", " ")} today with a sentiment score of ${sentimentValue}/100. ` +
		...
		return {
			...
			keyMetrics: {
				...
				sentiment: {
					...
					meaning:
						sentimentLevel.replace("-", " ").charAt(0).toUpperCase() +
						sentimentLevel.replace("-", " ").slice(1),
				},
				topSectors,
				yieldSpread: {
					value:
						data.economicIndicators.yieldCurve.spread10Y2Y !== undefined
							? `${data.economicIndicators.yieldCurve.spread10Y2Y.toFixed(2)}%`
							: data.economicIndicators.yieldCurve.level,
					meaning:
						data.economicIndicators.yieldCurve.spread10Y2Y !== undefined &&
						data.economicIndicators.yieldCurve.spread10Y2Y < 0
	...
	return (
		<div className="market-explanation-overlay" onClick={onClose}>
			<div className="market-explanation-modal" onClick={e => e.stopPropagation()}>
				...
				<div className="modal-content">
					...
					{error && (
						<div className="error-state">
							...
							<p>{error}</p>
							<button className="retry-button" onClick={generateExplanation}>
								Try Again
							</button>
						</div>
					)}
								...
								<div className="metrics-grid">
									<div className="metric-card">
										...
										<div className="metric-value">
											{explanation.keyMetrics.sentiment.value}/100
										</div>
										<div className="metric-meaning">
											{explanation.keyMetrics.sentiment.meaning}
										</div>
									</div>
									<div className="metric-card">
										<div className="metric-label">VIX (Fear Gauge)</div>
										<div className="metric-value">
											{explanation.keyMetrics.vix.value.toFixed(1)}
										</div>
										<div className="metric-meaning">
											{explanation.keyMetrics.vix.meaning}
										</div>
									</div>
									<div className="metric-card">
										<div className="metric-label">Yield Spread</div>
										<div className="metric-value">
											{explanation.keyMetrics.yieldSpread.value}
										</div>
										<div className="metric-meaning">
											{explanation.keyMetrics.yieldSpread.meaning}
										</div>
									</div>
								</div>
							</div>
