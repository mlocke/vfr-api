...
export default function PreMarketAfterHoursDisplay({
  ...
  const hasPreMarketData = extendedHoursData?.preMarketPrice && extendedHoursData?.preMarketChange !== undefined
  const hasAfterHoursData = extendedHoursData?.afterHoursPrice && extendedHoursData?.afterHoursChange !== undefined
        ...
        `}>
          ...
          <div className="mt-2 flex items-center justify-between">
            ...
            <div className="text-right">
              ...
              <div className={`text-sm ${getChangeColor(extendedHoursData.preMarketChange!)}`}>
                {formatChangePercent(
                  ((extendedHoursData.preMarketPrice! - regularPrice) / regularPrice) * 100
                )}
              </div>
            </div>
          </div>
          ...
          <div className="mt-2 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              `}>
                ...
                <span className="text-sm">
                  ({formatChangePercent(extendedHoursData.afterHoursChangePercent!)})
                </span>
              </div>
            </div>
            ...
            <div className="text-right">
              <div className="text-xs text-gray-500">vs Regular</div>
              <div className={`text-sm ${getChangeColor(extendedHoursData.afterHoursChange!)}`}>
                {formatChangePercent(
                  ((extendedHoursData.afterHoursPrice! - regularPrice) / regularPrice) * 100
                )}
              </div>
            </div>
          </div>
      ...
      {(hasPreMarketData || hasAfterHoursData) && (
        <div className="mt-2 p-2 bg-yellow-900/20 border border-yellow-600/30 rounded text-xs text-yellow-400">
          <div className="flex items-center gap-1">
            <span>⚠️</span>
            <span>Extended hours trading may have higher volatility and lower liquidity</span>
          </div>
        </div>
      )}
    </div>
