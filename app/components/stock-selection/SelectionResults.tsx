			...
			<div className="p-4">
				...
				<div className="flex items-center justify-between mb-3">
					<div className="flex items-center">
						...
						<span
							className={`ml-3 px-2 py-1 rounded text-xs font-medium ${getActionColor(stock.action)}`}
						>
							{stock.action}
						</span>
					</div>
					...
					<div>
						<div
							className={`text-lg font-medium ${
								(stock.context.priceChange24h || 0) >= 0
									? "text-green-400"
									: "text-red-400"
							}`}
						>
				...
				<div className="flex items-center justify-between text-sm">
					...
					<div>
						...
						<span className="text-white ml-1">
							{formatMarketCap(stock.context.marketCap)}
						</span>
					</div>
				</div>
		...
		<div className="bg-gray-800 rounded-lg border border-gray-700 p-6 mb-6">
			...
			<div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
				<div className="text-center">
					<div className="text-2xl font-bold text-white">
						{sectorAnalysis.overview.totalStocks}
					</div>
				...
				<div className="text-center">
					<div className="text-2xl font-bold text-green-400">
						{sectorAnalysis.overview.topPerformers}
					</div>
					<div className="text-sm text-gray-400">Top Performers</div>
				</div>
				<div className="text-center">
					<div className="text-2xl font-bold text-red-400">
						{sectorAnalysis.overview.underperformers}
					</div>
			...
			<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
				{[
					{
						label: "Momentum",
						value: sectorAnalysis.sectorMetrics.momentum,
						color: "blue",
					},
					{
						label: "Valuation",
						value: sectorAnalysis.sectorMetrics.valuation,
						color: "green",
					},
					{
						label: "Growth",
						value: sectorAnalysis.sectorMetrics.growth,
						color: "purple",
					},
					{
						label: "Stability",
						value: sectorAnalysis.sectorMetrics.stability,
						color: "orange",
					},
		...
		<div className="bg-gray-800 rounded-lg border border-gray-700 p-6 mb-6">
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
				...
				<div>
					...
					<div className="space-y-2">
						{Object.entries(portfolioMetrics.sectorBreakdown).map(
							([sector, weight]) => (
...
export default function SelectionResults({
			...
			<div className={`selection-results ${className}`}>
				<div className="text-center py-12">
					...
					<p className="text-gray-500">
						Select your analysis mode and enter symbols to get started
					</p>
				</div>
			</div>
		);
	...
	return (
		<div className={`selection-results ${className}`}>
			...
			<div className="mb-4">
				<ExtendedHoursIndicator marketStatus={topSelections[0]?.context?.marketStatus} />
			</div>
			...
			<div className="flex items-center justify-between mb-6">
				<div>
					...
					<p className="text-sm text-gray-400">
						Completed in {executionTime}ms • Cache hit rate:{" "}
						{(result.metadata.cacheHitRate * 100).toFixed(0)}% •
						{lastUpdate > 0 &&
					...
					>
						{topSelections.map((stock, index) => (
							<StockCard
								...
								onToggleDetails={() =>
									setExpandedStock(expandedStock === index ? null : index)
								}
							/>
						))}
					</div>
			...
			<div className="mt-8 pt-6 border-t border-gray-700">
				<div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
					...
					<div>
						...
						<span className="text-white ml-2">
							{result.metadata.dataSourcesUsed.length}
						</span>
					</div>
					<div>
