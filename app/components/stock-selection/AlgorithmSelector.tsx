...
export default function AlgorithmSelector({
	 ...
	 */
	const handleRiskToleranceChange = useCallback(
	...
	const handleCustomWeightsChange = useCallback(
		(weights: Partial<NonNullable<SelectionOptions["customWeights"]>>) => {
			onChange({
				customWeights: {
	 ...
	 */
	const handleDataPreferencesChange = useCallback(
		(preferences: Partial<NonNullable<SelectionOptions["dataPreferences"]>>) => {
			onChange({
				dataPreferences: {
	...
	return (
		<div className={`algorithm-selector ${className}`}>
			...
			<div className="mb-6">
              ...
              `}
                ...
                `}
							...
							<div className="mt-2 text-xs text-gray-500">{algorithm.timeframe}</div>
						</button>
					))}
				</div>
          ...
          "
				>
					<span
						className={`mr-2 transition-transform duration-200 ${showAdvanced ? "rotate-90" : ""}`}
					>
						▶
					</span>
										...
										value={
											options.customWeights?.[
												key as keyof NonNullable<
													SelectionOptions["customWeights"]
												>
											] || 0.5
										}
										onChange={e =>
											handleCustomWeightsChange({
					...
					<div>
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
							<div>
								...
								<div className="text-xs text-gray-500 mt-1">
									{(
										(options.dataPreferences?.minQualityScore || 0.7) * 100
									).toFixed(0)}
									%
								</div>
							</div>
							...
							<div>
								...
								<input
									...
									max="1440"
									value={Math.floor(
										(options.dataPreferences?.maxLatency || 300000) / 60000
									)}
									onChange={e =>
										handleDataPreferencesChange({
