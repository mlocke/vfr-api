...
export default function StockSelectionPanel({
	...
	const [state, setState] = useState<PanelState>({
		inputMode:
			defaultMode === SelectionMode.SECTOR_ANALYSIS
				? "sector"
				: defaultMode === SelectionMode.MULTIPLE_STOCKS
					? "multiple"
					: "single",
		singleStock: "",
		multipleStocks: [],
	 ...
	 */
	const addStockToMultiple = useCallback(
		(symbol: string) => {
		...
		try {
			...
			switch (state.inputMode) {
				case "single":
					...
					result = await stockSelection.analyzeStock(
						state.singleStock,
				...
				case "multiple":
					...
					result = await stockSelection.analyzeStocks(
						state.multipleStocks,
						state.algorithmOptions
					);
					break;
				...
				case "sector":
					...
					result = await stockSelection.analyzeSector(
	...
	return (
		<div className={`stock-selection-panel ${className}`}>
          ...
          "
				>
					<span
						className={`mr-2 transition-transform duration-200 ${state.showAdvanced ? "rotate-90" : ""}`}
					>
						▶
					</span>
					Advanced Options
