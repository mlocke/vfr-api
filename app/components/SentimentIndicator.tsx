...
export default function SentimentIndicator({ sector, realTimeEnabled = false }: SentimentIndicatorProps) {
    ...
    return (
      <div className="bg-red-500/10 backdrop-blur border border-red-500/30 rounded-xl p-4">
        <div className="text-red-400 text-sm">
          ⚠️ Failed to load sentiment: {error}
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="bg-gray-800/60 backdrop-blur border border-gray-700/50 rounded-xl p-4 transition-all duration-300">
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center space-x-2">
          <span className={`text-lg ${getSentimentColor(newsData.overallSentiment.label)}`}>
            {getSentimentEmoji(newsData.overallSentiment.label)}
          </span>
          <span className={`text-sm font-medium ${getSentimentColor(newsData.overallSentiment.label)}`}>
        ...
        <div className="flex items-center space-x-3 text-xs">
          ...
          <div className="flex items-center space-x-1">
            ...
            <span className="text-gray-400 capitalize">{newsData.overallSentiment.trending}</span>
          </div>
          
          {/* Market Impact */}
          <div className={`px-2 py-1 rounded-full ${getImpactColor(newsData.marketImpact)} bg-current/10 text-xs font-medium`}>
      ...
      <div className="text-xs text-gray-500 mb-3">
        {newsData.articles.length} articles • {newsData.source} • {formatTimeAgo(newsData.timestamp)}
      </div>
      ...
      {expanded && (
        <div className="border-t border-gray-700/50 pt-3 space-y-3">
          ...
          <div>
            ...
            <div className="space-y-2 max-h-40 overflow-y-auto">
              {newsData.articles.slice(0, 5).map((article, index) => (
                <div key={index} className="flex items-start space-x-2 p-2 rounded-lg bg-gray-700/30">
                  <span className={`text-xs ${getSentimentColor(article.sentiment.label)}`}>
                      ...
                      {article.keywords.length > 0 && (
                        <div className="flex space-x-1">
                          {article.keywords.slice(0, 2).map((keyword, idx) => (
                            <span key={idx} className="px-1 py-0.5 bg-cyan-500/20 text-cyan-400 rounded text-xs">
                              {keyword}
                            </span>
                          ))}
                        </div>
          ...
          <div>
            <h4 className="text-xs font-medium text-gray-300 mb-2">Sentiment Breakdown</h4>
            <div className="flex space-x-2">
              {['positive', 'neutral', 'negative'].map((sentiment) => {
                const count = newsData.articles.filter(a => a.sentiment.label === sentiment).length
                const percentage = (count / newsData.articles.length) * 100
                return (
                  <div key={sentiment} className="flex-1 text-center">
                    <div className={`text-xs ${getSentimentColor(sentiment)} font-medium`}>
