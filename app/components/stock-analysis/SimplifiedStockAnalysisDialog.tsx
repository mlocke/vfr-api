		...
		try {
			...
			return {
				...
				factorScores: {
					...
					technical: safeGet(
						firstResult,
						"score.factorScores.technical_overall_score",
						0.5
					),
					...
					vwap_deviation: safeGet(
						firstResult,
						"score.factorScores.vwap_deviation_score",
						0.5
					),
					vwap_signals: safeGet(
						firstResult,
						"score.factorScores.vwap_trading_signals",
						0.5
					),
				},
				...
				marketData: {
					...
					sector:
						safeGet(firstResult, "score.marketData.sector", "Unknown") ||
						safeGet(firstResult, "context.sector", "Unknown"),
				...
				dataQuality: {
					...
					completeness: safeGet(
						firstResult,
						"dataQuality.overall.metrics.completeness",
						0.8
					),
					...
					>
						<div>
							...
							>
								...
								<span>•</span>
								<span>
									{formatCurrency(stockData.marketData.price)} Current Price
								</span>
								<span>•</span>
								...
								{stockData.context.priceChange24h !== undefined && (
									<>
										...
										<span
											style={{
												color:
													stockData.context.priceChange24h > 0
														? "#00c853"
														: "#ef4444",
											}}
										>
					...
					<div style={{ padding: "16px 24px" }}>
						...
						>
							...
							<div>
								...
								>
									{formatScore(stockData.confidence)}% Confidence • Overall Score:{" "}
									{formatScore(stockData.overallScore)}/100
								</div>
							</div>
						</div>
					</div>
					...
					<div style={{ padding: "16px 24px 24px 24px" }}>
						...
						<InfoCard title="Market Summary" icon="💰">
							...
							>
								<div>
									<strong>Price:</strong>{" "}
									{formatCurrency(stockData.marketData.price)}
								</div>
								<div>
									<strong>Volume:</strong>{" "}
									{formatNumber(stockData.marketData.volume)}
								</div>
								<div>
									<strong>Sector:</strong> {stockData.marketData.sector}
								</div>
								<div>
									<strong>Exchange:</strong> {stockData.marketData.exchange}
								</div>
								<div>
									<strong>Market Cap:</strong>{" "}
									{formatCurrency(stockData.marketData.marketCap)}
								</div>
								<div>
									<strong>Beta:</strong> {stockData.context.beta}
								</div>
							</div>
						</InfoCard>
						...
						{stockData.reasoning.opportunities.length > 0 && (
							<InfoCard title="Opportunities" icon="🚀">
								...
								>
									{stockData.reasoning.opportunities
										.slice(0, 3)
							...
							>
								<div>
									<div>
										...
										{formatScore(stockData.dataQuality.overall)}/100
									</div>
								</div>
								<div>
									<div>
										<strong>Freshness:</strong>{" "}
										{formatScore(stockData.dataQuality.freshness)}/100
									</div>
								</div>
								<div>
									<div>
										<strong>Completeness:</strong>{" "}
										{formatScore(stockData.dataQuality.completeness)}/100
									</div>
								</div>
								<div>
									<div>
										<strong>Accuracy:</strong>{" "}
										{formatScore(stockData.dataQuality.accuracy)}/100
									</div>
								</div>
							</div>
						</InfoCard>
