 ...
 */
export const DialogHeader: React.FC<DialogHeaderProps> = ({ stockData, onClose, onRefresh }) => {
					...
					>
						{stockData.context.preMarketPrice && (
							<div style={{ marginBottom: "0.25rem" }}>
								...
								{stockData.context.preMarketPrice.toFixed(2)}
								{stockData.context.preMarketChangePercent && (
									<span
										style={{
											marginLeft: "0.5rem",
											color:
												stockData.context.preMarketChangePercent >= 0
													? "rgba(0, 200, 83, 0.9)"
													: "rgba(239, 68, 68, 0.9)",
										}}
									>
										({stockData.context.preMarketChangePercent >= 0 ? "+" : ""}
										{stockData.context.preMarketChangePercent.toFixed(2)}%)
									</span>
								)}
							</div>
						)}
						{stockData.context.afterHoursPrice && (
							<div>
								...
								{stockData.context.afterHoursPrice.toFixed(2)}
								{stockData.context.afterHoursChangePercent && (
									<span
										style={{
											marginLeft: "0.5rem",
											color:
												stockData.context.afterHoursChangePercent >= 0
													? "rgba(0, 200, 83, 0.9)"
													: "rgba(239, 68, 68, 0.9)",
										}}
									>
										({stockData.context.afterHoursChangePercent >= 0 ? "+" : ""}
										{stockData.context.afterHoursChangePercent.toFixed(2)}%)
									</span>
								)}
							</div>
						)}
