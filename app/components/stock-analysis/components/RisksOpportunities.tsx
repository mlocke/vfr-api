...
const processRisksAndOpportunities = (
	...
	risks.forEach((risk, index) => {
		...
		if (
			...
			riskLower.includes("significant") ||
			riskLower.includes("major")
		) {
			severity = "high";
		} else if (
			riskLower.includes("low") ||
			riskLower.includes("minor") ||
			riskLower.includes("limited")
		) {
			severity = "low";
		}
		...
		if (
			riskLower.includes("debt") ||
			riskLower.includes("financial") ||
			riskLower.includes("cash")
		) {
			category = "Financial";
	...
	opportunities.forEach((opportunity, index) => {
		...
		if (
			oppLower.includes("significant") ||
			oppLower.includes("major") ||
			oppLower.includes("strong")
		) {
			severity = "high";
	...
	if (items.length === 0) {
		items.push(
			...
			},
			{
				...
				description:
					"Company positioned for potential growth in current market environment",
				type: "opportunity",
				severity: "medium",
				category: "Growth",
			}
	...
	return (
		<div
			...
			onMouseEnter={e => {
				...
				e.currentTarget.style.borderColor = isRisk
					? "rgba(239, 68, 68, 0.5)"
					: "rgba(34, 197, 94, 0.5)";
			}}
			onMouseLeave={e => {
...
const RisksOpportunities: React.FC<RisksOpportunitiesProps> = ({
	...
	return (
		<>
			...
			<section className="section-container" aria-labelledby="risks-opportunities-heading">
				{/* Section Header */}
				<div
				...
				>
					<div className="header-content">
						<div className="header-left">
							...
							<h3 className="section-title" id="risks-opportunities-heading">
								Risks & Opportunities
							</h3>
						</div>
				...
				{/* Expandable Content */}
				<div className="content" id="risks-opportunities-content" aria-hidden={!isExpanded}>
					<div className="content-inner">
							...
							<div>
								...
								<div className="items-grid">
									{riskItems.map((item, index) => (
										<ItemCard key={`risk-${index}`} item={item} index={index} />
									))}
								</div>
							</div>
						)}
						...
						>
							...
							>
								Comprehensive analysis of {totalItems} key factors affecting
