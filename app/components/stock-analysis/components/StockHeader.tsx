...
const RecommendationBadge: React.FC<{
	...
	const getActionStyles = () => {
		const styles = {
			...
			MODERATE_SELL: {
				background:
					"linear-gradient(135deg, rgba(251, 146, 60, 0.8), rgba(234, 88, 12, 0.8))",
			...
			<header className="stock-header-container">
				...
				<div className="stock-info-section">
					<div className="stock-title-row">
						<h1 className="stock-symbol" id="dialog-title">
							{symbol}
						</h1>
					...
					<div className="context-info">
						<div style={{ marginBottom: "0.25rem" }}>
							<strong>{context.sector}</strong> • {formatMarketCap(context.marketCap)}{" "}
							Market Cap
						</div>
						{context.beta && (
							<div style={{ fontSize: "0.8rem", opacity: 0.8 }}>
								...
								{context.volumeChange24h && (
									<span style={{ marginLeft: "1rem" }}>
										Volume: {context.volumeChange24h > 0 ? "+" : ""}
										{context.volumeChange24h.toFixed(1)}%
									</span>
								)}
							</div>
						)}
						...
						{(context.preMarketPrice || context.afterHoursPrice) && (
							...
							>
								{context.marketStatus === "pre-market" &&
									context.preMarketPrice && (
										<div>
											...
											{context.preMarketChangePercent && (
												<span
													style={{
														color:
															context.preMarketChangePercent >= 0
																? "rgba(0, 200, 83, 0.8)"
																: "rgba(239, 68, 68, 0.8)",
														marginLeft: "0.5rem",
													}}
												>
													(
													{context.preMarketChangePercent >= 0 ? "+" : ""}
													{context.preMarketChangePercent.toFixed(2)}%)
												</span>
											)}
										</div>
									)}
								{context.marketStatus === "after-hours" &&
									context.afterHoursPrice && (
										<div>
											...
											{context.afterHoursChangePercent && (
												<span
													style={{
														color:
															context.afterHoursChangePercent >= 0
																? "rgba(0, 200, 83, 0.8)"
																: "rgba(239, 68, 68, 0.8)",
														marginLeft: "0.5rem",
													}}
												>
													...
													{context.afterHoursChangePercent >= 0
														? "+"
														: ""}
													{context.afterHoursChangePercent.toFixed(2)}%)
												</span>
											)}
										</div>
									)}
