...
export const useStockDialog = (
   ...
   * Cache dialog data
   */
  const setCachedData = useCallback((symbol: string, data: DialogStockData) => {
   ...
   */
  const fetchDialogData = useCallback(async (symbol: string): Promise<DialogStockData> => {
   ...
   */
  const openDialog = useCallback(async (symbol: string) => {
  ...
  const refreshDialog = useCallback(async () => {
    ...
    try {
      ...
      if (retryCountRef.current < opts.maxRetries!) {
        ...
        console.warn(`Retry ${retryCountRef.current}/${opts.maxRetries} for ${currentSymbol}`)
