...
export const StockAnalysisDialog: React.FC<StockAnalysisDialogProps> = React.memo(
	({ symbol, stockData, analysisServices, isOpen, onClose, onActionTaken }) => {
		const [activeTab, setActiveTab] = useState<
			"overview" | "scores" | "analysis" | "services" | "technical"
		>("overview");
		...
		const dialogStockData = React.useMemo(
			() => transformToDialogStockData(validatedStockData),
			[validatedStockData]
		);
		const scoreVisualizationProps = React.useMemo(
			() => transformToScoreVisualizationProps(validatedStockData),
			[validatedStockData]
		);
		const risksOpportunitiesProps = React.useMemo(
			() => transformToRisksOpportunitiesProps(validatedStockData),
			[validatedStockData]
		);
		const dialogFooterProps = React.useMemo(
			() => transformToDialogFooterProps(validatedStockData, onActionTaken, onClose),
			[validatedStockData, onActionTaken, onClose]
		);
			...
			if (isOpen) {
				...
				const domCheckTimeout = setTimeout(() => {
					const dialogElements = document.querySelectorAll(
						'[class*="dialog-backdrop"], [style*="backdrop"], [style*="z-index: 10000"]'
					);
					...
					const stockDialogs = document.querySelectorAll(
						'[style*="zIndex: 10000"], [style*="z-index: 10000"]'
					);
						...
						>
							<div>
								...
								>
									...
									<span>•</span>
									<span>
										{formatPrice(validatedStockData.score.marketData.price)}{" "}
										Current Price
									</span>
									<span>•</span>
									<span>
										{formatNumber(validatedStockData.score.marketData.volume)}{" "}
										Volume
									</span>
									{validatedStockData.context.priceChange24h !== undefined && (
										<>
											...
											<span
												style={{
													color:
														validatedStockData.context.priceChange24h >
														0
															? "rgba(0, 200, 83, 0.9)"
															: "rgba(239, 68, 68, 0.9)",
												}}
						...
						<div style={{ padding: "0 2rem 1rem 2rem" }}>
							<div
								style={{
									...
									gap: "0.75rem",
									background:
										validatedStockData.action === "BUY"
											? "rgba(0, 200, 83, 0.15)"
											: validatedStockData.action === "SELL"
												? "rgba(239, 68, 68, 0.15)"
												: "rgba(255, 193, 7, 0.15)",
									border: `2px solid ${
										validatedStockData.action === "BUY"
							...
							>
								...
								<div>
									...
									>
										{formatScore(validatedStockData.confidence)}% Confidence •
										Overall Score:{" "}
										{formatScore(validatedStockData.score.overallScore)}/100
									</div>
								</div>
							</div>
						</div>
						...
						<div style={{ padding: "1rem 2rem 2rem 2rem" }}>
								...
								<div>
									...
									<ErrorBoundary componentName="RisksOpportunities">
										<RisksOpportunities
											...
											onToggle={() =>
												setRisksOpportunitiesExpanded(
													!risksOpportunitiesExpanded
												)
											}
										/>
									</ErrorBoundary>
									...
									>
										...
										>
											<div>
												<strong>Price:</strong>{" "}
												{formatPrice(
													validatedStockData.score.marketData.price
												)}
											</div>
											<div>
												<strong>Volume:</strong>{" "}
												{formatNumber(
													validatedStockData.score.marketData.volume
												)}
											</div>
											<div>
												<strong>Sector:</strong>{" "}
												{validatedStockData.score.marketData.sector}
											</div>
											<div>
												<strong>Exchange:</strong>{" "}
												{validatedStockData.score.marketData.exchange}
											</div>
											<div>
												<strong>Market Cap:</strong>{" "}
												{formatMarketCap(
													validatedStockData.context.marketCap
												)}
											</div>
											<div>
												<strong>Overall Score:</strong>{" "}
												{formatScore(validatedStockData.score.overallScore)}
												/100
											</div>
										</div>
									</div>
							...
							{activeTab === "scores" && (
								<div>
									...
									>
										...
										>
											The detailed score visualization above shows the
											comprehensive analysis of {validatedStockData.symbol}{" "}
											across all key investment factors.
										</p>
									</div>
								</div>
							)}
							...
							{activeTab === "analysis" && (
								<div>
									...
									>
										...
										>
											{stockData.reasoning.primaryFactors.map(
												(factor, index) => (
													<li
														key={index}
									...
									>
										...
										>
											<div>
												...
												{formatScore(stockData.dataQuality.overall.overall)}
												/100
											</div>
											<div>
												<strong>Freshness:</strong>{" "}
												{formatScore(
													stockData.dataQuality.overall.metrics.freshness
												)}
												/100
											</div>
											<div>
												...
												{formatScore(
													stockData.dataQuality.overall.metrics
														.completeness
												)}
												/100
											</div>
											<div>
												...
												{formatScore(
													stockData.dataQuality.overall.metrics.accuracy
												)}
												/100
											</div>
											<div>
												<strong>Source Reputation:</strong>{" "}
												{formatScore(
													stockData.dataQuality.overall.metrics
														.sourceReputation
												)}
											...
											<div>
												...
												{stockData.dataQuality.overall.source}
											</div>
										</div>
									</div>
								...
								<div>
									...
									>
											...
											>
												<div
													style={{
														...
														height: "100%",
														background:
															"linear-gradient(90deg, rgba(0, 200, 83, 0.8), rgba(0, 200, 83, 1))",
														borderRadius: "4px",
										...
										>
											...
											>
												{formatScore(
													stockData.score.factorScores.momentum_composite
												)}
												/100
											</div>
											<div
										...
										>
											...
											>
												{formatScore(
													stockData.score.factorScores.volatility_30d
												)}
												/100
											</div>
											<div
							...
							{activeTab === "services" && analysisServices && (
								<div>
									...
									>
										{Object.entries(analysisServices).map(
											([serviceName, service]) => (
												<div
													key={serviceName}
													style={{
