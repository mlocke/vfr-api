...
Usage:
    python3 build-eodhd-options-dataset.py
    python3 build-eodhd-options-dataset.py --tickers AAPL MSFT TSLA
    python3 build-eodhd-options-dataset.py --historical AAPL --start 2024-01-01 --end 2024-12-31
"""
...
EODHD_API_KEY = os.getenv('EODHD_API_KEY', '68cf08e135b402.52970225')
...
class EODHDOptionsDatasetBuilder:
    ...
    def get_options_data(self, ticker: str, date: Optional[str] = None) -> Optional[List[Dict]]:
        """Get options data for a specific ticker"""
        try:
            # Build URL
            url = f"{BASE_URL}/options/{ticker}.US"
            params = {
            ...
            if response.status_code == 200:
                ...
                if data and 'data' in data:
                    ...
                    print(f"  ✅ {ticker}: {len(options_data)} options contracts")
                    return options_data
                else:
                    print(f"  ⚠️  {ticker}: No data field in response")
                    return None
    ...
    def build_dataset(self, tickers: Optional[List[str]] = None, format: str = 'both'):
        ...
        print('=' * 80)
        print('EODHD OPTIONS DATASET BUILDER')
        print('=' * 80)
...
def main():
    parser = argparse.ArgumentParser(
        description='Build EODHD options dataset',
...
Examples:
  ...
  # JSON only
  python3 build-eodhd-options-dataset.py --format json

  # Test with 5 tickers
  python3 build-eodhd-options-dataset.py --tickers AAPL MSFT GOOGL AMZN TSLA
        """
    )
