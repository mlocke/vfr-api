...
CREATE TABLE IF NOT EXISTS ml_enhancement_definitions (
    ...
    INDEX idx_enhancement_active (is_active, enhancement_type),
    INDEX idx_enhancement_factor (target_factor, is_active)
);
...
CREATE TABLE IF NOT EXISTS ml_enhancement_store (
    ...
    -- Partial index for active enhancements
...
CREATE TABLE IF NOT EXISTS ml_feature_store (
    ...
    -- Performance indexes
    INDEX idx_ticker_feature_timestamp (ticker, feature_id, timestamp DESC),
...
CREATE TABLE IF NOT EXISTS ml_predictions (
    ...
    INDEX idx_high_confidence (confidence_score DESC, prediction_timestamp DESC)
...
CREATE TABLE IF NOT EXISTS ml_models (
    ...
    INDEX idx_model_status (status, model_type),
    INDEX idx_model_tier (tier_requirement, status)
);
...
CREATE TABLE IF NOT EXISTS ml_user_tiers (
    ...
    INDEX idx_tier_active (ml_tier, is_active),
    INDEX idx_usage_tracking (predictions_used_today, last_prediction_timestamp)
);
