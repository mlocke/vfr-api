	...
	try {
		...
		if (data.score?.factorScores) {
			...
			factors.forEach(([factor, score]) => {
				console.log(
					`   ${factor}: ${typeof score === "number" ? score.toFixed(4) : score}`
				);
			});
			if (Object.keys(data.score.factorScores).length > 10) {
				console.log(
					`   ... and ${Object.keys(data.score.factorScores).length - 10} more factors`
				);
			}
		}
		...
		if (data.analystData) {
			...
			console.log(
				`   Sentiment Score: ${data.analystData.sentimentScore?.toFixed(2) || "N/A"}`
			);
