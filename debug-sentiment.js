  ...
  try {
    ...
    for (const symbol of testSymbols) {
      ...
      try {
        ...
        const impact = await sentimentService.analyzeStockSentimentImpact(symbol, 'Technology', 0.75)
        if (impact) {
    ...
    console.log('NEWSAPI_KEY:', process.env.NEWSAPI_KEY ? (process.env.NEWSAPI_KEY === 'your_news_api_key_here' ? '❌ Placeholder' : '✅ Set') : '❌ Missing')
