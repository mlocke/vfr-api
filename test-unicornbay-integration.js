    ...
    try {
        ...
        if (data.results && data.results.length > 0) {
            ...
            if (result.data && result.data.overallStatus) {
                ...
                console.log(`   ✅ Tests successful: ${status.testsSuccessful}/${status.testsCompleted}`);
    ...
    try {
        ...
        if (data.results && data.results.length > 0) {
            ...
            if (result.data && result.data.unicornBayIntegration) {
                ...
                console.log(`   📈 Performance grade: ${integration.performanceReport?.performanceGrade || 'N/A'}`);
                console.log(`   ⚡ Average latency: ${integration.performanceReport?.averageLatency || 'N/A'}ms`);
                console.log(`   💾 Cache hit rate: ${integration.performanceReport?.cacheHitRate || 'N/A'}%`);

                if (integration.recommendations) {
                    console.log(`   💡 Use UnicornBay: ${integration.recommendations.useUnicornBay}`);
                }
            }
