...
async function testMSFTData() {
  ...
  try {
    ...
    if (prevData.data?.status === 'OK' && prevData.data.results?.[0]) {
      ...
      if (info.market_cap) console.log(`  Market Cap: $${(info.market_cap/1000000000).toFixed(2)}B`);
    }
  } catch (error) {
  ...
  try {
    ...
    if (quoteResp.data['Global Quote'] && Object.keys(quoteResp.data['Global Quote']).length > 0) {
      ...
      console.log(`  Market Cap: $${(parseInt(overview.MarketCapitalization)/1000000000).toFixed(2)}B`);
  ...
  try {
    ...
    if (quoteResp.data && quoteResp.data.length > 0) {
      ...
      if (ratios.returnOnEquityTTM) console.log(`  ROE: ${(ratios.returnOnEquityTTM * 100).toFixed(2)}%`);
    }
  } catch (error) {
  ...
  try {
    ...
    const [quoteResp, statsResp] = await Promise.all(
      urls.map(url => axios.get(url, { timeout: 10000 }).catch(e => ({ data: { error: e.message } })))
    );
    ...
    if (quoteResp.data && !quoteResp.data.code) {
      ...
      console.log(`  52W Range: $${quote.fifty_two_week?.low} - $${quote.fifty_two_week?.high}`);
    }
  } catch (error) {
  ...
  if (successful.length > 1) {
    ...
    if (priceData.length > 1) {
      ...
      priceData.forEach(p => {
        ...
        console.log(`  ${p.source.padEnd(18)} $${p.price.toFixed(2).padEnd(10)} ${deviation}% ${flag}`);
      });
      console.log('  ' + '-'.repeat(45));
  ...
  if (pricesForCheck.length > 0) {
    ...
    const priceStdDev = Math.sqrt(
      pricesForCheck.reduce((sum, p) => sum + Math.pow(p - avgPrice, 2), 0) / pricesForCheck.length
    );
