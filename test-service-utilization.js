  ...
  try {
    ...
    if (response.data.success) {
      ...
      if (stockData && stockData.factorScores) {
        ...
        const vwapFactors = Object.keys(factors).filter(key =>
          key.includes('vwap') || key === 'vwap_deviation_score' || key === 'vwap_trading_signals'
        )
        ...
        const macroFactors = Object.keys(factors).filter(key =>
          key.includes('macro') || key === 'macroeconomic_sector_impact' || key === 'macroeconomic_composite'
        )
      ...
      const macroUtilization = analysisServices.macroeconomicAnalysis?.utilizationInResults || '0%'
      ...
      if (vwapUtilization !== '0%' || macroUtilization !== '0%') {
        ...
        console.log('⚠️ WARNING: Services not being utilized - check API keys and service availability')
      }
