...
} = require("./app/services/financial-data/OptionsAnalysisService.ts");
...
async function testOptionsSentimentIntegration() {
	...
	try {
		...
		if (indicators) {
			...
			if (indicators.news) {
				console.log(
					`- News Sentiment: ${(indicators.news.sentiment * 100).toFixed(1)}% (${indicators.news.articleCount} articles)`
				);
			}

			if (indicators.reddit) {
				console.log(
					`- Reddit Sentiment: ${(indicators.reddit.sentiment * 100).toFixed(1)}% (${indicators.reddit.postCount} posts)`
				);
			}

			if (indicators.options) {
				console.log(
					`- Options Sentiment: ${(indicators.options.sentiment * 100).toFixed(1)}%`
				);
				console.log(
					`  - P/C Ratio: ${indicators.options.putCallRatio.toFixed(2)} (${indicators.options.sentimentSignal})`
				);
				...
				console.log(
					`  - Unusual Activity: ${indicators.options.unusualActivity ? "YES" : "NO"}`
				);
				console.log(
					`  - Total Volume: ${indicators.options.volumeAnalysis.totalVolume.toLocaleString()}`
				);
			...
			if (sentimentImpact) {
				...
				console.log(
					`- Adjusted Score: ${(sentimentImpact.adjustedScore * 100).toFixed(1)}%`
				);
				console.log(
					`- Sentiment Weight: ${(sentimentImpact.sentimentWeight * 100).toFixed(1)}%`
				);
				console.log(
					`- Overall Sentiment: ${(sentimentImpact.sentimentScore.overall * 100).toFixed(1)}%`
				);

				if (sentimentImpact.sentimentScore.components.options) {
					console.log(
						`- Options Component: ${(sentimentImpact.sentimentScore.components.options * 100).toFixed(1)}%`
					);
				}
		...
		console.log(
			`- Yahoo Finance Sentiment: ${health.details.yahooFinanceSentiment ? "✅" : "❌"}`
		);
