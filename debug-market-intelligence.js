  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.metadata) {
        ...
        console.log(`  - Data Sources: ${data.metadata.dataSourcesUsed?.join(', ') || 'None'}`);
        console.log(`  - Analysis Mode: ${data.metadata.analysisMode}`);
        console.log(`  - Cache Hit Rate: ${(data.metadata.cacheHitRate * 100).toFixed(1)}%`);

        if (data.metadata.qualityScore) {
          console.log(`  - Quality Score: ${(data.metadata.qualityScore.overall * 100).toFixed(1)}%`);
        }
      }
      ...
      if (data.topSelections && data.topSelections.length > 0) {
        ...
        data.topSelections.forEach((stock, index) => {
          ...
          if (stock.analysis?.reasoning) {
            console.log(`     Reasoning: ${stock.analysis.reasoning.substring(0, 100)}...`);
          }
